...
static int ndef_print_signature(uint8_t *data, uint8_t data_len, uint8_t *signature, uint8_t sign_len) {
    ...
    PrintAndLogEx(INFO, "IC 签名公钥值：%s", ndef_public_keys[i].value);
...
static int ndefDecodeSig1(uint8_t *sig, size_t siglen) {
    ...
    if (sigType == stECDSA_P192 || sigType == stECDSA_P256) {
        ...
        PrintAndLogEx(SUCCESS, "\\t签名 [%u]...", intsiglen);
        ...
        PrintAndLogEx(SUCCESS, "\\t数据 [%u]...", intsiglen);
        print_hex_noascii_break(&sig[indx], intsiglen, 32);
    }
    ...
    for (uint8_t i = 0; i < certCount; i++) {
        ...
        PrintAndLogEx(INFO, "");
        PrintAndLogEx(SUCCESS, "\\t证书 %u [%u]...", i + 1, intcertlen);
...
static int ndefDecodeSig2(uint8_t *sig, size_t siglen) {
    ...
    PrintAndLogEx(SUCCESS, "\\t签名类型 :\\t" _GREEN_("%s"), ((sigType < stNA) ? ndefSigType_s[sigType] : ndefSigType_s[stNA]));
    PrintAndLogEx(SUCCESS, "\\t签名 URI :\\t\\t%s", (sigURI ? "present" : "not present"));
    PrintAndLogEx(SUCCESS, "\\t哈希类型 :\\t\\t%s", ((hashType == 0x02) ? _GREEN_("SHA-256") : _RED_("unknown")));
    ...
    if (sigURI) {
        indx += 2;
        PrintAndLogEx(SUCCESS, "\\t签名 URI [%zu]: %.*s", intsiglen, (int)intsiglen, &sig[indx]);
        indx += intsiglen;
    } else {
        PrintAndLogEx(SUCCESS, "\\t签名 [%zu]: %s", intsiglen, sprint_hex_inrow(&sig[indx], intsiglen));
        if (sigType == stECDSA_P192 || sigType == stECDSA_P256) {
    ...
    PrintAndLogEx(SUCCESS, "\\t证书数量 : %d", certCount);
    ...
    for (int i = 0; i < certCount; i++) {
        ...
        PrintAndLogEx(SUCCESS, "\\t证书 %d [%zu]: %s", i + 1, intcertlen, sprint_hex_inrow(&sig[indx], intcertlen));
        indx += intcertlen;
    }
    ...
    if ((indx <= siglen) && certURI) {
        ...
        indx += 2;
        PrintAndLogEx(SUCCESS, "\\t证书 URI [%zu]: %.*s", inturilen, (int)inturilen, &sig[indx]);
    }
...
static int ndefDecodeMime_wifi_wsc(NDEFHeader_t *ndef) {
    ...
    while (n) {
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x4A", 2) == 0) {
            uint8_t len = 1;
            PrintAndLogEx(INFO, "版本......... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x0E", 2) == 0) {
            //  10 0E 00 39
            uint8_t len = 2;
            PrintAndLogEx(INFO, "凭证...... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x20", 2) == 0) {
            ...
            PrintAndLogEx(INFO, "MAC 地址..... %s", sprint_hex_ascii(&ndef->Payload[pos + 4], len));
            n -= 4;
            n -= len;
            pos += 4;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x26", 2) == 0) {
            // 10 26 00 01 01
            uint8_t len = 3;
            PrintAndLogEx(INFO, "网络索引... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x27", 2) == 0) {
            ...
            uint8_t len = ndef->Payload[pos + 3];
            PrintAndLogEx(INFO, "网络密钥..... %s", sprint_hex_ascii(&ndef->Payload[pos + 4], len));
            n -= 4;
            n -= len;
            pos += 4;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x45", 2) == 0) {
            ...
            uint8_t len = ndef->Payload[pos + 3];
            PrintAndLogEx(INFO, "网络名称.... %s", sprint_hex_ascii(&ndef->Payload[pos + 4], len));
            n -= 4;
            n -= len;
            pos += 4;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x2C", 2) == 0) {
            uint8_t len = 1;
            PrintAndLogEx(INFO, "OOB密码......... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x49", 2) == 0) {
            uint8_t len = 1;
            PrintAndLogEx(INFO, "厂商扩展......... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
        ...
        if (memcmp(&ndef->Payload[pos], "\x10\x30\x2A", 3) == 0) {
            uint8_t len = 1;
            PrintAndLogEx(INFO, "厂商 WFA......... %s", sprint_hex(&ndef->Payload[pos + 2], len));
            n -= 2;
            n -= len;
            pos += 2;
            pos += len;
...
static int ndefDecodeMime_bt(NDEFHeader_t *ndef) {
    ...
    PrintAndLogEx(INFO, "OOB数据长度.... %u", ooblen);
    ...
    switch (ndef->Payload[8]) {
        case 0x02:
            PrintAndLogEx(INFO, "可选数据... 不完整的16位UUID列表");
            break;
        case 0x03:
            PrintAndLogEx(INFO, "可选数据... 完整的16位UUID列表");
            break;
        case 0x04:
            PrintAndLogEx(INFO, "可选数据... 不完整的32位UUID列表");
            break;
        case 0x05:
            PrintAndLogEx(INFO, "可选数据... 完整的32位UUID列表");
            break;
        case 0x06:
            PrintAndLogEx(INFO, "可选数据... 不完整的128位UUID列表");
            break;
        case 0x07:
            PrintAndLogEx(INFO, "可选数据... 完整的128位UUID列表");
            break;
        default:
            PrintAndLogEx(INFO, "可选数据... [ %02x ]", ndef->Payload[8]);
            break;
    }
...
static int ndefDecodeExternal_record(NDEFHeader_t *ndef) {
    ...
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(INFO, "载荷 [%zu]...", ndef->PayloadLen);
...
static int ndefDecodePayload(NDEFHeader_t *ndef, bool verbose) {
    ...
    switch (ndef->TypeNameFormat) {
        case tnfEmptyRecord: {
            PrintAndLogEx(INFO, "空记录");
            if (ndef->TypeLen != 0 || ndef->IDLen != 0 || ndef->PayloadLen != 0) {
                PrintAndLogEx(FAILED, "空记录中出现意外数据");
                break;
            }
            break;
        }
        ...
        case tnfMIMEMediaRecord: {
            PrintAndLogEx(INFO, "MIME 媒体记录");
            if (ndef->TypeLen == 0)  {
                PrintAndLogEx(INFO, "类型长度为零");
                break;
            }
        ...
        default: {
            PrintAndLogEx(FAILED, "意外的 tnf 值... 0x%02x", ndef->TypeNameFormat);
            break;
        }
    }
    PrintAndLogEx(INFO, "");
    ...
    while (len < ndefRecordLen) {
        ...
        if (firstRec) {
            if (!NDEFHeader.MessageBegin) {
                PrintAndLogEx(ERR, "NDEF第一条记录的MessageBegin为false！");
                return PM3_ESOFT;
            }
            firstRec = false;
        }
        ...
        if (NDEFHeader.MessageEnd && len + NDEFHeader.RecLen != ndefRecordLen) {
            PrintAndLogEx(ERR, "NDEF 记录长度错误。必须为 %zu，计算为 %zu", ndefRecordLen, len + NDEFHeader.RecLen);
            return PM3_ESOFT;
        }
    ...
    while (indx < ndefLen) {
        ...
        switch (ndef[indx]) {
            case 0x00: {
                ...
                if (len)
                    PrintAndLogEx(WARNING, "NDEF NULL块大小必须为0，实际为%d字节", len);
                indx += len;
                break;
            }
            case 0x01: {
                ...
                if (len != 3) {
                    PrintAndLogEx(WARNING, "NDEF锁定控制块大小必须为3，而不是%d", len);
                } else {
            ...
            case 0x02: {
                ...
                if (len != 3) {
                    PrintAndLogEx(WARNING, "NDEF内存控制块大小必须为3，而不是%u", len);
                } else {
                    uint8_t pages_addr = (ndef[indx] >> 4) & 0x0f;
                    uint8_t byte_offset = ndef[indx] & 0x0f;
                    uint8_t Size = ndef[indx + 1];
                    uint8_t bytes_per_page = ndef[indx + 2] & 0x0f;
                    PrintAndLogEx(SUCCESS, "页地址(页数).... %u", pages_addr);
                    PrintAndLogEx(SUCCESS, "字节偏移（字节数）... %u", byte_offset);
                    PrintAndLogEx(SUCCESS, "保留区域大小（位）...... %u（%u 字节）", Size, Size / 8);
            ...
            case 0x03: {
                ...
                if (len == 0) {
                    PrintAndLogEx(SUCCESS, "已找到零长度的 NDEF 消息");
                } else {
            ...
            case 0xFE: {
                if (verbose) {
                    PrintAndLogEx(SUCCESS, "检测到NDEF终止符");
                }
                return PM3_SUCCESS;
            }
            default: {
                if (verbose) {
                    PrintAndLogEx(ERR, "未知标签 0x%02x", ndef[indx]);
                }
                return PM3_ESOFT;
            }
        }
