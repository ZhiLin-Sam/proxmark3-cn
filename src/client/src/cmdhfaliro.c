...
static void parse_extended_length_info(const uint8_t *buf, size_t len,
    ...
    while (left > 0) {
        ...
        if (tlv_parse_tl(&cursor, &left, &tlv) == false || tlv.len > left) {
            PrintAndLogEx(WARNING, "格式错误的标签 7F66 值");
            return;
        }
        ...
        if (tlv.tag == 0x02) {
            ...
            if (trimmed_len > 0 && trimmed_len <= 4) {
                ...
                PrintAndLogEx(WARNING, "无法从标签 7F66 解析 INTEGER");
            }
        }
...
static int parse_aliro_select_response(const uint8_t *buf, size_t len, aliro_select_info_t *info) {
    ...
    while (top_left > 0) {
        ...
        if (tlv_parse_tl(&top_cursor, &top_left, &tlv) == false || tlv.len > top_left) {
            PrintAndLogEx(ERR, "格式错误的 SELECT 响应");
    ...
    if (fci_value == NULL || fci_len == 0) {
        PrintAndLogEx(ERR, "SELECT 响应不包含 FCI 模板 (标签 6F)");
        return PM3_ECARDEXCHANGE;
    }
    ...
    while (fci_left > 0) {
        ...
        if (tlv_parse_tl(&fci_cursor, &fci_left, &tlv) == false || tlv.len > fci_left) {
            PrintAndLogEx(ERR, "格式错误的 FCI 模板");
            return PM3_ECARDEXCHANGE;
        }
        ...
        if (tlv.tag == 0x84) {
            ...
            if (info->fci_aid_len > sizeof(info->fci_aid)) {
                ...
                PrintAndLogEx(WARNING, "返回的 FCI AID 过长，正在截断输出");
            }
            ...
            if (a5_res == PM3_SUCCESS) {
                ...
                PrintAndLogEx(WARNING, "专有信息TLV过长，密钥派生可能失败");
                ...
                switch (field.tag) {
                    case 0x80:
                        if (field.len == 2) {
                            ...
                            PrintAndLogEx(WARNING, "意外的应用类型大小: %zu", field.len);
                        }
                        break;
                    case 0x5C:
                        ...
                        if (info->versions_len > sizeof(info->versions)) {
                            ...
                            PrintAndLogEx(WARNING, "支持的协议版本列表过长，截断输出");
                        }
    ...
    if (info->have_proprietary_tlv == false) {
        PrintAndLogEx(ERR, "SELECT 响应不包含专有信息 (标签 A5)");
        return PM3_ECARDEXCHANGE;
    }
...
static void print_aliro_select_info(const aliro_select_info_t *info) {
    if (info->have_fci_aid) {
        ...
        PrintAndLogEx(INFO, "AID....................... 不存在");
    }

    if (info->have_versions && info->versions_len >= 2) {
        PrintAndLogEx(INFO, "支持的协议版本:");
        ...
        if ((info->versions_len % 2) != 0) {
            PrintAndLogEx(WARNING, "尾部协议版本字节被忽略: %02X", info->versions[info->versions_len - 1]);
        }
    } else {
        PrintAndLogEx(INFO, "支持的协议版本: 不存在");
    }
    ...
    if (info->has_max_command || info->has_max_response) {
        ...
        PrintAndLogEx(INFO, "最大APDU大小........ 未提供");
    }
}
...
static int aliro_exchange_chained(bool activate_field, bool leave_field_on,
    ...
    if (data_len > 255) {
        PrintAndLogEx(ERR, "APDU负载过长: %zu (最大255)", data_len);
        return PM3_EINVARG;
    }
...
static int aliro_select_with_info(aliro_select_info_t *info, bool keep_field_on) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "SELECT APDU 交换错误");
        return res;
    }

    if (sw != ISO7816_OK) {
        if (sw) {
            PrintAndLogEx(INFO, "未找到Aliro小程序。APDU响应：%04x - %s",
...
static int aliro_parse_auth0_response(const uint8_t *buf, size_t len, aliro_auth0_response_t *out) {
    ...
    while (left > 0) {
        struct tlv tlv = {0};
        if (tlv_parse_tl(&cursor, &left, &tlv) == false || tlv.len > left) {
            PrintAndLogEx(ERR, "格式错误的 AUTH0 响应");
            return PM3_ECARDEXCHANGE;
        }
        ...
        switch (tlv.tag) {
            case 0x86:
                if (tlv.len != 65 || value[0] != 0x04) {
                    PrintAndLogEx(ERR, "无效的 AUTH0 标签 0x86 长度/格式 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x9D:
                if (tlv.len != 64) {
                    PrintAndLogEx(ERR, "无效的 AUTH0 密码长度 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0xB2: {
                ...
                if (tlv_res != PM3_SUCCESS) {
                    PrintAndLogEx(WARNING, "AUTH0 厂商扩展过长，忽略");
    ...
    if (out->have_endpoint_ephemeral_public_key == false) {
        PrintAndLogEx(ERR, "AUTH0 响应不包含端点临时公钥（标签 0x86）");
        return PM3_ECARDEXCHANGE;
    }
    return PM3_SUCCESS;
}
...
static int aliro_parse_auth1_plaintext(const uint8_t *buf, size_t len, aliro_auth1_response_t *out) {
    ...
    while (left > 0) {
        struct tlv tlv = {0};
        if (tlv_parse_tl(&cursor, &left, &tlv) == false || tlv.len > left) {
            PrintAndLogEx(ERR, "格式错误的 AUTH1 明文响应");
            return PM3_ECARDEXCHANGE;
        }
        ...
        switch (tlv.tag) {
            case 0x4E:
                if (tlv.len != sizeof(out->key_slot)) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 密钥槽大小 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x5A:
                if (tlv.len != 65 || value[0] != 0x04) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 端点公钥大小/格式 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x9E:
                if (tlv.len != 64) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 签名大小 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x5E:
                if (tlv.len != 2) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 信令位图大小 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x91:
                if (tlv.len != sizeof(out->credential_signed_timestamp)) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 凭证时间戳大小 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
            ...
            case 0x92:
                if (tlv.len != sizeof(out->revocation_signed_timestamp)) {
                    PrintAndLogEx(ERR, "无效的 AUTH1 撤销时间戳大小 (%zu)", tlv.len);
                    return PM3_ECARDEXCHANGE;
                }
    ...
    if (out->have_signature == false) {
        PrintAndLogEx(ERR, "AUTH1 明文缺少签名 (标签 0x9E)");
        return PM3_ECARDEXCHANGE;
    }
    if (out->have_signaling_bitmap == false) {
        PrintAndLogEx(ERR, "AUTH1 明文缺少信令位图 (标签 0x5E)");
        return PM3_ECARDEXCHANGE;
    }
    if (out->have_key_slot == false && out->have_endpoint_public_key == false) {
        PrintAndLogEx(ERR, "AUTH1 明文缺少密钥槽和端点公钥");
        return PM3_ECARDEXCHANGE;
    }
    return PM3_SUCCESS;
}
...
static void aliro_print_timestamp(const char *label, const uint8_t *timestamp, bool have) {
    if (have == false) {
        PrintAndLogEx(INFO, "%s 不存在", label);
        return;
    }
    if (aliro_is_zeroed(timestamp, 20)) {
        PrintAndLogEx(INFO, "%s 全零", label);
        return;
    }
...
static int aliro_read_prepare_session(aliro_read_state_t *state,
    ...
    if (state->select_info.have_proprietary_tlv == false) {
        PrintAndLogEx(ERR, "SELECT 响应中缺少专有信息 TLV");
        return PM3_ESOFT;
    }

    if (aliro_choose_protocol_version(&state->select_info, state->protocol_version) == false) {
        PrintAndLogEx(ERR, "无法从 SELECT 响应中确定 ALIRO 协议版本");
        return PM3_ESOFT;
    }
    PrintAndLogEx(INFO, "已选择协议版本 0x%02X%02X", state->protocol_version[0], state->protocol_version[1]);
...
aliro_append_tlv(0x4D, state->reader_identifier, 32, auth0_data, sizeof(auth0_data), &auth0_data_len) != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "编码AUTH0命令失败");
        return PM3_ESOFT;
    }

    PrintAndLogEx(INFO, "");
    PrintAndLogInfoHeader("AUTH0");
    PrintAndLogEx(INFO, "读取器组ID........... %s", sprint_hex_inrow(reader_group_identifier, 16));
    PrintAndLogEx(INFO, "读取器子ID............. %s", sprint_hex_inrow(reader_group_sub_identifier, 16));
    PrintAndLogEx(INFO, "读取器ID................. %s", sprint_hex_inrow(state->reader_identifier, 32));
    PrintAndLogEx(INFO, "读取器公钥......... %s", sprint_hex_inrow(state->reader_public_key, 65));
    PrintAndLogEx(INFO, "读取器ePubK.............. %s", sprint_hex_inrow(state->reader_ephemeral_public_key, 65));
    PrintAndLogEx(INFO, "交易标识符.... %s", sprint_hex_inrow(state->transaction_identifier, 16));
    PrintAndLogEx(INFO, "请求的 AUTH0 流程...... %s", auth0_fast_requested ? "fast" : "standard");
    if (flow == ALIRO_FLOW_STANDARD && auth0_fast_requested) {
        PrintAndLogEx(INFO, "AUTH0 流程覆盖....... 快速（提供了 k-persistent）");
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "AUTH0 APDU 交换失败");
        return res;
    }
    if (auth0_sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "AUTH0 失败: %04x - %s", auth0_sw, GetAPDUCodeDescription(auth0_sw >> 8, auth0_sw & 0xff));
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "AUTH0 凭证 ePubK.... %s", sprint_hex_inrow(state->auth0_parsed.endpoint_ephemeral_public_key, 65));
    if (state->auth0_parsed.have_cryptogram) {
        PrintAndLogEx(INFO, "AUTH0 密码.......... %s", sprint_hex_inrow(state->auth0_parsed.cryptogram, 64));
    } else {
        PrintAndLogEx(INFO, "AUTH0 密码.......... 不存在");
    }
    if (endpoint_public_key_x_in != NULL) {
        PrintAndLogEx(INFO, "端点公钥X（输入）. %s", sprint_hex_inrow(endpoint_public_key_x_in, 32));
    }
    if (auth0_fast_requested == false && state->auth0_parsed.have_cryptogram) {
        PrintAndLogEx(WARNING, "请求标准流程时 AUTH0 返回了密码");
    }
    if (auth0_fast_requested && state->auth0_parsed.have_cryptogram == false) {
        PrintAndLogEx(WARNING, "请求 AUTH0 快速流程，但缺少密码");
        PrintAndLogEx(INFO, "快速流程................. 无密码，继续标准流程");
    }
    if (state->auth0_suffix_len > 0) {
        PrintAndLogEx(INFO, "AUTH0 厂商扩展.... %s", sprint_hex_inrow(state->auth0_suffix, state->auth0_suffix_len));
    }

    if (state->auth0_parsed.have_cryptogram) {
        if (kpersistent_len != 32) {
            PrintAndLogEx(INFO, "AUTH0 快速验证......... 跳过（未提供 --k-persistent）");
        } else if (endpoint_public_key_x_in == NULL) {
            PrintAndLogEx(INFO, "AUTH0 快速验证......... 跳过（端点公钥不可用）");
            if (auth0_fast_requested) {
                PrintAndLogEx(INFO, "快速流程................. 端点公钥不可用，继续标准流程");
            }
        } else {
            ...
            if (res == PM3_SUCCESS && state->fast_result.verified) {
                ...
                if (auth0_fast_requested && res == PM3_ECRYPTO) {
                    PrintAndLogEx(INFO, "快速流程................. 无法解密密码，继续标准流程");
                }
...
static int aliro_read_prepare_auth1_keys(aliro_read_state_t *state,
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "派生用于AUTH1解密的易失性密钥失败");
        return res;
    }
...
static int aliro_read_do_auth1(aliro_read_state_t *state,
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "构建AUTH1签名数据失败");
        return res;
    }
...
aliro_append_tlv(0x9E, auth1_signature, 64,
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "AUTH1 APDU 交换失败");
        return res;
    }
    if (auth1_sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "AUTH1 失败: %04x - %s", auth1_sw, GetAPDUCodeDescription(auth1_sw >> 8, auth1_sw & 0xff));
        return PM3_ESOFT;
    }
    if (auth1_response_enc_len < 16) {
        PrintAndLogEx(ERR, "AUTH1 响应太短");
        return PM3_ESOFT;
    }
...
static void aliro_read_print_auth1_report(const aliro_read_state_t *state) {
    ...
    PrintAndLogEx(INFO, "AUTH1 签名........... %s", sprint_hex_inrow(state->auth1_parsed.signature, 64));
    if (state->auth1_parsed.have_key_slot) {
        PrintAndLogEx(INFO, "AUTH1 密钥槽............ %s", sprint_hex_inrow(state->auth1_parsed.key_slot, 8));
    }
    if (state->auth1_parsed.have_endpoint_public_key) {
        PrintAndLogEx(INFO, "AUTH1 端点公钥..... %s", sprint_hex_inrow(state->auth1_parsed.endpoint_public_key, 65));
        if (state->standard_result.signature_checked) {
            PrintAndLogEx(INFO, "AUTH1 签名有效...... %s", state->standard_result.signature_valid ? _GREEN_("yes") : _RED_("no"));
        } else {
            PrintAndLogEx(INFO, "AUTH1 签名有效...... 未检查");
        }
    } else {
        PrintAndLogEx(INFO, "AUTH1 端点公钥..... 不存在");
        PrintAndLogEx(INFO, "AUTH1 签名有效...... 未检查");
    }
...
static int aliro_add_scope(aliro_step_up_scopes_t *scopes, const char *scope) {
    ...
    if (strlen(scope) > ALIRO_MAX_STEP_UP_SCOPE_LEN) {
        PrintAndLogEx(ERR, "step-up 作用域 `%s` 过长（最多 %d 个字符）", scope, ALIRO_MAX_STEP_UP_SCOPE_LEN);
        return PM3_EINVARG;
    }
    ...
    if (scopes->count >= ALIRO_MAX_STEP_UP_SCOPES) {
        PrintAndLogEx(ERR, "步进范围过多（最多%d个）", ALIRO_MAX_STEP_UP_SCOPES);
        return PM3_EOVFLOW;
    }
...
static int aliro_parse_step_up_scopes(struct arg_str *scope_arg, aliro_step_up_scopes_t *scopes) {
    ...
    while (token != NULL) {
        ...
        if (token_len == 0) {
            PrintAndLogEx(ERR, "step-up 作用域列表包含空值");
            return PM3_EINVARG;
        }
...
static void aliro_print_step_up_scopes(const aliro_step_up_scopes_t *scopes) {
    if (scopes == NULL || scopes->count == 0) {
        PrintAndLogEx(INFO, "Step-up 范围............ %s", ALIRO_DEFAULT_STEP_UP_SCOPE);
        return;
    }
    ...
    PrintAndLogEx(INFO, "Step-up 范围............ %s", joined);
}
...
static int aliro_send_reader_status_exchange(aliro_secure_channel_state_t *channel,
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "编码EXCHANGE读取器状态负载失败");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "加密 EXCHANGE 读取器状态负载失败");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "读取器状态 EXCHANGE APDU 交换失败");
        return res;
    }

    if (exchange_sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "读取器状态 EXCHANGE 失败: %04x - %s",
                      exchange_sw, GetAPDUCodeDescription(exchange_sw >> 8, exchange_sw & 0xff));
        return PM3_ESOFT;
    }
    PrintAndLogEx(INFO, "读取器状态 EXCHANGE.... %04x", exchange_sw);
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "解密读取器状态EXCHANGE响应失败");
        return res;
    }
    if (response_plaintext_len > 0) {
        PrintAndLogEx(INFO, "读取器状态响应.... %s",
...
static int aliro_step_up_build_device_request(const char *const *document_types, size_t document_type_count,
    ...
    if (err != CborNoError) {
        PrintAndLogEx(ERR, "编码升级 DeviceRequest CBOR 失败 (%s)", cbor_error_string(err));
        return PM3_ESOFT;
    }
...
static int aliro_step_up_wrap_session_data(const uint8_t *ciphertext, size_t ciphertext_len,
    ...
    if (err == CborNoError) {
        err = cbor_encode_text_stringz(&map, "数据");
    }
    if (err == CborNoError) {
    ...
    if (err != CborNoError) {
        PrintAndLogEx(ERR, "编码 SessionData CBOR 失败 (%s)", cbor_error_string(err));
        return PM3_ESOFT;
    }
...
static int aliro_step_up_extract_session_data_ciphertext_entry(const CborValue *key, const CborValue *value, void *ctx_ptr) {
    ...
    if (!aliro_cbor_key_equals(key, -1, "数据") || cbor_value_is_byte_string(value) == false) {
        return PM3_SUCCESS;
    }
...
static bool aliro_cbor_print_scalar(const char *label, const CborValue *value) {
    ...
    if (cbor_value_is_undefined(&cursor)) {
        PrintAndLogEx(INFO, "%s 未定义", label);
        return true;
    }
    ...
    if (aliro_cbor_key_equals(key, 4, "4")) {
        if (!aliro_cbor_print_scalar_fmt(fmt, "element_value", value)) {
            ...
            if (aliro_cbor_decode_value_as_type(value, CborMapType, &value_decoded) == PM3_SUCCESS) {
                ...
                PrintAndLogEx(INFO, "%s <访问数据>", label);
...
static void aliro_step_up_print_issuer_signed_item(const CborValue *raw_item, size_t item_idx, const aliro_print_format_t *fmt) {
    ...
    if (aliro_cbor_decode_value_as_type(raw_item, CborMapType, &item_decoded) != PM3_SUCCESS) {
        if (item_decoded.state == ALIRO_CBOR_DECODE_NOT_TAG24) {
            ...
            PrintAndLogEx(INFO, "%*sIssuerSignedItem[%zu]... <无效>", (int)indent, "", item_idx);
        }
...
static int aliro_step_up_print_device_response(const uint8_t *device_response, size_t device_response_len) {
    ...
    if (err != CborNoError) {
        PrintAndLogEx(ERR, "解析 DeviceResponse CBOR 失败 (%s)", cbor_error_string(err));
        return PM3_ESOFT;
    }
    if (!cbor_value_is_map(&root)) {
        PrintAndLogEx(ERR, "DeviceResponse 不是 CBOR 映射");
        return PM3_ESOFT;
    }
...
static int aliro_read_do_step_up(aliro_read_state_t *state,
    ...
    if (!state->standard_result.keys.step_up_keys_present) {
        PrintAndLogEx(ERR, "Step-up 密钥不可用");
        return PM3_ESOFT;
    }
    ...
    if (document_type_count == 0) {
        PrintAndLogEx(WARNING, "信令位图未指示可检索的升级文档");
        return PM3_SUCCESS;
    }

    if (state->auth1_parsed.signaling_bitmap & ALIRO_SIGNALING_ACCESS_DOCUMENT_RETRIEVABLE) {
        PrintAndLogEx(INFO, "信令位0............ 已设置 -> 请求 `%s`", ALIRO_ACCESS_DOCUMENT_TYPE);
    }
    if (state->auth1_parsed.signaling_bitmap & ALIRO_SIGNALING_REVOCATION_DOCUMENT_RETRIEVABLE) {
        PrintAndLogEx(INFO, "信令位1............ 已设置 -> 请求 `%s`", ALIRO_REVOCATION_DOCUMENT_TYPE);
    }

    if (document_type_count == 2) {
        PrintAndLogEx(INFO, "请求的文档类型....... %s, %s", document_types[0], document_types[1]);
    } else {
        PrintAndLogEx(INFO, "请求的文档类型........ %s", document_types[0]);
    }
    ...
    if (state->auth1_parsed.signaling_bitmap & ALIRO_SIGNALING_STEP_UP_SELECT_REQUIRED_FOR_DOC_RETRIEVAL) {
        ...
        if (sel_res != PM3_SUCCESS) {
            PrintAndLogEx(ERR, "Step-up SELECT APDU 交换失败");
            return sel_res;
        }
        if (stepup_select_sw != ISO7816_OK) {
            PrintAndLogEx(ERR, "Step-up SELECT 失败：%04x - %s",
                          stepup_select_sw, GetAPDUCodeDescription(stepup_select_sw >> 8, stepup_select_sw & 0xff));
            return PM3_ESOFT;
        }
        PrintAndLogEx(INFO, "Step-up SELECT............ 正常");
    }
    ...
    PrintAndLogEx(INFO, "设备请求 CBOR........ %s", sprint_hex_inrow(device_request, device_request_len));
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "加密升级 DeviceRequest 失败");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "编码ENVELOPE命令失败");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "ENVELOPE APDU 交换失败");
        return res;
    }
    if (envelope_sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "ENVELOPE 失败: %04x - %s",
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "ENVELOPE 响应不包含标签 0x53");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "解密ENVELOPE DeviceResponse失败");
        return res;
    }

    PrintAndLogEx(INFO, "设备响应 CBOR....... %s",
...
static int aliro_read_auth_flow(const uint8_t *kpersistent, size_t kpersistent_len,
    ...
    do {
        ...
        if (fast_flow_complete) {
            ...
            if (res != PM3_SUCCESS) {
                PrintAndLogEx(WARNING, "快速认证后完成EXCHANGE失败，继续执行");
            }
            status = PM3_SUCCESS;
            break;
        }
        ...
        res = aliro_send_reader_status_exchange(completion_channel,
                                                ALIRO_READER_STATUS_STATE_UNSECURE);
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "%s认证后完成EXCHANGE失败，继续执行", completion_flow_name);
        }
...
static int CmdHFAliroRead(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf aliro 读取",
                  "执行ALIRO加速流程和可选的升级文档检索。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("k", "k-persistent,key-persistent,kpersistent,keypersistent,kp", "<hex>", "Kpersistent（32字节，可选；用于快速密码验证）"),
        arg_str1("g", "reader-group-id,readergroupid,rgi", "<hex>", "读取器组标识符（16 字节）"),
        arg_str0("s", "reader-sub-group-id,readersubid,rsi", "<hex>", "读取器子组标识符（16 字节，默认：全零）"),
        arg_str1("p", "reader-private-key,readerprivkey,rpk", "<pem|der-b64|der-hex|scalar-b64|scalar-hex|path>", "读取器私钥（P-256）：PEM、DER 十六进制、标量十六进制/base64 或文件路径"),
        arg_str0("t", "transaction-id,ti", "<hex>", "交易标识符（16 字节，可选；省略则随机生成）"),
        arg_str0("e", "endpoint-public-key,endpointpublickey,epk", "<hex>", "用于AUTH0快速验证的端点公钥（32字节X或65字节未压缩）"),
        arg_str0("f", "flow", "<step-up|standard|fast>", "交易流程（默认：step-up）"),
    ...
    if (reader_group_identifier_len != 16) {
        PrintAndLogEx(ERR, "readergroupid 必须是 16 字节（实际 %d）", reader_group_identifier_len);
        return PM3_EINVARG;
    }
    if (reader_group_sub_identifier_len != 0 && reader_group_sub_identifier_len != 16) {
        PrintAndLogEx(ERR, "readersubid 必须是 16 字节（实际 %d）", reader_group_sub_identifier_len);
        return PM3_EINVARG;
    }
    if (ensure_ec_private_key(reader_private_key_text, MBEDTLS_ECP_DP_SECP256R1,
        ...
        PrintAndLogEx(ERR, "readerprivkey 格式无效");
        PrintAndLogEx(INFO, "接受的格式:");
    ...
    if (kpersistent_len != 0 && kpersistent_len != 32) {
        PrintAndLogEx(ERR, "kpersistent 提供时必须为32字节（实际为 %d）", kpersistent_len);
        return PM3_EINVARG;
    }
    if (transaction_identifier_len != 0 && transaction_identifier_len != 16) {
        PrintAndLogEx(ERR, "transaction-id 若提供则必须为16字节（实际为 %d）", transaction_identifier_len);
        return PM3_EINVARG;
    }
    ...
    if (endpoint_public_key_input_len != 0) {
        if (endpoint_public_key_input_len == 32) {
            ...
            if (key_res != PM3_SUCCESS) {
                PrintAndLogEx(ERR, "endpoint-public-key不是有效的未压缩P-256密钥");
                return PM3_EINVARG;
            }
        } else {
            PrintAndLogEx(ERR, "endpoint-public-key必须为32字节（X坐标）或65字节（未压缩）（实际为%d）", endpoint_public_key_input_len);
            return PM3_EINVARG;
        }
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdHFAliroList, AlwaysAvailable, "列出ISO 14443A/7816历史"},
    {"-----------", CmdHelp,        IfPm3Iso14443a,  "--------------------- " _CYAN_("操作") " ----------------------"},
    {"info",        CmdHFAliroInfo, IfPm3Iso14443a,  "获取Aliro小程序信息"},
    {"read",        CmdHFAliroRead, IfPm3Iso14443a,  "运行SELECT-AUTH0-AUTH1及可选的升级文档检索"},
    {NULL, NULL, NULL, NULL}
};
