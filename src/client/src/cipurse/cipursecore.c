...
static int CIPURSEExchangeEx(bool activate_field, bool leave_field_on, sAPDU_t apdu, bool include_le,
    ...
    if (result_len == NULL) {
        PrintAndLogEx(FAILED, "CIPURSEExchangeEx, result_len 为空");
        return PM3_EINVARG;
    }
    ...
    *result_len = rlen;
    ...
    if (isw != ISO7816_OK) {
        if (GetAPDULogging()) {
            if (*sw >> 8 == 0x61) {
                PrintAndLogEx(ERR, "APDU链接长度:%02x -->", *sw & 0xff);
            } else {
                PrintAndLogEx(ERR, "APDU(%02x%02x)错误: [%4X] %s", apdu.CLA, apdu.INS, isw, GetAPDUCodeDescription(*sw >> 8, *sw & 0xff));
                return 5;
            }
        }
    }
...
static void CIPURSEPrintManufacturerInfo(uint8_t data) {
    if (data == 0)
        PrintAndLogEx(INFO, "制造商... 无");
    else
        PrintAndLogEx(INFO, "制造商... %s", getTagInfo(data)); // getTagInfo from cmfhf14a.h
}

void CIPURSEPrintInfoFile(uint8_t *data, size_t len) {
    if (len < 2) {
        PrintAndLogEx(FAILED, "信息文件长度太短");
        return;
    }
...
void CIPURSEPrintFileDescriptor(uint8_t desc) {
    if (desc == 0x01)
        PrintAndLogEx(INFO, "二进制文件");
    else if (desc == 0x11)
        PrintAndLogEx(INFO, "带事务的二进制文件");
    else if (desc == 0x02)
        PrintAndLogEx(INFO, "线性记录文件");
    else if (desc == 0x12)
        PrintAndLogEx(INFO, "带事务的线性记录文件");
    else if (desc == 0x06)
        PrintAndLogEx(INFO, "循环记录文件");
    else if (desc == 0x16)
        PrintAndLogEx(INFO, "带事务的循环记录文件");
    else if (desc == 0x1E)
        PrintAndLogEx(INFO, "线性值记录文件");
    else if (desc == 0x1F)
        PrintAndLogEx(INFO, "带事务的线性值记录文件");
    else
        PrintAndLogEx(INFO, "未知文件 0x%02x", desc);
}

void CIPURSEPrintDGIArray(uint8_t *dgi, size_t dgilen) {
    if (dgilen < 3) {
        PrintAndLogEx(WARNING, "DGI 太小。长度: %zu", dgilen);
        return;
    }
...
void CIPURSEPrintDGI(uint8_t *dgi, size_t dgilen) {
    ...
    uint8_t len = dgi[2];
    if (len + 3 != dgilen) {
        PrintAndLogEx(ERR, "DGI 大小与记录大小不匹配。记录长度: %zu，DGI 大小: %d", dgilen, len);
        return;
    }

    // check DGI
    if (dgi[0] == 0x92 && dgi[1] == 0x00) {
        PrintAndLogEx(INFO, "DGI 9200 - ADF 文件属性");
        CIPURSEPrintFileAttrEx(&dgi[3], len, true);

    } else if (dgi[0] == 0x92 && dgi[1] == 0x01) {
        PrintAndLogEx(INFO, "DGI 9201 - EF 文件属性");
        CIPURSEPrintFileAttrEx(&dgi[3], len, true);

    } else if (dgi[0] == 0xa0 && dgi[1] == 0x0f) {
        PrintAndLogEx(INFO, "DGI a00f - 所有密钥值");

        if (len % 20 != 0) {
            PrintAndLogEx(ERR, "密钥值大小必须是20字节记录的数组。ADF大小: %d", len);
            return;
        }
        ...
        for (int i = 0; i < len / 20; i++) {
            ...
            PrintAndLogEx(INFO, "密钥[%d]............ %s", i + 1, sprint_hex_inrow(aeskey, sizeof(aeskey)));
        ...
        PrintAndLogEx(WARNING, "未知的DGI %02x%02x", dgi[0], dgi[1]);
    }
}
...
static void CIPURSEPrintKeyAttrib(uint8_t *attr) {
    ...
    PrintAndLogEx(INFO, "附加信息... 0x%02x", attr[0]);
    PrintAndLogEx(INFO, "密钥长度........ %d", attr[1]);
    PrintAndLogEx(INFO, "算法ID...... 0x%02x (%s)", attr[2], (attr[2] == 0x09) ? "AES" : "unknown");
    PrintAndLogEx(INFO, "安全属性... 0x%02x", attr[3]);
...
static void CIPURSEPrintKeyAttribDGI(uint8_t *attr) {
    ...
    PrintAndLogEx(INFO, "安全属性... 0x%02x", attr[0]);
...
void CIPURSEPrintEFFileAttr(uint8_t *attr, size_t len) {
    ...
    if (attr[1] == 0)
        PrintAndLogEx(INFO, "SFI.... 未分配");
    else
        PrintAndLogEx(INFO, "SFI.... 0x%02x", attr[1]);

    PrintAndLogEx(INFO, "文件 ID... 0x%02x%02x", attr[2], attr[3]);

    if (attr[0] == 0x01 || attr[0] == 0x11)
        PrintAndLogEx(INFO, "文件大小... %d", (attr[4] << 8) + attr[5]);
    else
        PrintAndLogEx(INFO, "Record num " _YELLOW_("%d") " record size " _YELLOW_("%d"), attr[4], attr[5]);

    PrintAndLogEx(INFO, "已分配密钥... %d", attr[6]);

    if (len >= 9) {
        PrintAndLogEx(INFO, "SMR 条目... %02x%02x", attr[7], attr[8]);
...
void CIPURSEPrintFileAttrEx(uint8_t *attr, size_t len, bool isDGI) {
    if (len < 7) {
        PrintAndLogEx(FAILED, "属性长度太短");
        return;
    }

    PrintAndLogEx(INFO, "--- " _CYAN_("File Attributes") "---------------------");
    if (attr[0] == 0x38 || attr[0] == 0x3F) {
        PrintAndLogEx(INFO, "类型... MF, ADF");

        if (attr[1] == 0x00) {
            if (attr[0] == 0x3F)
                PrintAndLogEx(INFO, "类型... PxSE");
            else
                PrintAndLogEx(INFO, "类型... MF");
        } else {
            if ((attr[1] & 0xe0) == 0x00)
                PrintAndLogEx(INFO, "类型... 未知");

            if ((attr[1] & 0xe0) == 0x20)
                PrintAndLogEx(INFO, "类型... CIPURSE L");

            if ((attr[1] & 0xe0) == 0x40)
                PrintAndLogEx(INFO, "类型... CIPURSE S");

            if ((attr[1] & 0xe0) == 0x60)
                PrintAndLogEx(INFO, "类型... CIPURSE T");

            if ((attr[1] & 0x02) == 0x00)
                PrintAndLogEx(INFO, "PxSE 选择关闭时自动选择关闭");
            else
                PrintAndLogEx(INFO, "PxSE 选择开启时自动选择开启");

            if ((attr[1] & 0x01) == 0x00)
                PrintAndLogEx(INFO, "PxSE 选择返回 FCPTemplate 关闭");
            else
                PrintAndLogEx(INFO, "PxSE 选择返回 FCPTemplate 开启");
        }

        PrintAndLogEx(INFO, "文件 ID................... 0x%02x%02x", attr[2], attr[3]);
        PrintAndLogEx(INFO, "最大自定义EF数量...... %d", attr[4]);
        PrintAndLogEx(INFO, "最大带SFID的EF数量... %d", attr[5]);

        uint8_t keynum = attr[6];
        PrintAndLogEx(INFO, "已分配密钥... %d", keynum);

        int idx = 7;
        if (keynum > 0) {
            if (len >= idx + 2) {
                PrintAndLogEx(INFO, "SMR 条目... %02x%02x", attr[idx], attr[idx + 1]);
            ...
            if (len >= idx + keynum * reclen) {
                for (int i = 0; i < keynum; i++) {
                    PrintAndLogEx(INFO, "密钥 %d 属性... %s", i + 1, sprint_hex(&attr[idx + i * reclen], reclen));
                    if (isDGI)
        ...
        if (len >= idx + 1) {
            ...
            if (attr[1] == 0x00 && attr[0] != 0x3F)
                xlen = xlen - 6;
            if (xlen > 0 && xlen < 200) {
                PrintAndLogEx(INFO, "TLV文件控制参数... [%d] %s", xlen, sprint_hex(&attr[idx], xlen));
        ...
        // MF only
        if (attr[1] == 0x00 && attr[0] != 0x3F) {
            PrintAndLogEx(INFO, "总内存大小... %d", (attr[len - 6] << 16) + (attr[len - 5] << 8) + attr[len - 4]);
            PrintAndLogEx(INFO, "空闲内存大小.... %d", (attr[len - 3] << 16) + (attr[len - 2] << 8) + attr[len - 1]);

        }
    } else {
        PrintAndLogEx(INFO, "类型... EF");
