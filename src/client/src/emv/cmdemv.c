...
static int emv_parse_track1(const uint8_t *d, size_t n, bool verbose) {
    ...
    while (token != NULL) {
        ...
        switch (i) {
            ...
            case 1:
                PrintAndLogEx(INFO, "持卡人............... %s", token);
                break;
            case 2:
                if (strlen(token) < 14) {
                    break;
                }
                PrintAndLogEx(INFO, "到期日期.............. %.*s ( %c%c/%c%c )", 4, token, token[2], token[3], token[0], token[1]);
                token += 4;

                PrintAndLogEx(INFO, "服务代码............. %.*s", 3, token);
                token += 3;

                PrintAndLogEx(INFO, "未知.................. %.*s", 4, token);
                token += 4;

                PrintAndLogEx(INFO, "CVV / iCvv............... %.*s", 3, token);
                token += 3;

                PrintAndLogEx(INFO, "尾部................. %s", token);
                break;
            default:
                break;
        }
...
static int emv_parse_track2(const uint8_t *d, size_t n, bool verbose) {
    ...
    PrintAndLogEx(INFO, "到期日期.............. %.*s ( %c%c/%c%c )", 4, tmp, tmp[2], tmp[3], tmp[0], tmp[1]);
    tmp += 4;

    PrintAndLogEx(INFO, "服务代码............. %.*s", 3, tmp);
    tmp += 3;

    PrintAndLogEx(INFO, "PIN验证值... %.*s", 4, tmp);
    tmp += 4;

    PrintAndLogEx(INFO, "CVV / iCvv............... %.*s", 3, tmp);
    tmp += 3;

    PrintAndLogEx(INFO, "尾部................. %s", tmp);
...
static int CmdEMVSelect(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("k",  "keep",    "保持场以执行下一条命令"),
        arg_lit0("a",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",     "TLV解码结果"),
        arg_lit0("w",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
    ...
    if (sw)
        PrintAndLogEx(INFO, "APDU响应状态: %04x - %s", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
...
static int CmdEMVSmartToNFC(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "EMV 智能卡转NFC",
                  "执行ISO14443a支付，使用ISO7816接口进行认证的TX",
                  "emv smart2nfc -t     -> test that the attached card is working (must be VISA)\n");

    void *argtable[] = {
        arg_param_begin,
        arg_lit0("t",  "test",    "测试连接的卡片是否工作（必须是VISA卡）"),
        arg_str0("u", "uid", "<hex>", "可选7个十六进制字节UID"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (uidlen == 0) {
        ...
        PrintAndLogEx(FAILED, "UID长度必须为7字节。");
        return PM3_EINVARG;
    }

    PrintAndLogEx(SUCCESS, "UID 长度为 %d", uidlen);

    bool testMode = arg_get_lit(ctx, 1);
    bool show_apdu = true;

    if (testMode) {
        PrintAndLogEx(SUCCESS, "测试模式已启用。");
    } else {
        PrintAndLogEx(SUCCESS, "测试模式已禁用。");
    }
...
static int CmdEMVSearch(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("s",  "select",  "激活场并选择卡片"),
        arg_lit0("k",  "keep",    "保持场开启以执行下一条命令"),
        arg_lit0("a",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",     "选定小程序的TLV解码结果"),
        arg_lit0("w",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdEMVPPSE(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("s",  "select",  "激活场并选择卡片"),
        arg_lit0("k",  "keep",    "保持场开启以执行下一条命令"),
        arg_lit0("1",  "pse",     "PSE（1PAY.SYS.DDF01）模式"),
        arg_lit0("2",  "ppse",    "PPSE（2PAY.SYS.DDF01）模式（默认）"),
        arg_lit0("a",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",     "选定小程序的TLV解码结果"),
        arg_lit0("w",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
        arg_param_end
...
static int CmdEMVGPO(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("k",  "keep",    "保持场开启以执行下一条命令"),
        arg_lit0("p",  "params",  "从`emv_defparams.json`文件加载参数，用于从PDOL和参数生成PDOLdata"),
        arg_lit0("m",  "make",    "从PDOL（标签9F38）和参数生成PDOLdata（默认：使用默认参数）"),
        arg_lit0("a",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",     "选定小程序的TLV解码结果"),
        arg_lit0("w",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
    ...
    if (dataMakeFromPDOL) {
        ...
        if (!pdol_data_tlv) {
            PrintAndLogEx(ERR, "无法创建 PDOL TLV。");
            tlvdb_free(tmp_ext);
            tlvdb_free(tlvRoot);
            SetAPDULogging(false);
            return PM3_ESOFT;
        ...
        if (paramsLoadFromFile) {
            PrintAndLogEx(WARNING, "无需加载参数。发送明文PDOL数据...");
        }
    ...
    PrintAndLogEx(INFO, "PDOL 数据[%zu]: %s", pdol_data_tlv_data_len, sprint_hex(pdol_data_tlv_data, pdol_data_tlv_data_len));
...
static int CmdEMVReadRecord(const char *Cmd) {
    ...
    if (datalen != 2) {
        PrintAndLogEx(ERR, "命令需要有 2 字节数据");
        return PM3_EINVARG;
    }
...
static int CmdEMVAC(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("k",  "keep",     "保持场开启以执行下一条命令"),
        arg_lit0("c",  "cda",      "执行CDA交易。需要在结果中获取SDAD。"),
        arg_str0("d",  "decision", "<aac|tc|arqc>", "终端决策。aac - 拒绝，tc - 批准，arqc - 请求在线授权"),
        arg_lit0("p",  "params",   "从`emv_defparams.json`文件加载参数，用于从CDOL和参数生成CDOLdata"),
        arg_lit0("m",  "make",     "从CDOL（标签8C和8D）和参数生成CDOLdata（默认：使用默认参数）"),
        arg_lit0("a",  "apdu",     "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",      "选定小程序的TLV解码结果"),
        arg_lit0("w",  "wired",    "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
    ...
    if (arg_get_str_len(ctx, 3)) {
        ...
        if (termDecision == 0xFF) {
            PrintAndLogEx(ERR, "错误：找不到终端决策 '%s'", arg_get_str(ctx, 3)->sval[0]);
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    } else {
    ...
    if (dataMakeFromCDOL) {
        ...
        if (!cdol_data_tlv) {
            PrintAndLogEx(ERR, "无法创建 CDOL TLV。");
            tlvdb_free(tmp_ext);
            tlvdb_free(tlvRoot);
            SetAPDULogging(false);
            return PM3_ESOFT;
        ...
        if (paramsLoadFromFile) {
            PrintAndLogEx(WARNING, "无需加载参数。发送明文CDOL数据...");
        }
        cdol_data_tlv = &data_tlv;
    }

    PrintAndLogEx(INFO, "CDOL 数据[%zu]: %s", cdol_data_tlv->len, sprint_hex(cdol_data_tlv->value, cdol_data_tlv->len));
...
static int CmdEMVGenerateChallenge(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "挑战: %s", sprint_hex(buf, len));
...
static int CmdEMVInternalAuthenticate(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("k",  "keep",    "保持场开启以执行下一条命令"),
        arg_lit0("p",  "params",  "从`emv_defparams.json`文件加载参数，用于从DDOL和参数生成DDOLdata"),
        arg_lit0("m",  "make",    "从DDOL（标签9F49）和参数生成DDOLdata（默认：使用默认参数）"),
        arg_lit0("a",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",     "选定小程序的TLV解码结果"),
        arg_lit0("w",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
    ...
    if (dataMakeFromDDOL) {
        ...
        if (!ddol_data_tlv) {
            PrintAndLogEx(ERR, "无法创建 DDOL TLV。");
            tlvdb_free(tmp_ext);
            tlvdb_free(tlvRoot);
            SetAPDULogging(false);
            return PM3_ESOFT;
        ...
        if (paramsLoadFromFile) {
            PrintAndLogEx(WARNING, "无需加载参数。发送明文DDOL数据...");
        }
        ddol_data_tlv = &data_tlv;
    }

    PrintAndLogEx(INFO, "DDOL数据[%zu]: %s", ddol_data_tlv->len, sprint_hex(ddol_data_tlv->value, ddol_data_tlv->len));
...
static void InitTransactionParameters(struct tlvdb *tlvRoot, bool paramLoadJSON, enum TransactionType TrType, bool GenACGPO) {
    ...
    if (paramLoadJSON) {
        PrintAndLogEx(INFO, "* * 从 JSON 加载交易参数...");
        ParamLoadFromJson(tlvRoot);
    }
...
static void ProcessGPOResponseFormat1(struct tlvdb *tlvRoot, uint8_t *buf, size_t len, bool decodeTLV) {
    if (buf[0] == 0x80) {
        if (decodeTLV) {
            PrintAndLogEx(SUCCESS, "GPO 响应格式1：");
            TLVPrintFromBuffer(buf, len);
        }

        if (len < 4 || (len - 4) % 4) {
            PrintAndLogEx(ERR, "GPO 响应格式1解析错误。长度 = %zu", len);
        } else {
            // AIP
...
static int CmdEMVExec(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("a",  "apdu",     "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",      "TLV解码结果"),
        arg_lit0("j",  "jload",    "从`emv_defparams.json`文件加载交易参数"),
        arg_lit0(NULL, "force",    "Force search AID. Search AID instead of execute PPSE"),
        arg_rem("By default:",     "交易类型 - MSD"),
        arg_lit0(NULL, "qvsdc",    "Transaction type - qVSDC or M/Chip"),
        arg_lit0("c",  "qvsdccda", "交易类型 - qVSDC 或 M/Chip plus CDA（生成 SDAD）"),
        arg_lit0("x",  "vsdc",     "交易类型 - VSDC。仅用于测试。非标准行为"),
        arg_lit0("g",  "acgpo",    "VISA。从 GPO 生成 AC"),
        arg_lit0("w",  "wired",    "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (IfPm3Smartcard() == false) {
        if (channel == CC_CONTACT) {
            PrintAndLogEx(WARNING, "PM3 不支持 SMARTCARD。退出。");
    ...
    if (!forceSearch) {
        ...
        if (res) {
            PrintAndLogEx(INFO, "检查PPSE而非PSE，反之亦然...");
    ...
    if (!AIDlen) {
        PrintAndLogEx(WARNING, "无法选择AID。未找到EMV AID");
        dreturn(PM3_ERFTRANS);
    }

    // Select
    PrintAndLogEx(INFO, "\\n* 正在选择AID:%s", sprint_hex_inrow(AID, AIDlen));
    SetAPDULogging(show_apdu);
    res = EMVSelect(channel, false, true, AID, AIDlen, buf, sizeof(buf), &len, &sw, tlvRoot);

    if (res) {
        PrintAndLogEx(WARNING, "无法选择AID (%d)。退出...", res);
        dreturn(PM3_ERFTRANS);
    }
    ...
    if (!pdol_data_tlv) {
        PrintAndLogEx(ERR, "错误：无法创建 PDOL TLV。");
        dreturn(PM3_ESOFT);
    }
    ...
    unsigned char *pdol_data_tlv_data = tlv_encode(pdol_data_tlv, &pdol_data_tlv_data_len);
    if (!pdol_data_tlv_data) {
        PrintAndLogEx(ERR, "错误：无法创建 PDOL 数据。");
        dreturn(PM3_ESOFT);
    }
    PrintAndLogEx(INFO, "PDOL 数据[%zu]: %s", pdol_data_tlv_data_len, sprint_hex(pdol_data_tlv_data, pdol_data_tlv_data_len));
    ...
    if (res) {
        PrintAndLogEx(ERR, "GPO 错误(%d)：%4x。退出...", res, sw);
        dreturn(PM3_ERFTRANS);
    }
    ...
    // extract PAN from track2
    {
        ...
        if (!tlvdb_get(tlvRoot, 0x5a, NULL) && track2 && track2->len >= 8) {
            ...
            if (pan) {
                ...
                PrintAndLogEx(INFO, "\\n* * 从磁道2提取的PAN：%s", sprint_hex(pantlv->value, pantlv->len));
            } else {
    ...
    if (!AFL || !AFL->len) {
        PrintAndLogEx(WARNING, "警告：未找到AFL。");
    }

    while (AFL && AFL->len) {
        if (AFL->len % 4) {
            PrintAndLogEx(WARNING, "警告：AFL长度错误：%zu", AFL->len);
            break;
        }
        ...
        for (int i = 0; i < AFL->len / 4; i++) {
            ...
            PrintAndLogEx(INFO, "* * SFI[%02x] 起始:%02x 结束:%02x 离线计数:%02x", SFI, SFIstart, SFIend, SFIoffline);
            if (SFI == 0 || SFI == 31 || SFIstart == 0 || SFIstart > SFIend) {
                PrintAndLogEx(WARNING, "SFI 错误！已跳过...");
                continue;
            }
            ...
            for (int n = SFIstart; n <= SFIend; n++) {
                ...
                if (res) {
                    PrintAndLogEx(WARNING, "错误 SFI[%02x]。APDU 错误 %4x", SFI, sw);
                    continue;
                }
                ...
                if (SFIoffline > 0) {
                    if (SFI < 11) {
                        ...
                        if (tlv_parse_tl(&abuf, &elmlen, &e)) {
                            ...
                            PrintAndLogEx(WARNING, "错误 SFI[%02x]。创建离线数据认证输入列表错误。", SFI);
                        }
                    } else {
    ...
    if (ODAiListLen) {
        ...
        PrintAndLogEx(INFO, "* 离线数据认证输入列表已添加到 TLV。长度=%zu \\n", ODAiListLen);
    }
        ...
        if (AC) {
            ...
            if (ATC) {
                ...
                if (IAD) {
                    ...
                    PrintAndLogEx(WARNING, "警告：未找到IAD。");
                }

            } else {
                PrintAndLogEx(WARNING, "警告 AC：未找到应用交易计数器（ATC）。");
            }
        }
    }
    ...
    if (GetCardPSVendor(AID, AIDlen) == CV_MASTERCARD && (TrType == TT_QVSDCMCHIP || TrType == TT_CDA)) {
        ...
        if (CDOL1 && GetCardPSVendor(AID, AIDlen) == CV_MASTERCARD) { // and m/chip transaction flag
            ...
            PrintAndLogEx(INFO, "* * 生成挑战码");
            res = EMVGenerateChallenge(channel, true, buf, sizeof(buf), &len, &sw, tlvRoot);
            if (res) {
                PrintAndLogEx(ERR, "错误 GetChallenge。APDU 错误 %4x", sw);
                dreturn(PM3_ERFTRANS);
            }

            if (len < 4) {
                PrintAndLogEx(ERR, "错误 GetChallenge。挑战长度错误 %zu", len);
                dreturn(PM3_ESOFT);
            }
            ...
            PrintAndLogEx(INFO, "* * 计算 CDOL1");
            struct tlv *cdol_data_tlv = dol_process(tlvdb_get(tlvRoot, 0x8c, NULL), tlvRoot, 0x01); // 0x01 - dummy tag
            if (!cdol_data_tlv) {
                PrintAndLogEx(ERR, "错误：无法创建 CDOL1 TLV。");
                dreturn(PM3_ESOFT);
            }

            PrintAndLogEx(INFO, "CDOL1 数据[%zu]: %s", cdol_data_tlv->len, sprint_hex(cdol_data_tlv->value, cdol_data_tlv->len));
            ...
            if (res) {
                PrintAndLogEx(ERR, "AC1 错误(%d): %4x. 退出...", res, sw);
                dreturn(PM3_ERFTRANS);
            }
            ...
            if (tlvdb_get(ac_tlv, 0x9f4b, NULL)) {
                ...
                if (res) {
                    PrintAndLogEx(WARNING, "CDA 错误 (%d)", res);
                }
            } else {
                PrintAndLogEx(INFO, "\\n* 签名动态应用数据(0x9f4b)不存在");
            }
            ...
            if (CID) {
                ...
                if (CID->len) {
                    switch (CID->value[0] & EMVAC_AC_MASK) {
                        case EMVAC_AAC:
                            PrintAndLogEx(INFO, "交易被拒绝。");
                            break;
                        case EMVAC_TC:
                            PrintAndLogEx(INFO, "交易离线批准。");
                            break;
                        case EMVAC_ARQC:
                            PrintAndLogEx(INFO, "交易在线批准。");
                            break;
                        default:
                            PrintAndLogEx(WARNING, "警告：CID交易代码错误 %2x", CID->value[0] & EMVAC_AC_MASK);
                            break;
                    }
                } else {
                    PrintAndLogEx(WARNING, "警告：CID长度错误 %zu", CID->len);
                }
            } else {
                PrintAndLogEx(WARNING, "警告：未找到CID(9F27)。");
            }
    ...
    if (AIP & 0x8000 && TrType == TT_MSD) {
        ...
        PrintAndLogEx(INFO, "* MSD dCVV 路径。检查 dCVV");

        const struct tlv *track2 = tlvdb_get(tlvRoot, 0x57, NULL);
        if (track2) {
            PrintAndLogEx(INFO, "磁道2: %s", sprint_hex(track2->value, track2->len));

            struct tlvdb *dCVV = GetdCVVRawFromTrack2(track2);
            PrintAndLogEx(INFO, "dCVV 原始数据：");
            ...
            if (GetCardPSVendor(AID, AIDlen) == CV_MASTERCARD) {
                ...
                if (!UDOL) {
                    PrintAndLogEx(INFO, "使用默认 UDOL。");
                }

                struct tlv *udol_data_tlv = dol_process(UDOL ? UDOL : &defUDOL, tlvRoot, 0x01); // 0x01 - dummy tag
                if (!udol_data_tlv) {
                    PrintAndLogEx(ERR, "错误：无法创建 UDOL TLV。");
                    dreturn(PM3_ESOFT);
                }

                PrintAndLogEx(INFO, "UDOL 数据[%zu]: %s", udol_data_tlv->len, sprint_hex(udol_data_tlv->value, udol_data_tlv->len));

                PrintAndLogEx(INFO, "\\n* 万事达卡计算加密校验和(UDOL)");

                res = MSCComputeCryptoChecksum(channel, true, (uint8_t *)udol_data_tlv->value, udol_data_tlv->len, buf, sizeof(buf), &len, &sw, tlvRoot);
                if (res) {
                    PrintAndLogEx(ERR, "错误 计算加密校验和。APDU 错误 %4x", sw);
                    free(udol_data_tlv);
                    dreturn(PM3_ESOFT);
                }
            ...
            PrintAndLogEx(ERR, "错误 MSD：未找到 Track2 数据。");
        }
    }
    ...
    if (GetCardPSVendor(AID, AIDlen) == CV_VISA && (TrType == TT_VSDC || TrType == TT_CDA)) {
        ...
        PrintAndLogEx(INFO, "CDOL1 数据[%zu]: %s", cdol1_data_tlv->len, sprint_hex(cdol1_data_tlv->value, cdol1_data_tlv->len));
        ...
        // AC1 print result
        PrintAndLogEx(NORMAL, "");
        if ((CID & EMVAC_AC_MASK) == EMVAC_AAC)     PrintAndLogEx(INFO, "AC1 结果：AAC（交易拒绝）");
        if ((CID & EMVAC_AC_MASK) == EMVAC_TC)      PrintAndLogEx(INFO, "AC1 结果：TC（交易批准）");
        if ((CID & EMVAC_AC_MASK) == EMVAC_ARQC)    PrintAndLogEx(INFO, "AC1 结果：ARQC（请求在线授权）");
        if ((CID & EMVAC_AC_MASK) == EMVAC_AC_MASK) PrintAndLogEx(INFO, "AC1 结果：RFU");
        ...
        if (IAD && (IAD->len > 1)) {
            ...
            PrintAndLogEx(INFO, "IAD 长度：%zu", IAD->len);
            ...
            PrintAndLogEx(INFO, "加密版本: %d", CryptoVersion);
            PrintAndLogEx(INFO, "派生密钥索引: %d", DerivKeyIndex);

            // Card Verification Results (CVR) decode
            if ((VDDlen - 2) > 0) {
                uint8_t CVRlen = IAD->value[3];
                if (CVRlen == (VDDlen - 2 - 1)) {
                    PrintAndLogEx(INFO, "CVR 长度: %d", CVRlen);
                    PrintAndLogEx(INFO, "CVR: %s", sprint_hex(&IAD->value[4], CVRlen));
                } else {
                    PrintAndLogEx(WARNING, "CVR 长度错误！CVR: %s", sprint_hex(&IAD->value[3], VDDlen - 2));
                }
            }
        ...
        PrintAndLogEx(INFO, "主机响应：`%s`", HostResponse);
        ...
        if (CryptoVersion == 10) {
            ...
            if (AC && (AC->len > 0)) {
                ...
                PrintAndLogEx(INFO, "原始 ARPC: %s", sprint_hex(rawARPC, rawARPClen));

                // here must be calculation of ARPC, but we don't know a bank keys.
                PrintAndLogEx(INFO, "ARPC: 不可用");

            } else {
                PrintAndLogEx(WARNING, "未找到应用密码（AC）。");
            }
        ...
        if ((CID & EMVAC_AC_MASK) == EMVAC_ARQC) {
            ...
            if (!cdol2_data_tlv) {
                PrintAndLogEx(ERR, "错误：无法创建 CDOL2 TLV。");
                free(cdol1_data_tlv);
                dreturn(PM3_ESOFT);
            }

            PrintAndLogEx(INFO, "CDOL2 数据[%zu]: %s", cdol2_data_tlv->len, sprint_hex(cdol2_data_tlv->value, cdol2_data_tlv->len));
...
static int CmdEMVScan(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("a",  "apdu",     "显示 APDU 请求和响应"),
        arg_lit0("t",  "tlv",      "TLV解码结果"),
        arg_lit0("e",  "extract",  "提取TLV元素并填充应用数据"),
        arg_lit0("j",  "jload",    "从`emv_defparams.json`文件加载交易参数"),
        arg_rem("By default:",     "交易类型 - MSD"),
        arg_lit0(NULL,  "qvsdc",   "Transaction type - qVSDC or M/Chip"),
        arg_lit0("c",  "qvsdccda", "交易类型 - qVSDC 或 M/Chip plus CDA（生成 SDAD）"),
        arg_lit0("x",  "vsdc",     "交易类型 - VSDC。仅用于测试。非标准行为"),
        arg_lit0("g",  "acgpo",    "VISA。从 GPO 生成 AC"),
        arg_lit0("m",  "merge",    "将输出文件与卡片数据合并。（警告：文件可能损坏！）"),
        arg_lit0("w",  "wired",    "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
    ...
    if (MergeJSON) {
        ...
        if (!json_is_object(root)) {
            PrintAndLogEx(ERR, "无效的json格式。根必须为对象");
            return PM3_EFILE;
        }
    } else {
        root = json_object();
    ...
    if (channel == CC_CONTACTLESS) {
        ...
        PrintAndLogEx(INFO, "获取 ATR");

        smart_card_atr_t card;
        smart_select(true, &card);
        if (!card.atr_len) {
            PrintAndLogEx(ERR, "无法从智能卡获取ATR。");
            return PM3_ERFTRANS;
        }
    ...
    if (!res) {
        ...
        SetAPDULogging(false);
        PrintAndLogEx(INFO, "AID搜索。");
        if (EMVSearch(channel, false, true, decodeTLV, tlvSelect, false)) {
            PrintAndLogEx(ERR, "未找到任何EMV AID，退出...");
            tlvdb_free(tlvSelect);
            DropFieldEx(channel);
            return PM3_ERFTRANS;
        }
    ...
    if (!AIDlen) {
        PrintAndLogEx(INFO, "无法选择AID。未找到EMV AID，退出中...");
        DropFieldEx(channel);
        return PM3_ERFTRANS;
    }
    ...
    if (res) {
        PrintAndLogEx(ERR, "无法选择AID (%d)，退出...", res);
        tlvdb_free(tlvRoot);
        DropFieldEx(channel);
        return PM3_ERFTRANS;
    }
    ...
    PrintAndLogEx(INFO, "初始化交易参数");
    InitTransactionParameters(tlvRoot, paramLoadJSON, TrType, GenACGPO);

    PrintAndLogEx(INFO, "计算 PDOL");
    ...
    PrintAndLogEx(INFO, "从AFL读取记录");
    const struct tlv *AFL = tlvdb_get(tlvRoot, 0x94, NULL);

    while (AFL && AFL->len) {
        if (AFL->len % 4) {
            PrintAndLogEx(ERR, "AFL 长度错误: %zu", AFL->len);
            break;
        }
        ...
        if (!json_is_array(sfijson)) {
            PrintAndLogEx(ERR, "内部逻辑错误。`$.Application.Records` 不是数组。");
            break;
        }
        ...
        for (int i = 0; i < AFL->len / 4; i++) {
            ...
            for (int n = SFIstart; n <= SFIend; n++) {
                ...
                res = EMVReadRecord(channel, true, SFI, n, buf, sizeof(buf), &len, &sw, tlvRoot);
                if (res) {
                    PrintAndLogEx(ERR, "SFI[%02x]. APDU 错误 %4x", SFI, sw);
                    continue;
                }
    ...
    if (ODAI_listlen) {
        ...
        tlvdb_add(tlvRoot, oda);
        PrintAndLogEx(INFO, "用于离线数据认证的输入列表已添加到 TLV [%zu 字节]", ODAI_listlen);
    }

    // getting certificates
    if (tlvdb_get(tlvRoot, 0x90, NULL)) {
        PrintAndLogEx(INFO, "正在恢复证书");
    ...
    if (res) {
        PrintAndLogEx(ERR, "无法保存文件：%s", filename);
        return PM3_EFILE;
    }
...
static int CmdEMVTest(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("i", "ignore", "忽略VM的时序测试"),
        arg_lit0("l", "long",   "同时运行长时间测试"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdEMVRoca(const char *Cmd) {
    ...
    if (!AIDlen) {
        PrintAndLogEx(INFO, "无法选择AID或未找到EMV AID，退出");
        DropFieldEx(channel);
        return PM3_ERFTRANS;
    }
    ...
    unsigned char *pdol_data_tlv_data = tlv_encode(pdol_data_tlv, &pdol_data_tlv_data_len);
    if (!pdol_data_tlv_data) {
        PrintAndLogEx(ERR, "无法创建PDOL数据，退出");
        tlvdb_free(tlvRoot);
        DropFieldEx(channel);
        free(pdol_data_tlv);
        return PM3_ESOFT;
    ...
    if (tlvdb_get(tlvRoot, 0x90, NULL)) {
        ...
        if (!issuer_pk) {
            emv_pk_free(pk);
            PrintAndLogEx(WARNING, "警告：未找到发行者证书，退出");
            ret = PM3_ESOFT;
            goto out;
        }
        ...
        if (!icc_pk) {
            ...
            PrintAndLogEx(WARNING, "警告：未找到ICC证书，退出");
            ret = PM3_ESOFT;
            goto out;
        }
...
static command_t CommandTable[] =  {
    ...
    {"list",        CmdEMVList,                     AlwaysAvailable, "列出 ISO7816 历史"},
    {"test",        CmdEMVTest,                     AlwaysAvailable, "执行加密逻辑自检"},
    {"-----------", CmdHelp,                        IfPm3Iso14443a,  "---------------------- " _CYAN_("Operations") " ---------------------"},
    {"challenge",   CmdEMVGenerateChallenge,        IfPm3Iso14443,   "生成挑战"},
    {"exec",        CmdEMVExec,                     IfPm3Iso14443,   "执行EMV非接触式交易"},
    {"genac",       CmdEMVAC,                       IfPm3Iso14443,   "生成应用密码"},
    {"gpo",         CmdEMVGPO,                      IfPm3Iso14443,   "执行GetProcessingOptions"},
    {"intauth",     CmdEMVInternalAuthenticate,     IfPm3Iso14443,   "内部认证"},
    {"pse",         CmdEMVPPSE,                     IfPm3Iso14443,   "执行PPSE。选择2PAY.SYS.DDF01或1PAY.SYS.DDF01目录"},
    {"reader",      CmdEMVReader,                   IfPm3Iso14443a,  "模拟 EMV 读取器"},
    {"readrec",     CmdEMVReadRecord,               IfPm3Iso14443,   "从卡片读取文件"},
    {"roca",        CmdEMVRoca,                     IfPm3Iso14443,   "提取公钥并运行ROCA测试"},
    {"scan",        CmdEMVScan,                     IfPm3Iso14443,   "扫描EMV卡并将其内容保存为json文件供模拟器使用"},
    {"search",      CmdEMVSearch,                   IfPm3Iso14443,   "尝试从小程序列表中选择所有小程序并打印已安装的小程序"},
    {"select",      CmdEMVSelect,                   IfPm3Iso14443,   "选择小程序"},
    {"-----------", CmdHelp,                        IfPm3Iso14443a,  "---------------------- " _CYAN_("simulation") " ---------------------"},
    {"smart2nfc",   CmdEMVSmartToNFC,               IfPm3Smartcard,  "作为NFC智能卡完成交易，使用ISO-7816接口进行认证"},
    /*
