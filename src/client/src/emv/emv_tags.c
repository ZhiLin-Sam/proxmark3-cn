    ...
    if (tlv->value[0] & EMVCID_REASON_MASK) {
        ...
        switch ((tlv->value[0] & EMVCID_REASON_MASK)) {
            case 0:
                ...
                break;
            case 1:
                PrintAndLogEx(NORMAL, "服务不允许");
                break;
            case 2:
                PrintAndLogEx(NORMAL, "超过 PIN 尝试限制");
                break;
            case 3:
...
static void emv_tag_dump_afl(const struct tlv *tlv, const struct emv_tag *tag, int level) {
    ...
    for (int i = 0; i < tlv->len / 4; i++) {
        PrintAndLogEx(INFO, "%*s" NOLF, (level * 4), " ");
        PrintAndLogEx(NORMAL, "SFI[%02x] 起始:%02x 结束:%02x 离线:%02x", tlv->value[i * 4 + 0] >> 3, tlv->value[i * 4 + 1], tlv->value[i * 4 + 2], tlv->value[i * 4 + 3]);
    }
}

bool emv_tag_dump(const struct tlv *tlv, int level) {
    if (tlv == NULL) {
        PrintAndLogEx(WARNING, "tlv 为空");
        return false;
    }
