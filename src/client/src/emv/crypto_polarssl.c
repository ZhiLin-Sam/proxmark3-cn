...
static struct crypto_pk *crypto_pk_polarssl_open_rsa(va_list vl) {
    ...
    if (res != 0) {
        PrintAndLogEx(WARNING, "PolarSSL公钥错误 res=%x exp=%d mod=%d", res * -1, explen, modlen);
        free(cp);
        return NULL;
    }
...
// int invlen = va_arg(vl, size_t);
    ...
    if (res != 0) {
        PrintAndLogEx(WARNING, "PolarSSL私钥错误 res=%x exp=%d mod=%d", res * -1, explen, modlen);
        free(cp);
        return NULL;
    }
...
static struct crypto_pk *crypto_pk_polarssl_genkey_rsa(va_list vl) {
    ...
    if (res) {
        PrintAndLogEx(WARNING, "PolarSSL私钥生成错误 res=%x exp=%u nbits=%u", res * -1, exp, nbits);
        free(cp);
        return NULL;
    }
...
static unsigned char *crypto_pk_polarssl_encrypt(const struct crypto_pk *_cp, const unsigned char *buf, size_t len, size_t *clen) {
    ...
    *clen = 0;
    ...
    if (!result) {
        PrintAndLogEx(WARNING, "RSA 加密失败。无法分配结果内存");
        return NULL;
    }

    int res = mbedtls_rsa_public(&cp->ctx, buf, result);
    if (res) {
        PrintAndLogEx(WARNING, "RSA 加密失败。错误: %x 数据长度: %zu 密钥长度: %zu", res * -1, len, keylen);
        free(result);
        return NULL;
    }
...
static unsigned char *crypto_pk_polarssl_decrypt(const struct crypto_pk *_cp, const unsigned char *buf, size_t len, size_t *clen) {
    ...
    *clen = 0;
    ...
    if (res) {
        PrintAndLogEx(WARNING, "RSA 解密失败。错误: %x 数据长度: %zu 密钥长度: %zu", res * -1, len, keylen);
        free(result);
        return NULL;
    }
    ...
    switch (param) {
        ...
        case 1:
            *plen = mbedtls_mpi_size(&cp->ctx.E);
            ...
            break;
        default:
            PrintAndLogEx(WARNING, "错误 获取参数。参数 = %u", param);
            break;
    }
    return result;
}
