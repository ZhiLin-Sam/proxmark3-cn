...
int roca_self_test(void) {
    ...
    PrintAndLogEx(INFO, "ROCA 检查漏洞测试");
    ...
    if (emv_rocacheck(keyn, 64, false)) {
        ret++;
        PrintAndLogEx(FAILED, "强模数（%s）", _RED_("fail"));
    } else {
        PrintAndLogEx(SUCCESS, "强模数（%s）", _GREEN_("ok"));
    }
    return ret;
}
