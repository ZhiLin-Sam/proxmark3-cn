...
static unsigned char *emv_pki_decode_message(const struct emv_pk *enc_pk,
    ...
    if (!cert_tlv) {
        PrintAndLogEx(WARNING, "错误：找不到证书");
        return NULL;
    }

    if (cert_tlv->len != enc_pk->mlen) {
        PrintAndLogEx(WARNING, "错误：证书长度(%zu)与密钥长度(%zu)不相等", cert_tlv->len, enc_pk->mlen);
        return NULL;
    }
    ...
    if (data[data_len - 1] != 0xbc || data[0] != 0x6a || data[1] != msgtype) {
        PrintAndLogEx(WARNING, "错误：证书格式错误");
        free(data);
        return NULL;
    }
    ...
    if (hash_pos == 0 || hash_pos > data_len) {
        PrintAndLogEx(WARNING, "错误：无法获取证书中的哈希位置");
        free(data);
        return NULL;
    }
    ...
    if (!ch) {
        PrintAndLogEx(WARNING, "错误：无法进行哈希运算");
        free(data);
        return NULL;
    }
    ...
    if (memcmp(data + data_len - 1 - hash_len, hash, hash_len)) {
        PrintAndLogEx(WARNING, "错误：计算出的哈希值错误");
...
static struct emv_pk *emv_pki_decode_key_ex(const struct emv_pk *enc_pk,
    ...
    if (msgtype == 2)
        ...
        PrintAndLogEx(WARNING, "错误：消息类型必须为 2 或 4");
        return NULL;
    }
    ...
    if (!data || data_len < 11 + pan_length) {
        PrintAndLogEx(WARNING, "错误：无法解码消息");
        return NULL;
    }

    if (showData) {
        PrintAndLogEx(SUCCESS, "恢复的数据:");
        print_buffer(data, data_len, 1);
    }
    ...
    if (((msgtype == 2) && (pan2_len < 4 || pan2_len > pan_len)) ||
        ...
        PrintAndLogEx(WARNING, "错误：无效的PAN长度");
        free(data);
    ...
    for (i = 0; i < pan2_len; i++)
        if (emv_cn_get(pan_tlv, i) != emv_cn_get(&pan2_tlv, i)) {
            PrintAndLogEx(WARNING, "错误：PAN 数据不匹配");
    ...
    if (pk_len > data_len - 11 - pan_length + rem_tlv->len) {
        PrintAndLogEx(WARNING, "错误：无效的pk长度");
        free(data);
        return NULL;
    }
...
struct tlvdb *emv_pki_perform_cda_ex(const struct emv_pk *enc_pk, const struct tlvdb *db,
    ...
    unsigned char *data = emv_pki_decode_message(enc_pk, 5, &data_len,
                                                 ...
                                                 (uint8_t *)NULL);
    if (!data || data_len < 3) {
        PrintAndLogEx(WARNING, "错误：无法解码消息。[%zu 字节]", data_len);
        return NULL;
    }
    ...
    if (data[3] < 30 || data[3] > data_len - 4) {
        PrintAndLogEx(WARNING, "错误：无效的数据长度");
        free(data);
        return NULL;
    }

    if (!cid_tlv || cid_tlv->len != 1 || cid_tlv->value[0] != data[5 + data[4]]) {
        PrintAndLogEx(WARNING, "错误：CID不匹配");
        free(data);
        return NULL;
    }
    ...
    if (!ch) {
        PrintAndLogEx(WARNING, "错误：无法创建哈希");
        free(data);
        return NULL;
    }
    ...
    if (memcmp(data + 5 + data[4] + 1 + 8, crypto_hash_read(ch), 20)) {
        PrintAndLogEx(WARNING, "错误：计算哈希错误");
        crypto_hash_close(ch);
        free(data);
        return NULL;
    }
    ...
    size_t idn_len = data[4];
    if (idn_len > data[3] - 1) {
        PrintAndLogEx(WARNING, "错误：无效的IDN长度");
        free(data);
        return NULL;
    }
