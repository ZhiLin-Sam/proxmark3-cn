...
bool TLVPrintFromBuffer(uint8_t *data, int datalen) {
    ...
    if (t) {
        ...
        PrintAndLogEx(WARNING, "TLV错误：无法将响应解析为TLV树。");
    }
    return false;
}
    ...
    do {
        ...
        if (res && res != 5) {
            if (++retrycnt < 3) {
                ...
                if (res == 1) {
                    PrintAndLogEx(WARNING, "正在退出...");
                    return 1;
                }

                PrintAndLogEx(FAILED, "重试失败 [%s]。已跳过...", sprint_hex_inrow(AID, AIDLen));
                return res;
            }
        }
    } while (res && res != 5);
...
int EMVSearchPSE(Iso7816CommandChannel channel, bool ActivateField, bool LeaveFieldON, uint8_t PSENum, bool decodeTLV, struct tlvdb *tlv) {
    ...
    if (!res) {
        if (sw != ISO7816_OK) {
            PrintAndLogEx(FAILED, "选择PSE错误。APDU错误: %04x。", sw);
            return 1;
        }
        ...
        if (t) {
            ...
            if (tsfi) {
                ...
                PrintAndLogEx(INFO, "* PPSE 获取 SFI: 0x%02x。", sfin);

                for (uint8_t ui = 0x01; ui <= 0x10; ui++) {
                    PrintAndLogEx(INFO, "* * 获取 SFI: 0x%02x. 编号: 0x%02x", sfin, ui);
                    res = EMVReadRecord(channel, true, sfin, ui, sfidata[ui], APDU_RES_LEN, &sfidatalen[ui], &sw, NULL);

                    // end of records
                    if (sw == 0x6a83) {
                        sfidatalen[ui] = 0;
                        PrintAndLogEx(INFO, "* * PPSE 获取 SFI。记录结束。");
                        break;
                    }
                    ...
                    if (sw != ISO7816_OK) {
                        sfidatalen[ui] = 0;
                        PrintAndLogEx(FAILED, "PPSE 获取错误。APDU 错误: %04x。", sw);
                        break;
                    }
                ...
                for (uint8_t ui = 0x01; ui <= 0x10; ui++) {
                    if (sfidatalen[ui]) {
                        ...
                        if (tsfi_a) {
                            ...
                            if (!tsfitmp) {
                                PrintAndLogEx(FAILED, "SFI 0x%02zu 没有任何记录。", sfidatalen[ui]);
                                continue;
                            }
            ...
            if (!fileFound)
                PrintAndLogEx(FAILED, "PPSE 没有任何记录。");

            tlvdb_free(t);
        } else {
            PrintAndLogEx(WARNING, "%s 错误：无法从响应中获取TLV。", PSE_or_PPSE);
        }
    } else {
        PrintAndLogEx(ERR, "%s 错误：无法选择PPSE AID。错误：%d", PSE_or_PPSE, res);
    }
    ...
    for (int i = 0; i < ARRAYLEN(AIDlist); i ++) {
        ...
        if (res && res != 5) {
            if (++retrycnt < 3) {
                ...
                retrycnt = 0;
                if (verbose) {
                    PrintAndLogEx(FAILED, "重试失败 [%s]。已跳过...", AIDlist[i].aid);
                }
            }
            continue;
        }
...
int EMVReadRecord(Iso7816CommandChannel channel, bool LeaveFieldON, uint8_t SFI, uint8_t SFIrec, uint8_t *Result, size_t MaxResultLen, size_t *ResultLen, uint16_t *sw, struct tlvdb *tlv) {
    ...
    if (*sw == 0x6700 || *sw == 0x6f00) {
        PrintAndLogEx(INFO, ">>> 尝试重新发出不带 Le 的命令...");
...
struct emv_pk *get_ca_pk(struct tlvdb *db) {
    ...
    PrintAndLogEx(INFO, "CA 公钥索引 0x%0x", caidx_tlv->value[0]);
...
int trSDA(struct tlvdb *tlv) {
    ...
    if (!issuer_pk) {
        emv_pk_free(pk);
        PrintAndLogEx(ERR, "错误：未找到发行者证书，退出");
        return 2;
    }
    ...
    if (!sda_tlv || sda_tlv->len < 1) {
        ...
        emv_pk_free(pk);
        PrintAndLogEx(WARNING, "找不到离线数据认证的输入列表，退出");
        return 3;
    }
    ...
    if (dac_db) {
        ...
        PrintAndLogEx(INFO, "SDA 验证通过 ( %s ) (数据认证码: %02hhx:%02hhx)", _GREEN_("ok"), dac_tlv->value[0], dac_tlv->value[1]);
        tlvdb_add(tlv, dac_db);
    } else {
        emv_pk_free(issuer_pk);
        emv_pk_free(pk);
        PrintAndLogEx(ERR, "SSAD 验证错误");
        return 4;
    }
...
int trDDA(Iso7816CommandChannel channel, bool decodeTLV, struct tlvdb *tlv) {
    ...
    */
    ...
    if (!icc_pk) {
        emv_pk_free(pk);
        emv_pk_free(issuer_pk);
        PrintAndLogEx(ERR, "错误：未找到 ICC 证书，退出");
        return 2;
    }
    ...
    if (tlvdb_get(tlv, 0x9f2d, NULL)) {
        ...
        if (!icc_pe_pk) {
            PrintAndLogEx(WARNING, "警告：ICC PE公钥恢复错误");
        } else {
        ...
        PrintAndLogEx(INFO, "未找到 ICC PE 公钥（PIN 加密公钥证书）。\\n");
    }
    ...
    if (sdad_tlv) {
        PrintAndLogEx(INFO, "* * 从 GPO 获取签名动态应用数据 (9F4B)。可能是 fDDA...");

        struct tlvdb *atc_db = emv_pki_recover_atc_ex(icc_pk, tlv, true);
        if (!atc_db) {
            PrintAndLogEx(ERR, "错误：无法恢复 IDN（ICC 动态号码）");
            emv_pk_free(pk);
            emv_pk_free(issuer_pk);
            emv_pk_free(icc_pk);
            return 8;
        ...
        if (atc_tlv) {
            PrintAndLogEx(INFO, "ATC（应用交易计数器）[%zu] %s", atc_tlv->len, sprint_hex_inrow(atc_tlv->value, atc_tlv->len));

            const struct tlv *core_atc_tlv = tlvdb_get(tlv, 0x9f36, NULL);
            if (tlv_equal(core_atc_tlv, atc_tlv)) {
                PrintAndLogEx(SUCCESS, "ATC 检查通过。");
                PrintAndLogEx(SUCCESS, "fDDA（快速 DDA）验证成功。");
            } else {
                PrintAndLogEx(WARNING, "错误：fDDA 验证通过，但证书中的 ATC 与记录中的 ATC 不一致。");
            }
        } else {
            PrintAndLogEx(WARNING, "错误：fDDA（快速 DDA）验证错误");
            emv_pk_free(pk);
            emv_pk_free(issuer_pk);
            emv_pk_free(icc_pk);
            tlvdb_free(atc_db);
        ...
        if (dac_db) {
            const struct tlv *dac_tlv = tlvdb_get(dac_db, 0x9f45, NULL);
            PrintAndLogEx(INFO, "SDAD 验证通过 ( %s ) (数据认证码: %02hhx:%02hhx)\\n", _GREEN_("ok"), dac_tlv->value[0], dac_tlv->value[1]);
            tlvdb_add(tlv, dac_db);
        } else {
            PrintAndLogEx(ERR, "错误：SSAD 验证错误");
            emv_pk_free(pk);
            emv_pk_free(issuer_pk);
            emv_pk_free(icc_pk);
            return 4;
        ...
        if (buf[0] == 0x80) {
            if (len < 3) {
                PrintAndLogEx(WARNING, "警告：内部认证格式1解析错误。长度=%zu", len);
            } else {
                // parse response 0x80
            ...
            if (!dda_db) {
                PrintAndLogEx(ERR, "错误：无法将内部认证结果解析为 TLV");
                free(ddol_data_tlv);
                emv_pk_free(pk);
                emv_pk_free(issuer_pk);
                emv_pk_free(icc_pk);
        ...
        if (idn_tlv) {
            PrintAndLogEx(INFO, "IDN (ICC 动态号码) [%zu] %s", idn_tlv->len, sprint_hex_inrow(idn_tlv->value, idn_tlv->len));
            PrintAndLogEx(INFO, "DDA验证成功。");
            tlvdb_add(tlv, idn_db);
            tlvdb_free(idn_db);
        } else {
            PrintAndLogEx(ERR, "DDA验证错误");
            tlvdb_free(idn_db);
...
int trCDA(struct tlvdb *tlv, struct tlvdb *ac_tlv, struct tlv *pdol_data_tlv, struct tlv *ac_data_tlv) {
    ...
    const struct tlv *sda_tlv = tlvdb_get(tlv, 0x21, NULL);
    if (!sda_tlv || sda_tlv->len < 1) {
        PrintAndLogEx(ERR, "错误：找不到离线数据认证的输入列表，退出");
        emv_pk_free(pk);
        return 3;
    }
    ...
    if (ssad_tlv && ssad_tlv->len > 1) {
        ...
        if (dac_db) {
            const struct tlv *dac_tlv = tlvdb_get(dac_db, 0x9f45, NULL);
            PrintAndLogEx(SUCCESS, "签名静态应用数据 (SSAD) 已验证 ( %s ) (%02hhx:%02hhx)", _GREEN_("ok"), dac_tlv->value[0], dac_tlv->value[1]);
            tlvdb_add(tlv, dac_db);
        } else {
            PrintAndLogEx(ERR, "错误：签名静态应用数据（SSAD）验证错误");
            emv_pk_free(pk);
            emv_pk_free(issuer_pk);
            emv_pk_free(icc_pk);
            return 4;
    ...
    PrintAndLogEx(INFO, "* * 检查签名动态应用数据 (SDAD)");
    ...
    if (idn_db) {
        const struct tlv *idn_tlv = tlvdb_get(idn_db, 0x9f4c, NULL);
        PrintAndLogEx(INFO, "IDN (ICC 动态号码) [%zu] %s", idn_tlv->len, sprint_hex_inrow(idn_tlv->value, idn_tlv->len));
        PrintAndLogEx(SUCCESS, "CDA 验证通过 ( %s )", _GREEN_("ok"));
        tlvdb_add(tlv, idn_db);
    } else {
        PrintAndLogEx(ERR, "错误：CDA验证错误");
    }
