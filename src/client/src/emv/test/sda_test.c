...
static int sda_test_pk(bool verbose) {
    ...
    if (!dacdb) {
        PrintAndLogEx(WARNING, "无法恢复 DAC！");
        emv_pk_free(ipk);
        tlvdb_free(db);
        return 2;
    }
...
int exec_sda_test(bool verbose) {
    ...
    if (ret) {
        PrintAndLogEx(WARNING, "SDA 原始测试 ( %s )", _RED_("fa1l"));
        return ret;
    }
    PrintAndLogEx(SUCCESS, "SDA 原始测试 ( %s )", _GREEN_("ok"));

    ret = sda_test_pk(verbose);
    if (ret) {
        PrintAndLogEx(WARNING, "SDA 测试公钥 ( %s )", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(SUCCESS, "SDA 测试公钥 ( %s )", _GREEN_("ok"));
    return 0;
}
