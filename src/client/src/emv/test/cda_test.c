...
static int cda_test_pk(bool verbose) {
    ...
    if (!ipk) {
        PrintAndLogEx(WARNING, "无法恢复颁发者证书！");
        tlvdb_free(db);
        return 2;
    }
    ...
    if (!iccpk) {
        PrintAndLogEx(WARNING, "无法恢复 ICC 证书！");
        emv_pk_free(ipk);
        tlvdb_free(db);
        return 2;
    }
    ...
    struct tlvdb *idndb = emv_pki_perform_cda(iccpk, db, cda_db,
                                              ...
                                              NULL);
    if (!idndb) {
        PrintAndLogEx(WARNING, "无法恢复 IDN！");
        tlvdb_free(cda_db);
        emv_pk_free(iccpk);
        emv_pk_free(ipk);
        tlvdb_free(db);
...
int exec_cda_test(bool verbose) {
    ...
    if (ret) {
        PrintAndLogEx(WARNING, "CDA 原始测试（ %s ）", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(INFO, "CDA 原始测试（ %s ）", _GREEN_("ok"));

    ret = cda_test_pk(verbose);
    if (ret) {
        PrintAndLogEx(WARNING, "CDA 测试公钥 ( %s )", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(INFO, "CDA 测试公钥 ( %s )", _GREEN_("ok"));
    return 0;
}
