...
static int test_genkey(unsigned int keylength, unsigned char *msg, size_t msg_len, bool verbose) {
    ...
    if (!pk) {
        PrintAndLogEx(WARNING, "错误：密钥生成错误");
        goto out;
    }

    tmp_len = crypto_pk_get_nbits(pk);
    if (tmp_len != keylength) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_get_nbits");
        goto close;
    }

    tmp = crypto_pk_decrypt(pk, msg, msg_len, &tmp_len);
    if (!tmp) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_decrypt");
        goto close;
    }

    tmp2 = crypto_pk_encrypt(pk, tmp, tmp_len, &tmp2_len);
    if (!tmp2) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_encrypt");
        goto free_tmp;
    }
    ...
    if (tmp2_len == msg_len && !memcmp(tmp2, msg, tmp2_len)) {
        ...
        PrintAndLogEx(WARNING, "错误：加密-解密序列长度或数据错误");
    }
...
static int test_pk(bool verbose) {
    ...
    PrintAndLogEx(INFO, "测试公钥接口");

    pubk = crypto_pk_open(PK_RSA,
                          pk_N, sizeof(pk_N),
                          pk_E, sizeof(pk_E));
    if (!pubk) {
        PrintAndLogEx(WARNING, "错误：打开公钥");
        return 1;
    }

    tmp_len = crypto_pk_get_nbits(pubk);
    if (tmp_len != sizeof(pk_N) * 8) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_get_nbits 不匹配");
        goto close_pub;
    }

    tmp = crypto_pk_get_parameter(pubk, 0, &tmp_len);
    if (tmp_len != sizeof(pk_N) || memcmp(tmp, pk_N, tmp_len)) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_get_parameter(0) 模数。参数长度 %zu 长度 %zu", tmp_len, sizeof(pk_N));
        free(tmp);
        goto close_pub;
    }
    free(tmp);
    ...
    if (tmp_len != sizeof(pk_E) || memcmp(tmp, pk_E, tmp_len)) {
        PrintAndLogEx(WARNING, "错误：crypto_pk_get_parameter(1) 指数");
        free(tmp);
        goto close_pub;
    }
    free(tmp);
...
int exec_crypto_test(bool verbose, bool include_slow_tests) {
    ...
    if (ret) {
        PrintAndLogEx(WARNING, "加密原始测试 ( %s )", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(SUCCESS, "加密原始测试 ( %s )", _GREEN_("ok"));

    for (int i = 0; i < ARRAYLEN(keylengths); i++) {
        unsigned int kl = keylengths[i];
        ret = test_genkey(kl, message, kl / 8, verbose);
        if (ret) {
            PrintAndLogEx(WARNING, "加密生成密钥[ %u ]测试 ( %s )", kl, _RED_("fail"));
            return ret;
        }
    }
