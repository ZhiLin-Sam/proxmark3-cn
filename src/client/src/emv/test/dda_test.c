...
int exec_dda_test(bool verbose) {
    ...
    if (ret) {
        PrintAndLogEx(WARNING, "DDA原始测试 ( %s )", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(SUCCESS, "DDA原始测试 ( %s )", _GREEN_("ok"));

    ret = dda_test_pk(verbose);
    if (ret) {
        PrintAndLogEx(WARNING, "DDA测试公钥 ( %s )", _RED_("fail"));
        return ret;
    }
    PrintAndLogEx(SUCCESS, "DDA测试公钥 ( %s )", _GREEN_("ok"));
    return 0;
}
