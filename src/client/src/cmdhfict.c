...
static int ict_select(void) {
    ...
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "选择 SEOS 小程序 aid 失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int CmdHfIctInfo(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf ict 信息",
                  "从ICT编码凭证标签（MIFARE Classic / DESfire）获取信息。",
                  "hf ict 信息");
...
static int CmdHfIctReader(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "派生密钥A... %s", sprint_hex_inrow(key, MIFARE_KEY_SIZE));

    memset(key, 0, sizeof(key));
    derive_mifare_key_b(card.uid, key);
    PrintAndLogEx(INFO, "派生密钥B... %s", sprint_hex_inrow(key, MIFARE_KEY_SIZE));
...
static command_t CommandTable[] = {
    ...
    {"credential", CmdHfIctCredential, IfPm3Iso14443a,  "读取ICT凭证并解码"},
    {"info",       CmdHfIctInfo,       IfPm3Iso14443a,  "标签信息"},
    {"list",       CmdHfIctList,       AlwaysAvailable, "列出ICT历史"},
    {"reader",     CmdHfIctReader,     AlwaysAvailable, "模拟 IS14443-a 读取器"},
    {NULL, NULL, NULL, NULL}
};
