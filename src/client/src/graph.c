...
void AppendGraph(bool redraw, uint16_t clock, int bit) {
    ...
    if ((MAX_GRAPH_TRACE_LEN - g_GraphTraceLen) < half) {
        PrintAndLogEx(DEBUG, "警告：AppendGraph() - 请求超出最大图形长度");
...
void convertGraphFromBitstreamEx(int hi, int low) {
    ...
    if (size == 0) {
        PrintAndLogEx(WARNING, "从图形缓冲区复制失败");
        free(bits);
        return;
    }
...
int GetAskClock(const char *str, bool verbose) {
    ...
    if (verbose || g_debugMode) {
        PrintAndLogEx(SUCCESS, "自动检测时钟速率: %d, 最佳起始位置: %d", clock1, idx);
    }
...
int GetPskCarrier(bool verbose) {
    ...
    // Only print this message if we're not looping something
    if (verbose) {
        PrintAndLogEx(SUCCESS, "自动检测 PSK 载波速率: %d", carrier);
    }
...
int GetPskClock(const char *str, bool verbose) {
    ...
    // Only print this message if we're not looping something
    if (verbose) {
        PrintAndLogEx(SUCCESS, "自动检测时钟速率: %d", clock1);
    }
...
int GetFskClock(const char *str, bool verbose) {
    ...
    if ((fc1 == 10 && fc2 == 8) || (fc1 == 8 && fc2 == 5)) {
        if (verbose) {
            PrintAndLogEx(SUCCESS, "检测到场时钟: FC/%d, FC/%d - 位时钟: RF/%d", fc1, fc2, rf1);
        }
    ...
    PrintAndLogEx(DEBUG, "DEBUG: 检测到未知 FSK 场时钟");
    PrintAndLogEx(DEBUG, "检测到场时钟: FC/%d, FC/%d - 位时钟: RF/%d", fc1, fc2, rf1);
    return 0;
}
...
bool fskClocks(uint8_t *fc1, uint8_t *fc2, uint8_t *rf1, int *firstClockEdge) {
    ...
    if (ans == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 未找到数据");
        free(bits);
        return false;
    }
    ...
    *rf1 = detectFSKClk(bits, size, *fc1, *fc2, firstClockEdge);
    ...
    if (*rf1 == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 时钟检测错误");
        return false;
    }
    return true;
}
...
void add_temporary_marker(uint32_t position, const char *label) {
    ...
    if (strlen(markerLabel) > 30) {
        PrintAndLogEx(WARNING, "临时标记的标签太长！正在截断...");
        markerLabel[30] = '\0';
    }
...
buffer_savestate_t save_buffer8(uint8_t *src, size_t length) {
    ...
    if (length % 4) {
        buffSize++;
        PrintAndLogEx(DEBUG, "(save_buffer8) 新 buffSize = %llu", buffSize);
    }
...
size_t restore_buffer32(buffer_savestate_t saveState, uint32_t *dest) {
    if (saveState.type != sizeof(uint32_t)) {
        PrintAndLogEx(WARNING, "无效的保存状态类型！期望 uint32_t！");
        PrintAndLogEx(WARNING, "缓冲区未修改！\\n");
        return 0;
    }
...
size_t restore_bufferS32(buffer_savestate_t saveState, int32_t *dest) {
    if (saveState.type != (sizeof(int32_t) >> 8)) {
        PrintAndLogEx(WARNING, "无效的保存状态类型！期望 int32_t");
        PrintAndLogEx(WARNING, "缓冲区未修改！\\n");
        return 0;
    }
...
size_t restore_buffer8(buffer_savestate_t saveState, uint8_t *dest) {
    if (saveState.type != sizeof(uint8_t)) {
        PrintAndLogEx(WARNING, "无效的保存状态类型！期望 uint8_t！");
        PrintAndLogEx(WARNING, "缓冲区未修改！\\n");
        return 0;
    }
