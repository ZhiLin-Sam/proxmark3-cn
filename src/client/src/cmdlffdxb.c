...
static void verify_values(uint64_t *animalid, uint32_t *countryid, uint32_t *extended) {
    ...
    if ((*countryid & 0x3FF) != *countryid) {
        *countryid &= 0x3FF;
        PrintAndLogEx(INFO, "国家ID截断为10位:" _YELLOW_("%03d"), *countryid);
    }
...
int demodFDXB(bool verbose) {
    ...
    if (ASKbiphaseDemod(0, 32, 1, 100, false) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B ASK双相解调失败");
        return PM3_ESOFT;
    }
    ...
    if (preambleIndex < 0) {
        ...
        if (preambleIndex == -1)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B 找到的比特太少");
        else if (preambleIndex == -2)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B 前导码未找到");
        else if (preambleIndex == -3)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B 大小不正确: %zu", size);
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B 答案: %d", preambleIndex);
        return PM3_ESOFT;
    }
    ...
    if (size != 104) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - FDX-B 移除奇偶校验错误: %zu", size);
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(SUCCESS, "保留/RFU...... %u (0x%04X)", reservedCode,  reservedCode);
    PrintAndLogEx(SUCCESS, "动物位设置?... %s", animalBit ? _YELLOW_("True") : "False");
    PrintAndLogEx(SUCCESS, "Data block?....... %s  ( 0x%X )", dataBlockBit ? _YELLOW_("True") : "False", extended);
    PrintAndLogEx(SUCCESS, "RUDI 位?......... %s", rudiBit ? _YELLOW_("True") " ( advanced transponder )" : "False");
    PrintAndLogEx(SUCCESS, "用户信息?........ %u %s", userInfo, (userInfo == 0) ? "( RFU )" : "");
    PrintAndLogEx(SUCCESS, "替换编号？... %u %s", replacementNr, replacementNr == 0 ? "( RFU )" : "");
    ...
    if (is_bt_temperature) {
        ...
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(SUCCESS, "检测到Bio-Thermo");
...
static int CmdFdxBDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 FDXB 解调",
                  "尝试查找FDX-B前导码，如果找到则解码/解扰数据",
                  "低频 FDXB 解调"
                 );
...
static int CmdFdxBReader(const char *Cmd) {
    ...
    do {
        ...
        if (curr_div == LF_DIVISOR_125) {
            ...
            if (res != PM3_SUCCESS) {
                PrintAndLogEx(ERR, "切换到134 KHz低频配置失败");
                return res;
            }
        } else {
            ...
            res = lf_setconfig(&config);
            if (res != PM3_SUCCESS) {
                PrintAndLogEx(ERR, "切换到125 KHz低频配置失败");
                return res;
            }
        }
...
static int CmdFdxBClone(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 FDXB 克隆",
                  "将 FDX-B 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_u64_1("c", "country", "<dec>", "国家代码"),
        arg_u64_1("n", "national", "<dec>", "国家代码"),
        arg_str0(NULL, "extended", "<hex>", "extended data"),
        arg_lit0("a", "animal", "可选 - 设置动物位"),
    ...
    PrintAndLogEx(INFO, "国家代码........ %"PRIu32, country_code);
    PrintAndLogEx(INFO, "国家代码....... %"PRIu64, national_code);
    PrintAndLogEx(INFO, "设置动物位...... %c", (is_animal) ? 'Y' : 'N');
    PrintAndLogEx(INFO, "设置数据块位.. %c", (has_extended) ? 'Y' : 'N');
    PrintAndLogEx(INFO, "扩展数据....... 0x%"PRIX32, extended);
...
static int CmdFdxBSim(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "设置动物位...... %c", (is_animal) ? 'Y' : 'N');
    PrintAndLogEx(INFO, "设置数据块位.. %c", (has_extended) ? 'Y' : 'N');
    PrintAndLogEx(INFO, "扩展数据....... 0x%"PRIX16, extended);
    PrintAndLogEx(INFO, "RFU................. 0");
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdFdxBDemod,  AlwaysAvailable, "从 GraphBuffer 解调 FDX-B ISO11784/85 标签"},
    {"reader",  CmdFdxBReader, IfPm3Lf,         "尝试在134kHz读取并提取标签数据"},
    {"clone",   CmdFdxBClone,  IfPm3Lf,         "克隆动物 ID 标签到 T55x7、Q5/T5555 或 EM4305/4469"},
    {"sim",     CmdFdxBSim,    IfPm3Lf,         "模拟动物ID标签"},
    {NULL, NULL, NULL, NULL}
};
