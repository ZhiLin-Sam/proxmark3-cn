...
static int demod_guard_raw(uint8_t *raw, uint8_t rlen) {
    ...
    if (len != 72) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 移除间隔后未产生72比特: %zu, 起始: %zu", len, start_idx);
        return PM3_ESOFT;
    }
    ...
    for (size_t idx = 0; idx < 8; idx++) {
        ...
        PrintAndLogEx(DEBUG, "DEBUG: gProxII 字节 %zu 异或后: %02x", idx, plain[idx]);
    }
...
int demodGuard(bool verbose) {
    ...
    if (ASKbiphaseDemod(0, 64, 0, 0, false) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII ASK双相解调失败");
        return PM3_ESOFT;
    }
    ...
    if (preambleIndex < 0) {
        ...
        if (preambleIndex == -1)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 找到的比特太少");
        else if (preambleIndex == -2)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 未找到前导码");
        else if (preambleIndex == -3)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 大小不正确: %zu", size);
        else if (preambleIndex == -5)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 间隔比特错误");
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII ans: %d", preambleIndex);
        return PM3_ESOFT;
    }
    ...
    size_t len = removeParity(bits_no_spacer, 0, 5, 3, 90); // source, startloc, paritylen, ptype, length_to_run
    if (len != 72) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - gProxII 移除间隔后未产生72比特: %zu, 起始: %zu", len, startIdx);
        return PM3_ESOFT;
    }
    ...
    for (size_t idx = 0; idx < 8; idx++) {
        plain[idx] = ((uint8_t)bytebits_to_byteLSBF(bits_no_spacer + 8 + (idx * 8), 8)) ^ xorKey;
        PrintAndLogEx(DEBUG, "DEBUG: gProxII 字节 %zu 异或后: %02x (异或前 %02x)", idx, plain[idx], bytebits_to_byteLSBF(bits_no_spacer + 8 + (idx * 8), 8));
    }
    ...
    switch (fmtLen) {
        case 36:
            ...
            PrintAndLogEx(DEBUG, "DEBUG: 卡片1: %x", (plain[4] & 1) << 19);
            PrintAndLogEx(DEBUG, "DEBUG: 卡片2: %x", plain[5] << 11);
            PrintAndLogEx(DEBUG, "DEBUG: 卡片3: %x", plain[6] << 3);
            PrintAndLogEx(DEBUG, "DEBUG: 卡片4: %x", (plain[7] & 0xE0) >> 5);
            FC = ((plain[3] & 0x7F) << 7) | (plain[4] >> 1);
            Card = ((plain[4] & 1) << 19) | (plain[5] << 11) | (plain[6] << 3) | ((plain[7] & 0xE0) >> 5);
            break;
        case 26:
...
static int CmdGuardDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 GProxII 解调",
                  "尝试查找Guardall Prox-II前导码，如果找到则解码/解扰数据",
...
static int CmdGuardReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 GProxII 读取",
                  "读取 Guardall 标签",
...
static int CmdGuardClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 GProxII 克隆",
...
static int CmdGuardSim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 GProxII 模拟",
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdGuardDemod,  AlwaysAvailable, "从 GraphBuffer 解调 G Prox II 标签"},
    {"reader",  CmdGuardReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",   CmdGuardClone,  IfPm3Lf,         "克隆Guardall标签到T55x7或Q5/T5555"},
    {"sim",     CmdGuardSim,    IfPm3Lf,         "模拟Guardall标签"},
    {NULL, NULL, NULL, NULL}
};
