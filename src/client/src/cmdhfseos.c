...
static int create_cmac(uint8_t *key, uint8_t *input, uint8_t *out, int input_len, int output_len, int encryption_algorithm) {
    ...
    if (encryption_algorithm == 0x09) {
        ...
        PrintAndLogEx(ERR, _RED_("未知加密算法"));
        return PM3_ESOFT;
    }
...
static int seos_get_data(uint8_t *rndICC, uint8_t *rndIFD, uint8_t *diversified_enc_key, uint8_t *diversified_mac_key, uint8_t *sioOutput,  int *sio_size, int encryption_algorithm, uint8_t *data_tag, int data_tag_len) {
    ...
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "获取数据失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int seos_write_data(uint8_t *rndICC, uint8_t *rndIFD, uint8_t *diversified_enc_key, uint8_t *diversified_mac_key, uint8_t *sio,  int sio_size, int encryption_algorithm, uint8_t *write_data_tag, int write_data_tag_len) {
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "更新数据失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int seos_challenge_get(uint8_t *RNDICC, uint8_t RNDICC_len, uint8_t keyslot) {
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "获取挑战失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int select_df_decode(uint8_t *response, uint8_t response_length, int *ALGORITHM_INFO_value1, int *ALGORITHM_INFO_value2, uint8_t *CRYPTOGRAM_encrypted_data, uint8_t *MAC_value) {
    ...
    */
    if (response == NULL || ALGORITHM_INFO_value1 == NULL || ALGORITHM_INFO_value2 == NULL || CRYPTOGRAM_encrypted_data == NULL || MAC_value == NULL) {
        PrintAndLogEx(ERR, "ADF响应解码器的输入无效");
        return PM3_ESOFT;
    }
    ...
    for (size_t i = 0; i + 1 < response_length;) {
        ...
        if (i + 2 + tag_len > response_length) {
            PrintAndLogEx(ERR, "格式错误的 ADF 响应: 标签 0x%02X 长度 %u 超过有效载荷大小 %u", tag, tag_len, response_length);
            return PM3_ESOFT;
        }

        if (tag == 0xCD) {
            if (tag_len < 2) {
                PrintAndLogEx(ERR, "格式错误的 ADF 响应: ALGORITHM_INFO (CD) 长度 %u 过短", tag_len);
                return PM3_ESOFT;
            }
            ...
            *ALGORITHM_INFO_value2 = response[i + 3];
            ...
            if (tag_len == 0 || tag_len > 64) {
                PrintAndLogEx(ERR, "格式错误的 ADF 响应: CRYPTOGRAM (85) 长度 %u，应为64", tag_len);
                return PM3_ESOFT;
            }
            ...
            if (tag_len != 8) {
                PrintAndLogEx(ERR, "格式错误的 ADF 响应: MAC (8E) 长度 %u，应为8", tag_len);
                return PM3_ESOFT;
            }
    ...
    if (has_algorithm_info == false || (plaintext_adf == false && (has_cryptogram == false || has_mac == false))) {
        if (has_algorithm_info == false) {
            PrintAndLogEx(ERR, "ADF 响应缺少 ALGORITHM_INFO 标签 (CD)");
        }
        if (plaintext_adf == false && has_cryptogram == false) {
            PrintAndLogEx(ERR, "ADF 响应缺少 CRYPTOGRAM 标签 (85)");
        }
        if (plaintext_adf == false && has_mac == false) {
            PrintAndLogEx(ERR, "ADF 响应缺少 MAC 标签 (8E)");
        }
        PrintAndLogEx(ERR, "原始 ADF 响应.................. %s", sprint_hex_inrow(response, response_length));
        return PM3_ESOFT;
    }
    ...
    if (algorithm_name1 != NULL) {
        ...
        PrintAndLogEx(ERR, "algoIdCipher（加密）........ %d（未知）", ALGORITHM_INFO_value1_n);
    }

    if (algorithm_name2 != NULL) {
        PrintAndLogEx(SUCCESS, "algoIdHash (MAC)................. "_YELLOW_("%i (%s)"), ALGORITHM_INFO_value2_n, algorithm_name2);
    } else {
        PrintAndLogEx(ERR, "algoIdHash（MAC）............... %d（未知）", ALGORITHM_INFO_value2_n);
    }
    ...
    for (int i = iv_size * 2; i < CRYPTOGRAM_decrypted_data_length; i++) {
        ...
        if (CRYPTOGRAM_decrypted_data[i] == 0x06 && CRYPTOGRAM_decrypted_data[i + 1] <= MAX_OID_LEN) {
            ...
            if (strcmp(CRYPTOGRAM_ADF_UPPER + 4, selectADFOID_UPPER + 4) != 0) {
                PrintAndLogEx(ERR, "ADF 与解密后的 ADF 不匹配");
                PrintAndLogEx(ERR, "可能是错误的认证密钥");
...
static int seos_mutual_auth(uint8_t *adfOID, size_t adfoid_len, uint8_t *randomICC, uint8_t *CRYPTOGRAM_Diversifier, uint8_t diversifier_len, uint8_t *mutual_auth_randomIFD, uint8_t *mutual_auth_keyICC, uint8_t *randomIFD, uint8_t randomIFD_len, uint8_t *keyIFD, uint8_t keyIFD_len, int encryption_algorithm, int hash_algorithm, int auth_key_index, uint8_t auth_key_slot) {
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "相互认证请求失败 (%04x - %s).", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
    ...
    if (memcmp(randomICC, mutual_auth_RandomICC, 8) != 0) {
        PrintAndLogEx(ERR, "RandomICC 与解密后的 RandomICC 不匹配");
        PrintAndLogEx(ERR, "可能是错误的密钥或 IV");
        return PM3_ESOFT;
    }
...
static int seos_try_aid_select(const uint8_t *aid, int aid_len, bool activate_field, bool keep_field_on) {
    ...
    if (resplen < 2) {
        PrintAndLogEx(ERR, "选择 SEOS 小程序 aid %s 失败（响应过短）。", sprint_hex_inrow(aid, aid_len));
        DropField();
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "选择 SEOS 小程序 aid %s 失败 (%04x - %s)。", sprint_hex_inrow(aid, aid_len), sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int seos_pacs_adf_select(char *oid, int oid_len, uint8_t *data_tag, int data_tag_len, int privacy_key_index, int auth_key_index, uint8_t *write, int write_len) {
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "选择 ADF 文件失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
    ...
    if (resplen <= 2) {
        PrintAndLogEx(ERR, "选择 ADF 文件失败（响应过短）。");
        return PM3_EWRONGANSWER;
    }
    resplen -= 2;
...
static int seos_gdf_select(int key_index) {
    ...
    uint16_t sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "获取 Global_df 失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
    ...
    if (resplen <= 2) {
        PrintAndLogEx(ERR, "获取 Global_df 失败（响应过短）。");
        return PM3_EWRONGANSWER;
    }
    resplen -= 2;
...
static int seos_load_keys(char *filename) {
    ...
    if (bytes_read > sizeof(keys)) {
        PrintAndLogEx(WARNING, "文件太长无法加载 - 预期：%zu 实际：%zu", sizeof(keys), bytes_read);
        free(dump);
        return PM3_EFILE;
    }
    ...
    size_t i = 0;
    if ((bytes_read % kn) != 0) {
        PrintAndLogEx(ERR, "密钥文件大小对于当前 SEOS 密钥布局无效");
        free(dump);
        return PM3_EFILE;
    }
    ...
    free(dump);
    PrintAndLogEx(SUCCESS, "已加载" _GREEN_("%2zd") " keys from %s", i, filename);
    return PM3_SUCCESS;
}

static int CmdHfSeosInfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频SEOS信息",
                  "从SEOS卡的默认ADF请求未认证信息\\n"
    ...
    if (privacy_key_len != 0 && seos_parse_key_index(privacy_key_str, &privacy_key_index) == false) {
        CLIParserFree(ctx);
        PrintAndLogEx(ERR, "无效的隐私密钥。预期--privacy-key N，索引范围为0-%zu", ARRAYLEN(keys) - 1);
        return PM3_EINVARG;
    }
    if (auth_key_len != 0 && seos_parse_key_index(auth_key_str, &auth_key_index) == false) {
        CLIParserFree(ctx);
        PrintAndLogEx(ERR, "无效的认证密钥。期望 --auth-key N，索引范围为 0-%zu", ARRAYLEN(keys) - 1);
        return PM3_EINVARG;
    }
...
static int CmdHfSeosPACS(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("o", "oid", "<hex>", "<0-100> 十六进制字节，用于OID（默认：2B0601040181E438010102011801010202）"),
        arg_str0(NULL, "privacy-key", "<idx>", "Privacy key slot index. The selected slot provides both privacy encryption and MAC subkeys and is sent as the privacy slot identifier"),
...
static int CmdHfSeosWrite(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0(NULL, "auth-key", "<idx>", "Auth key slot index. The selected slot provides the auth key and is sent as the auth slot identifier"),
        arg_str0(NULL, "aid", "<hex>", "Use a custom SEOS AID"),
        arg_str0("d", "数据", "<hex>", "<0-128> 十六进制字节，用于数据（必须是有效的BER-TLV）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHfSeosADF(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("t", "tag", "<hex>", "<0-100> 十六进制字节，用于要读取的标签（默认：FF00）"),
        arg_str0("o", "oid", "<hex>", "<0-100> 十六进制字节，用于OID（默认：2B0601040181E438010102011801010202）"),
        arg_str0(NULL, "privacy-key", "<idx>", "Privacy key slot index. The selected slot provides both privacy encryption and MAC subkeys and is sent as the privacy slot identifier"),
        arg_str0(NULL, "auth-key", "<idx>", "Auth key slot index. The selected slot provides the auth key and is sent as the auth slot identifier"),
        arg_str0(NULL, "aid", "<hex>", "Use a custom SEOS AID"),
...
static int CmdHfSeosSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("t", "tag", "<hex>", "<0-100> 十六进制字节，用于要模拟的标签（默认：FF00）"),
        arg_str0("o", "oid", "<hex>", "<0-100> 十六进制字节，用于OID（默认：2B0601040181E438010102011801010202）"),
        ...
        arg_str0("d", "数据", "<hex>", "<0-128> 十六进制字节，用于数据（必须是有效的BER-TLV）"),
        arg_str0("u", "uid", "<hex>", "<0-10> 十六进制字节，用于UID（必须是RID，即[0]=0x08）"),
        arg_lit0("l", "legacy", "使用旧版算法（3DES/SHA1）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHfSeosManageKeys(const char *Cmd) {
    ...
    if (privacy_len != 0 && privacy_len != 32) {
        PrintAndLogEx(ERR, "隐私密钥必须恰好为32字节");
        return PM3_EINVARG;
    }
    if (auth_len != 0 && auth_len != 16) {
        PrintAndLogEx(ERR, "认证密钥必须恰好为16字节");
        return PM3_EINVARG;
    }
...
static command_t CommandTable[] = {
    ...
    {"list",    CmdHfSeosList,          AlwaysAvailable, "列出 SEOS 历史"},
    {"sam",     CmdHfSeosSAM,           IfPm3Smartcard,  "SAM测试"},
    {"-----------", CmdHelp,            IfPm3Iso14443a,  "---------------------- " _CYAN_("操作") " ---------------------"},
    {"info",    CmdHfSeosInfo,          IfPm3Iso14443a,  "标签信息"},
    {"pacs",    CmdHfSeosPACS,          IfPm3Iso14443a,  "从卡片提取PACS信息"},
    {"write",   CmdHfSeosWrite,         IfPm3Iso14443a,  "将ADF写入卡片"},
    {"adf",     CmdHfSeosADF,           IfPm3Iso14443a,  "从卡中读取ADF"},
    {"gdf",     CmdHfSeosGDF,           IfPm3Iso14443a,  "从卡中读取GDF"},
    {"-----------", CmdHelp,            IfPm3Seos,       "---------------------- " _CYAN_("Simulation") " ---------------------"},
    {"sim",     CmdHfSeosSim,           IfPm3Seos,       "模拟Seos标签"},
    {"-----------", CmdHelp,            AlwaysAvailable, "------------------------ " _CYAN_("Utils") " ------------------------"},
    {"managekeys", CmdHfSeosManageKeys, AlwaysAvailable, "管理与 SEOS 命令一起使用的密钥"},
    {NULL, NULL, NULL, NULL}
};
