...
static uint16_t extractChallenges(uint16_t tracepos, uint16_t traceLen, uint8_t *trace) {
    ...
    if (tracepos + TRACELOG_HDR_LEN + data_len + TRACELOG_PARITY_LEN(hdr) > traceLen) {
        PrintAndLogEx(DEBUG, "跟踪位置偏移 %"PRIu64 " larger than reported tracelen %u",
    ...
    */
    ...
    switch (frame[0]) {
        ...
        case MIFARE_ULC_AUTH_1: {
            ...
            PrintAndLogEx(INFO, "已找到一个 MFU-C 认证尝试");
    ...
    for (uint8_t i = 0; i < 2; i++, pos++) {
        ...
        switch (frame[pos]) {
            ...
            case MFDES_AUTHENTICATE: {
                ...
                PrintAndLogEx(INFO, "已找到一个 MFDES Auth NATIVE (keyNo %d)", frame[pos + long_jmp]);
            ...
            case MFDES_AUTHENTICATE_ISO: {
                // Assume wrapped or unwrapped
                PrintAndLogEx(INFO, "已找到一个 MFDES Auth ISO (keyNo %d)", frame[pos + long_jmp]);
                if (next_record_is_response(tracepos, trace) == false) {
                    break;
                }
            ...
            case MFDES_AUTHENTICATE_AES: {
                // Assume wrapped or unwrapped
                PrintAndLogEx(INFO, "已找到一个 MFDES Auth AES (keyNo %d)", frame[pos + long_jmp]);
            ...
            case MFDES_AUTHENTICATE_EV2F: {
                PrintAndLogEx(INFO, "已找到一个 MFDES Auth EV2 First");
            ...
            case MFDES_AUTHENTICATE_EV2NF: {
                PrintAndLogEx(INFO, "已找到一个 MFDES Auth EV2 Non First");
                uint16_t tmp = extractChall_ev2(tracepos, trace, pos, long_jmp);
                if (tmp == 0)
                    break;
                else
...
#define TRACE_MAX_HEX_BYTES  18
    ...
    if (data_len == 0) {
        if (protocol == ICLASS && duration == 2048) {
            ...
            snprintf(line[0], sizeof(line[0]), "<空跟踪 - 可能出错>");
        }
    }
...
static int download_trace(void) {
    ...
    if (IfPm3Present() == false) {
        PrintAndLogEx(FAILED, "您在离线模式下请求了跟踪上传，请考虑使用参数`" _YELLOW_("-1") "` for working from Tracebuffer");
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(DEBUG, "从设备下载跟踪日志数据");
...
static int CmdTraceExtract(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("1", "buffer", "使用来自跟踪缓冲区的数据"),
        arg_param_end
    };
    ...
    if (use_buffer == false) {
        ...
        PrintAndLogEx(FAILED, "您在离线模式下请求了跟踪列表，但没有跟踪记录。");
        PrintAndLogEx(FAILED, "考虑使用 `" _YELLOW_("加载跟踪") "` or removing parameter `" _YELLOW_("-1") "`");
        return PM3_EINVARG;
    }
...
static int CmdTraceLoad(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("trace list -1 -t ...") "` to view trace.  Remember the " _YELLOW_("`-1`") " param");
    return PM3_SUCCESS;
}
...
static int CmdTraceSave(const char *Cmd) {
    ...
    if (gs_traceLen == 0) {
        download_trace();
        if (gs_traceLen == 0) {
            PrintAndLogEx(WARNING, "跟踪为空，无内容可保存");
            return PM3_SUCCESS;
        }
    }
...
int CmdTraceList(const char *Cmd) {
    ...
    if (CLIParamStrToBuf(arg_get_str(ctx, 8), (uint8_t *)dictionary, FILE_PATH_SIZE, &diclen)) {
        PrintAndLogEx(FAILED, "字典文件名过长或无效。");
        diclen = 0;
    }
    ...
    if (strcmp(type, "14a") == 0)      protocol = ISO_14443A;
        ...
        PrintAndLogEx(FAILED, "未知协议 \\\"%s\\\"", type);
        return PM3_EINVARG;
    }
    ...
    if (use_buffer == false) {
        ...
        if (IfPm3Present() == false) {
            ...
            PrintAndLogEx(FAILED, "您请求了跟踪列表，但没有跟踪记录。");
        }
    ...
    if (show_hex) {
        ...
        if (use_relative) {
            ...
            PrintAndLogEx(INFO, _YELLOW_("开始") " = start of start frame. " _YELLOW_("end") " = end of frame. " _YELLOW_("src") " = source of transfer.");
        }
        ...
        if (protocol == PROTO_MIFARE || protocol == PROTO_MFPLUS) {
            ...
            if (diclen > 0) {
                ...
                if (res != PM3_SUCCESS || dicKeysCount == 0 || keyBlock == NULL) {
                    PrintAndLogEx(FAILED, "加载字典时发生错误！（现在将使用默认密钥）");
                } else {
    ...
    if (show_hex) {
        PrintAndLogEx(HINT, "提示: 使用的语法: `" _YELLOW_("text2pcap -t \"%%S.\" -l 264 -n <input-text-file> <output-pcapng-file>") "`");
    }
...
static command_t CommandTable[] = {
    ...
    {"extract", CmdTraceExtract,  AlwaysAvailable, "提取跟踪中找到的认证挑战"},
    {"list",    CmdTraceList,     AlwaysAvailable, "列出跟踪缓冲区中的协议数据"},
    {"load",    CmdTraceLoad,     AlwaysAvailable, "从文件加载跟踪"},
    {"save",    CmdTraceSave,     AlwaysAvailable, "将跟踪缓冲区保存到文件"},
    {NULL, NULL, NULL, NULL}
};
