...
static void mqtt_publish_callback(void **unused, struct mqtt_response_publish *published) {
    ...
    if (ps) {
        ...
        if (res == PM3_SUCCESS) {
            PrintAndLogEx(INFO, "已获取JSON文件 ( %s )", _GREEN_("ok"));
        }
    } else {
...
static int mqtt_receive(const char *addr, const char *port, const char *topic, const char *fn) {
    ...
    */
    ...
    if (client.error != MQTT_OK) {
        PrintAndLogEx(FAILED, "错误：%s", mqtt_error_str(client.error));
    ...
    if (pthread_create(&client_daemon, NULL, mqtt_client_refresher, &client)) {
        PrintAndLogEx(FAILED, "启动客户端守护进程失败");
        return mqtt_exit(PM3_ESOFT, sockfd, NULL);
    }
...
static command_t CommandTable[] = {
    ...
    {"send",     CmdMqttSend,      AlwaysAvailable, "通过MQTT发送消息或json文件"},
    {"receive",  CmdMqttReceive,   AlwaysAvailable, "通过MQTT接收消息或JSON文件"},
    {NULL, NULL, NULL, NULL}
};
