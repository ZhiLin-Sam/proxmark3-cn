    ...
    if (is_response) {
        ...
        switch (command) {
            ...
            case HITAGU_CMD_READ_MULTIPLE_BLOCK: {
                ...
                exp_len += snprintf(exp + exp_len, size - exp_len, "读取多块 起始:%d 数量:%d"
                                    , reflect8(block_addr)
            ...
            case HITAGU_CMD_WRITE_SINGLE_BLOCK: {
                ...
                exp_len += snprintf(exp + exp_len, size - exp_len, "写入单块 起始:%d 数据:[%s]"
                                    , reflect8(block_addr)
            ...
            default: {
                exp_len += snprintf(exp + exp_len, size - exp_len, "未知 0x%02X", command);
                break;
            }
        }
    }
...
static bool htu_get_uid(uint64_t *uid) {
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - 获取 Hitag µ UID 失败");
        return false;
    }
...
static int process_hitagu_common_args(CLIParserContext *ctx, lf_hitag_data_t *const packet) {
    ...
    if (key_len != 0 && key_len != HITAG_PASSWORD_SIZE) {
        PrintAndLogEx(WARNING, "错误的KEY长度，期望0或4，实际为%d", key_len);
        return PM3_EINVARG;
    }
...
static void print_error(int8_t reason) {
    ...
    switch (reason) {
        ...
        case -2: {
            PrintAndLogEx(FAILED, "读取 UID 失败！");
            break;
        }
        case -3: {
            PrintAndLogEx(FAILED, "获取系统信息/配置失败！");
            break;
        }
        case -4: {
            PrintAndLogEx(FAILED, "登录失败！密码错误？");
            break;
        }
        case -5: {
            PrintAndLogEx(FAILED, "块无写入权限。未授权？");
            break;
        }
        case -6: {
            PrintAndLogEx(FAILED, "响应 CRC 无效！");
            break;
        }
        case -7: {
            PrintAndLogEx(FAILED, "读取块失败！");
            break;
        }
        default: {
            // PM3_REASON_UNKNOWN
            PrintAndLogEx(FAILED, "错误 - Hitag µ 失败");
            break;
        }
    }
}
...
static int CmdLFHitagURead(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf hitag htu 读块",
                  "读取 Hitag µ 存储器。\\n\\n"
                  "  82xx password mode: \n"
    ...
    void *argtable[] = {arg_param_begin,
                        arg_lit0("8", "82xx", "82xx模式"),
                        arg_str0("k", "key", "<hex>", "密码，4个十六进制字节"),
                        arg_int0("p", "page", "<dec>", "读取的块地址（默认：0）"),
                        arg_int0("c", "count", "<dec>", "要读取的块数。'0' 读取所有块（默认：1）"),
                        arg_param_end
                       };
    ...
    if (page >= HITAGU_MAX_BLOCKS) {
        PrintAndLogEx(WARNING, "块地址超出范围。最大为 255，实际为 %u", page);
        return PM3_EINVARG;
    }
    ...
    if (count > HITAGU_MAX_BLOCKS) {
        PrintAndLogEx(WARNING, "一次最多读取 %d 个块", HITAGU_MAX_BLOCKS);
        return PM3_EINVARG;
    }
    ...
    if (icr == HITAGU_ICR_STANDARD) {
        ...
        PrintAndLogEx(INFO, "Hitag µ Standard (ICR=0x%02X)，用户块：0x%02X", icr, user_blocks);
    } else if (icr == HITAGU_ICR_ADVANCED) {
        user_blocks = HITAGU_MAX_PAGE_ADVANCED;
        PrintAndLogEx(INFO, "Hitag µ Advanced (ICR=0x%02X)，用户块：0x%02X", icr, user_blocks);
        ...
        PrintAndLogEx(INFO, "Hitag µ 8265 (ICR=0x%02X)，用户块：0x%02X", icr, user_blocks);
    } else {
        user_blocks = HITAGU_MAX_PAGE_STANDARD;
        PrintAndLogEx(INFO, "未知的ICR (0x%02X)", icr);
    }
...
static int CmdLFHitagUDump(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag htu 转储",
                  "读取所有Hitag µ内存并保存到文件\\n"
                  "  82xx password mode: \n"
                  "    - default password 00000000\n",
...
static int CmdLFHitagUWrite(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag htu 写块",
                  "写入Hitag µ存储器中的一个块。\\n"
                  "  82xx password mode: \n"
                  "    - default password 00000000\n",
...
static int CmdLFHitagUReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag htu 读取器", "作为 Hitag µ 读取器。持续寻找 Hitag µ 标签，直到按下 Enter 或 pm3 按钮\\n",
                  "lf hitag htu reader\n"
...
static int CmdLFHitagUSim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag htu 模拟",
                  "模拟Hitag µ应答器\\n"
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdLFHitagUList,    AlwaysAvailable, "列出Hitag µ跟踪历史"},
    {"-----------", CmdHelp,            IfPm3Hitag,      "----------- " _CYAN_("常规") " -----------"},
    {"reader",      CmdLFHitagUReader,  IfPm3Hitag,      "模拟 Hitag µ 读取器"},
    {"rdbl",        CmdLFHitagURead,    IfPm3Hitag,      "读取Hitag µ块"},
    {"dump",        CmdLFHitagUDump,    IfPm3Hitag,      "将Hitag µ块转储到文件"},
    {"wrbl",        CmdLFHitagUWrite,   IfPm3Hitag,      "写入Hitag µ块"},
    {"-----------", CmdHelp,            IfPm3Hitag,      "----------- " _CYAN_("Simulation") " -----------"},
    {"sim",         CmdLFHitagUSim,     IfPm3Hitag,      "模拟Hitag µ应答器"},
    {NULL, NULL, 0, NULL}
};
