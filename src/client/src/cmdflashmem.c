...
int pm3_get_flash_pages64k(uint8_t *pages64k) {
    ...
    if (WaitForResponseTimeout(CMD_FLASHMEM_PAGES64K, &resp, 2500) == false) {
        PrintAndLogEx(WARNING, "pm3_get_flash_pages64k() 等待回复超时");
        return PM3_ETIMEOUT;
    }

    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "从闪存读取失败（64k 页）");
        return PM3_EFLASH;
    }
...
static int pm3_get_flash_info(spi_flash_t *info) {
    ...
    if (WaitForResponseTimeout(CMD_FLASHMEM_GET_INFO, &resp, 2500) == false) {
        PrintAndLogEx(WARNING, "pm3_get_flash_info() 等待回复超时");
        return PM3_ETIMEOUT;
    }

    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "读取闪存信息失败");
        return PM3_EFLASH;
    }
...
static int pm3_sign_write(uint8_t *signature, uint8_t slen) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "获取闪存页失败 (%x)", res);
        return res;
    }
...
static void pm3_print_public_keys(void) {
    ...
    for (uint8_t i = 0; i < ARRAYLEN(rsa_keypairs); i++) {
        ...
        PrintAndLogEx(INFO, "长度........ %"PRIu64, rsa.len);
        PrintAndLogEx(INFO, "指数... %s", str_exp);
        PrintAndLogEx(INFO, "公钥模数N");
...
static int CmdFlashMemLoad(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("o", "offset", "<dec>", "内存中的偏移量"),
        arg_lit0("m", "mfc", "上传6字节密钥（MIFARE Classic字典）"),
        arg_lit0("i", "iclass", "上传8字节密钥（iClass字典）"),
        arg_lit0("t", "t55xx", "上传4字节密钥（T55xx字典）"),
    ...
    if (is_mfc) {
        d = DICTIONARY_MIFARE;
        PrintAndLogEx(INFO, "将文件视为MIFARE Classic密钥");
    } else if (is_iclass) {
        d = DICTIONARY_ICLASS;
        PrintAndLogEx(INFO, "将文件视为iCLASS密钥");
    } else if (is_t55xx) {
        d = DICTIONARY_T55XX;
        PrintAndLogEx(INFO, "将文件视为T55xx密码");
    } else if (is_ulc) {
        d = DICTIONARY_MIFARE_ULC;
        PrintAndLogEx(INFO, "将文件视为MIFARE Ultralight-C密钥");
    } else if (is_ulaes) {
        d = DICTIONARY_MIFARE_ULAES;
        PrintAndLogEx(INFO, "将文件视为MIFARE Ultralight AES密钥");
    }

    uint8_t spi_flash_pages = 0;
    int res = pm3_get_flash_pages64k(&spi_flash_pages);
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "获取闪存页数失败 (%x)", res);
        return res;
    }
    ...
    switch (d) {
        case DICTIONARY_MIFARE: {
            ...
            if (datalen > FLASH_MEM_MAX_SIZE_P(spi_flash_pages)) {
                PrintAndLogEx(ERR, "错误，文件大小超过可用内存");
                free(data);
                return PM3_EOVFLOW;
            }
    ...
    if (d != DICTIONARY_NONE) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "将密码写入文件 %s 失败", spiffsDest);
            free(data);
            return res;
        }
        ...
        while (bytes_remaining > 0) {
            ...
            if (resp.status != PM3_SUCCESS) {
                g_conn.block_after_ACK = false;
                PrintAndLogEx(FAILED, "Flash写入失败 [偏移 %u]", bytes_sent);
                free(data);
                return PM3_EFLASH;
            }
        }
...
static int CmdFlashMemDump(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("o", "offset", "<dec>", "内存中的偏移量"),
        arg_int0("l", "len", "<dec>", "长度"),
        arg_lit0("v", "view", "查看转储"),
        arg_str0("f", "file", "<fn>", "保存文件名"),
        arg_int0("c", "cols", "<dec>", "列分隔（默认32）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
//    initialwipe = arg_get_lit(ctx, 2);
    ...
    if (page < 0 || page > (spi_flash_pages - 2)) {
        PrintAndLogEx(WARNING, "页必须在0到%d之间", spi_flash_pages - 2);
        return PM3_EINVARG;
    }
...
static int CmdFlashMemInfo(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("s", "sign", "创建签名"),
        arg_str0("d", NULL, "<hex>", "flash memory id, 8 hex bytes"),
        arg_str0("p", "pem",  "<fn>", "PEM格式的密钥"),
        arg_lit0("v", "详细", "详细输出"),
        arg_lit0("w", "write", "将签名写入闪存"),
        arg_param_end
    };
    ...
    if (pemlen) {
        ...
        PrintAndLogEx(INFO, "正在加载文件 `" _YELLOW_("%s") "`" NOLF, path);
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(WARNING, "获取闪存信息失败。确定您的设备有外部闪存吗？");
        return res;
    }
    ...
    if (got_private) {
        ...
        if (verbose) {
            PrintAndLogEx(
                ...
                (is_keyok) ? _GREEN_("ok") : _YELLOW_("不适用")
            );
        }

        if (is_keyok == false) {
            PrintAndLogEx(FAILED, "无可用私钥进行签名");
            return PM3_ECRYPTO;
        }
        ...
        if (dlen == 8) {
            ...
            PrintAndLogEx(INFO, "正在使用 ID......... %s", sprint_hex_inrow(id, dlen));
        } else {
            PrintAndLogEx(INFO, "正在使用 ID......... %s", sprint_hex_inrow(mem.flashid, sizeof(mem.flashid)));
        }

        PrintAndLogEx(INFO, "签名中.......... %s", sprint_hex_inrow(sha_hash, sizeof(sha_hash)));
...
static command_t CommandTable[] = {
    {"spiffs",   CmdFlashMemSpiFFS,  IfPm3Flash,  "{ SPI 文件系统 }"},
    {"help",     CmdHelp,            AlwaysAvailable, "此帮助"},
    {"-----------", CmdHelp,            IfPm3Flash,      "------------------- " _CYAN_("操作") " -------------------"},
    {"波特率", CmdFlashmemSpiBaud, IfPm3Flash,  "设置闪存SPI波特率"},
    {"dump",     CmdFlashMemDump,    IfPm3Flash,  "从闪存转储数据"},
    {"info",     CmdFlashMemInfo,    IfPm3Flash,  "闪存信息"},
    {"load",     CmdFlashMemLoad,    IfPm3Flash,  "加载数据到闪存"},
    {"wipe",     CmdFlashMemWipe,    IfPm3Flash,  "擦除闪存中的数据"},
    {NULL, NULL, NULL, NULL}
};
