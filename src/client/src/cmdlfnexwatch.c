...
static int nexwatch_magic_bruteforce(uint32_t cn, uint8_t calc_parity, uint8_t chk) {
    ...
    PrintAndLogEx(DEBUG, "DEBUG: 错误 - 未找到魔数");
    return PM3_ESOFT;
}
...
int demodNexWatch(bool verbose) {
    ...
    if (PSKDemod(0, 0, 100, false) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - NexWatch 无法解调信号");
        return PM3_ESOFT;
    }
    bool invert = false;
    ...
    if (idx < 0) {
        if (idx == -1)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - NexWatch 样本不足");
        // else if (idx == -2)
        ...
        // PrintAndLogEx(DEBUG, "DEBUG: Error - NexWatch problem during PSK demod");
            ...
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - NexWatch 未找到前导码");
        // else if (idx == -5)
        // PrintAndLogEx(DEBUG, "DEBUG: Error - NexWatch size not correct: %d", size);
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - NexWatch 错误 %d", idx);
...
static int CmdNexWatchDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 Nexwatch 解调",
                  "尝试查找Nexwatch前导码，如果找到则解码/解扰数据",
                  "低频 Nexwatch 解调"
                 );
...
static int CmdNexWatchReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Nexwatch 读取",
                  "读取 Nexwatch 标签",
...
static int CmdNexWatchClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Nexwatch 克隆",
    ...
    void *argtable[] = {
        ...
        arg_u64_0("m", "mode", "<dec>", "模式（十进制）（0-15，默认为1）"),
    ...
    if (use_nexkey && use_quadrakey) {
        PrintAndLogEx(FAILED, "不能同时指定Nexkey和Quadrakey");
        return PM3_EINVARG;
    }
    ...
    if (mlen > 1) {
        PrintAndLogEx(FAILED, "不能指定超过一个字节的魔数");
        return PM3_EINVARG;
    }

    if (use_raw && cn != -1) {
        PrintAndLogEx(FAILED, "不能同时指定Raw和Card ID");
        return PM3_EINVARG;
    }
...
static int CmdNexWatchSim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Nexwatch 模拟",
...
static command_t CommandTable[] = {
    ...
    {"demod",  CmdNexWatchDemod,  AlwaysAvailable, "从 GraphBuffer 解调 NexWatch 标签 (nexkey, quadrakey)"},
    {"reader", CmdNexWatchReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",  CmdNexWatchClone,  IfPm3Lf,         "克隆NexWatch标签到T55x7、Q5/T5555或EM4305/4469"},
    {"sim",    CmdNexWatchSim,    IfPm3Lf,         "模拟NexWatch标签"},
    {NULL, NULL, NULL, NULL}
};
