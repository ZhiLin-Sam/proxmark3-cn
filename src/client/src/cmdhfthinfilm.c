...
static int print_barcode(uint8_t *barcode, const size_t barcode_len, bool verbose) {
    ...
    switch (barcode[1]) {
        ...
        case 5:
            if (barcode_len < 16) {
                PrintAndLogEx(WARNING, "EPC: (部分数据) %s", sprint_hex(barcode + 2, barcode_len - 2));
                return PM3_ESOFT;
            }
...
int infoThinFilm(bool verbose) {
    ...
    if (resp.status == PM3_SUCCESS) {
        if (resp.length == 16 || resp.length == 32)  {
            ...
            if (verbose)
                PrintAndLogEx(WARNING, "响应长度错误。（%d）", resp.length);
...
int CmdHfThinFilmSim(const char *Cmd) {
    ...
    if (ret) {
        PrintAndLogEx(INFO, "客户端中断");
        PrintAndLogEx(WARNING, "模拟仍在 Proxmark3 上运行，直到下一个命令或按钮按下");
    } else {
...
static command_t CommandTable[] = {
    ...
    {"info",    CmdHfThinFilmInfo,  IfPm3NfcBarcode, "标签信息"},
    {"list",    CmdHfThinFilmList,  AlwaysAvailable, "列出 NFC Barcode / Thinfilm 历史 - 不正确"},
    {"sim",     CmdHfThinFilmSim,   IfPm3NfcBarcode, "伪造Thinfilm标签"},
    {NULL, NULL, NULL, NULL}
};
