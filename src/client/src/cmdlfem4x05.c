...
static const char *em4x05_annotation[] = {"Info/User", "UID", "密码", "User", "配置", "User", "User", "User", "User", "User", "User", "User", "User", "User", "Lock", "Lock"};
static const char *em4x69_annotation [] = {"Info", "UID", "密码", "Lock", "配置", "User", "User", "User", "User", "User", "User", "User", "User", "User", "User", "User"};

static int CmdHelp(const char *Cmd);

static int CmdEM4x05CloneHelp(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 4x05克隆帮助",
                  "显示用于克隆EM4305/4469标签特定技术的可用命令列表",
                  "低频EM 4x05克隆帮助"
                 );
    void *argtable[] = {
        arg_param_begin,
    ...
    CLIParserFree(ctx);
    PrintAndLogEx(NORMAL, "克隆EM4305/4469标签特定技术，请参见相应低频子菜单中的命令，例如：");
...
static bool em4x05_download_samples(void) {
    ...
    if (!GetFromDevice(BIG_BUF, got, sizeof(got), 0, NULL, 0, NULL, 2500, false)) {
        PrintAndLogEx(WARNING, "(em_download_samples) 命令执行超时");
        return false;
    }
    ...
    if (getSignalProperties()->isnoise) {
        PrintAndLogEx(DEBUG, "未找到标签 - 信号看起来像噪声");
        return false;
    }
    return true;
}
...
static int doPreambleSearch(size_t *startIdx) {
    ...
    if (g_DemodBufferLen < EM_PREAMBLE_LEN) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM4305解调缓冲区太小");
        return PM3_ESOFT;
    }
    ...
    *startIdx = 0;
    ...
    if (!preambleSearchEx(g_DemodBuffer, preamble, EM_PREAMBLE_LEN, &size, startIdx, true)) {
        ...
        if (!preambleSearchEx(g_DemodBuffer, errpreamble, EM_PREAMBLE_LEN, &size, startIdx, true)) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM4305未找到前导码 :: %zu", *startIdx);
            return PM3_ESOFT;
        }
...
static bool detectFSK(void) {
    ...
    if (GetFskClock("", false) == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: FSK时钟失败");
        return false;
    }
    // demod
    int ans = FSKrawDemod(0, 0, 0, 0, false);
    if (ans != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: FSK解调失败");
        return false;
    }
    return true;
}
...
static bool detectPSK(void) {
    ...
    if (ans <= 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: PSK时钟失败");
        return false;
    }
    //demod
    ...
    if (ans != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: PSK1解调失败");
        return false;
    }
    ...
    if (!preambleSearchEx(g_DemodBuffer, preamble, EM_PREAMBLE_LEN, &size, &startIdx, true)) {
        ...
        if (ans != PM3_SUCCESS) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: PSK1反相解调失败");
            return false;
        }

        if (!preambleSearchEx(g_DemodBuffer, preamble, EM_PREAMBLE_LEN, &size, &startIdx, true)) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: PSK1反相解调失败2");
            return false;
        }
    }
...
static bool detectASK_MAN(void) {
    ...
    if (ASKDemod_ext(0, 0, 50, 0, false, false, false, 1, &stcheck) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: ASK/Manchester解调失败");
        return false;
    }
    return true;
}
...
static bool detectASK_BI(void) {
    ...
    if (ans != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: ASK/双相正常解调失败");

        ans = ASKbiphaseDemod(0, 1, 1, 50, false);
        if (ans != PM3_SUCCESS) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: ASK/双相反相解调失败");
            return false;
        }
    }
    return true;
...
static bool detectNRZ(void) {
    ...
    if (ans != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: NRZ正常解调失败");

        ans = NRZrawDemod(0, 1, 1, false);
        if (ans != PM3_SUCCESS) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM: NRZ反相解调失败");
            return false;
        }
    }
...
static int em4x05_setdemod_buffer(uint32_t *word, size_t idx) {
    ...
    if (!em4x05_col_parity_test(g_DemodBuffer + idx + EM_PREAMBLE_LEN, 45, 5, 9, 0)) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - 结束奇偶校验失败");
        return PM3_ESOFT;
    }

    // test for even parity bits and remove them. (leave out the end row of parities so 36 bits)
    if (!removeParity(g_DemodBuffer, idx + EM_PREAMBLE_LEN, 9, 0, 36)) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - EM, 移除奇偶校验失败");
        return PM3_ESOFT;
    }
...
int em4x05_clone_tag(uint32_t *blockdata, uint8_t numblocks, uint32_t pwd, bool use_pwd) {
    ...
    for (int8_t i = 0; i < numblocks; i++) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "(em4x05_clone_tag) 写入标签超时");
            return res;
        }
    }
...
static int em4x05_login_ext(uint32_t pwd) {
    ...
    if (WaitForResponseTimeout(CMD_LF_EM4X_LOGIN, &resp, 10000) == false) {
        PrintAndLogEx(WARNING, "(em4x05_login_ext) 等待回复超时");
        return PM3_ETIMEOUT;
    }
...
int em4x05_read_word_ext(uint8_t addr, uint32_t pwd, bool use_pwd, uint32_t *word) {
    ...
    if (WaitForResponseTimeout(CMD_LF_EM4X_READWORD, &resp, 10000) == false) {
        PrintAndLogEx(WARNING, "(em4x05_read_word_ext) 等待回复超时");
        return PM3_ETIMEOUT;
    }
...
static void printEM4x05config(em_tech_type_t card_type, uint32_t wordData) {
    ...
    PrintAndLogEx(INFO, "");
    PrintAndLogEx(INFO, "配置........... %08X", wordData);
    PrintAndLogEx(INFO, "--------------------------------");
...
static void printEM4x05info(uint32_t block0, uint32_t serial) {
    ...
    */
    ...
    PrintAndLogEx(SUCCESS, "块0........ %08x", block0);

    switch (cap) {
        case 3:
            PrintAndLogEx(SUCCESS, "电容类型...... 330pF ( %u )", cap);
            break;
        case 2:
            PrintAndLogEx(SUCCESS, "电容类型...... %spF ( %u )", (chipType == 4) ? "75" : "210", cap);
            break;
        case 1:
            PrintAndLogEx(SUCCESS, "电容类型...... 250pF ( %u )", cap);
            break;
        case 0:
            PrintAndLogEx(SUCCESS, "电容类型...... 无谐振电容 ( %u )", cap);
            break;
        default:
            PrintAndLogEx(SUCCESS, "电容类型...... 未知 ( %u )", cap);
            break;
    }

    PrintAndLogEx(SUCCESS, "自定义代码... %s ( %u )", (custCode == 0x200) ? "default" : "unknown", custCode);
}
...
int CmdEM4x05Demod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 解调",
                  "尝试查找EM 4x05前导码，如果找到则解码/解扰数据",
                  "lf em 4x05 解调"
                 );
...
int CmdEM4x05Dump(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 转储",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("p", "pwd", "<hex>", "密码(00000000)"),
    ...
    if (usePwd) {
        ...
        if (status == PM3_SUCCESS) {
            ...
            PrintAndLogEx(WARNING, "登录尝试：标签无应答");
            return status;
        }
    }
...
int CmdEM4x05Read(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 读取",
                  "读取 EM4x05/EM4x69。标签必须在天线上。",
                  "lf em 4x05 read -a 1\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int1("a", "addr", "<dec>", "要读取的内存地址（0-15）"),
        arg_str0("p", "pwd", "<hex>", "可选 - 密码，4字节十六进制"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (status == PM3_SUCCESS)
        PrintAndLogEx(SUCCESS, "地址 %02d | %08X - %s", addr, word, (addr > 13) ? "Lock" : "");
    else if (status == PM3_EFAILED)
        PrintAndLogEx(ERR, "标签拒绝读取操作");
    else
...
int CmdEM4x05Write(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 写入",
                  "写入EM4x05/EM4x69。标签必须在天线上。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("a", "addr", "<dec>", "要写入的内存地址（0-13）"),
        arg_str1("d", "数据", "<hex>", "要写入的数据（4个十六进制字节）"),
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(WARNING, "数据必须为 4 个十六进制字节");
        return PM3_EINVARG;
    } else if (res == 0) {
        CLIParserFree(ctx);
        PrintAndLogEx(WARNING, "数据必须为 4 个十六进制字节");
    ...
    if ((addr > 13) && (protect_operation == false)) {
        PrintAndLogEx(WARNING, "地址必须在 0 到 13 之间");
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("lf em 4x05 读取") "` to verify");
    return status;
}
...
int CmdEM4x05Wipe(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 擦除",
                  "擦除EM4x05/EM4x69。标签必须在天线上。",
    ...
    if (foo > 1) {
        PrintAndLogEx(ERR, "不能同时针对多种芯片类型");
        return PM3_EINVARG;
    }
...
int CmdEM4x05Info(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 信息",
                  "标签信息EM4205/4305/4469/4569标签。标签必须在天线上。",
                  "lf em 4x05 info\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("p", "pwd", "<hex>", "可选 - 密码，4个十六进制字节"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (em4x05_read_word_ext(EM_CONFIG_BLOCK, pwd, use_pwd, &word) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "(CmdEM4x05Info) 读取配置块失败");
        return PM3_ESOFT;
    }
...
int CmdEM4x05Chk(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 4x05检查",
                  "此命令对EM4205/4305/4469/4569使用字典攻击",
                  "lf em 4x05 chk\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "加载默认密钥字典文件 <*.dic>"),
        arg_str0("e", "em", "<pwd>", "尝试基于EM4100 ID从某些克隆器计算出的密码"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    int fnlen = 0;
    ...
    if (card_id > 0) {
        ...
        PrintAndLogEx(INFO, "测试 %08"PRIX32" generated ", pwd);
...
int CmdEM4x05Brute(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 4x05暴力破解",
    ...
    if (res != 1) {
        CLIParserFree(ctx);
        PrintAndLogEx(WARNING, "检查 `start_pwd` 参数");
        return PM3_EINVARG;
    }
    ...
    if (WaitForResponseTimeout(CMD_LF_EM4X_BF, &resp, 1000) == false) {
        PrintAndLogEx(WARNING, "(EM4x05 暴力破解) 等待回复超时");
        return PM3_ETIMEOUT;
    }
    PrintAndLogEx(INFO, "暴力破解正在设备端运行，按下按钮中断");
    return PM3_SUCCESS;
}
...
static int unlock_write_protect(bool use_pwd, uint32_t pwd, uint32_t data, bool verbose) {
    ...
    if (status == PM3_SUCCESS && verbose) {
        ...
        PrintAndLogEx(ERR, "标签拒绝PROTECT操作");
    } else {
        PrintAndLogEx(DEBUG, "No answer from tag");
    }
...
int CmdEM4x05Unlock(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 解锁",
                  "对 EM4205/4305/4469/4569 执行撕扯操作",
                  "lf em 4x05 unlock\n"
    ...
    void *argtable[] = {
        ...
        arg_int0("s", "开始", "<us>", "从延迟（微秒）开始扫描"),
        arg_int0("e", "end", "<us>", "在延迟（微秒）后结束扫描"),
        arg_str0("p", "pwd", "<hex>", "密码(默认00000000)"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (start > end) {
        PrintAndLogEx(FAILED, "起始延迟不能大于结束延迟 %.0lf vs %.0lf", start, end);
        return PM3_EINVARG;
    }
...
#define ACTIVE_MASK 0x00008000
    ...
    if (search_value == ACTIVE_MASK) {
        PrintAndLogEx(SUCCESS, "标签已完全解锁，无需操作");
        return PM3_SUCCESS;
    }
...
int CmdEM4x05Sniff(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 嗅探",
                  "嗅探编程器发送的EM4x05命令",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("1", "buf", "使用缓冲区中的数据"),
        arg_lit0("r", "rev", "反转数据块的位顺序"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdEM4x05View(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x05 查看",
    ...
    PrintAndLogEx(INFO, "全零密码块可能是填充数据");
    return PM3_SUCCESS;
}
...
static int CmdEM4x05Config(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "默认....... %08X", EM4305_DEFAULT_CONFIG_BLOCK);
    ...
    PrintAndLogEx(INFO, "SECURA 密钥.... %08X", EM4305_SECURAKEY_CONFIG_BLOCK);
...
static command_t CommandTable[] = {
    ...
    {"clonehelp",   CmdEM4x05CloneHelp, IfPm3Lf,         "显示可用的克隆命令"},
    {"brute",       CmdEM4x05Brute,     IfPm3Lf,         "暴力破解密码"},
    {"chk",         CmdEM4x05Chk,       IfPm3Lf,         "检查密码"},
    {"config",      CmdEM4x05Config,    AlwaysAvailable, "创建通用配置字"},
    {"demod",       CmdEM4x05Demod,     AlwaysAvailable, "从GraphBuffer解调EM4x05/EM4x69标签"},
    {"dump",        CmdEM4x05Dump,      IfPm3Lf,         "转储EM4x05/EM4x69标签"},
    {"info",        CmdEM4x05Info,      IfPm3Lf,         "标签信息"},
    {"read",        CmdEM4x05Read,      IfPm3Lf,         "从EM4x05/EM4x69读取字数据"},
    {"sniff",       CmdEM4x05Sniff,     AlwaysAvailable, "尝试从样本缓冲区恢复em4x05命令"},
    {"unlock",      CmdEM4x05Unlock,    IfPm3Lf,         "对EM4x05/EM4x69执行撕开操作"},
    {"view",        CmdEM4x05View,      AlwaysAvailable, "显示标签转储文件的内容"},
    {"wipe",        CmdEM4x05Wipe,      IfPm3Lf,         "擦除EM4x05/EM4x69标签"},
    {"write",       CmdEM4x05Write,     IfPm3Lf,         "向EM4x05/EM4x69写入字数据"},
    {NULL, NULL, NULL, NULL}
};
