...
static int iclass_load_transport(uint8_t *key, uint8_t n) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(INFO, "找不到任何解密方法");
        return PM3_EINVARG;
    }

    if (keylen != 16) {
        PrintAndLogEx(ERR, "从文件加载传输密钥失败");
        free(keyptr);
        return PM3_EINVARG;
    }

    if (keylen != n) {
        PrintAndLogEx(ERR, "数组大小不匹配");
        free(keyptr);
        return PM3_EINVARG;
    }
...
static int generate_config_card(const iclass_config_card_item_t *o,  uint8_t *key, bool got_kr, uint8_t *card_key, bool got_eki, bool use_elite, bool got_mk, uint8_t *master_key) {
    ...
    if (res == PM3_SUCCESS) {
        ...
        PrintAndLogEx(FAILED, "读取卡片失败");
        PrintAndLogEx(INFO, "回退到默认配置卡");
    }
    ...
    PrintAndLogEx(INFO, "总字节数: %u", tot_bytes);
    // normal size
    ...
    if (res_key != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "未找到 iclass_decryptionkey.bin");
        free(data);
        return PM3_EINVARG;
    }
    ...
    if (strstr(o->desc, "ELITE") != NULL) {
        ...
        if (got_kr == false) {
            PrintAndLogEx(ERR, "请指定 ELITE 密钥!");
            free(data);
            return PM3_EINVARG;
        }
        ...
        if (app1_limit < 0x16) {
            ...
            PrintAndLogEx(WARNING, "正在调整 applimit1 以适应密钥滚动...");
        ...
        if (IsCardHelperPresent(false) != false) {
            if (Encrypt(lkey, enckey1) == false) {
                PrintAndLogEx(WARNING, "加密key1失败");
            } else {
                PrintAndLogEx(NORMAL, "( " _GREEN_("ok") " )");
            }
        } else {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass 查看") "` to view dump file");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass sim -t 3") "` to start simulating config card");
    return PM3_SUCCESS;
}
...
void print_picopass_header(const picopass_hdr_t *hdr) {
    ...
    PrintAndLogEx(SUCCESS, "电子钱包... %s 卡片挑战，CC", sprint_hex(hdr->epurse, sizeof(hdr->epurse)));
...
static int CmdHFiClassSniff(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf iclass 嗅探",
                  "嗅读取器与标签之间的通信",
                  "hf iclass sniff\n"
    ...
    if (jam_epurse_update) {
        PrintAndLogEx(INFO, "嗅探带干扰的iCLASS电子钱包更新...");
    }
    ...
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass list") "` to view captured tracelog");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("trace save -f hf_iclass_mytrace") "` to save tracelog for later analysing");
    if (jam_epurse_update) {
        PrintAndLogEx(HINT, "提示：通过比较trace和block 2中的值来验证干扰是否生效");
    }
    PrintAndLogEx(NORMAL, "");
    return PM3_SUCCESS;
}
...
static int CmdHFiClassSim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 模拟",
                  "模拟iCLASS传统/标准标签",
    ...
    if (sim_type > 4 && sim_type != 6 && sim_type != 7) {
        PrintAndLogEx(ERR, "未定义的模拟类型 %d", sim_type);
        return PM3_EINVARG;
    }
    ...
    switch (sim_type) {
        ...
        case ICLASS_SIM_MODE_READER_ATTACK: {
            PrintAndLogEx(INFO, "开始 iCLASS 模拟 2 攻击（精英模式）");
            ...
            PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass loclass -f iclass_mac_attack.bin") "` to recover elite key");
            break;
        }
        case ICLASS_SIM_MODE_READER_ATTACK_KEYROLL: {
            ...
            PrintAndLogEx(INFO, "开始 iCLASS 模拟 4 攻击（精英模式，读卡器处于密钥滚动模式）");
            PrintAndLogEx(INFO, "Press " _GREEN_("<Enter>") " to abort");
            PacketResponseNG resp;
            clearCommandBuffer();
            SendCommandMIX(CMD_HF_ICLASS_SIMULATE, sim_type, NUM_CSNS, 1, csns, NUM_CSNS * PICOPASS_BLOCK_SIZE);
            ...
            PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass loclass -f iclass_mac_attack_keyroll_A.bin") "` to recover elite key");
            PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass loclass -f iclass_mac_attack_keyroll_B.bin") "` to recover elite key");
            break;
        }
        ...
        default: {
            PrintAndLogEx(INFO, "开始 iCLASS 模拟");
            ...
            if (sim_type == ICLASS_SIM_MODE_FULL || sim_type ==  ICLASS_SIM_MODE_FULL_GLITCH || sim_type ==  ICLASS_SIM_MODE_FULL_GLITCH_KEY)
                PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass esave -h") "` to save the emulator memory to file");
            break;
        }
    }
    return PM3_SUCCESS;
...
static int CmdHFiClassTagSim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 标签模拟",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("w",   "wiegand",  "<format>",  "Wiegand 格式（默认 H10301），参见 `wiegand list`"),
    ...
    // --- validation
    if ((rawkey + elite) > 1) {
        PrintAndLogEx(ERR, "不能同时使用 --elite 和 --raw");
        return PM3_EINVARG;
    }

    if (csn_len > 0 && csn_len != 8) {
        PrintAndLogEx(ERR, "CSN必须恰好为8个十六进制字节");
        return PM3_EINVARG;
    }

    if (bin_len > 64) {
        PrintAndLogEx(ERR, "二进制Wiegand字符串最多64位");
        return PM3_EINVARG;
    }

    if (bin_len == 0 && card.FacilityCode == 0 && card.CardNumber == 0) {
        PrintAndLogEx(ERR, "必须提供 --cn/--fc 或 --bin");
        return PM3_EINVARG;
    }
    ...
    if (kd_len == 0) {
        ...
        if (key_nr >= ICLASS_KEYS_MAX) {
            PrintAndLogEx(ERR, "借记密钥索引超出范围（最大 %d）", ICLASS_KEYS_MAX - 1);
            return PM3_EINVARG;
        }
        ...
        PrintAndLogEx(ERR, "借记密钥必须恰好为8个十六进制字节");
        return PM3_EINVARG;
    } else if (key_nr >= 0) {
        PrintAndLogEx(SUCCESS, "Using debit key[%d] " _GREEN_("%s"), key_nr, sprint_hex(kd_master, 8));
    }
    ...
    if (kc_len == 0) {
        ...
        if (credit_nr >= ICLASS_KEYS_MAX) {
            PrintAndLogEx(ERR, "信用密钥索引超出范围（最大 %d）", ICLASS_KEYS_MAX - 1);
            return PM3_EINVARG;
        }
        ...
        PrintAndLogEx(ERR, "信用密钥必须恰好为8个十六进制字节");
        return PM3_EINVARG;
    } else if (credit_nr >= 0) {
        PrintAndLogEx(SUCCESS, "Using credit key[%d] " _GREEN_("%s"), credit_nr, sprint_hex(kc_master, 8));
    }
    ...
    if (enc_key_len > 0) {
        if (enc_key_len != 16) {
            PrintAndLogEx(ERR, "传输密钥必须是16个十六进制字节");
            return PM3_EINVARG;
        }
        have_enc_key = true;
    }
    ...
    if (have_enc_key == false) {
        ...
        if (use_sc == false) {
            ...
            if (res == PM3_SUCCESS && keylen == 16) {
                ...
                if (enckeyptr != NULL)
                    free(enckeyptr);
                PrintAndLogEx(WARNING, "未找到传输密钥 - 凭证块将以未加密方式写入");
            }
        } else {
    ...
    // --- start simulation
    PrintAndLogEx(INFO, "开始 iCLASS 完整模拟");
    if (bin_len == 0) {
...
static int CmdHFiClassInfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 信息",
                  "作为 iCLASS 读取器。读取/指纹识别 iCLASS 标签。",
                  "hf iclass 信息");
        ...
        if (WaitForResponseTimeout(CMD_HF_ICLASS_READER, &resp, 2000)) {
            ...
            if (loop) {
                ...
                if (r->status == FLAG_ICLASS_NULL || resp.status == PM3_ERFTRANS) {
                    if (verbose) PrintAndLogEx(WARNING, "iCLASS / Picopass 选卡失败 ( %d , %d)", r->status, resp.status);
                    res = PM3_EOPABORTED;
                    break;
                }
            }
...
static int CmdHFiClassReader(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 读卡器",
                  "作为 iCLASS 读取器。持续寻找 iCLASS 标签，直到按下 Enter 或 pm3 按钮",
...
static int CmdHFiClassELoad(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 加载",
                  "从(bin/json) iCLASS转储文件加载数据到模拟器内存",
    ...
    PrintAndLogEx(HINT, "提示：您可以模拟了。参见 `" _YELLOW_("hf iclass sim -h") "`");
...
static int CmdHFiClassESave(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 保存",
    ...
    void *argtable[] = {
        ...
        arg_int0("s", "size", "<256|2048>", "要保存的字节数（默认256）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (bytes > 4096) {
        PrintAndLogEx(WARNING, "模拟器内存最大4096字节。将%u截断为4096", bytes);
        bytes = 4096;
    }
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass view -f") "` to view dump file");
    return PM3_SUCCESS;
}

static int CmdHFiClassEView(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 查看",
                  "显示模拟器内存。\\n"
    ...
    if (bytes % 8 != 0) {
        bytes &= 0xFFF8;
        PrintAndLogEx(WARNING, "数字不能被8整除，截断为%u", bytes);
    }
...
static int CmdHFiClassESetBlk(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 设置块",
                  "设置模拟器内存中的单个块。",
    ...
    void *argtable[] = {
        ...
        arg_str0("d", "数据", "<hex>", "要写入的字节，8个十六进制字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHFiClassDecrypt(const char *Cmd) {
    ...
    CLIParserInit(&clictx, "hf iclass 解密",
                  "3DES 解密数据\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "指定转储文件的文件名"),
        arg_str0("d", "数据", "<hex>", "3DES加密数据"),
        arg_str0("k", "key", "<hex>", "3DES传输密钥"),
        arg_lit0("v", "详细", "详细输出"),
    ...
    if (enc_data_len > 0) {
        if (enc_data_len != PICOPASS_BLOCK_SIZE) {
            PrintAndLogEx(ERR, "数据必须为 8 个十六进制字节（16 个十六进制符号）");
            return PM3_EINVARG;
        }
        have_data = true;
    }
    ...
    if (key_len > 0) {
        if (key_len != 16) {
            PrintAndLogEx(ERR, "传输密钥必须是16个十六进制字节（32个十六进制字符）");
            return PM3_EINVARG;
        }
        have_key = true;
    }
    ...
    if (have_data) {
        ...
        PrintAndLogEx(SUCCESS, "加密中... %s", sprint_hex_inrow(enc_data, sizeof(enc_data)));
...
static int CmdHFiClassEncryptBlk(const char *Cmd) {
    CLIParserContext *clictx;
    CLIParserInit(&clictx, "hf iclass 加密",
                  "3DES 加密数据\\n"
    ...
    if (blk_data_len != 8) {
        PrintAndLogEx(ERR, "块数据必须为 8 个十六进制字节（16 个十六进制符号）");
        CLIParserFree(clictx);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(SUCCESS, "明文....... %s", sprint_hex_inrow(blk_data, sizeof(blk_data)));
...
static bool select_only(uint8_t *CSN, uint8_t *CCNR, bool verbose, bool shallow_mod) {
    ...
    if (verbose) {
        ...
        PrintAndLogEx(SUCCESS, "电子钱包........ %s", sprint_hex_inrow(CCNR, PICOPASS_BLOCK_SIZE));
    }
    return true;
}
...
static int iclass_dump_non_secure(bool shallow_mod, uint8_t *tag_data, uint16_t *taglen) {
    ...
    if (resp.status == PM3_ERFTRANS) {
        PrintAndLogEx(FAILED, "未找到标签");
        return PM3_ESOFT;
    }

    iclass_card_select_resp_t *r = (iclass_card_select_resp_t *)resp.data.asBytes;
    if (r->status == FLAG_ICLASS_NULL) {
        PrintAndLogEx(FAILED, "读取块0,1,2失败");
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "非安全页面模式");
    PrintAndLogEx(INFO, "转储所有可用内存，块3 - %u (0x%02x)", app_limit1, app_limit1);
...
static int CmdHFiClassDump(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 转储",
                  "转储iCLASS标签的所有内存",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "保存文件名"),
        arg_str0("k", "key", "<hex>", "借记密钥或重放用的NR/MAC，8个十六进制字节"),
    ...
    if (key_len > 0 && deb_key_nr >= 0) {
        PrintAndLogEx(ERR, "请指定扣款密钥或索引，不要同时指定");
        return PM3_EINVARG;
    }
    ...
    if (deb_key_nr >= 0) {
        if (deb_key_nr < ICLASS_KEYS_MAX) {
            ...
            PrintAndLogEx(ERR, "密钥编号无效");
            return PM3_EINVARG;
        }
    }

    if (credit_key_len > 0 && credit_key_nr >= 0) {
        PrintAndLogEx(ERR, "请指定充值密钥或索引，不要同时指定");
        return PM3_EINVARG;
    }
    ...
    if ((use_replay + rawkey + elite) > 1) {
        PrintAndLogEx(ERR, "不能同时使用 'elite'、'raw' 和 'nr'");
        return PM3_EINVARG;
    }
    ...
    if (r->status & (FLAG_ICLASS_CSN | FLAG_ICLASS_CONF | FLAG_ICLASS_CC)) {
        ...
        if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) {
            ...
            PrintAndLogEx(INFO, "检测到卡片处于非安全页模式");

            app_limit1 = card_app2_limit[type];
            app_limit2 = 0;
        } else if (hdr->conf.app_limit >= hdr->conf.mem_config) {
            PrintAndLogEx(WARNING, "AA1 配置 >= 卡片大小，使用卡片大小作为 AA1 限制");
            app_limit1 = card_app2_limit[type];
        } else {
    ...
    if (force) {
        ...
        PrintAndLogEx(INFO, "强制非安全页转储");
    }

    if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) {
        PrintAndLogEx(INFO, "转储所有可用内存，块3 - %u (0x%02x)", app_limit1, app_limit1);
        if (auth) {
            PrintAndLogEx(INFO, "无需密钥，忽略用户提供的密钥");
        }
    } else {
        ...
        if (app_limit2 != 0) {
            PrintAndLogEx(INFO, "卡片至少有2个应用区域。AA1限制 %u (0x%02X) AA2限制 %u (0x%02X)", app_limit1, app_limit1, app_limit2, app_limit2);
        } else {
            PrintAndLogEx(INFO, "卡片有1个应用区域。AA1限制 %u (0x%02X)", app_limit1, app_limit1);
        }
    }
    ...
    if (packet->isOK == false) {
        PrintAndLogEx(WARNING, "读取 AA1 块失败");
        return PM3_ESOFT;
    }
    ...
    if (have_credit_key && pagemap != PICOPASS_NON_SECURE_PAGEMODE && app_limit2 != 0) {
        ...
        if (packet->isOK == false) {
            PrintAndLogEx(WARNING, "使用信用密钥读取块失败");
            goto write_dump;
        }
...
write_dump:
    ...
    if (have_credit_key && pagemap != 0x01 && aa2_success == false) {
        PrintAndLogEx(INFO, "读取 AA2 失败，将 AA1 数据转储到文件");
    }
    ...
    PrintAndLogEx(SUCCESS, "保存转储文件 - 已读取%u个块", bytes_got / 8);

    pm3_save_dump(filename, tag_data, bytes_got, jsfIclass);

    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass decrypt -f") "` to decrypt dump file");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass view -f") "` to view dump file");
    PrintAndLogEx(NORMAL, "");
    return PM3_SUCCESS;
}
...
static int iclass_write_block(uint8_t blockno, uint8_t *bldata, uint8_t *macdata, uint8_t *KEY, bool use_credit_key,
    ...
    if (resp.status != PM3_SUCCESS) {
        if (verbose) PrintAndLogEx(ERR, "与卡片通信失败");
        return resp.status;
    }
...
static int CmdHFiClass_WriteBlock(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 写块",
                  "将数据写入iCLASS标签",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("k", "key", "<hex>", "访问密钥，8 个十六进制字节"),
        arg_int0(NULL, "ki", "<dec>", "Key index to select key from memory 'hf iclass managekeys'"),
        arg_int1(NULL, "blk", "<dec>", "块号"),
        arg_str1("d", "数据", "<hex>", "以8个十六进制字节写入的数据"),
        arg_str0("m", "mac", "<hex>", "重放MAC数据（4个十六进制字节）"),
    ...
    if (key_len > 0 && key_nr >= 0) {
        PrintAndLogEx(ERR, "请指定密钥或索引，不要同时指定");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (mac_len) {
        if (mac_len != 4) {
            PrintAndLogEx(ERR, "MAC 必须是 4 个十六进制字节（8 个十六进制符号）");
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
    ...
    do {
        ...
        switch (isok) {
            ...
            case PM3_ETEAROFF: {
                if (verbose) {
                    PrintAndLogEx(INFO, "写入撕除已触发");
                }
                break;
            }
            default: {
                PrintAndLogEx(FAILED, "写入失败");
                break;
            }
        }
...
static int CmdHFiClassCreditEpurse(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 充值钱包",
    ...
    void *argtable[] = {
        ...
        arg_int0(NULL, "ki", "<dec>", "Key index to select key from memory 'hf iclass managekeys'"),
        arg_str1("d", "数据", "<hex>", "以4个十六进制字节写入的数据"),
        arg_lit0(NULL, "elite", "elite computations applied to key"),
        arg_lit0(NULL, "raw", "no computations applied to key"),
        arg_lit0("v", "详细", "详细输出"),
        arg_lit0(NULL, "shallow", "use shallow (ASK) reader modulation instead of OOK"),
    ...
    if (data_len != 4) {
        PrintAndLogEx(ERR, "数据必须为 4 个十六进制字节（8 个十六进制符号）");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    switch (isok) {
        case PM3_SUCCESS:
            PrintAndLogEx(SUCCESS, "电子钱包充值成功");
            break;
        case PM3_ETEAROFF:
            if (verbose)
                PrintAndLogEx(INFO, "写入撕除已触发");
...
static int CmdHFiClassRestore(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 恢复",
                  "从转储文件（bin/eml/json）恢复数据到iCLASS标签",
    ...
    if (rawkey + elite > 1) {
        PrintAndLogEx(FAILED, "不能同时使用 'e' 和 'r'");
        return PM3_EINVARG;
    }

    if (startblock < 5) {
        PrintAndLogEx(WARNING, "你不能这样写密钥块。请确保起始块 > 4");
        return PM3_EINVARG;
    }
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "准备恢复块范围%02d..%02d", startblock, endblock);
    ...
    if (resp.status == PM3_SUCCESS) {
        ...
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass 读块") "` to verify data on card");
    } else {
...
static int iclass_read_block_ex(uint8_t *KEY, uint8_t blockno, uint8_t keyType, bool elite, bool rawkey, bool replay, bool verbose,
    ...
    if (packet->isOK == false) {
        if (verbose) PrintAndLogEx(FAILED, "认证错误");
        return PM3_ESOFT;
    }
...
static int CmdHFiClass_ReadBlock(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 读块",
                  "从标签读取iCLASS块",
    ...
    if (verbose) {
        if (key_len > 0)
            PrintAndLogEx(SUCCESS, "正在使用密钥 %s", sprint_hex(key, 8));
    }

    if (auth == false && verbose) {
        PrintAndLogEx(WARNING, "警告：读取时未使用认证。通常适用于配置为`非安全页`的卡片");
    ...
    switch (blockno) {
        ...
        case 7: {
            ...
            if (starts && ones) {
                ...
                PrintAndLogEx(INFO, "数据看起来未加密，尝试解码");
            }
...
static int CmdHFiClass_TearBlock(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 撕毁",
                  "撕下 iCLASS 标签块\\n"
    ...
    void *argtable[] = {
        ...
        arg_int0(NULL, "ki", "<dec>", "Key index to select key from memory 'hf iclass managekeys'"),
        arg_int1(NULL, "blk", "<dec>", "块号"),
        arg_str0("d", "数据", "<hex>", "以8个十六进制字节写入的数据"),
        arg_str0("m", "mac", "<hex>", "重放MAC数据（4个十六进制字节）"),
        arg_lit0(NULL, "credit", "key is assumed to be the credit key"),
        arg_lit0(NULL, "elite", "elite computations applied to key"),
        arg_lit0(NULL, "raw", "no computations applied to key"),
    ...
    if (tearoff_end <= tearoff_start) {
        PrintAndLogEx(ERR, "撕离结束延迟必须大于开始延迟");
        return PM3_EINVARG;
    }

    if (tearoff_start <= 0) {
        PrintAndLogEx(ERR, "撕离开始延迟必须大于0");
        return PM3_EINVARG;
    }

    if (tearoff_end <= 0) {
        PrintAndLogEx(ERR, "撕离结束延迟必须大于0");
        return PM3_EINVARG;
    }
    ...
    if (data_len && auth == false) {
        PrintAndLogEx(SUCCESS, "未提供密钥。尝试无认证读写");
    }
            ...
            if ((tear_success == false) &&
                ...
                if (blockno == 2) {
                    if (memcmp(data_read, ff_data, 8) == 0 && memcmp(data_read_orig, ff_data, 8) != 0) {
                        PrintAndLogEx(SUCCESS, "电子钱包已被撕毁（ %s ）", _GREEN_("ok"));
                        PrintAndLogEx(HINT, "提示：尝试 `hf iclass creditepurse -d FEFFFEFF --ki 1`");
                        PrintAndLogEx(HINT, "提示：尝试 `hf iclass wrbl -d 'FFFFFFFF FFFF FEFF' --blk 2 --ki 1 --credit`");
                        isok = PM3_SUCCESS;
                        goto_out = true;
                    }
                }
                ...
                if (blockno == 1) {
                    ...
                    if (data_read[1] > data_read_orig[1] ||
                        ...
                        PrintAndLogEx(SUCCESS, "更多 OTP 位被设置!!!");
...
static int CmdHFiClass_BlackTears(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_int0("e", NULL, "<dec>", "tearoff delay end (in us) must be a higher value than the start delay"),
        arg_str0("o", "otp", "<hex>", "自定义OTP值，2个十六进制字节"),
    ...
    if (memcmp(r->header.hdr.csn + 4, "\xFE\xFF\x12\xE0", 4) == 0) {
        ...
        PrintAndLogEx(ERR, "检测到旧版芯片 ( %s )",  _RED_("fail"));
        PrintAndLogEx(INFO, "不支持此操作");
        PrintAndLogEx(INFO, "CSN... %s", sprint_hex_inrow(r->header.hdr.csn, PICOPASS_BLOCK_SIZE));
        DropField();
        return PM3_ESOFT;
    }
    ...
    uint8_t pagemap = get_pagemap(hdr);
    if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) {
        PrintAndLogEx(INFO, "卡片已处于非安全页模式");
        PrintAndLogEx(HINT, "尝试 `" _YELLOW_("hf iclass 转储") "` - to extract all memory");
        DropField();
        return PM3_SUCCESS;
    }
    ...
    while ((tearoff_start <= tearoff_end) && (read_ok == false)) {
        ...
        if ((tear_success == false) &&
            ...
            if (data_read[1] > data_read_orig[1] || data_read[2] > data_read_orig[2]) {
                ...
                res = iclass_write_block(blockno, data_read, mac, key, use_credit_key, elite, rawkey, use_replay, verbose, auth, shallow_mod);
                if (res != PM3_SUCCESS) {
                    PrintAndLogEx(INFO, "稳定位 ( %s )", _RED_("fail"));
                }
...
out:
    ...
    if (b7 == TEAR_INITAL) {
        ...
        if (do_not_stabilize) {
            PrintAndLogEx(INFO, "未稳定位...");
        } else {
            PrintAndLogEx(INFO, "正在稳定位...");
            iclass_write_block(blockno, data, mac, key, use_credit_key, elite, rawkey, use_replay, false, auth, shallow_mod);
        }
...
//    if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) { }
    ...
    */
    ...
    while (i <= endblock) {
        ...
        if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) {
            const char *info_nonks[] = {"CSN", "配置", "AIA", "User"};
            if (i < 3) {
            ...
            const char *info_ks[] = {"CSN", "配置", "E-purse", "Debit", "Credit", "AIA", "User", "User AA2"};
...
static int CmdHFiClassView(const char *Cmd) {
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "文件大小 %zu 字节，文件块 %d (0x%x)", bytes_read, (uint16_t)(bytes_read >> 3), (uint16_t)(bytes_read >> 3));
...
static void HFiClassCalcNewKey(uint8_t *CSN, uint8_t *OLDKEY, uint8_t *NEWKEY, uint8_t *xor_div_key, bool elite, bool oldElite, bool verbose) {
    ...
    if (verbose) {
        PrintAndLogEx(SUCCESS, "旧分散密钥.... %s", sprint_hex_inrow(old_div_key, PICOPASS_BLOCK_SIZE));
...
static int CmdHFiClassCalcNewKey(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 计算新密钥",
                  "计算用于更新的新密钥（块3和4）",
    ...
    if (old_key_len > 0 && old_key_nr >= 0) {
        PrintAndLogEx(ERR, "请指定旧密钥或索引，不要同时指定");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (old_key_len > 0) {
        ...
        PrintAndLogEx(ERR, "请指定旧密钥或旧密钥索引");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (new_key_len > 0 && new_key_nr >= 0) {
        PrintAndLogEx(ERR, "请指定新密钥或索引，不要同时指定");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (new_key_len > 0) {
        ...
        PrintAndLogEx(ERR, "请指定新密钥或旧密钥索引");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
...
static int iclass_load_keys(char *filename) {
    ...
    if (bytes_read > ICLASS_KEYS_MAX *  PICOPASS_BLOCK_SIZE) {
        PrintAndLogEx(WARNING, "文件太长无法加载 - 字节数：%zu", bytes_read);
        free(dump);
        return PM3_EFILE;
    }
    size_t i = 0;
...
static int CmdHFiClassManageKeys(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 管理密钥",
                  "管理客户端内存中的 iCLASS 密钥",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "指定加载/保存操作的文件名"),
        arg_str0("k", "key", "<hex>", "访问密钥，8 个十六进制字节"),
    ...
    if (operation == 0) {
        PrintAndLogEx(ERR, "未指定操作（load、save 或 print）\\n");
        return PM3_EINVARG;
    }
    if (operation > 6) {
        PrintAndLogEx(ERR, "指定的操作过多\\n");
        return PM3_EINVARG;
    }
    if (operation > 4 && fnlen == 0) {
        PrintAndLogEx(ERR, "加载或保存时必须输入文件名\\n");
        return PM3_EINVARG;
    }
    if (key_len > 0 && key_nr == -1) {
        PrintAndLogEx(ERR, "指定密钥时请指定密钥索引");
        return PM3_EINVARG;
    }
...
static void add_key(uint8_t *key) {
    ...
    if (i == ICLASS_KEYS_MAX) {
        PrintAndLogEx(INFO, "找不到空的密钥槽");
    } else {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass managekeys -p") "` to view keys");
    }
}

static int CmdHFiClassCheckKeys(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 检查",
                  "Checkkeys加载包含8字节十六进制密钥的字典文本文件，测试对iCLASS标签的认证",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "包含默认iclass密钥的字典文件"),
        arg_lit0(NULL, "credit", "key is assumed to be the credit key"),
        arg_lit0(NULL, "elite", "elite computations applied to key"),
    ...
    if (keycount > 100000) {
        PrintAndLogEx(FAILED, "文件包含超过100,000个密钥，正在中止...");
        free(keyBlock);
        return PM3_EFILE;
    }

    // Get CSN / UID and CCNR
    PrintAndLogEx(SUCCESS, "读取标签 CSN / CCNR...");

    bool got_csn = false;
    for (uint8_t i = 0; i < ICLASS_AUTH_RETRY; i++) {
        got_csn = select_only(CSN, CCNR, false, shallow_mod);
        if (got_csn == false)
            PrintAndLogEx(WARNING, "再试一次");
        else
            break;
    }

    if (got_csn == false) {
        PrintAndLogEx(WARNING, "尝试了%d次。无法选择卡片，正在中止...", ICLASS_AUTH_RETRY);
        free(keyBlock);
        DropField();
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "正在生成多样化密钥 %s", (use_elite || use_raw) ? NOLF : "");
    ...
    for (chunk_offset = 0; chunk_offset < keycount; chunk_offset += max_chunk_size) {
        ...
        if (resp.status == PM3_SUCCESS) {
            ...
            PrintAndLogEx(INPLACE, "块 [%03d/%d]", chunk_offset, keycount);
            fflush(stdout);
        }
    }
...
static int iclass_recover(uint8_t key[8], uint32_t index_start, uint32_t loop, uint8_t no_first_auth[8], bool debug, bool test, bool fast, bool short_delay, bool allnight) {
    ...
    while (repeat == true) {
        ...
        if (resp.status == PM3_SUCCESS) {
            ...
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(WARNING, "iCLASS Key Bits Recovery: " _YELLOW_("通过键盘中止！"));
            repeat = false;
            ...
            if (memcmp(verification_mac, args->MAC_TAG2, 4) == 0) {
                ...
                if (!*(args->found)) {
                    *args->found = true;
                    ...
                    PrintAndLogEx(HINT, "提示: 运行 `"_YELLOW_("hf iclass unhash -k %s")"` to find the needed pre-images", sprint_hex_inrow(div_key, 8));
                    PrintAndLogEx(INFO, "完成！");
                    PrintAndLogEx(NORMAL, "");
                }
                pthread_mutex_unlock(args->log_lock);
...
static int CmdHFiClassLegBrute_MT(uint8_t epurse[8], uint8_t macs[8], uint8_t macs2[8], uint8_t startingKey[8], uint64_t index, int threads, bool debug) {
    ...
    if (thread_count > max_threads) {
        PrintAndLogEx(INFO, "线程数上限为可用 CPU 数量 (%d)", max_threads);
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        if (pthread_create(&tids[i], NULL, brute_thread, &args[i]) != 0) {
            PrintAndLogEx(WARNING, "创建线程%d失败，以%d个线程运行", i, i);
            thread_count = i;
            break;
        }
    }
    ...
    if (debug) {
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(INFO, "索引范围摘要 (%d 线程, 密钥空间 2^40 = %" PRIu64 "):", thread_count, (uint64_t)1 << 40);
    ...
    if (found == false) {
        PrintAndLogEx(WARNING, "在给定的密钥空间中未找到密钥");
    }
...
static int CmdHFiClassLegBrute(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 暴力破解",
...
static int CmdHFiClassLegacyRecover(const char *Cmd) {
    ...
    if (loop > 10000) {
        PrintAndLogEx(ERR, "循环过多，ARM容易崩溃。为安全起见，请指定小于10000的数字");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    } else if (test) {
        loop = 1;
    ...
    PrintAndLogEx(HINT, "提示：运行 `" _YELLOW_("hf iclass legbrute -h") "` with the partial key found");
    PrintAndLogEx(NORMAL, "");
    return PM3_SUCCESS;
...
static int CmdHFiClassUnhash(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 去哈希",
    ...
    PrintAndLogEx(INFO, "分散密钥... %s", sprint_hex_inrow(div_key, sizeof(div_key)));
    PrintAndLogEx(INFO, "-----------------------------------");
    invert_hash0(div_key);
    PrintAndLogEx(INFO, "-----------------------------------");
    PrintAndLogEx(INFO, "您现在可以通过使用 hashcat 破解 DES 来获取主密钥。");
    PrintAndLogEx(INFO, "创建一个文本文件，每行包含<preimage>:<csn>，然后与hashcat一起使用。");
    PrintAndLogEx(NORMAL, "");
...
static int CmdHFiClassLookUp(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 查找",
    ...
    if (live) {
        ...
        PrintAndLogEx(INFO, "模拟标签 - 等待读取器 CHECK 命令...");
        PrintAndLogEx(INFO, "Press " _GREEN_("`pm3 button`") " to abort");
        ...
        uint8_t num_mac = resp.oldarg[1];
        if (num_mac == 0) {
            PrintAndLogEx(WARNING, "未从读卡器捕获到CHECK命令");
            return PM3_ESOFT;
        }
        ...
        PrintAndLogEx(SUCCESS, "捕获的 CHECK:");
        ...
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(INFO, "正在搜索内置密钥表（标准+精英）...");
        ...
        if (check_known_default(csn, cap_epurse, nr, mac_r, found_key)) {
            ...
            PrintAndLogEx(WARNING, "在内置表中未找到密钥");
        }
        ...
        if (!live_found) {
            ...
            if (res == PM3_SUCCESS && live_keycount > 0) {
                ...
                if (live_item != NULL) {
                    ...
                    PrintAndLogEx(WARNING, "在 %s 中未找到密钥", std_file);
                }
                free(live_prekey);
            } else {
                PrintAndLogEx(WARNING, "加载字典失败: %s", std_file);
            }
            free(live_keyBlock);
        }
        ...
        if (!live_found) {
            ...
            if (res == PM3_SUCCESS && live_keycount > 0) {
                ...
                if (live_item != NULL) {
                    ...
                    PrintAndLogEx(WARNING, "在 %s 中未找到密钥", elite_file);
                }
                free(live_prekey);
            } else {
                PrintAndLogEx(WARNING, "加载字典失败: %s", elite_file);
            }
            free(live_keyBlock);
        }
        ...
        if (csn_len != 8 || epurse_len != 8 || macs_len != 8) {
            PrintAndLogEx(ERR, "需要CSN、ePurse和MAC（或使用--live）");
            return PM3_EINVARG;
        }
        ...
        PrintAndLogEx(SUCCESS, "CSN....... " _GREEN_("%s"), sprint_hex(csn, sizeof(csn)));
        PrintAndLogEx(SUCCESS, "电子钱包.... %s", sprint_hex(epurse, sizeof(epurse)));
        PrintAndLogEx(SUCCESS, "MACS...... %s", sprint_hex(macs, sizeof(macs)));
        PrintAndLogEx(SUCCESS, "CCNR...... " _GREEN_("%s"), sprint_hex(CCNR, sizeof(CCNR)));
        PrintAndLogEx(SUCCESS, "标签MAC... %s", sprint_hex(MAC_TAG, sizeof(MAC_TAG)));
    }
    ...
    PrintAndLogEx(INFO, "排序中...");

    // Sort mac list
    qsort(prekey, keycount, sizeof(iclass_prekey_t), cmp_uint32);

    PrintAndLogEx(SUCCESS, "正在搜索%s密钥...", _YELLOW_("DEBIT"));
...
void GenerateMacFrom(uint8_t *CSN, uint8_t *CCNR, bool use_raw, bool use_elite, uint8_t *keys, uint32_t keycnt, iclass_premac_t *list) {
    ...
    for (int i = 0; i < iclass_tc; i++) {
        ...
        if (res) {
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(WARNING, "创建pthreads失败。退出中");
            return;
        }
    }
...
void PrintPreCalc(iclass_prekey_t *list, uint32_t itemcnt) {
    ...
    for (int i = 0; i < itemcnt; i++) {
        ...
        if (i < 10) {
            ...
            PrintAndLogEx(SUCCESS, "... 跳过打印剩余部分");
        }
    }
}
...
static void generate_rev(uint8_t *data, uint8_t len) {
    ...
    PrintAndLogEx(SUCCESS, "置换密钥..... %s", sprint_hex_inrow(data, len));
    permute_rev(data, len, key);
    PrintAndLogEx(SUCCESS, "未置换密钥... %s", sprint_hex_inrow(key, len));
    shave(key, len);
    PrintAndLogEx(SUCCESS, "密钥.............. %s", sprint_hex_inrow(key, len));
    free(key);
}
static void generate(uint8_t *data, uint8_t len) {
    ...
    PrintAndLogEx(SUCCESS, "输入密钥...... %s", sprint_hex_inrow(data, len));
    permute(data, len, pkey);
    PrintAndLogEx(SUCCESS, "置换密钥... %s", sprint_hex_inrow(pkey, len));
    simple_crc(pkey, len, key);
    PrintAndLogEx(SUCCESS, "CRC校验密钥..... %s", sprint_hex_inrow(key, len));
    free(key);
    free(pkey);
}
...
static int CmdHFiClassPermuteKey(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 密钥置换",
                  "来自《黑暗之心》论文的置换函数。",
...
static int CmdHFiClassEncode(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 编码",
    ...
    if (use_emulator_memory == false) {
        if (key_nr < 0) {
            PrintAndLogEx(ERR, "缺少 --ki 或 --emu 的必需参数");
            return PM3_EINVARG;
        }
    ...
    if (bin_len > 64) {
        PrintAndLogEx(ERR, "二进制Wiegand字符串必须小于64位");
        return PM3_EINVARG;
    }
    ...
    if (use_emulator_memory) {
        ...
        PrintAndLogEx(HINT, "提示：您现在可以模拟了。参见 `" _YELLOW_("hf iclass sim -h") "`");
    } else {
        for (uint8_t i = 0; i < 4; i++) {
...
static int CmdHFiClassConfigCard(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf iclass 配置卡",
    ...
    if (midx >= 0) {
        if (midx < ICLASS_KEYS_MAX) {
            ...
            PrintAndLogEx(SUCCESS, "使用密钥[%d]作为新的读卡器主密钥" _GREEN_("%s"), midx, sprint_hex(iClass_Key_Table[midx], 8));
        } else {
            PrintAndLogEx(ERR, "--mrki 编号无效");
            return PM3_EINVARG;
        }
    }
    ...
    if (ccidx > -1 && ccidx < ARRAYLEN(iclass_config_options)) {
        ...
        if (strlen(item->desc) == 0) {
            PrintAndLogEx(ERR, "超出范围, %u (得到 %u)",  ARRAYLEN(iclass_config_options), ccidx);
            return PM3_EINVARG;
        }
        if (strstr(item->desc, "ELITE") != NULL && got_kr == false) {
            PrintAndLogEx(ERR, "请指定 ELITE 密钥 (--eki) !");
            return PM3_EINVARG;
        }
        if (strstr(item->desc, "Custom") != NULL && got_mk == false) {
            PrintAndLogEx(ERR, "请指定新标准主密钥 (--mrki) !");
            return PM3_EINVARG;
        }
        if (strstr(item->desc, "Restore") != NULL && card_kidx == -1) {
            PrintAndLogEx(ERR, "请指定当前读卡器密钥 (--ki) !");
            return PM3_EINVARG;
        }
...
static int CmdHFiClassSAM(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("v",  "详细",  "详细输出"),
        arg_lit0("k",  "keep",     "命令执行后保持场激活"),
        arg_lit0("n",  "nodetect", "跳过选择卡片和向 SAM 发送卡片详细信息"),
        arg_lit0("t",  "tlv",      "解码TLV"),
        arg_lit0(NULL, "break",    "stop tag interaction on nr-mac"),
        arg_lit0("p",  "prevent",  "假电子钱包更新"),
        arg_lit0(NULL, "shallow",  "shallow mod"),
        arg_strx0("d", "数据", "<hex>", "发送给SAM的DER编码命令"),
        arg_lit0("s", "snmp",  "数据为SNMP格式，无头部"),
        arg_lit0(NULL, "info",  "get SAM infos (version, serial number)"),
        arg_str0("f",  "file", "<fn>", "转储文件以模拟SAM而非真实卡片"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    switch (resp.status) {
        case PM3_SUCCESS:
            break;
        case PM3_ENOPACS:
            PrintAndLogEx(SUCCESS, "未找到PACS数据。卡为空？");
            return resp.status;
        default:
            PrintAndLogEx(WARNING, "SAM 选择失败");
            return resp.status;
    }
    ...
    if (d[0] == 0xbd && d[2] == 0x8a && d[4] == 0x03) {
        ...
        if (verbose) {
            PrintAndLogEx(INFO, "重放 Nr-MAC 以转储 SIO：");
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdHFiClassList,            AlwaysAvailable, "列出 iCLASS 历史"},
//    {"-----------", CmdHelp,                    AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"-----------", CmdHelp,                    IfPm3Iclass,     "------------------- " _CYAN_("操作") " -------------------"},
//    {"clone",       CmdHFiClassClone,           IfPm3Iclass,     "Create a HID credential to Picopass / iCLASS tag"},
    {"dump",        CmdHFiClassDump,            IfPm3Iclass,     "将Picopass / iCLASS标签转储到文件"},
    {"info",        CmdHFiClassInfo,            IfPm3Iclass,     "标签信息"},
    {"rdbl",        CmdHFiClass_ReadBlock,      IfPm3Iclass,     "读取Picopass / iCLASS块"},
    {"reader",      CmdHFiClassReader,          IfPm3Iclass,     "模拟 Picopass / iCLASS 读取器"},
    {"restore",     CmdHFiClassRestore,         IfPm3Iclass,      "将转储文件恢复到Picopass / iCLASS标签"},
    {"sniff",       CmdHFiClassSniff,           IfPm3Iclass,     "窃听Picopass / iCLASS通信"},
    {"view",        CmdHFiClassView,            AlwaysAvailable, "显示标签转储文件的内容"},
    {"wrbl",        CmdHFiClass_WriteBlock,     IfPm3Iclass,     "写入Picopass / iCLASS块"},
    {"creditepurse", CmdHFiClassCreditEpurse,   IfPm3Iclass,     "充值电子钱包值"},
    {"tear",        CmdHFiClass_TearBlock,      IfPm3Iclass,     "对iCLASS块执行撕拉攻击"},
    {"-----------", CmdHelp,                    AlwaysAvailable, "--------------------- " _CYAN_("Recovery") " --------------------"},
//    {"autopwn",     CmdHFiClassAutopwn,         IfPm3Iclass,     "Automatic key recovery tool for iCLASS"},
    {"chk",         CmdHFiClassCheckKeys,       IfPm3Iclass,     "检查密钥"},
    {"loclass",     CmdHFiClass_loclass,        AlwaysAvailable, "使用loclass执行暴力破解读卡器攻击"},
    {"lookup",      CmdHFiClassLookUp,          AlwaysAvailable, "使用认证跟踪检查字典文件中的密钥"},
    {"legrec",      CmdHFiClassLegacyRecover,   IfPm3Iclass,     "在提供有效的nr-mac组合的情况下，恢复旧卡分散密钥的24位"},
    {"legbrute",    CmdHFiClassLegBrute,        AlwaysAvailable, "暴力破解部分多样化密钥的40位，提供密钥的24位和两个有效的nr-macs"},
    {"unhash",      CmdHFiClassUnhash,          AlwaysAvailable, "反转分散密钥以在DES加密后检索hash0前像"},
    {"blacktears",  CmdHFiClass_BlackTears,     IfPm3Iclass,     "对新硅卡自动撕脱攻击以启用非安全页面模式"},
    {"-----------", CmdHelp,                    IfPm3Iclass,     "-------------------- " _CYAN_("Simulation") " -------------------"},
    {"sim",         CmdHFiClassSim,             IfPm3Iclass,     "模拟iCLASS标签"},
    {"tagsim",      CmdHFiClassTagSim,          IfPm3Iclass,     "从FC/CN和密钥模拟完整的iCLASS 2K标签"},
    {"eload",       CmdHFiClassELoad,           IfPm3Iclass,     "上传文件到模拟器内存"},
    {"esave",       CmdHFiClassESave,           IfPm3Iclass,     "将模拟器内存保存到文件"},
    {"esetblk",     CmdHFiClassESetBlk,         IfPm3Iclass,     "设置模拟器内存块数据"},
    {"eview",       CmdHFiClassEView,           IfPm3Iclass,     "查看模拟器内存"},
    {"-----------", CmdHelp,                    AlwaysAvailable, "---------------------- " _CYAN_("Utils") " ----------------------"},
    {"configcard",  CmdHFiClassConfigCard,      IfPm3Iclass,     "读卡器配置卡生成器"},
    {"calcnewkey",  CmdHFiClassCalcNewKey,      AlwaysAvailable, "计算多样化密钥（块3和4）以写入新密钥"},
    {"encode",      CmdHFiClassEncode,          AlwaysAvailable, "将二进制wiegand编码到块7"},
    {"encrypt",     CmdHFiClassEncryptBlk,      AlwaysAvailable, "加密给定块数据"},
    {"decrypt",     CmdHFiClassDecrypt,         AlwaysAvailable, "解密给定的块数据或标签转储文件" },
    {"managekeys",  CmdHFiClassManageKeys,      AlwaysAvailable, "管理与 iCLASS 命令一起使用的密钥"},
    {"permutekey",  CmdHFiClassPermuteKey,      AlwaysAvailable, "来自《黑暗之心》论文的置换函数"},
    {"-----------", CmdHelp,                    IfPm3Smartcard,  "----------------------- " _CYAN_("SAM") " -----------------------"},
    {"sam",         CmdHFiClassSAM,             IfPm3Smartcard,  "SAM测试"},
    {NULL, NULL, NULL, NULL}
};
