...
relay_socket_t relay_listen_accept(uint16_t port, relay_socket_t *listen_sock) {
    ...
    if (sock < 0) {
        PrintAndLogEx(ERR, "创建套接字失败 (%s)", strerror(errno));
    ...
    if (listen(sock, 1) < 0) {
        PrintAndLogEx(ERR, "监听套接字失败 (%s)", strerror(errno));
        close(sock);
        return RELAY_SOCKET_INVALID;
    }
    ...
    if (client < 0) {
        PrintAndLogEx(ERR, "接受连接失败 (%s)", strerror(errno));
        close(sock);
        return RELAY_SOCKET_INVALID;
    }
...
relay_socket_t relay_connect(const char *ip, uint16_t port) {
    ...
    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        PrintAndLogEx(ERR, "连接到%s:%u时出错 (%s)", ip, port, strerror(errno));
        close(sock);
        return RELAY_SOCKET_INVALID;
    }
