...
int relay_init(void) {
    ...
    if (ret != 0) {
        PrintAndLogEx(ERR, "WSAStartup失败，错误代码 %d", ret);
        return PM3_EFAILED;
    }
...
relay_socket_t relay_listen_accept(uint16_t port, relay_socket_t *listen_sock) {
    ...
    if (sock == INVALID_SOCKET) {
        PrintAndLogEx(ERR, "创建套接字失败 (WSA %d)", WSAGetLastError());
        return RELAY_SOCKET_INVALID;
    }
    ...
    if (listen(sock, 1) == SOCKET_ERROR) {
        PrintAndLogEx(ERR, "监听套接字失败 (WSA %d)", WSAGetLastError());
        closesocket(sock);
        return RELAY_SOCKET_INVALID;
    }
    ...
    if (client == INVALID_SOCKET) {
        PrintAndLogEx(ERR, "接受连接失败 (WSA %d)", WSAGetLastError());
        closesocket(sock);
        return RELAY_SOCKET_INVALID;
    }
...
relay_socket_t relay_connect(const char *ip, uint16_t port) {
    ...
    if (inet_pton(AF_INET, ip, &addr.sin_addr) <= 0) {
        PrintAndLogEx(ERR, "无效的 IP 地址... %s:%u", ip, port);
        closesocket(sock);
        return RELAY_SOCKET_INVALID;
    }

    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) == SOCKET_ERROR) {
        PrintAndLogEx(ERR, "连接到%s:%u时出错 (WSA %d)", ip, port, WSAGetLastError());
        closesocket(sock);
        return RELAY_SOCKET_INVALID;
    }
