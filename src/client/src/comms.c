...
#ifdef COMMS_DEBUG
    PrintAndLogEx(NORMAL, "正在发送 %s", "OLD");
#endif
...
#ifdef COMMS_DEBUG
    PrintAndLogEx(INFO, "正在发送 %s", ng ? "NG" : "MIX");
#endif

    if (!g_session.pm3_present) {
        PrintAndLogEx(INFO, "发送字节到proxmark失败 - 离线");
        return;
    }
    if (len > PM3_CMD_DATA_SIZE) {
...
static void storeReply(const PacketResponseNG *packet) {
    ...
    if ((cmd_head + 1) % CMD_BUFFER_SIZE == cmd_tail) {
        ...
        // circular buffer.
        PrintAndLogEx(FAILED, "警告：命令缓冲区即将覆盖命令！需要修复！");
        fflush(stdout);
    }
...
*uart_communication(void *targ) {
    ...
    while (connection->run) {
        ...
        if (is_receiving_raw) {
            ...
            if (bufferPos < bufferLen) {
                ...
                if (res == PM3_SUCCESS) {
                    ...
                    PrintAndLogEx(WARNING, "读取原始数据时出错：%zu/%zu, %d", bufferPos, bufferLen, res);
                    error = true;
                    if (res == PM3_ENOTTY) {
                        commfailed = true;
                    }
            ...
            if ((res == PM3_SUCCESS) && (rxlen == sizeof(PacketResponseNGPreamble))) {
                ...
                if (rx.magic == RESPONSENG_PREAMBLE_MAGIC) { // New style NG reply
                    ...
                    if (length > PM3_CMD_DATA_SIZE) {
                        PrintAndLogEx(WARNING, "收到长度不兼容的数据包帧：0x%04x", length);
                        error = true;
                    }
                    ...
                    if ((!error) && (length > 0)) { // Get the variable length payload
                        ...
                        if ((res != PM3_SUCCESS) || (rxlen != length)) {
                            ...
                            PrintAndLogEx(WARNING, "收到可变部分太短的数据包帧？%d/%d", rxlen, length);
                            error = true;
                            ...
                            if (rx.ng) {      // Received a valid NG frame
                                ...
                                if (length < sizeof(arg)) {
                                    PrintAndLogEx(WARNING, "收到长度不兼容的 MIX 数据包帧：0x%04x", length);
                                    error = true;
                                }
                        ...
                        if (rx.ng) {
                            ...
                            PrintAndLogEx(WARNING, "收到空的 MIX 数据包帧（长度：0x00）");
                            error = true;
                        }
                    ...
                    if (!error) {                        // Get the postamble
                        ...
                        if ((res != PM3_SUCCESS) || (rxlen != sizeof(PacketResponseNGPostamble))) {
                            PrintAndLogEx(WARNING, "收到没有后同步码的数据包帧");
                            error = true;
                        }
                    }
                    ...
                    if (!error) {                        // Check CRC, accept MAGIC as placeholder
                        ...
                        if (rx.crc != RESPONSENG_POSTAMBLE_MAGIC) {
                            ...
                            if ((first << 8) + second != rx.crc) {
                                PrintAndLogEx(WARNING, "收到 CRC 无效的数据包帧 %02X%02X <> %04X", first, second, rx.crc);
                                error = true;
                            }
                        }
                    }
...
#ifdef COMMS_DEBUG
                        PrintAndLogEx(NORMAL, "正在接收 %s：", rx.ng ? "NG" : "MIX");
#endif
#ifdef COMMS_DEBUG_RAW
                    ...
                    if ((res != PM3_SUCCESS) || (rxlen != sizeof(PacketResponseOLD) - sizeof(PacketResponseNGPreamble))) {
                        PrintAndLogEx(WARNING, "收到旧帧数据包，有效载荷太短？%d/%zu", rxlen, sizeof(PacketResponseOLD) - sizeof(PacketResponseNGPreamble));
                        error = true;
                    }
                    if (!error) {
#ifdef COMMS_DEBUG
                        PrintAndLogEx(NORMAL, "正在接收旧数据：");
#endif
#ifdef COMMS_DEBUG_RAW
                ...
                if (rxlen > 0) {
                    PrintAndLogEx(WARNING, "收到数据包帧前导码太短：%d/%zu", rxlen, sizeof(PacketResponseNGPreamble));
                    error = true;
                }
                if (res == PM3_ENOTTY) {
                    commfailed = true;
        ...
        if (connection->block_after_ACK) {
            ...
            if (ACK_received) {
#ifdef COMMS_DEBUG
                PrintAndLogEx(NORMAL, "收到 ACK，快速 TX 模式：忽略其他 RX 直到 TX");
#endif
                while (!txBuffer_pending) {
                    pthread_cond_wait(&txBufferSig, &txBufferMutex);
                }
    ...
    if (sp == INVALID_SERIAL_PORT) {
        ...
        if (*dev == NULL) {
            *dev = calloc(sizeof(pm3_device_t), sizeof(uint8_t));
            if (*dev == NULL) {
                PrintAndLogEx(ERR, "为 pm3_device_t 分配内存失败");
                uart_close(sp);
                sp = NULL;
                return false;
            }
...
bool OpenProxmark(pm3_device_t **dev, const char *port, bool wait_for_port, int timeout, bool flash_mode, uint32_t speed) {
    ...
    if (sp == INVALID_SERIAL_PORT) {
        ...
        PrintAndLogEx(HINT, "提示：尝试使用shell脚本 `" _YELLOW_("`./pm3 --list") "` to get a list of possible serial ports");
        sp = NULL;
        return false;
    } else if (sp == CLAIMED_SERIAL_PORT) {
...
int SetHfFieldTimeout(uint32_t timeout_sec, bool quiet) {
    ...
    if (WaitForResponseTimeoutW(CMD_SET_HF_FIELD_TIMEOUT, &resp, 1000, false) == false) {
        if (!quiet) {
            PrintAndLogEx(WARNING, "设置HF场超时时超时");
        }
        return PM3_ETIMEOUT;
    }

    if (resp.status != PM3_SUCCESS && !quiet) {
        PrintAndLogEx(WARNING, "HF场超时命令失败 (%d)", resp.status);
    }
    return resp.status;
}
...
int TestProxmark(pm3_device_t *dev) {
    ...
    PrintAndLogEx(SUCCESS, "通过 %s%s%s%s 与 PM3 通信",
    ...
    if (g_session.hf_field_timeout_sec > 0) {
        ...
        if (timeout_res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "应用高频场超时失败 (" _YELLOW_("%u") " s)", g_session.hf_field_timeout_sec);
        }
    }
    return PM3_SUCCESS;
}
    ...
    while (true) {
        ...
        while (getReply(response)) {
            ...
            if (response->cmd == CMD_WTX && response->length == sizeof(uint16_t)) {
                ...
                PrintAndLogEx(DEBUG, "已获取等待时间扩展请求 %i 毫秒", wtx);
                if (ms_timeout != (size_t) - 1) {
                    ms_timeout += wtx;
                }
            }
...
static bool dl_it(uint8_t *dest, uint32_t bytes, PacketResponseNG *response, size_t ms_timeout, bool show_warning, uint32_t rec_cmd) {
    ...
    while (true) {
        ...
        if (msclock() - tmp_clk > ms_timeout) {
            PrintAndLogEx(FAILED, "尝试从设备下载数据时超时");
            break;
        }

        if (msclock() - tmp_clk > 3000 && show_warning) {
            // 3 seconds elapsed (but this doesn't mean the timeout was exceeded)
            PrintAndLogEx(INFO, "等待Proxmark3响应...");
            PrintAndLogEx(INFO, "您可以通过按下 pm3 按钮取消此操作");
            show_warning = false;
        }
    }
