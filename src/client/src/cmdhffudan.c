...
static int CmdHFFudanReader(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf fudan 读卡器",
                  "读取复旦标签",
                  "hf fudan reader\n"
...
static int CmdHFFudanDump(const char *Cmd) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "选择复旦卡失败，退出中...");
        DropField();
        return PM3_SUCCESS;
    }
    ...
    if (t == FUDAN_NONE) {
        PrintAndLogEx(FAILED, "未检测到复旦卡。正在退出...");
        DropField();
        return PM3_SUCCESS;
    }
        ...
        for (uint8_t tries = 0; tries < FUDAN_BLOCK_READ_RETRY; tries++) {
            ...
            if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
                if (resp.status == PM3_SUCCESS) {
                    ...
                    PrintAndLogEx(NORMAL, "");
                    PrintAndLogEx(FAILED, "无法读取块%2d", b);
                }
            } else {
                PrintAndLogEx(NORMAL, "");
                PrintAndLogEx(WARNING, "尝试读取块 %2d 时命令执行超时", b);
            }
        }
...
static int CmdHFFudanWrBl(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0("d", "数据", "<hex>", "要写入的字节，4个十六进制字节"),
        arg_param_end
    };
    ...
    if (blen != MAX_FUDAN_BLOCK_SIZE) {
        PrintAndLogEx(WARNING, "块数据必须包含4个十六进制字节。得到 %i", blen);
        return PM3_EINVARG;
    }
...
static int CmdHFFudanView(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf fudan 查看",
                  "打印 FUDAN 转储文件 (bin/eml/json)",
...
static command_t CommandTable[] = {
    ...
    {"reader",  CmdHFFudanReader,   IfPm3Iso14443a,   "模拟复旦读取器"},
    {"dump",    CmdHFFudanDump,     IfPm3Iso14443a,   "将FUDAN标签转储为二进制文件"},
    //{"sim",    CmdHFFudanSim,     IfPm3Iso14443a,   "Simulate a fudan tag"},
    {"rdbl",    CmdHFFudanRdBl,     IfPm3Iso14443a,   "读取复旦标签"},
    {"view",    CmdHFFudanView,     AlwaysAvailable,  "显示标签转储文件的内容"},
    {"wrbl",    CmdHFFudanWrBl,     IfPm3Iso14443a,   "写入复旦标签"},
    {NULL,      NULL,               0, NULL}
};
