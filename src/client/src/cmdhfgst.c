...
static void gst_print_ose_info(const gst_ose_info_t *info) {
    ...
    if (info->have_wallet_type) {
        ...
        if (gst_wallet_type_is_android_pay(info->wallet_type, info->wallet_type_len) == false) {
            PrintAndLogEx(WARNING, "钱包类型不是AndroidPay。这可能不是Google Smart Tap。");
    ...
    if (info->have_ose_device_nonce) {
        ...
        PrintAndLogEx(INFO, "设备随机数 (C2)......... 不存在");
    }
    ...
    if (info->have_directory_device_nonce) {
        ...
        PrintAndLogEx(INFO, "设备随机数 (DF6E)....... 不存在");
    }
}
...
static int gst_select_ose(gst_ose_info_t *info, bool activate_field, bool keep_field_on) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "SELECT OSE 交换错误");
        return res;
    }
    if (sw != ISO7816_OK) {
        if (sw) {
            PrintAndLogEx(INFO, "未找到OSE.VAS.01。APDU响应：%04x - %s",
...
static int gst_select_smart_tap_v2(gst_select_info_t *info, bool keep_field_on) {
    ...
    int res = gst_exchange_chained(false, keep_field_on, 0x00, ISO7816_SELECT_FILE, 0x04, 0x00,
                                   ...
                                   response, sizeof(response), &response_len, &sw);
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "SELECT Smart Tap v2 交换错误");
        return res;
    }
    if (sw != ISO7816_OK) {
        if (sw) {
            PrintAndLogEx(INFO, "未找到 Smart Tap v2 小程序。APDU 响应: %04x - %s",
                          sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        } else {
            PrintAndLogEx(ERR, "APDU交换错误。卡片返回0x0000");
        }
...
static int gst_perform_get_data(uint32_t collector_id,
    ...
    for (uint8_t get_data_retry_count = 0; local_sw == 0x9100; get_data_retry_count++) {
        if (get_data_retry_count >= GST_MAX_GET_DATA_CHAINED_RETRIES) {
            PrintAndLogEx(ERR, "GET DATA 返回链式响应次数过多（最大=%" PRIu8 ")", GST_MAX_GET_DATA_CHAINED_RETRIES);
            return PM3_ETIMEOUT;
        }
...
static int gst_selftest_zlib(void) {
    ...
    PrintAndLogEx(INFO, "zlib 示例解压... ( %s )", inflate_ok ? _GREEN_("ok") : _RED_("fail"));
    ...
    PrintAndLogEx(INFO, "zlib 示例解析... ( %s )", parse_ok ? _GREEN_("ok") : _RED_("fail"));

    free(decompressed_payload);

    return (inflate_ok && parse_ok) ? PM3_SUCCESS : PM3_ESOFT;
#else
    PrintAndLogEx(INFO, "zlib 支持......... ( %s )", _YELLOW_("unsupported"));
    return PM3_ENOTIMPL;
#endif
}
...
static void gst_print_formatted_payload(const char *label, const uint8_t *payload, size_t payload_len) {
    ...
    if (is_printable_ascii(value, value_len)) {
        PrintAndLogEx(INFO, "%s \\\"%.*s\\\" (格式=0x%02X, ASCII)", label, (int)value_len, (const char *)value, format);
    } else {
        PrintAndLogEx(INFO, "%s %s（格式=0x%02X）", label, sprint_hex_inrow(value, value_len), format);
    }
}
...
static void gst_print_service_objects(const uint8_t *message, size_t message_len) {
    ...
    while (offset < message_len) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "解析顶层 NDEF 记录失败 (err=%d)", res);
            break;
        }
        ...
        if (res != PM3_SUCCESS || issuer_record.payload_len < 2) {
            PrintAndLogEx(WARNING, "服务值记录缺少有效的发行者字段");
            if (top_record.me) {
                break;
            }
            continue;
...
static int gst_read(const gst_read_config_t *cfg) {
    ...
    if (cfg->verbose) {
        ...
        PrintAndLogEx(ERR, "钱包类型不是AndroidPay。这可能不是Google Smart Tap。");
    ...
    if (device_nonce_len == 0) {
        ...
        PrintAndLogEx(ERR, "无法获取听筒/设备随机数");
        status = PM3_ESOFT;
        goto out;
    }
    ...
    if (cfg->verbose) {
        ...
        PrintAndLogEx(INFO, "收集器 ID.............. %" PRIu32 " (0x%08" PRIX32 ")", cfg->collector_id, cfg->collector_id);
        PrintAndLogEx(INFO, "长期密钥版本..... %" PRIu32 " (0x%08" PRIX32 ")", cfg->key_version, cfg->key_version);
        ...
        PrintAndLogEx(INFO, "实时认证....... %s", cfg->live_authentication ? _GREEN_("yes") : "no");
    }
    ...
    if (gst_status_is_transient_failure(negotiate_sw)) {
        PrintAndLogEx(WARNING, "NEGOTIATE 临时失败（序列=1），正在使用序列=3重试...");
        status = gst_build_negotiate_message(reader_nonce, reader_ephemeral_public_key,
                                             reader_signature, reader_signature_len,
                                             cfg->collector_id, cfg->key_version,
    ...
    status = gst_parse_negotiate_response(negotiate_response, negotiate_response_len,
                                          ...
                                          device_ephemeral_public_key, &device_ephemeral_public_key_len);
    if (status != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "解析 NEGOTIATE 响应失败");
        goto out;
    }
    ...
    if (gst_status_is_transient_failure(get_data_sw)) {
        PrintAndLogEx(WARNING, "GET DATA 临时失败，正在重试...");
    ...
    if (status != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "解析 SERVICE_RESPONSE / RECORD_BUNDLE 失败");
        goto out;
    }
    ...
    if (bundle_flags & 0x01) {
        ...
        if (status != PM3_SUCCESS) {
            PrintAndLogEx(ERR, "解密RECORD_BUNDLE负载失败");
            goto out;
        }
    ...
    if (bundle_flags & 0x02) {
        ...
        if (status != PM3_SUCCESS) {
            if (status == PM3_EOVFLOW) {
                PrintAndLogEx(ERR, "压缩载荷超过%d字节限制", GST_MAX_DECOMPRESSED_PAYLOAD);
            } else {
                PrintAndLogEx(ERR, "解压RECORD_BUNDLE负载失败 (zlib)");
            }
            goto out;
        }
...
#else
        PrintAndLogEx(WARNING, "RECORD_BUNDLE 已压缩（设置了 0x02 标志），但 zlib 不可用。跳过解压缩。");
#endif
    }
...
static int CmdHFGSTTest(const char *Cmd) {
    ...
    if (res == PM3_ENOTIMPL) {
        PrintAndLogEx(INFO, "测试 ( %s )", _YELLOW_("skipped"));
    } else {
        PrintAndLogEx(SUCCESS, "测试 ( %s )", (res == PM3_SUCCESS) ? _GREEN_("ok") : _RED_("fail"));
    }
    return res;
}
...
static int CmdHFGSTInfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gst 信息",
                  "选择 OSE / Smart Tap 小程序并打印功能。",
                  "hf gst info\n"
...
static int CmdHFGSTRead(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gst 读取",
                  "执行Google Smart Tap读取流程并打印解析的通行证对象。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("c", "collector-id,collectorid,cid", "<hex|dec>", "收集器标识符（32位值）"),
        arg_str1("p", "reader-private-key,readerprivkey,rpk", "<pem|der-b64|der-hex|scalar-b64|scalar-hex|path>", "读取器私钥：PEM、DER 十六进制、标量十六进制/base64 或文件路径"),
    ...
    if (CLIParamStrToBuf(arg_get_str(ctx, 1), (uint8_t *)collector_id_text, sizeof(collector_id_text), &collector_id_text_len) != 0 ||
        ...
        PrintAndLogEx(ERR, "无效的collector-id");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (session_id_len == 8) {
        ...
        PrintAndLogEx(ERR, "提供的会话ID必须为8字节（实际为%d）", session_id_len);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (reader_nonce_len == 32) {
        ...
        PrintAndLogEx(ERR, "如果提供，reader-nonce 必须是 32 字节（实际 %d）", reader_nonce_len);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (eph_priv_len == 32) {
        ...
        PrintAndLogEx(ERR, "如果提供，reader-ephemeral-private-key 必须是 32 字节（实际 %d）", eph_priv_len);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdHFGSTList,  AlwaysAvailable, "列出ISO 14443A/7816历史"},
    {"test",        CmdHFGSTTest,  AlwaysAvailable, "执行自检"},
    {"-----------", CmdHelp,       IfPm3Iso14443a,  "--------------------- " _CYAN_("操作") " ----------------------"},
    {"info",        CmdHFGSTInfo,  IfPm3Iso14443a,  "获取Google Smart Tap小程序信息"},
    {"read",        CmdHFGSTRead,  IfPm3Iso14443a,  "读取并解码Google Smart Tap通行对象"},
    {NULL, NULL, NULL, NULL}
};
