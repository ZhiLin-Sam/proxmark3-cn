...
static int print_authentication1(void) {
    PrintAndLogEx(INFO, "发起相互认证。此命令必须在 Auth2 命令之前执行");
    PrintAndLogEx(INFO, "且仅在 Auth2 命令成功后实现相互认证。");
...
static int print_authentication2(void) {
    PrintAndLogEx(INFO, "完成相互认证。");
    PrintAndLogEx(INFO, "此命令只能在 Auth1 命令之后执行。");
...
static json_t *felica_get_system_list(void) {
    ...
    if (root == NULL) {
        PrintAndLogEx(WARNING, "解析 `%s` 第 %d 行失败: %s", path, error.line, error.text);
        free(path);
        return NULL;
    }

    if (json_is_array(root) == false) {
        PrintAndLogEx(WARNING, "无效的 `%s` 格式，期望数组根", path);
        json_decref(root);
        free(path);
        return NULL;
    }
...
static bool waitCmdFelicaEx(bool iSelect, PacketResponseNG *resp, bool verbose, bool logging, uint32_t timeout_ms) {
    ...
    if (resp->status != PM3_SUCCESS) {
        if (logging) {
            PrintAndLogEx(WARNING, "FeliCa 命令失败 (%d)", resp->status);
        }
        return false;
    }
    ...
    do {
        ...
        if (WaitForResponseTimeout(CMD_HF_FELICA_COMMAND, &resp, 2500)) {
            ...
            if (loop) {
                ...
                if (status != PM3_SUCCESS) {
                    if (verbose) {
                        PrintAndLogEx(WARNING, "FeliCa 卡片选择失败 (%d)", status);
                    }
                    res = status;
                    break;
                }
            ...
            if (resp.length < sizeof(felica_card_select_t)) {
                if (verbose) {
                    PrintAndLogEx(WARNING, "FeliCa 卡片选择返回无效载荷");
                }
                res = PM3_ESOFT;
                break;
            }
...
static int CmdHFFelicaReader(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf felica 读卡器",
                  "作为 ISO 18092 / FeliCa 读取器。持续寻找 FeliCa 标签，直到按下 Enter 或 pm3 按钮",
...
static int info_felica(bool verbose) {
    ...
    if (WaitForResponseTimeout(CMD_HF_FELICA_COMMAND, &resp, 2500) == false) {
        DropField();
        if (verbose) PrintAndLogEx(WARNING, "FeliCa 卡片选择失败");
        return PM3_ESOFT;
    }
    ...
    if (resp.status != PM3_SUCCESS) {
        switch (resp.status) {
            case PM3_ETIMEOUT:
                if (verbose) {
                    PrintAndLogEx(WARNING, "卡片超时");
                }
                break;
            case PM3_EWRONGANSWER:
                if (verbose) {
                    PrintAndLogEx(WARNING, "卡片应答错误");
                }
                break;
            case PM3_ECRC:
                if (verbose) {
                    PrintAndLogEx(WARNING, "CRC 校验失败");
                }
                break;
            default:
                if (verbose) {
                    PrintAndLogEx(WARNING, "FeliCa 卡片选择失败 (%d)", resp.status);
                }
                break;
        }
        DropField();
...
static int CmdHFFelicaInfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 信息",
                  "基于FeliCa的标签读取器",
                  "hf felica 信息");
...
static int CmdHFFelicaSeacInfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica SEAC信息",
                  "获取关于 FeliCa SEAC 卡的信息",
                  "hf felica SEAC信息");
...
static void clear_and_send_command_ex(uint8_t flags, uint16_t datalen, uint8_t *data, uint16_t numbits, bool verbose, bool normalize_frame) {
    ...
    clearCommandBuffer();
    if (verbose) {
        PrintAndLogEx(INFO, "发送原始命令 - 帧: %s", sprint_hex(payload, payload_len));
    }
...
static int send_felica_payload_with_retries(uint8_t flags, uint16_t datalen, uint8_t *data, bool verbose,
    ...
    for (uint32_t attempt = 0; attempt <= retries; attempt++) {
        if (attempt > 0) {
            if (logging) {
                if (request_name) {
                    PrintAndLogEx(WARNING, "正在重试 %s（%" PRIu32 "/%" PRIu32 ")",
                                  request_name, attempt, retries);
                } else {
                    PrintAndLogEx(WARNING, "正在重试请求（%" PRIu32 "/%" PRIu32 ")",
                                  attempt, retries);
                }
            }
        ...
        if (expected_response_cmd >= 0) {
            ...
            if (frame_response->cmd_code[0] != (uint8_t)expected_response_cmd) {
                if (logging && attempt == retries) {
                    PrintAndLogEx(FAILED, "错误响应命令 0x%02X (期望 0x%02X)。",
...
static int felica_ensure_target_present(const uint8_t *custom_idm,
    ...
    if (mode == FELICA_IDM_RESOLVE_CHAINED) {
        if (last_known_card.IDm[0] == 0 || last_known_card.IDm[1] == 0) {
            PrintAndLogEx(WARNING, "无最后已知卡片！使用 `" _YELLOW_("hf felica 读卡器") "` first or set a custom IDm");
            return PM3_EINVARG;
        }
    ...
    if (last_known_card.IDm[0] != 0 && last_known_card.IDm[1] != 0) {
        ...
        PrintAndLogEx(WARNING, "缓存的 IDm 不再存在。正在轮询新标签...");
    } else {
    ...
    if (ret != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "轮询时未检测到FeliCa标签。");
        return ret;
    }
...
static void felica_print_node_discovery_method_used(felica_node_discovery_method_t method) {
    ...
    PrintAndLogEx(INFO, "使用的节点发现方法：%s", name);
}
        ...
        for (size_t j = 0; j < ARRAYLEN(FELICA_REQUEST_SERVICE_PROBE_ATTRIBUTES); j++) {
            ...
            if (batch_count == FELICA_REQUEST_SERVICE_DISCOVERY_BATCH_SIZE) {
                if (felica_request_service_send_probe_batch(flags, idm, retry_count,
                    ...
                    if (supported) {
                        PrintAndLogEx(WARNING, "节点发现因通信丢失而中断。");
                    }
                    return false;
                }
                batch_count = 0;
    ...
    for (uint16_t node_number = 0; node_number <= FELICA_MAX_NODE_NUMBER; node_number++) {
        for (size_t i = 0; i < ARRAYLEN(FELICA_REQUEST_SERVICE_PROBE_ATTRIBUTES); i++) {
            ...
            if (send_read_without_encryption_ex(*flags, sizeof(data), data, false,
                ...
                PrintAndLogEx(WARNING, "因通信丢失（可能卡片已移除），停止无加密读取发现。");
                if (discovered_count) {
...
static int felica_parse_node_discovery_method(const char *method_str, felica_node_discovery_method_t *method_out) {
    ...
    *method_out = FELICA_NODE_DISCOVERY_NONE;
    ...
    PrintAndLogEx(ERR, "未知的 --method `%s`。", method_str);
    PrintAndLogEx(INFO, "有效值: auto, request_code_list, search_service_code, request_service, read_without_encryption");
    return PM3_EINVARG;
}
...
static int felica_dump_discovery_visitor(const felica_discovered_node_t *node, void *ctx) {
    ...
    PrintAndLogEx(INFO, "服务 %04X %s", felica_to_network_order(node->node_code_le), attrib_str);
...
static int CmdHFFelicaAuthentication1(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 认证1",
    ...
    void *argtable[] = {
        ...
        arg_str0("k", "key",  "<hex>", "3des密钥，16字节"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    PrintAndLogEx(INFO, "读取器挑战（未加密）：%s", sprint_hex(nonce, 8));
    ...
    if (keylen == 24) {
        ...
        PrintAndLogEx(INFO, "3DES 主密钥: %s", sprint_hex(master_key, keylen));
    ...
    PrintAndLogEx(INFO, "3DES 加密的 M1c: %s", sprint_hex(output, sizeof(output)));
    ...
    PrintAndLogEx(INFO, "客户端发送 AUTH1 帧: %s", sprint_hex(data, datalen));
    ...
    if (auth1_response.frame_response.IDm[0]) {
        ...
        if (is_key_correct) {
            PrintAndLogEx(SUCCESS, "Auth1使用正确的密钥材料完成！");
            PrintAndLogEx(SUCCESS, "现在使用 Auth2 与 M3C 和相同密钥");
        } else {
            PrintAndLogEx(INFO, "3DES 密钥（交换解密）: %s", sprint_hex(rev_master_key, 16));
            PrintAndLogEx(INFO, "P2c: %s", sprint_hex(p2c, 8));
            PrintAndLogEx(ERR, "无法使用主密钥解密M2C（P1c != P2c）！");
            PrintAndLogEx(ERR, "可能是错误的密钥或错误的解密方法");
        }
    }
    return PM3_SUCCESS;
}
...
static int CmdHFFelicaAuthentication2(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 认证2",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0(NULL, "idm", "<hex>", "set custom IDm"),
        arg_str0("c", "cc", "<hex>", "M3c卡片挑战，8字节"),
        arg_str0("k", "key",  "<hex>", "3des M3c解密密钥，16字节"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
//    unsigned char m3c[8];  == cc
    ...
    if (keylen == 16) {
        ...
        PrintAndLogEx(INFO, "3DES 主密钥（加密）... %s", sprint_hex_inrow(key, sizeof(key)));
        PrintAndLogEx(INFO, "3DES 主密钥（解密）... %s", sprint_hex_inrow(rev_key, sizeof(rev_key)));
    } else {
        PrintAndLogEx(ERR, "无效的密钥长度");
    ...
    PrintAndLogEx(INFO, "3DES 解密后的 M3c = P3c... %s", sprint_hex_inrow(p3c, sizeof(p3c)));

    // Encrypt p3c with master_key
    unsigned char m4c[8];
    mbedtls_des3_crypt_ecb(&des3_ctx_enc, p3c, m4c);
    PrintAndLogEx(INFO, "3DES 加密后的 M4c......... %s", sprint_hex_inrow(m4c, sizeof(m4c)));
    ...
    PrintAndLogEx(INFO, "客户端发送 AUTH2 帧: %s", sprint_hex(data, datalen));
    clear_and_send_command(flags, datalen, data, 0);
    ...
    if (auth2_response.code[0] != 0x12) {
        PrintAndLogEx(SUCCESS, "Auth2响应：");
        PrintAndLogEx(SUCCESS, "IDtc.............. %s", sprint_hex(auth2_response.IDtc, sizeof(auth2_response.IDtc)));
        PrintAndLogEx(SUCCESS, "IDi (加密)... %s", sprint_hex(auth2_response.IDi, sizeof(auth2_response.IDi)));
        PrintAndLogEx(SUCCESS, "PMi (加密)... %s", sprint_hex(auth2_response.PMi, sizeof(auth2_response.PMi)));
    } else {
        PrintAndLogEx(ERR, "帧格式错误");
    }
    return PM3_SUCCESS;
}
...
static int CmdHFFelicaWritePlain(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 写块",
                  "使用此命令向无需认证的服务写入块数据。\\n\\n"
                  " - Mode shall be Mode0.\n"
    ...
    if (send_write_without_encryption(flags, datalen, data, 1, &wr_noCry_resp) == PM3_SUCCESS) {
        ...
        PrintAndLogEx(SUCCESS, "状态标志1... %s", sprint_hex(wr_noCry_resp.status_flags.status_flag1, sizeof(wr_noCry_resp.status_flags.status_flag1)));
        PrintAndLogEx(SUCCESS, "状态标志2... %s\\n", sprint_hex(wr_noCry_resp.status_flags.status_flag2, sizeof(wr_noCry_resp.status_flags.status_flag2)));
        if (wr_noCry_resp.status_flags.status_flag1[0] == 0x00 && wr_noCry_resp.status_flags.status_flag2[0] == 0x00) {
            PrintAndLogEx(SUCCESS, "数据写入成功！");
        } else {
            PrintAndLogEx(FAILED, "出错了！检查状态标志。");
        }
    }
...
static int CmdHFFelicaReadPlain(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 读块",
                  "使用此命令从无需认证的服务读取块数据。\\n\\n"
                  " - Mode shall be Mode0.\n"
...
static int CmdHFFelicaRequestResponse(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 请求响应",
                  "使用此命令验证卡片及其模式的存在。\\n"
...
static int CmdHFFelicaRequestSpecificationVersion(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 请求规格版本",
                  "使用此命令获取卡片操作系统版本。\\n"
                  "Response:\n"
    ...
    if (send_request_specification_version(flags, sizeof(request_specification_version_request),
                                           ...
                                           &specification_version_info) != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "未收到卡片响应");
        return PM3_ERFTRANS;
    }

    PrintAndLogEx(SUCCESS, "已获取请求响应");
    ...
    if (specification_version_info.has_specification_version) {
        ...
        if (specification_version_info.option_version_count < specification_version_info.number_of_option) {
            PrintAndLogEx(WARNING, "截断的选项版本列表：卡片返回了%u个条目，显示%zu个",
...
static int CmdHFFelicaResetMode(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 重置模式",
                  "使用此命令将模式重置为模式0。",
                  "hf felica resetmode\n"
    ...
    if (reset_mode_response.frame_response.IDm[0] != 0) {
        ...
        PrintAndLogEx(SUCCESS, "状态标志1... %s", sprint_hex(reset_mode_response.status_flags.status_flag1, sizeof(reset_mode_response.status_flags.status_flag1)));
        PrintAndLogEx(SUCCESS, "状态标志2... %s", sprint_hex(reset_mode_response.status_flags.status_flag2, sizeof(reset_mode_response.status_flags.status_flag2)));
    }
    return PM3_SUCCESS;
}
...
static int CmdHFFelicaRequestSystemCode(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 请求系统代码",
                  "使用此命令获取卡片注册的系统代码。"
...
static int CmdHFFelicaDump(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 转储",
                  "转储所有现有的区域码和服务码。\\n"
    ...
    if (ret == PM3_EOPABORTED) {
        PrintAndLogEx(WARNING, "未经授权的服务转储被用户中止。已发现 %" PRIu32 " node(s), visited %" PRIu32 " service(s), dumped %" PRIu32 " public service(s).",
                      discovered_nodes, dump_ctx.service_count, dump_ctx.public_service_count);
        return ret;
    }

    if (ret != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "无法使用 RequestCodeList/SearchServiceCode/RequestService/ReadWithoutEncryption 发现节点。");
        return ret;
    }
    ...
    PrintAndLogEx(SUCCESS, "未经授权的服务转储完成。已发现 %" PRIu32 " node(s), visited %" PRIu32 " service(s), dumped %" PRIu32 " public service(s).",
                  discovered_nodes, dump_ctx.service_count, dump_ctx.public_service_count);
...
static int CmdHFFelicaRequestService(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 请求服务",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("a", "all", "自动节点编号模式，遍历所有节点 1 < n < 32"),
        arg_str0("n", "node", "<hex>", "节点数量"),
        arg_str0("c", "code", "<hex>", "节点代码列表（小端序）"),
        arg_str0(NULL, "idm", "<hex>", "use custom IDm"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHFFelicaDiscoverNodes(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 发现节点",
                  "转储所有现有的区域码和服务码。\\n"
    ...
    PrintAndLogEx(HINT, "区域和服务代码以网络字节序打印。");
    ...
    if (ret == PM3_EOPABORTED) {
        ...
        PrintAndLogEx(WARNING, "用户中止节点发现，在 % 之后" PRIu64 " ms. Discovered %" PRIu32 " node(s): %" PRIu32 " area(s), %" PRIu32 " service(s).",
    ...
    if (ret != PM3_SUCCESS) {
        if (selected_method != FELICA_NODE_DISCOVERY_NONE) {
            PrintAndLogEx(FAILED, "节点发现失败，使用 --method %s。", felica_node_discovery_method_cli_name(selected_method));
        } else {
            PrintAndLogEx(FAILED, "无法使用 RequestCodeList/SearchServiceCode/RequestService/ReadWithoutEncryption 发现节点。");
        }
        return ret;
    ...
    felica_scsvcode_print_footer(&scsv_ctx);
    PrintAndLogEx(INFO, "节点发现持续时间：%" PRIu64 " ms", discovery_duration_ms);

    PrintAndLogEx(SUCCESS, "服务代码和区域转储完成。已发现%" PRIu32 " node(s): %" PRIu32 " area(s), %" PRIu32 " service(s).",
...
static int CmdHFFelicaDumpServiceArea(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica SCSV代码",
                  "转储所有现有的区域码和服务码。",
                  "hf felica scsvcode\n"
    ...
    if (ret == PM3_EOPABORTED) {
        felica_scsvcode_print_footer(&scsv_ctx);
        PrintAndLogEx(WARNING, "用户中止了服务代码和区域转储。已发现%" PRIu32 " node(s): %" PRIu32 " area(s), %" PRIu32 " service(s).",
                      discovered_nodes, scsv_ctx.area_count, scsv_ctx.service_count);
        return ret;
    }

    if (ret != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "无法使用 SearchServiceCode 发现节点。");
        return ret;
    }
...
static int CmdHFFelicaSniff(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 嗅探",
                  "从现场收集数据并保存到命令缓冲区。\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_u64_0("s", "samples", "<dec>", "要跳过的样本数"),
    ...
    if (payload.samples > 9999) {
        ...
        PrintAndLogEx(INFO, "样本太大无法跳过值，使用最大值9999");
    }

    if (payload.triggers  > 9999) {
        payload.triggers  = 9999;
        PrintAndLogEx(INFO, "触发值太大无法跳过，使用最大值9999");
    }
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf felica list") "` to view");
...
static int CmdHFFelicaSimLite(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 精简模拟",
                  "模拟ISO/18092 FeliCa Lite标签",
                  "hf felica litesim -u 1122334455667788"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_str1("u", "uid", "<hex>", "UID/NDEF2 8 个十六进制字节"),
        arg_param_end
    };
...
static int felica_make_block_list(uint16_t *out, const uint8_t *blk_numbers, const size_t length) {
    if (length > 4) {
        PrintAndLogEx(ERR, "felica_make_block_list: 超过最大大小");
        return PM3_EINVARG;
    }
...
static int felica_internal_authentication(
    ...
    if (memcmp(mac_blk, mac, FELICA_BLK_HALF) != 0) {
        PrintAndLogEx(ERR, "\nInternal Authenticate: " _RED_("失败"));
        return PM3_ERFTRANS;
    }
...
static int felica_external_authentication(
    ...
    if (res.status_flags.status_flag1[0] != 0x00 || res.status_flags.status_flag2[0] != 0x00) {
        PrintAndLogEx(ERR, "\nExternal Authenticate: " _RED_("失败"));
        return PM3_ERFTRANS;
    }
...
static int felica_mutual_authentication(
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "会话密钥(SK): %s", sprint_hex(auth_ctx.session_key, sizeof(auth_ctx.session_key)));
    }
...
static int CmdHFFelicaAuthenticationLite(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 精简认证",
                  "认证",
    ...
    void *argtable[] = {
        ...
        arg_str0("c", "", "<hex>", "设置随机挑战，16字节"),
        arg_str0(NULL, "idm", "<hex>", "set custom IDm"),
        arg_lit0("k", "", "接收后保持信号场开启"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    PrintAndLogEx(INFO, "卡片密钥: %s", sprint_hex(key, sizeof(key)));
    PrintAndLogEx(INFO, "随机挑战(RC): %s", sprint_hex(rc, sizeof(rc)));

    PrintAndLogEx(SUCCESS, "FeliCa lite - 认证开始");
...
static uint16_t PrintFliteBlock(uint16_t tracepos, uint8_t *trace, uint16_t tracelen) {
    ...
    PrintAndLogEx(NORMAL, "块号 %02x, 状态: %02x %02x", blocknum, status1, status2);
    switch (blocknum) {
        case 0x00:
        ...
        case 0x87:
            ...
            break;
        case 0x88: {
            PrintAndLogEx(NORMAL,  "内存配置（MC）：");
            PrintAndLogEx(NORMAL,  "写入状态需要MAC: %s", trace[3 + 12] ? "on" : "off");
            ...
            PrintAndLogEx(NORMAL,  "写入 CK 和 CKV 需要 MAC：%s", trace[3 + 5] ? "on" : "off");
            PrintAndLogEx(NORMAL,  "RF 参数: %02x", (trace[3 + 4] & 0x7));
            PrintAndLogEx(NORMAL,  "兼容 NDEF: %s", trace[3 + 3] ? "yes" : "no");
            PrintAndLogEx(NORMAL,  "内存配置可写：%s", (trace[3 + 2] == 0xff) ? "yes" : "no");
        ...
        break;
        case 0x92:
            PrintAndLogEx(NORMAL,  "状态:");
            PrintAndLogEx(NORMAL,  "轮询已禁用: %s", trace[3 + 8] ? "yes" : "no");
            PrintAndLogEx(NORMAL,  "已认证：%s", trace[3] ? "yes" : "no");
            break;
        case 0xa0:
            PrintAndLogEx(NORMAL,  "所有块的 CRC 匹配 : %s", (trace[3 + 2] == 0xff) ? "no" : "yes");
            break;
        default:
            PrintAndLogEx(WARNING,  "无效 %d: %s", blocknum, line);
            break;
    }
    return tracepos + 19;
}
...
static int CmdHFFelicaDumpLite(const char *Cmd) {
    ...
    */
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf felica 精简转储",
    ...
    if (keylen != 0) {
        ...
        if (ret) {
            PrintAndLogEx(WARNING, "认证失败");
        }
    }

    PrintAndLogEx(NORMAL, "");

    PrintAndLogEx(SUCCESS, "FeliCa lite - 转储开始");
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(WARNING, "FeliCa lite 转储失败 (%d)", resp.status);
        return resp.status;
    }

    if (resp.length < sizeof(felica_lite_dump_resp_t)) {
        PrintAndLogEx(WARNING, "意外的转储响应长度");
        return PM3_ESOFT;
    }
    ...
    if (tracelen == 0) {
        PrintAndLogEx(WARNING, "无跟踪数据！可能不是 FeliCa Lite 卡？");
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(SUCCESS, "记录的活动 (trace len = %"PRIu32" bytes)", (uint32_t)tracelen);
...
static int CmdHFFelicaCmdRaw(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("c",  NULL, "计算并附加CRC"),
        arg_lit0("k",  NULL, "接收后保持信号场开启"),
        ...
        arg_str1(NULL, NULL, "<hex>", "要发送的原始字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (datalen > 254) {
        PrintAndLogEx(FAILED, "FeliCa 原始载荷过长 (%d)。最大为 254 字节。", datalen);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(SUCCESS, "数据: %s", sprint_hex(data, datalen));
    ...
    if (reply) {
        ...
        if (active_select) {
            PrintAndLogEx(SUCCESS, "主动选择等待 FeliCa。");
...
static command_t CommandTable[] = {
    ...
    {"list",            CmdHFFelicaList,                  AlwaysAvailable, "列出ISO 18092/FeliCa历史"},
    {"-----------",     CmdHelp,                          AlwaysAvailable, "----------------------- " _CYAN_("操作") " -----------------------"},
    {"info",            CmdHFFelicaInfo,                  IfPm3Felica,     "标签信息"},
    {"seacinfo",        CmdHFFelicaSeacInfo,              IfPm3Felica,     "FeliCa SEAC标签信息"},
    {"raw",             CmdHFFelicaCmdRaw,                IfPm3Felica,     "向标签发送原始十六进制数据"},
    {"rdbl",            CmdHFFelicaReadPlain,             IfPm3Felica,     "从无需认证的服务读取块数据。"},
    {"reader",          CmdHFFelicaReader,                IfPm3Felica,     "模拟 ISO18092/FeliCa 读取器"},
    {"sniff",           CmdHFFelicaSniff,                 IfPm3Felica,     "嗅探ISO 18092/FeliCa通信"},
    {"wrbl",            CmdHFFelicaWritePlain,            IfPm3Felica,     "将块数据写入无需认证的服务。"},
    {"-----------",     CmdHelp,                          AlwaysAvailable, "----------------------- " _CYAN_("FeliCa Standard") " -----------------------"},
    {"dump",            CmdHFFelicaDump,                  IfPm3Felica,     "等待并尝试转储FeliCa"},
    {"discnodes",       CmdHFFelicaDiscoverNodes,         IfPm3Felica,     "发现区域码和服务码节点。"},
    {"rqservice",       CmdHFFelicaRequestService,        IfPm3Felica,     "验证区域和服务的存在，并获取密钥版本。"},
    {"rqresponse",      CmdHFFelicaRequestResponse,       IfPm3Felica,     "验证卡片及其模式的存在。"},
    {"scsvcode",        CmdHFFelicaDumpServiceArea,       IfPm3Felica,     "获取区域码和服务码。"},
    {"rqsyscode",       CmdHFFelicaRequestSystemCode,     IfPm3Felica,     "获取卡片注册的系统码。"},
    {"auth1",           CmdHFFelicaAuthentication1,       IfPm3Felica,     "认证卡片。使用Auth1开始双向认证"},
    {"auth2",           CmdHFFelicaAuthentication2,       IfPm3Felica,     "允许卡片认证读写器。完成双向认证"},
    ...
    {"rqspecver",       CmdHFFelicaRequestSpecificationVersion, IfPm3Felica,  "获取卡片操作系统版本。"},
    {"resetmode",       CmdHFFelicaResetMode,             IfPm3Felica,     "将模式重置为模式0。"},
    ...
    {"litesim",         CmdHFFelicaSimLite,               IfPm3Felica,     "模拟ISO/18092 FeliCa Lite标签"},
    {"liteauth",        CmdHFFelicaAuthenticationLite,    IfPm3Felica,     "认证卡片。"},
    {"litedump",        CmdHFFelicaDumpLite,              IfPm3Felica,     "等待并尝试转储FelicaLite"},
