...
static int CmdIOProxWatch(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "正在监听 IO Prox 卡片 - 将标签放在天线上");
...
int demodIOProx(bool verbose) {
    ...
    if (size < 65) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox GraphBuffer 中样本不足");
        free(bits);
        return PM3_ESOFT;
    }
    ...
    if (idx < 0) {
        if (g_debugMode) {
            if (idx == -1) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 样本不足");
            } else if (idx == -2) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 仅检测到噪声");
            } else if (idx == -3) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox FSK 解调过程中出错");
            } else if (idx == -4) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 前导码未找到");
            } else if (idx == -5) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO 大小不正确, 大小 %zu", size);
            } else if (idx == -6) {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 分隔位未找到");
            } else {
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 解调 FSK 错误 %d", idx);
            }
        }
        free(bits);
        return PM3_ESOFT;
    ...
    if (idx == 0) {
        if (g_debugMode) {
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox 数据未找到 - FSK 比特: %zu", size);
    ...
    PrintAndLogEx(DEBUG, "%d%d%d%d%d%d%d%d %d 设施", bits[idx + 18], bits[idx + 19], bits[idx + 20], bits[idx + 21], bits[idx + 22], bits[idx + 23], bits[idx + 24], bits[idx + 25], bits[idx + 26]);
    PrintAndLogEx(DEBUG, "%d%d%d%d%d%d%d%d %d 版本", bits[idx + 27], bits[idx + 28], bits[idx + 29], bits[idx + 30], bits[idx + 31], bits[idx + 32], bits[idx + 33], bits[idx + 34], bits[idx + 35]);
    PrintAndLogEx(DEBUG, "%d%d%d%d%d%d%d%d %d 代码1", bits[idx + 36], bits[idx + 37], bits[idx + 38], bits[idx + 39], bits[idx + 40], bits[idx + 41], bits[idx + 42], bits[idx + 43], bits[idx + 44]);
    PrintAndLogEx(DEBUG, "%d%d%d%d%d%d%d%d %d 代码2", bits[idx + 45], bits[idx + 46], bits[idx + 47], bits[idx + 48], bits[idx + 49], bits[idx + 50], bits[idx + 51], bits[idx + 52], bits[idx + 53]);
    PrintAndLogEx(DEBUG, "%d%d%d%d%d%d%d%d %d%d 校验和", bits[idx + 54], bits[idx + 55], bits[idx + 56], bits[idx + 57], bits[idx + 58], bits[idx + 59], bits[idx + 60], bits[idx + 61], bits[idx + 62], bits[idx + 63]);
    ...
    if (g_debugMode) {
        if (crc != calccrc)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - IO prox CRC 失败");

        PrintAndLogEx(DEBUG, "DEBUG: IO prox 索引: %d, 长度: %zu, 打印解调缓冲区:", idx, size);
...
static int CmdIOProxDemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 IO 解调",
                  "尝试查找ioProx前导码，如果找到则解码/解扰数据",
                  "低频 IO 解调"
                 );
...
static int CmdIOProxSim(const char *Cmd) {
    ...
    if ((cn & 0xFFFF) != cn) {
        cn &= 0xFFFF;
        PrintAndLogEx(INFO, "卡片号截断为16位 (ioProx): %u", cn);
    }
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdIOProxDemod,  AlwaysAvailable, "从GraphBuffer解调ioProx标签"},
    {"reader",  CmdIOProxReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",   CmdIOProxClone,  IfPm3Lf,         "克隆 ioProx 标签到 T55x7 或 Q5/T5555"},
    {"sim",     CmdIOProxSim,    IfPm3Lf,         "模拟ioProx标签"},
    {"watch",   CmdIOProxWatch,  IfPm3Lf,         "持续监测卡片。读取器模式"},
    {NULL, NULL, NULL, NULL}
};
