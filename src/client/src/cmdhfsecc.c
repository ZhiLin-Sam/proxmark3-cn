...
static int CmdHFHIDConfigCardInfo(const char *Cmd) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "与卡片交换 APDU 失败");
        return res;
    }
    if (resplen < 2) {
        PrintAndLogEx(ERR, "响应过短（%d 字节）", resplen);
        return PM3_ESOFT;
    }
    ...
    if (sw1 != 0x90 || sw2 != 0x00) {
        PrintAndLogEx(ERR, "卡片返回错误SW %02X%02X", sw1, sw2);
        return PM3_ESOFT;
    }
    ...
    if (!tag66) {
        PrintAndLogEx(ERR, "响应中未找到标签66（卡片数据）");
        return PM3_ESOFT;
    }
    ...
    if (!tag73) {
        PrintAndLogEx(ERR, "未找到标签73（卡片识别数据）");
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "  %-20s  %s", "Property", "值");
...
static int CmdHFHIDConfigSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("f", "file", "<fn>",  "包含UID、AID、SCP02Key、可选KDD/KVN的JSON文件（无.json扩展名）"),
        arg_int0("n", "num",  "<dec>", "在<n>次读卡器交互后退出。0 = 无限"),
        arg_param_end
    };
    ...
    if (uidlen != 4 && uidlen != 7 && uidlen != 10) {
        PrintAndLogEx(ERR, "UID必须是4、7或10字节（实际为%d）", uidlen);
        json_decref(root);
        return PM3_EINVARG;
    }
...
static int CmdHFHIDConfigSniff(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("i", "interactive", "控制台在嗅探完成或中止前不会返回"),
        arg_lit0("j", "jam",         "干扰特定 APDU 的响应（参见 -d/-a）"),
        arg_str0("d", "apdu",  "<hex>", "要干扰的 APDU 字节（默认：A0D4000000）"),
        arg_str0("a", "resp",  "<hex>", "干扰时的响应负载（默认：00009000）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if ((has_apdu || has_resp) && !jam) {
        PrintAndLogEx(ERR, "-d 和 -r 需要 -j（jam 模式）");
        return PM3_EINVARG;
    }
    ...
    if (interactive) {
        ...
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf seos list") "` to view captured tracelog");
...
static command_t CommandTable[] = {
    ...
    {"info",      CmdHFHIDConfigCardInfo,   IfPm3Iso14443a,   "读取并解码卡片识别数据（GP标签0066）"},
    {"sim",       CmdHFHIDConfigSim,        IfPm3Iso14443a,   "模拟HID iCLASS SE配置卡"},
    {"sniff",     CmdHFHIDConfigSniff,      IfPm3Iso14443a,   "嗅探读写器与卡片通信，干扰 A0 D4 APDU"},
    {NULL, NULL, NULL, NULL}
};
