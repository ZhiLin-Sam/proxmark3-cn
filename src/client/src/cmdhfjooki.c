...
static int jooki_encode(uint8_t *iv, uint8_t tid, uint8_t fid, uint8_t *uid, uint8_t *out) {
    if (out == NULL) {
        PrintAndLogEx(ERR, "(encode jooki) base64ndef 参数为空");
        return PM3_EINVARG;
    }

    out[0] = 0x00;
    if (iv == NULL || uid == NULL) {
        PrintAndLogEx(ERR, "(encode jooki) iv 或 uid 参数为空");
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(DEBUG, "编码结果.... %s", sprint_hex(enc, sizeof(enc)));
...
static int jooki_decode(uint8_t *b64, uint8_t *result) {
    ...
    PrintAndLogEx(DEBUG, "(decode_jooki) 明文......... %s", sprint_hex(result, sizeof(ndef)));
    return PM3_SUCCESS;
}
...
static int jooki_create_ndef(uint8_t *b64ndef, uint8_t *ndefrecord) {
    ...
    if (ndefrecord == NULL) {
        PrintAndLogEx(ERR, "(jooki_create_ndef) ndefrecord 参数为空");
        return PM3_EINVARG;
    }
    memcpy(ndefrecord,
...
static void jooki_printEx(uint8_t *b64, uint8_t *iv, uint8_t tid, uint8_t fid, uint8_t *uid, bool verbose) {
    ...
    PrintAndLogEx(INFO, "Figurine..... %02x %02x - " _GREEN_("%s, %s")
                  , tid
                  , fid
                  , (idx != -1) ?  jooks_figures[idx].typedesc : "不适用"
                  , (idx != -1) ?  jooks_figures[idx].figdesc : "不适用"
                 );
    ...
    PrintAndLogEx(INFO, "NDEF 原始数据..... %s", sprint_hex_inrow(ndefmsg, sizeof(ndefmsg)));
...
static int jooki_selftest(void) {
    ...
    for (int i = 0; i < ARRAYLEN(jooks); i++) {
        ...
        PrintAndLogEx(INFO, "编码URL.. %s（ %s ）", sprint_hex((const uint8_t *)jooks[i].b64, 12), jooks[i].b64);
        PrintAndLogEx(INFO, "Type......... %02x - " _GREEN_("%s") " ( %s )", tid, (idx != -1) ? jooks_figures[idx].typedesc : "不适用", tid_ok ? _GREEN_("ok") : _RED_("fail"));
        PrintAndLogEx(INFO, "Figurine..... %02x - " _GREEN_("%s") " ( %s )", fid, (idx != -1) ? jooks_figures[idx].figdesc : "不适用", fid_ok ? _GREEN_("ok") : _RED_("fail"));
        ...
        uint8_t ndefmsg[52] = {0};
        jooki_create_ndef(b64, ndefmsg);
        PrintAndLogEx(INFO, "NDEF原始数据... %s", sprint_hex(ndefmsg, sizeof(ndefmsg)));
...
static int CmdHF14AJookiEncode(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf jooki 编码",
                  "将Jooki令牌编码为base64 NDEF URI格式",
    ...
    if (ftid > 0x04 || ffid > 0x20) {
        PrintAndLogEx(ERR, "使用有效的 Figure Type ID 和 Figure ID");
        return PM3_EINVARG;
    }
    ...
    if (figure_abbr_val > 1) {
        PrintAndLogEx(ERR, "选择一个标签类型或使用塑像类型ID和塑像ID");
        return PM3_EINVARG;
    }

    if (figure_abbr_val == 1 && !figure_abbr) {
        PrintAndLogEx(ERR, "使用 --tid 和 --fid 或其中一个 figurine 类型");
        return PM3_EINVARG;
    }
    ...
    if (use_tag) {
        ...
        if (ulen != JOOKI_UID_LEN) {
            PrintAndLogEx(ERR, "错误的UID长度，期望%u，实际为%d", JOOKI_UID_LEN, ulen);
            return PM3_EINVARG;
        }
    }
...
static int CmdHF14AJookiDecode(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf jooki 解码",
                  "解码NDEF URI格式的base64编码Jooki令牌",
                  "hf jooki decode -d 7WzlgEzqLgwTnWNy"
                 );

    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<base64>", "base64 URL参数"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF14AJookiSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("b", "b64", "<base64>", "base64 URL参数"),
        arg_param_end
    };
    ...
    while (datalen) {
        ...
        if (mf_eml_set_mem_xt(data + counter, blockno, blocks_to_send, blockwidth) != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "无法设置仿真块: %3d", blockno);
            free(data);
            return PM3_ESOFT;
        }
    ...
    free(data);
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 14a list") "` to view trace log");
...
static int CmdHF14AJookiClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf jooki 克隆",
                  "将Jooki令牌写入Ultralight或NTAG标签",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("b", "b64",  "<base64>", "base64 URL参数"),
        arg_str0("d", "数据", "<hex>",    "原始NDEF字节"),
        arg_str0("p", "pwd",  "<hex>",    "认证密码(EV1/NTAG 4字节)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    uint8_t b64[JOOKI_B64_LEN] = {0x00};
    ...
    if (dlen != 52) {
        PrintAndLogEx(ERR, "错误的数据长度。期望52，实际为%d", dlen);
        return PM3_EINVARG;
    }
    ...
    while ((i * 4) < dlen) {
        ...
        PrintAndLogEx(SUCCESS, "写入块 %d ( %s )", packetw.block_no, resp.status == PM3_SUCCESS ? _GREEN_("ok") : _RED_("fail"));
        packetw.block_no++;
        i++;
    }

    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf mfu NDEF 读取") "` to view");
    PrintAndLogEx(INFO, "完成！");
    return PM3_SUCCESS;
}
...
static command_t CommandTable[] = {
    ...
    {"clone",  CmdHF14AJookiClone,   IfPm3Iso14443a,  "写入Jooki令牌"},
    {"decode", CmdHF14AJookiDecode,  AlwaysAvailable, "解码Jooki令牌"},
    {"encode", CmdHF14AJookiEncode,  AlwaysAvailable, "编码Jooki令牌"},
    {"sim",    CmdHF14AJookiSim,     IfPm3Iso14443a,  "模拟Jooki令牌"},
    {NULL, NULL, NULL, NULL}
};
