...
static void em4x70_print_info_result(const em4x70_tag_info_t *data) {
    ...
    PrintAndLogEx(INFO, "锁定位 0：%d", (data->Raw[3] & LOCKBIT_0) ? 1 : 0);
    PrintAndLogEx(INFO, "锁定位 1：%d", (data->Raw[3] & LOCKBIT_1) ? 1 : 0);
    PrintAndLogEx(INFO, "标签是%s。", (data->Raw[3] & LOCKBIT_0) ? _RED_("LOCKED") : _GREEN_("UNLOCKED"));
    PrintAndLogEx(INFO, "");
    PrintAndLogEx(NORMAL, "");
}
...
static int CmdEM4x70Info(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf em 4x70 信息",
                  "标签信息EM4x70\\n"
...
static int CmdEM4x70Write(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 EM 4x70 写入",
                  "写入EM4x70\\n",
    ...
    if (opts.block >= EM4X70_NUM_BLOCKS) {
        PrintAndLogEx(FAILED, "块必须在范围 [0, 15] 内，得到 %d", opts.block);
        return PM3_EINVARG;
    }
    if (value_len != 2) {
        PrintAndLogEx(FAILED, "字/数据长度必须为2字节，实际为 %d", value_len);
        return PM3_EINVARG;
    }
...
static int CmdEM4x70Brute(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x70 暴力破解",
    ...
    if (res == 2) {
        PrintAndLogEx(WARNING, "起始密钥参数必须在 [0, FFFF] 范围内");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    CLIParserFree(ctx);
    ...
    if (rnd_len != 7) {
        PrintAndLogEx(FAILED, "随机数长度必须为 7 字节，实际为 %d", rnd_len);
        return PM3_EINVARG;
    }

    if (frnd_len != 4) {
        PrintAndLogEx(FAILED, "F(RN)长度必须为4字节，实际为%d", frnd_len);
        return PM3_EINVARG;
    }
    ...
    if (result == PM3_EOPABORTED) {
        ...
        PrintAndLogEx(INFO, "部分密钥响应... %02X %02X", data.partial_key[0], data.partial_key[1]);
    } else {
...
static int CmdEM4x70Unlock(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 EM 4x70 解锁",
                  "通过发送PIN解锁EM4x70\\n"
                  "Default pin may be:\n"
                  " AAAAAAAA\n"
                  " 00000000\n",
    ...
    if (pin_len != 4) {
        PrintAndLogEx(FAILED, "PIN 长度必须为 4 字节，实际为 %d", pin_len);
        return PM3_EINVARG;
    }
...
static int CmdEM4x70Auth(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf em 4x70 认证",
    ...
    CLIParserFree(ctx);
    if (rn_len != 7) {
        PrintAndLogEx(FAILED, "随机数长度必须为 7 字节，实际为 %d", rn_len);
        return PM3_EINVARG;
    }
    if (frn_len != 4) {
        PrintAndLogEx(FAILED, "F(RN)长度必须为4字节，实际为%d", frn_len);
        return PM3_EINVARG;
    }
    ...
    if (PM3_SUCCESS == result) {
        PrintAndLogEx(INFO, "标签认证响应：%02X %02X %02X", data.grn.grn[0], data.grn.grn[1], data.grn.grn[2]);
...
static int CmdEM4x70SetPIN(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 EM 4x70 设置PIN",
                  "写入新PIN\\n",
...
static int CmdEM4x70SetKey(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 EM 4x70 设置密钥",
                  "向标签写入新的96位密钥\\n",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("k",  "key", "<hex>", "密钥为12个十六进制字节"),
        arg_param_end
    };
    ...
    CLIParserFree(ctx);
    if (key_len != 12) {
        PrintAndLogEx(FAILED, "密钥长度必须为 12 字节，实际为 %d", key_len);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(INFO,
                  "正在验证新密钥的认证: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
...
static int CmdEM4x70Recover_ParseArgs(const char *Cmd, em4x70_cmd_input_recover_t *out_results) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("k",  "key",    "<hex>", "密钥为6个十六进制字节"),
        arg_str1(NULL, "rnd",    "<hex>", "Random 56-bit"),
        arg_str1(NULL, "frn",    "<hex>", "F(RN) 28-bit as 4 hex bytes"),
    ...
    if (PM3_SUCCESS == result) {
        ...
        if (key_len != 6) {
            PrintAndLogEx(FAILED, "密钥长度必须为 6 字节，实际为 %d", key_len);
            result = PM3_EINVARG;
        }
        ...
        if (grn_len != 3) {
            PrintAndLogEx(FAILED, "G(RN)长度必须为3字节，实际为%d", grn_len);
            result = PM3_EINVARG;
        }
    }
...
static int CmdEM4x70Recover(const char *Cmd) {
    ...
    if (PM3_SUCCESS == result) {
        ...
        if (PM3_EOVFLOW == result) {
            PrintAndLogEx(ERR, "找到超过 %d 个潜在密钥。这出乎意料，可能是代码错误。", MAXIMUM_ID48_RECOVERED_KEY_COUNT);
            return result;
        } else if (PM3_SUCCESS != result) {
    ...
    PrintAndLogEx(INFO, "恢复 %d 个潜在密钥:", recover_ctx.data.potential_key_count);
    for (uint8_t i = 0; i < recover_ctx.data.potential_key_count; ++i) {
        ...
        PrintAndLogEx(INFO,
                      "潜在密钥 #%d: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
                      " -->  " _YELLOW_("lf em 4x70 auth --rnd %02X%02X%02X%02X%02X%02X%02X --frn %02X%02X%02X%02X")
                      " --> %02X%02X%02X",
                      i,
    ...
    if (recover_ctx.opts.verify) {
        ...
        PrintAndLogEx(WARNING, "尚未实现针对标签的自动验证。");
...
static int CmdEM4x70AutoRecover(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "步骤 1. 验证传递的参数与标签认证（安全检查）");
    ...
    if (PM3_ETIMEOUT == result) {
        ...
        PrintAndLogEx(FAILED, "使用新密钥认证返回 %02x %02x %02x"
                      , tag_grn.grn.grn[0]
                      , tag_grn.grn.grn[1]
                      , tag_grn.grn.grn[2]
                     );
    ...
    for (uint8_t block = 9; block > 6; --block) {
        ...
        //    lf em 4x70 write   -b N -d 0000
        PrintAndLogEx(INFO, "步骤 %d. 暴力破解块 %d 中的密钥位", step, block);
        ...
        if (PM3_ETIMEOUT == result) {
            PrintAndLogEx(FAILED, "等待回复超时");
            PrintAndLogEx(HINT, "提示: 块 %d 数据可能已被覆盖。请手动从步骤 %d 重新开始", block, step);
            return result;
        } else if (PM3_SUCCESS != result) {
        ...
        if (PM3_ETIMEOUT == result) {
            PrintAndLogEx(FAILED, "等待回复超时");
            PrintAndLogEx(HINT, "提示: 块 %d 数据已被覆盖。请手动从步骤 %d 重新开始", block, step);
            return result;
        } else if (PM3_SUCCESS != result) {
            PrintAndLogEx(FAILED, "Writing block %d ( " _RED_("fail") " )", block);
            PrintAndLogEx(HINT, "提示: 块 %d 数据已被覆盖。请手动从步骤 %d 重新开始", block, step);
        ...
        if (PM3_ETIMEOUT == result) {
            PrintAndLogEx(FAILED, "等待回复超时");
            PrintAndLogEx(HINT, "提示: 块 %d 数据 (" _GREEN_("%02X%02X") ") may need to be rewritten", block, brute.partial_key[0], brute.partial_key[1]);
            return result;
        } else if (PM3_SUCCESS != result) {
            PrintAndLogEx(FAILED, "Writing block %d ( " _RED_("fail") " )", block);
            PrintAndLogEx(HINT, "提示: 块 %d 数据 (" _GREEN_("%02X%02X") ") may need to be rewritten", block, brute.partial_key[0], brute.partial_key[1]);
    ...
    PrintAndLogEx(INFO, "步骤 5. 恢复密钥低48位的潜在值");
    ...
    PrintAndLogEx(INFO, "步骤 6. 验证哪个潜在密钥实际在标签上");
    ...
    if ((found_one_key == false) || found_more_than_one_key) {
        PrintAndLogEx(FAILED, "无法恢复多个潜在密钥中的任何一个");
        PrintAndLogEx(FAILED, "检查标签耦合/位置是否良好！");
        return PM3_EFAILED;
    } else {
...
static command_t CommandTable[] = {
    ...
    {"brute",       CmdEM4x70Brute,        IfPm3EM4x70,     "暴力破解EM4X70以找到部分密钥"},
    {"info",        CmdEM4x70Info,         IfPm3EM4x70,     "标签信息"},
    {"write",       CmdEM4x70Write,        IfPm3EM4x70,     "写入EM4x70"},
    {"unlock",      CmdEM4x70Unlock,       IfPm3EM4x70,     "解锁EM4x70以写入"},
    {"auth",        CmdEM4x70Auth,         IfPm3EM4x70,     "认证EM4x70"},
    {"setpin",      CmdEM4x70SetPIN,       IfPm3EM4x70,     "写入PIN"},
    {"setkey",      CmdEM4x70SetKey,       IfPm3EM4x70,     "写入密钥"},
    {"calc",        CmdEM4x70Calc,         AlwaysAvailable, "计算EM4x70挑战和响应"},
    {"recover",     CmdEM4x70Recover,      AlwaysAvailable, "从部分密钥恢复剩余密钥"},
    {"autorecover", CmdEM4x70AutoRecover,  IfPm3EM4x70,     "从可写标签恢复完整密钥"},
    {NULL, NULL, NULL, NULL}
};
