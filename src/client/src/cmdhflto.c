...
static void lto_print_cmwi(uint8_t *d) {
    ...
    PrintAndLogEx(INFO, "块 1 保护标志........ %s", (d[1] == 0) ? _GREEN_("uninitialised cartridge") : (d[1] == 1) ? "initialised cartridge" : "不适用");
...
//    uint32_t crc = bytes_to_num(d+60, 4);
    ...
    PrintAndLogEx(INFO, "                                                           ^^^^^^^^ CRC-32");
    PrintAndLogEx(INFO, "页 ID.................. ..." _YELLOW_("0x%04x"), page_id);
...
static int CmdHFLTOReader(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf lto 读取器",
                  "作为 LTO-CM 读取器。持续寻找 LTO-CM 标签，直到按下 Enter 或 pm3 按钮",
...
int infoLTO(bool verbose) {
    ...
    if (ret_val == PM3_SUCCESS) {
        ...
        if (type_info[1] > 3) {
            PrintAndLogEx(INFO, "未知的LTO标签，请报告给@iceman！");
        }
...
int wrblLTO(uint8_t blk, uint8_t *data, bool verbose) {
    ...
    if (ret_val == PM3_SUCCESS) {
        ...
        PrintAndLogEx(WARNING, "BLK %03d：写入错误。可能这是一个只读块地址。",  blk);
    }
...
static int CmdHfLTOWriteBlock(const char *Cmd) {
    ...
    if (res == PM3_SUCCESS)
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf lto 读块") "` to verify");
...
int dumpLTO(uint8_t *dump, bool verbose) {
    ...
    for (uint8_t i = 0; i < blocks; i++) {
        ...
        PrintAndLogEx(INPLACE, "...正在读取块 %d", i);
        fflush(stdout);
    }
...
static command_t CommandTable[] = {
    ...
    {"dump",     CmdHfLTODump,        IfPm3Iso14443a,  "将LTO-CM标签转储到文件"},
    {"info",     CmdHfLTOInfo,        IfPm3Iso14443a,  "标签信息"},
    {"list",     CmdHfLTOList,        AlwaysAvailable, "列出 LTO-CM 历史"},
    {"rdbl",     CmdHfLTOReadBlock,   IfPm3Iso14443a,  "读取块"},
    {"reader",   CmdHFLTOReader,      IfPm3Iso14443a,  "模拟 LTO-CM 读取器"},
    {"restore",  CmdHfLTRestore,      IfPm3Iso14443a,  "将转储文件恢复到LTO-CM标签"},
    {"wrbl",     CmdHfLTOWriteBlock,  IfPm3Iso14443a,  "写入块"},
    {NULL, NULL, NULL, NULL}
};
