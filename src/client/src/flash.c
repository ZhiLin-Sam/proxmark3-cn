...
static int build_segs_from_phdrs(flash_file_t *ctx, uint32_t flash_size) {
    ...
    if (!ctx->segments) {
        PrintAndLogEx(ERR, "内存不足");
        return PM3_EMALLOC;
    }
    ctx->num_segs = 0;
    seg = ctx->segments;

    PrintAndLogEx(SUCCESS, "正在加载可用的 ELF 段：");
    for (int i = 0; i < ctx->num_phdrs; i++) {
        ...
        PrintAndLogEx(SUCCESS, "   "_YELLOW_("%d")": V 0x%08x P 0x%08x (0x%08x->0x%08x) [%c%c%c] @0x%x",
                      ...
                      offset);
        if (filesz != memsz) {
            PrintAndLogEx(ERR, "错误：PHDR 文件大小不等于内存大小\\n"
                               "(DATA+BSS PHDRs do not make sense on ROM platforms!)");
            return PM3_EFILE;
        }
        if (paddr < last_end) {
            PrintAndLogEx(ERR, "错误：PHDR 未排序或重叠");
            return PM3_EFILE;
        }
        if (paddr < FLASH_START || (paddr + filesz) > flash_end) {
            PrintAndLogEx(ERR, "错误：PHDR 未包含在 Flash 中");
            if ((paddr + filesz) > flash_end) {
                PrintAndLogEx(ERR, "固件可能对于您的设备来说太大");
                PrintAndLogEx(ERR, "参见README.md了解如何在256KB闪存平台上编译");
            }
            return PM3_EFILE;
        }
        if (vaddr >= FLASH_START && vaddr < flash_end && (flags & PF_W)) {
            PrintAndLogEx(ERR, "错误：Flash VMA 段可写");
            return PM3_EFILE;
        }
        ...
        if (block_offset) {
            if (ctx->num_segs) {
                ...
                if (this_firstblock == prev_lastblock) {
                    ...
                    PrintAndLogEx(INFO, "注意: 将前一段从 0x%x 字节扩展到 0x%x 字节",
                                  prev_seg->length, new_length);
                    if (hole)
                        PrintAndLogEx(INFO, "注意: 创建了 0x%x 字节的空洞", hole);
                    free(data);
                    free(prev_seg->data);
            ...
            PrintAndLogEx(WARNING, "警告：段未从块边界开始，将进行填充");
...
static int check_segs(flash_file_t *ctx, int can_write_bl, uint32_t flash_size) {
    ...
    for (int i = 0; i < ctx->num_segs; i++) {
        ...
        if (seg->start & (BLOCK_SIZE - 1)) {
            PrintAndLogEx(ERR, "错误：段未对齐");
            return PM3_EFILE;
        }
        if (seg->start < FLASH_START) {
            PrintAndLogEx(ERR, "错误：段超出闪存边界");
            return PM3_EFILE;
        }
...
int flash_load(flash_file_t *ctx, bool force) {
    ...
    if (fd == NULL) {
        PrintAndLogEx(ERR, _RED_("无法打开文件") " %s  >>> ", ctx->filename);
        res = PM3_EFILE;
        goto fail;
    }
    ...
    if (bytes_read != fsize) {
        PrintAndLogEx(ERR, "错误，读取的字节数与文件大小不匹配");
        res = PM3_EFILE;
        goto fail;
    }
    ...
    if (memcmp(ehdr->e_ident, elf_ident, sizeof(elf_ident))
        ...
        PrintAndLogEx(ERR, "不是 ELF 文件或 ELF 类型错误");
        res = PM3_EFILE;
        goto fail;
    }

    if (le16(ehdr->e_type) != ET_EXEC) {
        PrintAndLogEx(ERR, "ELF 不可执行");
        res = PM3_EFILE;
        goto fail;
    }

    if (le16(ehdr->e_machine) != EM_ARM) {
        PrintAndLogEx(ERR, "ELF 架构错误");
        res = PM3_EFILE;
        goto fail;
    }

    if (!ehdr->e_phnum || !ehdr->e_phoff) {
        PrintAndLogEx(ERR, "ELF 没有 PHDR");
        res = PM3_EFILE;
        goto fail;
    }
    ...
    if (le16(ehdr->e_phentsize) != sizeof(Elf32_Phdr_t)) {
        ...
        PrintAndLogEx(ERR, "ELF文件或此代码存在缺陷");
        res = PM3_EFILE;
        goto fail;
    }
    ...
    if (res == PM3_EUNDEF)
        PrintAndLogEx(WARNING, "无法检查版本信息");

    if (force)
        return PM3_SUCCESS;

    PrintAndLogEx(INFO,  "请确保刷入正确且最新的版本");
    PrintAndLogEx(INFO,  "您可以使用 '--force' 选项强制刷写此固件");
fail:
    flash_free(ctx);
    return res;
}
...
static int enter_bootloader(char *serial_port_name, bool wait_appear) {
    ...
    if ((state & DEVICE_INFO_FLAG_CURRENT_MODE_OS) == DEVICE_INFO_FLAG_CURRENT_MODE_OS) {
        ...
        if (
            ...
            PrintAndLogEx(SUCCESS, "如果您的引导加载程序需要，请立即按住按钮。");
        }
        msleep(500);
...
static int wait_for_ack(PacketResponseNG *ack) {
    ...
    if (ack->cmd != CMD_ACK) {
        PrintAndLogEx(ERR, "错误：意外回复 0x%04x %s（期望 ACK）",
                      ack->cmd,
...
int flash_start_flashing(int enable_bl_writes, char *serial_port_name, uint32_t *max_allowed) {
    ...
    *max_allowed = 256;
    ...
    if (enable_bl_writes) {
        PrintAndLogEx(INFO, "允许的闪存范围：0x%08x-0x%08x", FLASH_START, flash_end);
    } else {
        PrintAndLogEx(INFO, "允许的闪存范围：0x%08x-0x%08x", BOOTLOADER_END, flash_end);
    }
...
int flash_write(flash_file_t *ctx) {
    ...
    PrintAndLogEx(SUCCESS, "正在为文件写入段: %s", ctx->filename);
    ...
    for (int i = 0; i < ctx->num_segs; i++) {
        ...
        while (length) {
            ...
            if (write_block(baddr, data, block_size) < 0) {
                PrintAndLogEx(ERR, "写入块 %d/%u 时出错", block, blocks);
                return PM3_EFATAL;
            }
