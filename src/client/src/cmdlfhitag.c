...
// [=]  5/0x05 | AC 3F 00 06 | .?..  | RW  | User
    ...
    if (show_header) {
        PrintAndLogEx(INFO, "");
        PrintAndLogEx(INFO, "--- " _CYAN_("可能的解扰模式") " -------------");
    }
...
static int ht2_check_dictionary(uint32_t key_count, uint8_t *keys,  uint8_t keylen, uint32_t *found_idx) {
    ...
    while (key_count--) {
        ...
        PrintAndLogEx(INPLACE, "正在检查密钥 %u / %u", cnt, toti);
...
static bool ht2_get_uid(uint32_t *uid) {
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - 获取 Hitag 2 UID 失败");
        return false;
    }
...
static int CmdLFHitagReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag 读取器",
...
static int CmdLFHitagRd(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0("k", "key", "<hex>", "密钥，4或6个十六进制字节"),
...
//    uint32_t page = arg_get_u32_def(ctx, 6, 0);
    ...
    if ((use_ht1 + use_ht2 + use_htm) > 1) {
        PrintAndLogEx(ERR, "错误，只能指定一种 Hitag 类型");
        return PM3_EINVARG;
    }
    if ((use_ht1 + use_ht2 + use_htm) == 0) {
        PrintAndLogEx(ERR, "错误，请指定一种 Hitag 类型");
        return PM3_EINVARG;
    }

    if (keylen != 0 && keylen != 4 && keylen != 6) {
        PrintAndLogEx(WARNING, "错误的KEY长度，期望0、4或6，实际为%d", keylen);
        return PM3_EINVARG;
    }

    if (nalen != 0 && nalen != 8) {
        PrintAndLogEx(WARNING, "错误的NR/AR长度，期望0或8，实际为%d", nalen);
        return PM3_EINVARG;
    }
    ...
    if (foo > 1) {
        PrintAndLogEx(WARNING, "仅指定一种认证模式");
        return PM3_EINVARG;
    } else if (foo == 0) {
        PrintAndLogEx(WARNING, "指定一种认证模式");
        return PM3_EINVARG;
    }

    if (use_ht2 && use_plain) { // not sure for the other types...
        PrintAndLogEx(WARNING, "选择的Hitag类型没有明文模式");
        return PM3_EINVARG;
    }
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - hitag 失败");
        return PM3_ESOFT;
    }
...
static int CmdLFHitagSCheckChallenges(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag 挑战-响应",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("f", "file", "<fn>", "要加载的文件名（无扩展名）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdLFHitagWriter(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0(NULL, "crypto", "crypto mode"),
        arg_str0("k", "key", "<hex>", "密钥，4或6个十六进制字节"),
        arg_int1("p", "page", "<dec>", "要写入的页地址"),
        arg_str1("d", "数据", "<hex>", "数据，4个十六进制字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (dlen != sizeof(data)) {
        PrintAndLogEx(WARNING, "数据长度错误，期望 4，实际 %d", dlen);
        return PM3_EINVARG;
    }
...
static int CmdLFHitag2Dump(const char *Cmd) {
    ...
    if (keylen != 0 &&
        ...
        PrintAndLogEx(WARNING, "错误的KEY长度，期望(0,4,6,8)，实际为%d", keylen);
        return PM3_EINVARG;
    }
    ...
    if (use_hts && use_pwd) { // not sure for the other types...
        PrintAndLogEx(WARNING, "选择的Hitag类型没有密码模式");
        return PM3_EINVARG;
    }
    ...
    if (use_ht2 && use_pwd) {
        ...
        do {
            ...
            switch (payload->status) {
                case -1: {
                    PrintAndLogEx(NORMAL, "");
                    PrintAndLogEx(FAILED, "无法选择标签！");
                    return PM3_ESOFT;
                }
                case -2: {
                    PrintAndLogEx(NORMAL, "");
                    PrintAndLogEx(FAILED, "找不到有效的加密命令！");
                    return PM3_ESOFT;
                }
                case -3: {
                    PrintAndLogEx(NORMAL, "");
                    PrintAndLogEx(FAILED, "找不到加密的 'read page0' 命令！");
                    return PM3_ESOFT;
                }
                case -4: {
                    PrintAndLogEx(NORMAL, "");
                    PrintAndLogEx(FAILED, "部分数据提取！");
                    continue;
                }
            }
...
static int CmdLFHitagView(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag 查看",
                  "打印 HITAG 转储文件 (bin/eml/json)",
...
static int CmdLFHitagEload(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag 电子加载",
                  "将hitag标签转储加载到设备上的模拟器内存",
                  "lf hitag eload -2 -f lf-hitag-11223344-dump.bin\n"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_str1("f", "file", "<fn>", "指定转储文件名"),
        arg_lit0("1", "ht1", "卡片类型 Hitag 1"),
        arg_lit0("2", "ht2", "卡片类型 Hitag 2"),
        arg_lit0("s", "hts", "卡片类型 Hitag S"),
        arg_lit0("m", "htm", "卡片类型 Hitag \\xce\\xbc"), // μ
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (bytes_read == HITAG2_MAX_BYTE_SIZE || bytes_read == (HITAG_BLOCK_SIZE * 64)) {
        ...
        PrintAndLogEx(ERR, "错误，转储文件大小错误。得到 %zu", bytes_read);
    }
...
static int CmdLFHitagEview(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag 电子查看",
                  "显示模拟器内存",
                  "lf hitag eview\n"
                 );
    void *argtable[] = {
...
static int CmdLFHitagSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("1", "ht1", "模拟 Hitag 1"),
        arg_lit0("2", "ht2", "模拟 Hitag 2"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdLFHitagSniff(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("lf hitag list")"` to view captured tracelog");
...
static int CmdLFHitag2Chk(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag 检查",
                  "对 Hitag 卡运行字典密钥或密码恢复。",
...
static int CmdLFHitag2Lookup(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "指定字典文件名"),
        arg_str0("k", "key", "<hex>", "将已知加密密钥指定为 6 个字节"),
        arg_str1("u", "uid", "<hex>", "将 UID 指定为 4 个十六进制字节"),
    ...
    if (inkeylen && inkeylen != 6) {
        PrintAndLogEx(INFO, "密钥长度错误。期望6，实际%i", inkeylen);
        return PM3_EINVARG;
    }

    if (ulen && ulen != 4) {
        PrintAndLogEx(INFO, "UID长度错误，期望4，实际为%i", ulen);
        return PM3_EINVARG;
    }

    if (nlen && nlen != 4) {
        PrintAndLogEx(INFO, "Nr长度错误。期望4，实际%i", nlen);
        return PM3_EINVARG;
    }

    if (alen && alen != 4) {
        PrintAndLogEx(INFO, "Ar长度错误，期望4，得到%i", alen);
        return PM3_EINVARG;
    }

    if (nalen && nalen != 8) {
        PrintAndLogEx(INFO, "NrAr长度错误。期望8，实际%i", nalen);
        return PM3_EINVARG;
    }
    ...
    */
    ...
    if (nlen && alen) {
        ...
        PrintAndLogEx(INFO, "未提供 nr 或 ar");
        return PM3_EINVARG;
    }
    ...
    if (inkeylen) {
        ...
        PrintAndLogEx(DEBUG, "密钥... %012" PRIx64, knownkey);
        ...
        PrintAndLogEx(DEBUG, "c位.............. %08x", cbits);
        PrintAndLogEx(DEBUG, "c位异或FFFFFFFF... %08x", cbits ^ 0xFFFFFFFF);
        PrintAndLogEx(DEBUG, "Ar.................. %08" PRIx32 "  ( %s )", ar, (isok) ? _GREEN_("ok") : _RED_("fail"));

        PrintAndLogEx(INFO, "Nr/Ar匹配密钥（%s）", (isok) ? _GREEN_("ok") : _RED_("fail"));
        PrintAndLogEx(NORMAL, "");
        return PM3_SUCCESS;
    }
...
static int CmdLFHitag2Crack2(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag 破解2",
...
//        fflush(stdout);
        ...
        if (resp.status == PM3_SUCCESS) {
            ...
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(SUCCESS, "Nonce 重放和长度扩展攻击 ( %s )", _GREEN_("ok"));
            PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("tools/hitag2crack/crack2/ht2crack2search <FILE_with_above_bytes>") "`");
            break;
        } else {
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(FAILED, "Nonce 重放和长度扩展攻击 ( %s )", _RED_("fail"));
            break;
        }
...
static uint64_t hitag2_verify_crypto_test(void) {
    ...
    PrintAndLogEx(DEBUG, "UID... %08" PRIx32, uid);
    PrintAndLogEx(DEBUG, "IV.... %08" PRIx32, iv);
    PrintAndLogEx(DEBUG, "密钥... %012" PRIx64, key);

    // initialise
    hitag_state_t state;
    ht2_hitag2_init_ex(&state, key, uid, iv);
    PrintAndLogEx(DEBUG, "hs 移位寄存器... %012" PRIx64, state.shiftreg);
...
static uint64_t hitag2_verify_crypto_test_round(void) {
    ...
    PrintAndLogEx(DEBUG, "hs 移位寄存器... %012" PRIx64, cs);
...
static int CmdLFHitag2Selftest(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "加密自检 ( %s )", test ? _GREEN_("ok") : _RED_("fail"));

    test |= hitag2_verify_crypto_test_round();
    PrintAndLogEx(INFO, "加密自检轮次 ( %s )", test ? _GREEN_("ok") : _RED_("fail"));

    test |= hitag2_benchtest(1);
    PrintAndLogEx(INFO, "Hitag 2 加密，初始化+生成32位 ( 微秒 %" PRIu64 " )", test);

    test |= hitag2_benchtest_gen32();
    PrintAndLogEx(INFO, "Hitag 2 加密，仅生成新32位 ( 微秒: %" PRIu64 " )", test);

    test |= hitag2_benchtest(1000);
    PrintAndLogEx(INFO, "Hitag 2 加密，初始化+生成32位，x1000 ( 微秒: %" PRIu64 " )", test);

    PrintAndLogEx(INFO, "--------------------------------------------------------");
    PrintAndLogEx(SUCCESS, "测试 ( %s )", (test) ? _GREEN_("ok") : _RED_("fail"));
    PrintAndLogEx(NORMAL, "");
    return PM3_SUCCESS;
}
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdLFHitagList,             AlwaysAvailable, "列出Hitag跟踪历史"},
    {"hts",         CmdLFHitagS,                AlwaysAvailable, "{ Hitag S/8211 操作 }"},
    {"htu",         CmdLFHitagU,                AlwaysAvailable, "{ Hitag µ/8265 操作 }"},
    {"-----------", CmdHelp,                    IfPm3Hitag,      "------------------------ " _CYAN_("常规") " ------------------------"},
    {"info",        CmdLFHitagInfo,             IfPm3Hitag,      "标签信息"},
    {"reader",      CmdLFHitagReader,           IfPm3Hitag,      "模拟 Hitag 2 读取器"},
    {"test",        CmdLFHitag2Selftest,        AlwaysAvailable, "执行自检"},
    {"-----------", CmdHelp,                    IfPm3Hitag,      "----------------------- " _CYAN_("操作") " -----------------------"},
//    {"demod",       CmdLFHitag2PWMDemod,        IfPm3Hitag,      "PWM Hitag 2 reader message demodulation"},
    {"dump",        CmdLFHitag2Dump,            IfPm3Hitag,      "转储Hitag 2标签"},
    {"read",        CmdLFHitagRd,               IfPm3Hitag,      "读取Hitag内存"},
    {"sniff",       CmdLFHitagSniff,            IfPm3Hitag,      "窃听Hitag通信"},
    {"view",        CmdLFHitagView,             AlwaysAvailable, "显示标签转储文件的内容"},
    {"wrbl",        CmdLFHitagWriter,           IfPm3Hitag,      "写入Hitag存储器中的一个块（页）"},
    {"-----------", CmdHelp,                    IfPm3Hitag,      "----------------------- " _CYAN_("Simulation") " -----------------------"},
    {"eload",       CmdLFHitagEload,            IfPm3Hitag,      "上传文件到模拟器内存"},
//    {"esave",       CmdLFHitagESave,            IfPm3Hitag,      "Save emulator memory to file"},
    {"eview",       CmdLFHitagEview,            IfPm3Hitag,      "查看模拟器内存"},
    {"sim",         CmdLFHitagSim,              IfPm3Hitag,      "模拟Hitag应答器"},
    {"-----------", CmdHelp,                    IfPm3Hitag,      "----------------------- " _CYAN_("Recovery") " -----------------------"},
    {"cc",          CmdLFHitagSCheckChallenges, IfPm3Hitag,      "Hitag S：测试所有提供的挑战"},
    {"crack2",      CmdLFHitag2Crack2,          IfPm3Hitag,      "恢复2048位加密流"},
    {"chk",         CmdLFHitag2Chk,             IfPm3Hitag,      "检查密钥"},
    {"lookup",      CmdLFHitag2Lookup,          AlwaysAvailable, "使用认证跟踪检查字典文件中的密钥"},
    {"ta",          CmdLFHitag2CheckChallenges, IfPm3Hitag,      "Hitag 2：测试所有记录的认证"},
    { NULL, NULL, 0, NULL }
};
