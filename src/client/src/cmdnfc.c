...
void print_type4_cc_info(uint8_t *d, uint8_t n) {
    if (n < 0x0F) {
        PrintAndLogEx(WARNING, "从 CC 文件读取的字节数不足");
        return;
    }
    ...
    PrintAndLogEx(SUCCESS, "    read   ( %02X ) protection: %s", d[13], ((d[13] & 0x80) == 0x80) ? _RED_("enabled") : _GREEN_("已禁用"));
    PrintAndLogEx(SUCCESS, "    write  ( %02X ) protection: %s", d[14], ((d[14] & 0x80) == 0x80) ? _RED_("enabled") : _GREEN_("已禁用"));
...
#define MAX_NDEF_LEN  2048
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("d",  "数据", "<hex>", "要解码的NDEF数据"),
        arg_str0("f", "file", "<fn>", "要加载的文件"),
    ...
    if (((datalen != 0) && (fnlen != 0)) || ((datalen == 0) && (fnlen == 0))) {
        PrintAndLogEx(ERR, "您必须提供十六进制数据或文件名");
        return PM3_EINVARG;
    }
    int res = PM3_SUCCESS;
    if (fnlen != 0) {
...
//    {"format",     CmdNFCType1Format,  IfPm3Iso14443a,  "format ISO-14443-a tag as NFC Tag"},
    {"read",        CmdNFCType1Read,  IfPm3Iso14443a,  "读取NFC论坛标签类型1"},
//    {"write",        CmdNFCType1Write, IfPm3Iso14443a, "write NFC Forum Tag Type 1"},
    {"--------",    CmdNFCType1Help,  AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",        CmdNFCType1Help,  AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
//    {"format",     CmdNFCType2Format,  IfPm3Iso14443a,  "format ISO-14443-a tag as NFC Tag"},
    {"read",        CmdNFCType2Read,  IfPm3Iso14443a,  "读取NFC论坛标签类型2"},
//    {"write",        CmdNFCType2Write, IfPm3Iso14443a, "write NFC Forum Tag Type 2"},
    {"--------",    CmdNFCType2Help,  AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",        CmdNFCType2Help,  AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
//    {"write",       CmdNFCType3Write, IfPm3Felica, "write NFC Forum Tag Type 3"},
    {"--------",    CmdNFCType3Help,  AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
...
static command_t CommandNFCType4ATable[] = {
    ...
    {"format",       CmdNFCType4AFormat, IfPm3Iso14443a,  "将ISO-14443-a标签格式化为NFC标签"},
    {"read",         CmdNFCType4ARead,   IfPm3Iso14443a,  "读取NFC论坛标签类型4 A"},
    {"write",        CmdNFCType4AWrite,  IfPm3Iso14443a,  "写入 NFC 论坛标签类型 4 A"},
...
//    {"mfdesformat",  CmdNFCMFDESFormat, IfPm3Iso14443a,  "format MIFARE DESfire as NFC Forum Tag Type 4"},
    {"st25taread",   CmdNFCST25TARead,   IfPm3Iso14443a,  "将ST25TA作为NFC论坛标签类型4读取"},

    {"--------",     CmdNFCType4AHelp,   AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",         CmdNFCType4AHelp,   AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
//    {"format",     CmdNFCType4BFormat,  IfPm3Iso14443b,  "format ISO-14443-b tag as NFC Tag"},
    {"read",        CmdNFCType4BRead,  IfPm3Iso14443b,  "读取NFC论坛标签类型4 B"},
//    {"write",       CmdNFCType4BWrite, IfPm3Iso14443b,  "write NFC Forum Tag Type 4 B"},
    {"--------",    CmdNFCType4BHelp,  AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",        CmdNFCType4BHelp,  AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
//    {"write",       CmdNFCType5Write, IfPm3Iso15693,   "write NFC Forum Tag Type 5"},
    {"--------",    CmdNFCType5Help,  AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
...
static command_t CommandMFTable[] = {
    ...
    {"cformat",     CmdNFCMFCFormat,  IfPm3Iso14443a,  "将MIFARE Classic标签格式化为NFC标签"},
    {"cread",       CmdNFCMFCRead,    IfPm3Iso14443a,  "读取NFC类型MIFARE Classic标签"},
    {"cwrite",      CmdNFCMFCWrite,  IfPm3Iso14443a,   "写入 NFC 类型 MIFARE Classic 标签"},
    {"pread",       CmdNFCMFPRead,    IfPm3Iso14443a,  "读取NFC类型MIFARE Plus标签"},
    {"--------",    CmdNFCMFHelp,     AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",        CmdNFCMFHelp,     AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
static command_t CommandBarcodeTable[] = {
    ...
    {"read",        CmdNFCBarcodeRead,     IfPm3Iso14443a,  "读取NFC条码"},
    {"sim",         CmdNFCBarcodeSim,      IfPm3Iso14443a,  "模拟NFC条码"},
    {"--------",    CmdNFCBarcodeHelp,     AlwaysAvailable, "--------------------- " _CYAN_("常规") " ---------------------"},
    {"help",        CmdNFCBarcodeHelp,     AlwaysAvailable, "此帮助"},
    {NULL, NULL, NULL, NULL}
};
...
//    {"type5",       CmdNFCType5,      AlwaysAvailable, "{ NFC Forum Tag Type 5...             }"},
    {"mf",          CmdNFCMF,         AlwaysAvailable, "{ NFC 类型 MIFARE Classic/Plus 标签... }"},
...
//    {"snep",        CmdNFCSNEP,       AlwaysAvailable, "{ Simple NDEF Exchange Protocol...    }"},
    ...
    {"decode",      CmdNfcDecode,     AlwaysAvailable, "解码NDEF记录"},
