...
static int CmdAnalyseLfsr(const char *Cmd) {
    ...
    if (res) {
        PrintAndLogEx(FAILED, "解析FIND字节错误");
        return PM3_EINVARG;
    }
...
static int CmdAnalyseLRC(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<hex>", "计算LRC中缺失XOR的字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    int dlen = 0;
...
static int CmdAnalyseCRC(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "\\n使用 (%d) | %s 测试", dlen, sprint_hex(data, (size_t)dlen));

    // 51  f5  7a  d6
    uint8_t uid[] = {0x51, 0xf5, 0x7a, 0xd6}; //12 34 56
    init_table(CRC_LEGIC);
    uint8_t legic8 = CRC8Legic(uid, sizeof(uid)) & 0xFF;
    PrintAndLogEx(INFO, "Legic 16 | %X (期望 EF6F) [legic8 = %02x]", crc16_legic(data, (size_t)dlen, legic8), legic8);
    ...
    PrintAndLogEx(INFO, "已确认的CRC实现");
    PrintAndLogEx(INFO, "-------------------------------------\n");
    PrintAndLogEx(INFO, "基于 CRC 8\\n\\n");
    PrintAndLogEx(INFO, "LEGIC: CRC8 : %X (期望 C6)", legic8);
    PrintAndLogEx(INFO, "MAXIM: CRC8 : %X (预期A1)", CRC8Maxim(dataStr, sizeof(dataStr)));
    PrintAndLogEx(INFO, "-------------------------------------\n");
    PrintAndLogEx(INFO, "基于CRC16\\n\\n");
    ...
    PrintAndLogEx(INFO, "FeliCa | %04X (期望 B37F)", Crc16ex(CRC_FELICA, poll + 2, sizeof(poll) - 4));
    PrintAndLogEx(INFO, "FeliCa | %04X (期望 0000)", Crc16ex(CRC_FELICA, poll + 2, sizeof(poll) - 2));

    uint8_t sel_corr[] = { 0x40, 0xe1, 0xe1, 0xff, 0xfe, 0x5f, 0x02, 0x3c, 0x43, 0x01};
    PrintAndLogEx(INFO, "iCLASS | %04x (期望 0143)", Crc16ex(CRC_ICLASS, sel_corr, sizeof(sel_corr) - 2));
    PrintAndLogEx(INFO, "---------------------------------------------------------------\n\n\n");
    ...
    PrintAndLogEx(INFO, "ISO15693 CRC X25| %04x 或 %04x (期望 906E)\\n", crcCC, Crc16ex(CRC_15693, dataStr, sizeof(dataStr)));
    ...
    PrintAndLogEx(INFO, "CRC32 (desfire) | %08x (期望值)", crc32);
    PrintAndLogEx(INFO, "---------------------------------------------------------------\n\n\n");
...
static int CmdAnalyseCHKSUM(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<hex>", "计算校验和的字节"),
        arg_str0("m", "mask", "<hex>", "限制输出的位掩码（最多4个十六进制字节）"),
        arg_lit0("v", "详细", "详细输出"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (mlen > 8) {
        PrintAndLogEx(FAILED, "掩码值最多4个十六进制字节");
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(INFO, "掩码值 0x%x", mask);
...
static int CmdAnalyseDates(const char *Cmd) {
    ...
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    CLIParserFree(ctx);
    PrintAndLogEx(NORMAL, "待实现。欢迎贡献！");
    return PM3_SUCCESS;
}

static int CmdAnalyseA(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "分析 a",
                  "Iceman的个人垃圾测试命令",
    ...
    if (res == PM3_SUCCESS) {
        PrintAndLogEx(SUCCESS, "密钥 A | %012" PRIx64, key);
    }
...
static int CmdAnalyseDemodBuffer(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "分析演示缓冲区",
                  "将二进制字符串加载到 DemodBuffer",
                  "analyse demodbuff -d 0011101001001011"
                 );

    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<bin>", "加载的二进制字符串"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    free(data);
    PrintAndLogEx(HINT, "提示：使用 `" _YELLOW_("数据 打印") "` to view DemodBuffer");
    return PM3_SUCCESS;
}

static int CmdAnalyseFreq(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "分析频率",
                  "计算波长",
                  "analyse freq\n"
                  ""
                 );
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("F", "freq", "<int>", "谐振频率F，单位赫兹（Hz）"),
        arg_int0("L", "cap",  "<int>", "电容C，单位微法（F）"),
        arg_int0("C", "ind",  "<int>", "电感，单位微亨（μH）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (L == 0) {
        ...
        PrintAndLogEx(INFO, "电感 %lf 亨利", calc_inductance);
    }
    ...
    if (C == 0) {
        ...
        PrintAndLogEx(INFO, "电容 %lf 法拉", calc_capacitance);
    }
    return PM3_SUCCESS;
}
...
static int CmdAnalyseUnits(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "分析单位",
                  "HF 中的单位转换实验。ETU (1/13.56MHz)、US 或 SSP_CLK (1/3.39MHz)",
    ...
    if (selftest) {
        PrintAndLogEx(INFO, "US 到 ETU 转换");
        ...
        PrintAndLogEx(INFO, "ETU到微秒(µS)转换");
        ...
        PrintAndLogEx(INFO, "微秒（µS）到SSP时钟3.39MHz转换");
        ...
        PrintAndLogEx(INFO, "SSP CLK 3.39MHz 到 US 转换");
        ...
        PrintAndLogEx(INFO, "ETU到SSP CLK 3.39MHz转换");
        ...
        PrintAndLogEx(INFO, "SSP CLK 3.39MHz 到 ETU 转换");
...
static int CmdAnalyseRegex(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("p", "pattern", "<str>", "正则表达式模式"),
        arg_str0("d", "text", "<str>", "要匹配的文本"),
        arg_lit0("i", "insensitive", "不区分大小写匹配"),
        arg_lit0("t", "test", "运行自检"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (selftest) {
        ...
        for (size_t i = 0; i < ARRAYLEN(tests); i++) {
            ...
            PrintAndLogEx(ok ? SUCCESS : FAILED, "%zu. 模式=`%s` 有效=%s 匹配=%s ( %s )",
                          i + 1,
                          tests[i].pattern,
                          "true",
                          matched ? "true" : "false",
        ...
        PrintAndLogEx(all_ok ? SUCCESS : FAILED, "测试 ( %s )", all_ok ? _GREEN_("ok") : _RED_("fail"));
        return all_ok ? PM3_SUCCESS : PM3_ESOFT;
    }

    if (arg_pattern->count == 0 || arg_text->count == 0) {
        CLIParserFree(ctx);
        PrintAndLogEx(ERR, "除非使用--test，否则需要模式和文本");
        return PM3_EINVARG;
    }
    ...
    CLIParserFree(ctx);
    PrintAndLogEx(matched ? SUCCESS : INFO, "正则匹配: %s", matched ? _GREEN_("true") : _YELLOW_("false"));
    return PM3_SUCCESS;
}

static command_t CommandTable[] = {
    {"help",    CmdHelp,            AlwaysAvailable, "此帮助"},
    {"lrc",     CmdAnalyseLRC,      AlwaysAvailable, "生成XOR LRC的最终字节"},
    {"crc",     CmdAnalyseCRC,      AlwaysAvailable, "用于 CRC 评估的存根方法"},
    {"chksum",  CmdAnalyseCHKSUM,   AlwaysAvailable, "通过加法、掩码和反码计算校验和"},
    {"dates",   CmdAnalyseDates,    AlwaysAvailable, "在给定的字节数组中查找日期戳"},
    {"lfsr",    CmdAnalyseLfsr,     AlwaysAvailable, "LFSR测试"},
    {"a",       CmdAnalyseA,        AlwaysAvailable, "位数测试"},
    {"nuid",    CmdAnalyseNuid,     AlwaysAvailable, "从 7 字节 UID 创建 NUID"},
    {"demodbuff", CmdAnalyseDemodBuffer, AlwaysAvailable, "加载二进制字符串到 DemodBuffer"},
    {"freq",    CmdAnalyseFreq,     AlwaysAvailable, "计算波长"},
    {"foo",     CmdAnalyseFoo,      AlwaysAvailable, "多路复用器"},
    {"regex",   CmdAnalyseRegex,    AlwaysAvailable, "正则表达式工具（子集：^ $ . * 带 \\\\\\\\ 转义）"},
    {"units",   CmdAnalyseUnits,    AlwaysAvailable, "转换 ETU <> US <> SSP_CLK (3.39MHz)"},
    {NULL, NULL, NULL, NULL}
};
