...
int FIDOCheckDERAndGetKey(uint8_t *der, size_t derLen, bool verbose, uint8_t *publicKey, size_t publicKeyMaxLen) {
    ...
    if (res < 0) {
        PrintAndLogEx(ERR, "错误：CA解析证书返回-0x%x - %s", -res, ecdsa_get_error(res));
    }
    if (verbose)
        PrintAndLogEx(SUCCESS, "CA 加载成功。跳过 %d 个", res);
    ...
    if (res) {
        PrintAndLogEx(ERR, "错误：DER解析返回0x%x - %s", (res < 0) ? -res : res, ecdsa_get_error(res));
    }
    ...
    if (verbose) {
        ...
        PrintAndLogEx(SUCCESS, "DER 证书信息:\\n%s", linfo);
    }
    ...
    if (res) {
        PrintAndLogEx(ERR, "错误：DER验证返回0x%x - %s\\n", (res < 0) ? -res : res, ecdsa_get_error(res));
    } else {
    ...
    if (verbose) {
        ...
        PrintAndLogEx(SUCCESS, "验证信息:\\n%s", linfo);
    }

    // get public key
    res = ecdsa_public_key_from_pk(&cert.pk, MBEDTLS_ECP_DP_SECP256R1, publicKey, publicKeyMaxLen);
    if (res) {
        PrintAndLogEx(ERR, "错误：从证书 0x%x - %s 获取公钥", (res < 0) ? -res : res, ecdsa_get_error(res));
    } else {
        if (verbose)
            PrintAndLogEx(SUCCESS, "从证书获取公钥:\\n%s", sprint_hex_inrow(publicKey, 65));
    }
...
static int FIDO2CheckSignature(json_t *root, uint8_t *publickey, uint8_t *sign, size_t signLen, uint8_t *authData, size_t authDataLen, bool verbose) {
    ...
    if (res == PM3_SUCCESS) {
        ...
        if (res || clientDataHashLen != 32) {
            PrintAndLogEx(ERR, "错误：无法从json获取clientDataHash！");
            return 2;
        }
        ...
        PrintAndLogEx(ERR, "无效签名。res = %d。", res);
        return res;
    }
...
int FIDO2MakeCredentionalParseRes(json_t *root, uint8_t *data, size_t dataLen, bool verbose, bool verbose2, bool showCBOR, bool showDERTLV) {
    ...
    PrintAndLogEx(INFO, "RP ID 哈希: %s", sprint_hex(ubuf, 32));
    ...
    PrintAndLogEx(INFO, "标志 0x%02x:", ubuf[32]);
    if (!ubuf[32])
        PrintAndLogEx(SUCCESS, "无");
    if (ubuf[32] & 0x01)
        PrintAndLogEx(SUCCESS, "up - 用户存在结果");
    if (ubuf[32] & 0x04)
        PrintAndLogEx(SUCCESS, "uv - 用户验证（指纹扫描或PIN等）结果");
    if (ubuf[32] & 0x40)
        PrintAndLogEx(SUCCESS, "at - 包含已认证凭据数据");
    if (ubuf[32] & 0x80)
        PrintAndLogEx(SUCCESS, "ed - 包含扩展数据");
    ...
    PrintAndLogEx(SUCCESS, "凭证ID[%d]: %s", cridlen, sprint_hex_inrow(&ubuf[55], cridlen));
    ...
    if (verbose2) {
        PrintAndLogEx(SUCCESS, "凭证公钥 (COSE_KEY)[%d]: %s", cplen, sprint_hex_inrow(&ubuf[55 + cridlen], cplen));
    } else {
        PrintAndLogEx(SUCCESS, "凭证公钥 (COSE_KEY)[%d]: %s...", cplen, sprint_hex(&ubuf[55 + cridlen], MIN(cplen, 16)));
    }
    JsonSaveBufAsHexCompact(root, "$.AppData.COSE_KEY", &ubuf[55 + cridlen], cplen);

    if (showCBOR) {
        PrintAndLogEx(INFO, "COSE 结构:");
    ...
    if (res) {
        PrintAndLogEx(ERR, "错误：无法获取COSE_KEY。");
    } else {
        PrintAndLogEx(SUCCESS, "COSE 公钥: %s", sprint_hex_inrow(coseKey, sizeof(coseKey)));
    ...
    while (!cbor_value_at_end(&mapsmt)) {
        ...
        if (!strcmp(key, "alg")) {
            ...
            PrintAndLogEx(INFO, "算法 [%lld] %s", (long long)alg, GetCOSEAlgDescription(alg));
        ...
        if (!strcmp(key, "sig")) {
            ...
            cbor_check(res);
            if (verbose2) {
                PrintAndLogEx(INFO, "签名 [%zu]: %s", signLen, sprint_hex_inrow(sign, signLen));
            } else {
                PrintAndLogEx(INFO, "签名 [%zu]: %s...", signLen, sprint_hex(sign, MIN(signLen, 16)));
            }
        }
        ...
        if (!strcmp(key, "x5c")) {
            ...
            cbor_check(res);
            if (verbose2) {
                PrintAndLogEx(INFO, "DER 证书[%zu]:", derLen);
                PrintAndLogEx(INFO, "------------------DER-------------------");
...
int FIDO2GetAssertionParseRes(json_t *root, uint8_t *data, size_t dataLen, bool verbose, bool verbose2, bool showCBOR) {
    ...
    while (!cbor_value_at_end(&mapint)) {
        ...
        if (!strcmp(key, "type")) {
            ...
            cbor_check(res);
            PrintAndLogEx(SUCCESS, "凭证类型: %s", ctype);
        }
        ...
        if (!strcmp(key, "id")) {
            ...
            cbor_check(res);
            PrintAndLogEx(SUCCESS, "凭证ID[%zu]: %s", n, sprint_hex(cid, n));
        }
    }
    ...
    if (res) {
        ...
        while (!cbor_value_at_end(&mapint)) {
            ...
            if (!strcmp(key, "name") || !strcmp(key, "displayName")) {
                ...
                cbor_check(res);
                PrintAndLogEx(SUCCESS, "用户实体 %s: %s", key, cname);
            }
            ...
            if (!strcmp(key, "id")) {
                ...
                res = CborGetBinStringValue(&mapint, cid, sizeof(cid), &n);
                cbor_check(res);
                PrintAndLogEx(SUCCESS, "用户实体 id [%zu]: %s", n, sprint_hex(cid, n));
                ...
                if (idbuflen == n && !memcmp(idbuf, cid, idbuflen)) {
                    ...
                    PrintAndLogEx(ERR, "错误：错误的 UserEntity id（来自 json：%s）", sprint_hex(idbuf, idbuflen));
                }
            }
        }
        res = cbor_value_leave_container(&map, &mapint);
    ...
    cbor_check(res);
    if (verbose2) {
        PrintAndLogEx(SUCCESS, "签名 [%zu]: %s", signLen, sprint_hex_inrow(sign, signLen));
    } else {
        PrintAndLogEx(SUCCESS, "签名 [%zu]: %s...", signLen, sprint_hex(sign, MIN(signLen, 16)));
    }
    ...
    if (res) {
        PrintAndLogEx(SUCCESS, "numberOfCredentials: 默认为1");
    } else {
