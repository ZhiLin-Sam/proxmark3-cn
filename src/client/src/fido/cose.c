...
int COSEGetECDSAKey(uint8_t *data, size_t datalen, bool verbose, uint8_t *public_key) {
    ...
    if (!res) {
        cbor_value_get_int64(&map, &i64);
        if (verbose)
            PrintAndLogEx(SUCCESS, "算法 [%lld] %s", (long long)i64, GetCOSEAlgDescription(i64));
        if (i64 != -7)
            PrintAndLogEx(ERR, "错误：算法必须为 -7。");
    }
    ...
    if (!res) {
        cbor_value_get_int64(&map, &i64);
        if (verbose)
            PrintAndLogEx(SUCCESS, "曲线 [%lld] %s", (long long)i64, GetCOSECurveDescription(i64));
        if (i64 != 1)
            PrintAndLogEx(ERR, "错误：曲线必须为 1。");
    }
    ...
    if (!res) {
        ...
        cbor_check(res);
        if (verbose)
            PrintAndLogEx(SUCCESS, "x - 坐标 [%zu]: %s", len, sprint_hex(&public_key[1], 32));
        if (len != 32)
            PrintAndLogEx(ERR, "错误：x坐标长度必须为32。");
    }
    ...
    if (!res) {
        ...
        cbor_check(res);
        if (verbose)
            PrintAndLogEx(SUCCESS, "y 坐标 [%zu]: %s", len, sprint_hex(&public_key[33], 32));
        if (len != 32)
            PrintAndLogEx(ERR, "错误：y坐标长度必须为32。");
    }
    ...
    if (!res) {
        ...
        cbor_check(res);
        if (verbose)
            PrintAndLogEx(SUCCESS, "d - 私钥 [%zu]: %s", len, sprint_hex(private_key, len));
    }
