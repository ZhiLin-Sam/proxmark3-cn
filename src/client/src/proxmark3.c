...
#define BANNERMSG1 ""
#define BANNERMSG2 
#define BANNERMSG3 "    Release... v4.21611 - BREAKMEIFYOUCAN!"
    ...
    if (script_cmds_file) {
        ...
        if (res == PM3_SUCCESS) {
            if (push_cmdscriptfile(path, stayInCommandLoop) == PM3_SUCCESS) {
                PrintAndLogEx(SUCCESS, "正在从文件执行命令：%s\\n", path);
            } else {
    ...
    if (g_session.incognito) {
        ...
        if (searchHomeFilePath(&g_session.history_path, NULL, PROXHISTORY, true) != PM3_SUCCESS) {
            ...
            if (pm3line_load_history(g_session.history_path) != PM3_SUCCESS) {
                PrintAndLogEx(INFO, "无法加载之前的历史记录");
            }
        }
    }
            ...
            if (execCommand) {
                ...
                if (stdinOnPipe) {
                    ...
                    if (fgets(script_cmd_buf, sizeof(script_cmd_buf), stdin) == NULL) {
                        PrintAndLogEx(ERR, "STDIN 意外结束，退出...");
                        break;
                    }
                    execCommand = true;
...
static void dumpAllHelp(int markdown, bool full_help) {
    ...
    PrintAndLogEx(NORMAL, "\\n%sProxmark3命令转储%s\\n\\n", markdown ? "# " : "", markdown ? "" : "\n======================");
    PrintAndLogEx(NORMAL, "某些命令仅在Proxmark3实际连接时可用。%s\\n", markdown ? "  " : "");
    PrintAndLogEx(NORMAL, "检查列\"offline\"以确认其可用性。\\n");
    PrintAndLogEx(NORMAL, "\n");
    command_t *cmds = getTopLevelCommandTable();
    dumpCommandsRecursive(cmds, markdown, full_help);
    g_session.help_dump_mode = false;
    PrintAndLogEx(NORMAL, "完整帮助转储完成。");
}
#endif //LIBPM3
...
static void show_help(bool showFullHelp, char *exec_name) {
    ...
    if (showFullHelp) {
        ...
        PrintAndLogEx(NORMAL, "你需要将 Proxmark3 加入 modem-manager 的黑名单 - 更多详情请参阅文档：");
...
static int dumpmem_pm3(char *serial_port_name, const char *filename, uint32_t addr, uint32_t len, bool raw) {
    ...
    if (in_bootloader) {
        if ((dev_info & DEVICE_INFO_FLAG_UNDERSTANDS_READ_MEM) != 0) {
            PrintAndLogEx(INFO, "设备正在运行引导加载程序。");
        } else {
            PrintAndLogEx(ERR, "设备正在运行引导加载程序，但引导加载程序"
...
finish:
    if (ret == PM3_SUCCESS) {
        ...
        PrintAndLogEx(FAILED, "用户中止");
    } else {
        PrintAndLogEx(ERR, "错误 %u 中止", ret);
    }
    return ret;
}
...
static int flash_pm3(char *serial_port_name, uint8_t num_files, const char *filenames[FLASH_MAX_FILES], bool can_write_bl, bool force) {
    ...
    PrintAndLogEx(SUCCESS, "即将使用以下文件%s:", num_files > 1 ? "s" : "");
    for (int i = 0 ; i < num_files; ++i) {
...
finish:
    if (ret != PM3_SUCCESS) {
        PrintAndLogEx(WARNING, "刷写过程失败，请按照建议步骤操作！");
    }
...
int main(int argc, char *argv[]) {
    ...
    if ((g_session.preferences_loaded == false) && (g_session.incognito == false)) {
        PrintAndLogEx(INFO, "正在创建初始偏好文件");  // json save reports file name, so just info msg here
