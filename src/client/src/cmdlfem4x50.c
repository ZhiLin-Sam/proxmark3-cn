...
static void em4x50_print_info_result(uint8_t *data, bool verbose) {
    ...
    PrintAndLogEx(INFO, "密码检查..... %s", (bpwc) ? _RED_("on") : _GREEN_("off"));
    PrintAndLogEx(INFO, "写后读取... %s", (braw) ? "on" : "off");
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(INFO, "--------- " _CYAN_("Protection") " ------------");
    PrintAndLogEx(INFO, "首字读保护.... %i", fwrp);
    PrintAndLogEx(INFO, "最后字读保护..... %i", lwrp);
    PrintAndLogEx(INFO, "首字写禁止... %i", fwwi);
    PrintAndLogEx(INFO, "最后字写禁止.... %i", lwwi);
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(INFO, "零值可能表示读保护");
    PrintAndLogEx(NORMAL, "");
}
...
static int em4x50_load_file(const char *filename, uint8_t *data, size_t data_len, size_t *bytes_read) {
    ...
    *bytes_read = 0;
    ...
    if (serial == device_id) {
        PrintAndLogEx(WARNING, "文件 %s 中无有效 EM4x50 数据", filename);
        free(dump);
        return PM3_ENODATA;
    }
...
static int CmdEM4x50ELoad(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf em 4x50 加载EEPROM",
                  "将EM4x50标签转储(bin/eml/json)加载到设备上的模拟器内存",
    ...
    if (em4x50_load_file(filename, data, EM4X50_DUMP_FILESIZE, &bytes_read) != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "读取错误");
        return PM3_EFILE;
    }

    // upload to emulator memory
    em4x50_seteml(data, 0, EM4X50_DUMP_FILESIZE);
    PrintAndLogEx(HINT, "提示：您可以模拟了。参见 `" _YELLOW_("lf em 4x50 sim -h") "`");
...
static int CmdEM4x50ESave(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 保存EEPROM",
    ...
    if (serial == device_id) {
        PrintAndLogEx(WARNING, "闪存中无有效 em4x50 数据。");
        return PM3_ENODATA;
    }
...
static int CmdEM4x50EView(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 查看EEPROM",
                  "显示模拟器内存中的em4x50内容。",
                  "lf em 4x50 eview\n"
                 );
    ...
    uint32_t device_id = bytes_to_num(data + 4 * EM4X50_DEVICE_ID, 4);
    if (serial == device_id) {
        PrintAndLogEx(WARNING, "模拟器内存中无有效 em4x50 数据。");
        return PM3_ENODATA;
    }
...
static int CmdEM4x50Login(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 登录",
                  "登录到 EM4x50 标签。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("p", "passsword", "<hex>", "密码，4字节，最低有效位"),
        arg_param_end
    };
    ...
    if (pwd_len != 4) {
        PrintAndLogEx(FAILED, "密码长度必须为4字节");
        return PM3_EINVARG;
    }
...
static int CmdEM4x50Brute(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 暴力破解",
                  "尝试暴力破解EM4x50卡的密码。\\n"
    ...
    PrintAndLogEx(INFO, "选择模式: %s", mode);
    ...
    if (strcmp(mode, "range") == 0) {
        ...
        PrintAndLogEx(FAILED, "未知的暴力破解模式: %s", mode);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (etd.bruteforce_mode == BF_MODE_RANGE) {
        ...
        if (begin_len != 4) {
            PrintAndLogEx(FAILED, "'begin' 参数必须为 4 字节");
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }

        if (end_len != 4) {
            PrintAndLogEx(FAILED, "'end' 参数必须为 4 字节");
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
        ...
        if (etd.bruteforce_charset == 0) {
            PrintAndLogEx(FAILED, "使用字符集暴力破解模式时请至少启用一个字符集。");
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }

        PrintAndLogEx(INFO, "启用的字符集：%s%s",
    ...
    if (no_iter > 0)
        PrintAndLogEx(INFO, "预计时长：%i 小时 %i 分 %i 秒", dur_h, dur_m, dur_s);
    else
        PrintAndLogEx(INFO, "预计时长：未知");
    ...
    if (resp.status == PM3_SUCCESS)
        ...
        PrintAndLogEx(WARNING, "暴力破解密码失败");
...
static int CmdEM4x50Chk(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 检查",
                  "对 EM4x50 卡运行字典密钥恢复。",
    ...
    if (strlen(filename) == 0) {
        ...
        PrintAndLogEx(INFO, "将文件视为 T55xx 密钥");
    }
    ...
    PrintAndLogEx(INFO, "您可以通过按下 pm3 按钮取消此操作");
    ...
    while (bytes_remaining > 0) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "SPIFFS 上传失败");
            return res;
        }
...
int em4x50_read(em4x50_data_t *etd, em4x50_word_t *out) {
    ...
    if (WaitForResponseTimeout(CMD_LF_EM4X50_READ, &resp, EM4X50_TIMEOUT_CMD) == false) {
        PrintAndLogEx(WARNING, "(em4x50) 等待回复超时");
        return PM3_ETIMEOUT;
    }
...
static int CmdEM4x50Read(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 读块",
                  "读取单个EM4x50块/字。",
                  "lf em 4x50 rdbl -b 3\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int1("b", "block", "<dec>", "块/字地址"),
        arg_str0("p", "pwd", "<hex>", "密码，4个十六进制字节，最低有效位"),
        arg_param_end
    };
    ...
    if (pwd_len) {
        if (pwd_len != 4) {
            PrintAndLogEx(FAILED, "密码长度必须为4字节，而不是%d", pwd_len);
            return PM3_EINVARG;
        } else {
...
static int CmdEM4x50Info(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 信息",
                  "EM4x50 标签信息。",
                  "lf em 4x50 info\n"
...
static int CmdEM4x50Reader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 读取器",
                  "显示EM4x50标签的标准读取数据。",
                  "lf em 4x50 reader\n"
    ...
    do {
        ...
        if (now > EM4X50_NO_WORDS) {
            PrintAndLogEx(WARNING, "字数：%d，限制为 %d", now, EM4X50_NO_WORDS);
            now = EM4X50_NO_WORDS;
        }
...
static int CmdEM4x50Dump(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 转储",
                  "读取EM4x50标签的所有块/字，并以(bin/json)格式保存转储",
                  "lf em 4x50 dump\n"
    ...
    PrintAndLogEx(INFO, "读取 EM4x50 标签");
    clearCommandBuffer();
    SendCommandNG(CMD_LF_EM4X50_INFO, (uint8_t *)&etd, sizeof(etd));
    PacketResponseNG resp;
    if (WaitForResponseTimeout(CMD_LF_EM4X50_INFO, &resp, EM4X50_TIMEOUT_CMD) == false) {
...
static int CmdEM4x50Write(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 写块",
                  "向EM4x50标签写入单个块/字。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int1("b", "block", "<dec>", "块/字地址，十进制"),
        arg_str1("d", "数据", "<hex>", "数据，4字节，LSB"),
        arg_str0("p", "pwd", "<hex>", "密码，4字节，最低有效位"),
        arg_param_end
    };
    ...
    if (addr <= 0 || addr >= EM4X50_NO_WORDS) {
        PrintAndLogEx(FAILED, "地址必须在范围 [0, 31] 内");
        return PM3_EINVARG;
    }

    if (word_len != 4) {
        PrintAndLogEx(FAILED, "字/数据长度必须为4字节，而不是 %d", word_len);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("lf em 4x50 rdbl -b %u") "` - to read your data", addr);
    PrintAndLogEx(INFO, "完成！");
    return PM3_SUCCESS;
}
...
static int CmdEM4x50WritePwd(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 写密码",
                  "写入EM4x50密码。",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("p", "pwd", "<hex>", "密码，4个十六进制字节，最低有效位"),
        arg_str1("n", "new", "<hex>", "新密码，4个十六进制字节，最低有效位"),
        arg_param_end
    };
    ...
    if (npwd_len != 4) {
        PrintAndLogEx(FAILED, "密码长度必须为4字节，而不是%d", npwd_len);
        return PM3_EINVARG;
    } else {
    ...
    PrintAndLogEx(SUCCESS, "正在写入新密码 %s ( %s )"
...
static int CmdEM4x50Wipe(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 擦除",
    ...
    for (int i = 1; i < EM4X50_DEVICE_SERIAL; i++) {
        ...
        PrintAndLogEx(INPLACE, "正在擦除块 %i", i);
...
static int CmdEM4x50Restore(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 恢复",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("u", "uid", "<hex>", "UID，4个十六进制字节，高位在前"),
    ...
    if ((uidLen && fnlen) || (!uidLen && !fnlen)) {
        PrintAndLogEx(FAILED, "请使用选项 'u' 或选项 'f'");
        return PM3_EINVARG;
    }
    ...
    for (int i = startblock; i < EM4X50_DEVICE_SERIAL; i++) {
        ...
        PrintAndLogEx(INPLACE, "正在恢复块 %i", i);
...
static int CmdEM4x50Sim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 模拟",
                  "模拟EM4x50标签\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("p", "passsword", "<hex>", "密码，4字节，最低有效位"),
        arg_param_end
    };
    ...
    if (pwd_len) {
        if (pwd_len != 4) {
            PrintAndLogEx(FAILED, "密码长度必须为4字节，实际为%d", pwd_len);
            return PM3_EINVARG;
        } else {
    ...
    if ((status == PM3_SUCCESS) || (status == PM3_EOPABORTED))
        ...
        PrintAndLogEx(FAILED, "内存中无有效 EM4x50 数据");
...
static int CmdEM4x50View(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf em 4x50 查看",
                  "打印 EM4x50 转储文件\\n",
...
static command_t CommandTable[] = {
    ...
    {"brute",   CmdEM4x50Brute,      IfPm3EM4x50,     "暴力破解攻击以找到密码"},
    {"chk",     CmdEM4x50Chk,        IfPm3EM4x50,     "检查密码"},
    {"dump",    CmdEM4x50Dump,       IfPm3EM4x50,     "转储EM4x50标签"},
    {"info",    CmdEM4x50Info,       IfPm3EM4x50,     "标签信息"},
    {"login",   CmdEM4x50Login,      IfPm3EM4x50,     "登录到 EM4x50 标签"},
    {"rdbl",    CmdEM4x50Read,       IfPm3EM4x50,     "读取EM4x50字数据"},
    {"reader",  CmdEM4x50Reader,     IfPm3EM4x50,     "显示标准读取模式数据"},
    {"restore", CmdEM4x50Restore,    IfPm3EM4x50,     "将EM4x50转储恢复到标签"},
    {"view",    CmdEM4x50View,       AlwaysAvailable, "显示标签转储文件的内容"},
    {"wipe",    CmdEM4x50Wipe,       IfPm3EM4x50,     "擦除EM4x50标签"},
    {"wrbl",    CmdEM4x50Write,      IfPm3EM4x50,     "写入EM4x50字数据"},
    {"wrpwd",   CmdEM4x50WritePwd,   IfPm3EM4x50,     "更改EM4x50密码"},
    {"-----------", CmdHelp,         AlwaysAvailable, "--------------------- " _CYAN_("模拟") " ---------------------"},
    {"eload",  CmdEM4x50ELoad,       IfPm3EM4x50,     "上传文件到模拟器内存"},
    {"esave",  CmdEM4x50ESave,       IfPm3EM4x50,     "将模拟器内存保存到文件"},
    {"eview",  CmdEM4x50EView,       IfPm3EM4x50,     "查看模拟器内存"},
    {"sim",    CmdEM4x50Sim,         IfPm3EM4x50,     "模拟EM4x50标签"},
    {NULL, NULL, NULL, NULL}
};
