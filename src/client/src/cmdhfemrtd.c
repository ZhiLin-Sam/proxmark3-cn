...
//  tag    dg# fileid  filename           desc                                                  pace   eac    req    fast   parser                          dumper
    ...
    {0x77, 0,  0x011D, "EF_SOD",          "文档安全对象",                           false, false, false, false, NULL,                           emrtd_dump_ef_sod},
...
static bool emrtd_exchange_commands(sAPDU_t apdu, bool include_le, uint16_t le, uint8_t *dataout, size_t maxdataoutlen, size_t *dataoutlen, bool activate_field, bool keep_field_on) {
    ...
    if (sw != ISO7816_OK) {
        PrintAndLogEx(DEBUG, "命令失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        return false;
    }
    return true;
}
...
static int emrtd_get_asn1_data_length(uint8_t *datain, int datainlen, int offset) {
    PrintAndLogEx(DEBUG, "asn1 数据长度, 数据输入: %s", sprint_hex_inrow(datain, datainlen));
    int lenfield = (int) * (datain + offset);
    PrintAndLogEx(DEBUG, "asn1 数据长度, 长度字段: %02X", lenfield);
    if (lenfield <= 0x7f) {
        return lenfield;
...
static int emrtd_get_asn1_field_length(uint8_t *datain, int datainlen, int offset) {
    PrintAndLogEx(DEBUG, "asn1 字段长度, 数据输入: %s", sprint_hex_inrow(datain, datainlen));
    int lenfield = (int) * (datain + offset);
    PrintAndLogEx(DEBUG, "asn1 字段长度, 长度字段: %02X", lenfield);
    if (lenfield <= 0x80) {
        return 1;
    } else if (lenfield == 0x81) {
        return 2;
...
static void emrtd_deskey(uint8_t *seed, const uint8_t *type, int length, uint8_t *dataout) {
    PrintAndLogEx(DEBUG, "种子.............. %s", sprint_hex_inrow(seed, 16));

    // combine seed and type
    uint8_t data[50] = { 0x00 };
    memcpy(data, seed, length);
    memcpy(data + length, type, 4);
    PrintAndLogEx(DEBUG, "数据.............. %s", sprint_hex_inrow(data, length + 4));

    // SHA1 the key
    unsigned char key[64] = { 0x00 };
    sha1hash(data, length + 4, key);
    PrintAndLogEx(DEBUG, "密钥............... %s", sprint_hex_inrow(key, length + 4));

    // Set parity bits
    for (int i = 0; i < ((length + 4) / 8); i++) {
        mbedtls_des_key_set_parity(key + (i * 8));
    }
    PrintAndLogEx(DEBUG, "后奇偶校验密钥... %s", sprint_hex_inrow(key, 20));
...
static bool emrtd_check_cc(uint8_t *ssc, uint8_t *key, uint8_t *rapdu, int rapdulength) {
    ...
    PrintAndLogEx(DEBUG, "rapdu 截断: %s", sprint_hex_inrow(rapdu + (rapdulength - 8), 8));
...
static bool emrtd_secure_select_file_by_ef(uint8_t *kenc, uint8_t *kmac, uint8_t *ssc, uint16_t file) {
    ...
    PrintAndLogEx(DEBUG, "命令: %s", sprint_hex_inrow(cmd, cmdlen));
    PrintAndLogEx(DEBUG, "数据：%s", sprint_hex_inrow(data, datalen));

    des3_encrypt_cbc(iv, kenc, data, datalen, temp);
    PrintAndLogEx(DEBUG, "临时: %s", sprint_hex_inrow(temp, datalen));
    ...
    PrintAndLogEx(DEBUG, "数据：%s", sprint_hex_inrow(data, lc));
...
static bool _emrtd_secure_read_binary_decrypt(uint8_t *kenc, uint8_t *kmac, uint8_t *ssc, int offset, int bytes_to_read, uint8_t *dataout, size_t *dataoutlen) {
    ...
    PrintAndLogEx(DEBUG, "secreadbindec，读取%i偏移%i：加密：%s", offset, bytes_to_read, sprint_hex_inrow(response, resplen));

    cutat = ((int) response[1]) - 1;

    des3_decrypt_cbc(iv, kenc, response + 3, cutat, temp);
    memcpy(dataout, temp, bytes_to_read);
    PrintAndLogEx(DEBUG, "secreadbindec，读取%i偏移%i：解密：%s", offset, bytes_to_read, sprint_hex_inrow(temp, cutat));
    PrintAndLogEx(DEBUG, "secreadbindec，读取%i偏移%i：解密并截断：%s", offset, bytes_to_read, sprint_hex_inrow(dataout, bytes_to_read));
...
static bool emrtd_lds_get_data_by_tag(uint8_t *datain, size_t datainlen, uint8_t *dataout, size_t *dataoutlen, int tag1, int tag2, bool twobytetag, bool entertoptag, size_t skiptagcount) {
    ...
    while (offset < datainlen) {
        PrintAndLogEx(DEBUG, "emrtd_lds_get_data_by_tag, 偏移: %i, 数据: %X", offset, *(datain + offset));
        ...
        if (*(datain + offset) == tag1 && (!twobytetag || *(datain + offset + 1) == tag2)) {
            if (skipcounter < skiptagcount) {
                ...
                *dataoutlen = e_datalen;
                ...
                PrintAndLogEx(ERR, "错误（emrtd_lds_get_data_by_tag）e_datalen越界");
                return false;
            }
        }
...
static int emrtd_dump_ef_dg5(uint8_t *file_contents, size_t file_length, const char *path) {
    ...
    if (datalen < EMRTD_MAX_FILE_SIZE) {
        ...
        PrintAndLogEx(ERR, "错误（emrtd_dump_ef_dg5）数据长度越界");
        return PM3_ESOFT;
    }
    return PM3_SUCCESS;
}
...
static int emrtd_dump_ef_dg7(uint8_t *file_contents, size_t file_length, const char *path) {
    ...
    if (datalen < EMRTD_MAX_FILE_SIZE) {
        ...
        PrintAndLogEx(ERR, "错误（emrtd_dump_ef_dg7）数据长度越界");
        return PM3_ESOFT;
    }
    return PM3_SUCCESS;
}
...
static int emrtd_dump_ef_sod(uint8_t *file_contents, size_t file_length, const char *path) {
    ...
    if (fieldlen + 1 > EMRTD_MAX_FILE_SIZE) {
        PrintAndLogEx(ERR, "错误（emrtd_dump_ef_sod）字段长度越界");
...
static bool emrtd_dump_file(uint8_t *ks_enc, uint8_t *ks_mac, uint8_t *ssc, uint16_t file, const char *name, bool use_secure, const char *path) {
    ...
    PrintAndLogEx(DEBUG, "内容（超过2k字符可能不完整）");
    PrintAndLogEx(DEBUG, "------------------------------------------");
...
static bool emrtd_do_bac(char *documentnumber, char *dob, char *expiry, uint8_t *ssc, uint8_t *ks_enc, uint8_t *ks_mac) {
    ...
    if (emrtd_external_authenticate(cmd_data, sizeof(cmd_data), response, sizeof(response), &resplen) == false) {
        PrintAndLogEx(ERR, "无法进行外部认证。您是否提供了正确的 MRZ 信息？");
        return false;
    }
    PrintAndLogEx(INFO, "使用BAC的外部认证成功");
    ...
    if (memcmp(rnd_ifd, dec_output + 8, 8) != 0) {
        PrintAndLogEx(ERR, "挑战失败，rnd_ifd 不匹配。");
        return false;
    }
...
static bool emrtd_do_auth(char *documentnumber, char *dob, char *expiry, bool BAC_available, bool *BAC, uint8_t *ssc, uint8_t *ks_enc, uint8_t *ks_mac) {
    ...
    if (emrtd_select_file_by_name(sizeof(aid), aid) == false) {
        PrintAndLogEx(ERR, "无法选择 MRTD 应用。");
        return false;
    }
    ...
    if (emrtd_select_file_by_ef(dg_table[EF_COM].fileid) == false) {
        *BAC = true;
        PrintAndLogEx(INFO, "强制认证");
        PrintAndLogEx(INFO, "切换到外部认证");
    } else {
        *BAC = false;
        // Select EF_DG1
    ...
    if (*BAC) {
        ...
        if (BAC_available == false) {
            PrintAndLogEx(ERR, "此 eMRTD 强制要求认证，但您未提供 MRZ 数据。无法继续。");
            PrintAndLogEx(HINT, "提示: 查看 `" _YELLOW_("hf emrtd info/dump --h") "`, supply data with `-n` `-d` and `-e`");
            return false;
        }
...
int dumpHF_EMRTD(char *documentnumber, char *dob, char *expiry, bool BAC_available, const char *path) {
    ...
    if (emrtd_dump_file(ks_enc, ks_mac, ssc, dg_table[EF_CardAccess].fileid, dg_table[EF_CardAccess].filename, BAC, path) == false) {
        PrintAndLogEx(INFO, "无法转储 EF_CardAccess，卡片不支持 PACE");
        PrintAndLogEx(HINT, "提示：对于没有PACE的卡片，这是预期行为，无需担心。");
    }
    ...
    if (emrtd_select_and_read(response, &resplen, dg_table[EF_COM].fileid, ks_enc, ks_mac, ssc, BAC) == false) {
        PrintAndLogEx(ERR, "读取 EF_COM 失败");
        DropField();
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "读取EF_COM，长度%zu", resplen);
    PrintAndLogEx(DEBUG, "内容（超过2k字符可能不完整）... %s", sprint_hex_inrow(response, resplen));
    saveFile(filepath, ".bin", response, resplen);
    ...
    if (emrtd_lds_get_data_by_tag(response, resplen, filelist, &filelistlen, 0x5c, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_COM 读取文件列表失败");
        DropField();
        return PM3_ESOFT;
    }

    PrintAndLogEx(DEBUG, "文件列表... %s", sprint_hex_inrow(filelist, filelistlen));
    ...
    for (int i = 0; i < filelistlen; i++) {
        ...
        if (dg == NULL) {
            PrintAndLogEx(INFO, "未找到文件标签，跳过... %02X", filelist[i]);
            continue;
        }
        PrintAndLogEx(DEBUG, "当前文件... %s", dg->filename);
        if (!dg->pace && !dg->eac) {
...
static bool emrtd_compare_check_digit(char *datain, int datalen, char expected_check_digit) {
    ...
    PrintAndLogEx(DEBUG, "emrtd_compare_check_digit, 期望 %c == %c 计算值 ( %s )"
...
static void emrtd_print_unknown_timestamp_5f85(uint8_t *data, size_t datalen) {
    ...
    PrintAndLogEx(HINT, "提示：这很可能是个性化时间戳，但使用了未记录的标签。");
}
...
static int emrtd_print_ef_dg1_info(uint8_t *data, size_t datalen) {
    ...
    if (emrtd_lds_get_data_by_tag(data, datalen, (uint8_t *) mrz, &mrzlen, 0x5f, 0x1f, true, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_DG1 读取 MRZ 失败。");
        return PM3_ESOFT;
    }
    ...
    if (mrzlen == 90) {
        ...
        PrintAndLogEx(ERR, "MRZ 长度 (%zu) 错误。", mrzlen);
        return PM3_ESOFT;
    }
...
static int emrtd_print_ef_dg11_info(uint8_t *data, size_t datalen) {
    ...
    if (emrtd_lds_get_data_by_tag(data, datalen, taglist, &taglistlen, 0x5c, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_DG11 读取文件列表失败。");
        return PM3_ESOFT;
    }
        ...
        if (taglist[i] == 0x5f) {
            switch (taglist[i + 1]) {
                ...
                case 0x2b:
                    ...
                    break;
                default:
                    PrintAndLogEx(SUCCESS, "未知字段 %02X%02X....... %s", taglist[i], taglist[i + 1], sprint_hex_inrow(tagdata, tagdatalen));
                    break;
            }
            ...
            // TODO: Account for A0
            PrintAndLogEx(SUCCESS, "未知字段 %02X......... %s", taglist[i], sprint_hex_inrow(tagdata, tagdatalen));
        }
    }
    return PM3_SUCCESS;
}
...
static int emrtd_print_ef_dg12_info(uint8_t *data, size_t datalen) {
    ...
    if (emrtd_lds_get_data_by_tag(data, datalen, taglist, &taglistlen, 0x5c, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_DG12 读取文件列表失败。");
        return PM3_ESOFT;
    }
...
static int emrtd_ef_sod_extract_signatures(uint8_t *data, size_t datalen, uint8_t *dataout, size_t *dataoutlen) {
    ...
    if (emrtd_lds_get_data_by_tag(data, datalen, top, &toplen, 0x30, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取 top 失败。");
        return false;
    }

    PrintAndLogEx(DEBUG, "顶部: %s。", sprint_hex_inrow(top, toplen));

    if (emrtd_lds_get_data_by_tag(top, toplen, signeddata, &signeddatalen, 0xA0, 0x00, false, false, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取 signedData 失败。");
        return false;
    }

    PrintAndLogEx(DEBUG, "签名数据: %s.", sprint_hex_inrow(signeddata, signeddatalen));
    ...
    if (emrtd_lds_get_data_by_tag(signeddata, signeddatalen, emrtdsigcontainer, &emrtdsigcontainerlen, 0x30, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取 eMRTDSignature 容器失败。");
        return false;
    }
    ...
    if (emrtd_lds_get_data_by_tag(emrtdsigcontainer, emrtdsigcontainerlen, emrtdsig, &emrtdsiglen, 0xA0, 0x00, false, false, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取 eMRTDSignature 失败。");
        return false;
    }
    ...
    if (emrtd_lds_get_data_by_tag(emrtdsig, emrtdsiglen, emrtdsigtext, &emrtdsigtextlen, 0x04, 0x00, false, false, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取 eMRTDSignature（文本）失败。");
        return false;
    }
...
static int emrtd_parse_ef_sod_hash_algo(uint8_t *data, size_t datalen, int *hashalgo) {
    ...
    *hashalgo = -1;
    ...
    if (emrtd_lds_get_data_by_tag(data, datalen, hashalgoset, &hashalgosetlen, 0x30, 0x00, false, true, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取哈希算法集失败。");
        return false;
    }

    PrintAndLogEx(DEBUG, "哈希算法设置：%s", sprint_hex_inrow(hashalgoset, hashalgosetlen));
    ...
    for (int hashi = 0; hashalg_table[hashi].name != NULL; hashi++) {
        PrintAndLogEx(DEBUG, "正在尝试: %s", hashalg_table[hashi].name);
        ...
        if (hashalg_table[hashi].descriptorlen != hashalgosetlen) {
            PrintAndLogEx(DEBUG, "长度不匹配: %zu", hashalgosetlen);
            continue;
        }
    ...
    PrintAndLogEx(ERR, "解析哈希列表失败（未知算法: %s）。哈希验证将不可用。", sprint_hex_inrow(hashalgoset, hashalgosetlen));
    return PM3_ESOFT;
}
...
static int emrtd_parse_ef_sod_hashes(uint8_t *data, size_t datalen, uint8_t *hashes, int *hashalgo) {
    ...
    PrintAndLogEx(DEBUG, "哈希数据... %s", sprint_hex_inrow(emrtdsig, emrtdsiglen));
    ...
    if (emrtd_lds_get_data_by_tag(emrtdsig, emrtdsiglen, hashlist, &hashlistlen, 0x30, 0x00, false, true, 1) == false) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取哈希列表失败");
        return false;
    }

    PrintAndLogEx(DEBUG, "哈希列表... %s", sprint_hex_inrow(hashlist, hashlistlen));
        ...
        switch (hashlist[offset]) {
            case 0x30: {
                ...
                if (hashlen <= 64) {
                    ...
                    PrintAndLogEx(ERR, "错误（emrtd_parse_ef_sod_hashes）哈希长度越界");
                }
                break;
            }
        }
...
static int emrtd_print_ef_sod_info(uint8_t *dg_hashes_calc, uint8_t *dg_hashes_sod, int hash_algo, bool fastdump) {
    ...
    PrintAndLogEx(INFO, "文档安全对象");
    PrintAndLogEx(INFO, "包含护照数据的数字签名");
    PrintAndLogEx(INFO, "");
    ...
    if (hash_algo == -1) {
        ...
        for (int i = 1; i <= 16; i++) {
            ...
            if (calc_all_zero == true) {
                ...
                PrintAndLogEx(SUCCESS, _YELLOW_("EF_DG%-2i") " %s%.*s " _GREEN_("有效"), i, dg_table[i].desc, n, pad);
            }
        }
    }
...
static int emrtd_print_ef_cardaccess_info(uint8_t *data, size_t datalen) {
    ...
    if (emrtd_lds_get_data_by_tag(dataset, datasetlen, datafromtag, &datafromtaglen, 0x06, 0x00, false, false, 0) == false) {
        PrintAndLogEx(ERR, "从 EF_CardAccess 读取 PACE 算法失败。");
        return PM3_ESOFT;
    }

    for (int pacei = 0; pacealg_table[pacei].name != NULL; pacei++) {
        PrintAndLogEx(DEBUG, "尝试：%s", pacealg_table[pacei].name);
    ...
    if (emrtd_lds_get_data_by_tag(dataset, datasetlen, datafromtag, &datafromtaglen, 0x02, 0x00, false, false, 1) == false) {
        PrintAndLogEx(ERR, "从 EF_CardAccess 读取 PACE 参数 ID 失败。");
        return PM3_ESOFT;
    }
    ...
    for (int pacepari = 0; pacesdp_table[pacepari].id != 32; pacepari++) {
        PrintAndLogEx(DEBUG, "尝试：%s", pacesdp_table[pacepari].name);
...
int infoHF_EMRTD(char *documentnumber, char *dob, char *expiry, bool BAC_available, bool only_fast) {
    ...
    if (emrtd_select_and_read(response, &resplen, dg_table[EF_CardAccess].fileid, ks_enc, ks_mac, ssc, BAC) == false) {
        ...
        PrintAndLogEx(HINT, "提示：上面的错误是正常的，仅表示您的eMRTD缺少PACE。");
    }
    ...
    PrintAndLogEx(SUCCESS, "通信标准... %s", use14b ? _YELLOW_("ISO/IEC 14443(B)") : _YELLOW_("ISO/IEC 14443(A)"));
    PrintAndLogEx(SUCCESS, "认证........... %s", BAC ? _GREEN_("Enforced") : _RED_("Not enforced"));
    PrintAndLogEx(SUCCESS, "PACE..................... %s", PACE_available ? _GREEN_("Available") : _YELLOW_("Not available"));
    PrintAndLogEx(SUCCESS, "认证结果.... %s", auth_result ? _GREEN_("Successful") : _RED_("失败"));
    ...
    if (!emrtd_select_and_read(response, &resplen, dg_table[EF_SOD].fileid, ks_enc, ks_mac, ssc, BAC)) {
        PrintAndLogEx(ERR, "读取 EF_SOD 失败。");
        DropField();
        return PM3_ESOFT;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "从 EF_SOD 读取哈希列表失败。哈希校验将失败");
    }
    ...
    for (int i = 0; i < filelistlen; i++) {
        ...
        if (((dg->fastdump && only_fast) || !only_fast) && !dg->pace && !dg->eac) {
            if (emrtd_select_and_read(response, &resplen, dg->fileid, ks_enc, ks_mac, ssc, BAC)) {
                ...
                PrintAndLogEx(DEBUG, "EF_DG%i 哈希算法... %i", dg->dgnum, hash_algo);
                // Check file hash
                if (hash_algo != -1) {
                    PrintAndLogEx(DEBUG, "EF_DG%i 在 EF_SOD 上的哈希... %s", dg->dgnum, sprint_hex_inrow(dg_hashes_sod[dg->dgnum], hashalg_table[hash_algo].hashlen));
                    hashalg_table[hash_algo].hasher(response, resplen, dg_hashes_calc[dg->dgnum]);
                    PrintAndLogEx(DEBUG, "EF_DG%i 哈希计算........ %s", dg->dgnum, sprint_hex_inrow(dg_hashes_calc[dg->dgnum], hashalg_table[hash_algo].hashlen));
                }
            }
        }
    }
...
int infoHF_EMRTD_offline(const char *path) {
    ...
    if ((loadFile_safeEx(filepath, ".BIN", (void **)&data, (size_t *)&datalen, false) == PM3_SUCCESS) ||
        ...
        PrintAndLogEx(HINT, "提示: 上面的错误是正常的。它只是意味着您的 eMRTD 缺少 PACE");
    }
...
static int CmdHFeMRTDDump(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("n", "doc", "<alphanum>", "证件号码，最多9个字符"),
        arg_str0("d", "date", "<YYMMDD>", "出生日期，YYMMDD格式"),
        arg_str0("e", "expiry", "<YYMMDD>", "有效期（YYMMDD格式）"),
        arg_str0("m", "mrz", "<[0-9A-Z<]>", "MRZ第二行，44个字符"),
    ...
    if (CLIParamStrToBuf(arg_get_str(ctx, 2), dob, 6, &slen) != 0 || slen == 0) {
        ...
        if (!validate_date(dob, slen)) {
            PrintAndLogEx(ERR, "出生日期格式错误，无法继续。");
            PrintAndLogEx(HINT, "提示：使用YYMMDD格式。");
            error = true;
        }
    }
    ...
    if (CLIParamStrToBuf(arg_get_str(ctx, 3), expiry, 6, &slen) != 0 || slen == 0) {
        ...
        if (!validate_date(expiry, slen)) {
            PrintAndLogEx(ERR, "到期日期格式错误，无法继续。");
            PrintAndLogEx(HINT, "提示：使用YYMMDD格式。");
            error = true;
        }
    }
    ...
    if (CLIParamStrToBuf(arg_get_str(ctx, 4), mrz, 44, &slen) == 0 && slen != 0) {
        if (slen != 44) {
            PrintAndLogEx(ERR, "MRZ 长度不正确，应为44，而非%i", slen);
            error = true;
        } else {
            BAC = true;
...
static command_t CommandTable[] = {
    ...
    {"-----------", CmdHelp,           IfPm3Iso14443,   "------------------- " _CYAN_("操作") " -------------------"},
    {"dump",    CmdHFeMRTDDump,    IfPm3Iso14443,   "将eMRTD文件转储为二进制文件"},
    {"info",        CmdHFeMRTDInfo,    AlwaysAvailable, "标签信息"},
    {"list",    CmdHFeMRTDList,    AlwaysAvailable, "列出ISO 14443A/7816历史"},
    {NULL, NULL, NULL, NULL}
};
