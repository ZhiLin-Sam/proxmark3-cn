...
static int ntag424_exchange_apdu(APDU_t apdu, int command_header_length, uint8_t *response, int *response_length, ntag424_communication_mode_t comm_mode, ntag424_session_keys_t *session_keys, uint8_t sw1_expected, uint8_t sw2_expected) {
    ...
    if (comm_mode != COMM_PLAIN) {
        if (session_keys == NULL) {
            PrintAndLogEx(ERR, "请求非明文通信模式但未提供会话密钥");
            return PM3_EINVARG;
        }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "交换 APDU 失败: %d", res);
        return res;
    }

    if (*response_length < 2) {
        PrintAndLogEx(ERR, "无响应");
        return PM3_ESOFT;
    }
    ...
    if (sw1 != sw1_expected || sw2 != sw2_expected) {
        PrintAndLogEx(ERR, "来自卡的错误：%02X %02X (%s)", sw1, sw2, GetAPDUCodeDescription(sw1, sw2));
        return PM3_ESOFT;
    }
...
static int ntag424_auth_first_step(uint8_t keyno, uint8_t *key, uint8_t *out) {
    ...
    if (response_length != 16 + 2) {
        PrintAndLogEx(ERR, "获取 RndB 失败（无效的密钥编号？）");
        return PM3_ESOFT;
    }
...
static int CmdHF_ntag424_view(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf ntag424 view",
                  "打印 NTAG 424 DNA 转储文件 (bin/eml/json)",
    ...
    if (verbose) {
        ...
        PrintAndLogEx(INFO, "文件大小 %zu 字节", bytes_read);
    }

    // to be implemented...
    PrintAndLogEx(INFO, "尚未实现");
    PrintAndLogEx(INFO, "欢迎贡献！");
...
static int CmdHF_ntag424_auth(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf ntag424 auth",
                  "使用选定密钥对 NTAG424 进行认证。",
                  "hf ntag424 auth --keyno 0 -k 00000000000000000000000000000000");

    void *argtable[] = {
        arg_param_begin,
        arg_int1(NULL, "keyno", "<dec>", "密钥编号"),
        arg_str1("k",  "key", "<hex>", "用于认证的密钥（十六进制16字节）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF_ntag424_read(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf ntag424 read",
                  "读取并打印NTAG424标签上文件的数据。如果提供密钥信息，将进行认证。",
    ...
    void *argtable[] = {
        ...
        arg_int0(NULL, "keyno",  "<dec>",   "密钥编号"),
        arg_str0("k",  "key",    "<hex>",   "用于认证的密钥（十六进制16字节）"),
        arg_int0("o",  "offset", "<dec>",   "文件中读取的偏移量（默认0）"),
        arg_int1("l",  "长度", "<dec>",   "要读取的字节数"),
    ...
    if (comm_mode != COMM_PLAIN && auth == false) {
        PrintAndLogEx(ERR, "未指定密钥时只能使用明文通信模式");
        return PM3_EINVARG;
    }
...
static int CmdHF_ntag424_write(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf ntag424 write",
                  "将数据写入NTAG424标签上的文件。如果提供密钥信息，将进行身份验证。",
    ...
    void *argtable[] = {
        ...
        arg_int0(NULL, "keyno",   "<dec>", "密钥编号"),
        arg_str0("k",  "key",     "<hex>", "用于认证的密钥（十六进制16字节）"),
        arg_int0("o",  "offset",  "<dec>", "文件中写入的偏移量（默认0）"),
        arg_str1("d",  "数据",    "<hex>", "待写入数据"),
        arg_str0("m",  "cmode",   "<plain|mac|encrypt>", "通信模式"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF_ntag424_changefilesettings(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_int1(NULL, "keyno", "<dec>", "密钥编号"),
        arg_str1("k",  "key", "<hex>", "用于认证的密钥（十六进制16字节）"),
        arg_str0("o",  "options", "<hex>", "文件选项字节（十六进制1字节）"),
        arg_str0("a",  "access", "<hex>", "文件访问设置（十六进制2字节）"),
        arg_str0("s",  "sdmoptions", "<hex>", "SDM 选项（十六进制 1 字节）"),
        arg_str0("c",  "sdmaccess", "<hex>", "SDM 访问设置（十六进制 2 字节）"),
    ...
    if (arg_get_str(ctx, 4)->count == 1) {
        ...
        if (len != 1) {
            PrintAndLogEx(ERR, "选项必须为1字节，实际为 ( %d )", len);
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
    ...
    if (arg_get_str(ctx, 5)->count == 1) {
        ...
        if (len != 2) {
            PrintAndLogEx(ERR, "访问权限必须是 2 字节，实际为 ( %d )", len);
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
    ...
    if (arg_get_str(ctx, 6)->count == 1) {
        ...
        if (len != 1) {
            PrintAndLogEx(ERR, "SDM 选项必须为 1 字节，实际为 ( %d )", len);
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
    ...
    if (arg_get_str(ctx, 7)->count == 1) {
        ...
        if (len != 2) {
            PrintAndLogEx(ERR, "SDM 访问权限必须为 2 字节，实际为 ( %d )", len);
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        if (arg_get_str(ctx, 8 + i)->count == 1) {
            ...
            if (len != 3) {
                PrintAndLogEx(ERR, "sdmdata必须为3字节，实际得到( %d )", len);
                CLIParserFree(ctx);
                return PM3_EINVARG;
            }
        } else {
    ...
    if (res == PM3_SUCCESS) {
        ...
        PrintAndLogEx(ERR, "写入设置 (" _RED_("fail") " )");
    }
    return res;
}
...
static command_t CommandTable[] = {
    ...
    {"info",         CmdHF_ntag424_info,               IfPm3Iso14443a,   "标签信息"},
    {"view",         CmdHF_ntag424_view,               AlwaysAvailable,  "显示标签转储文件的内容"},
    {"auth",         CmdHF_ntag424_auth,               IfPm3Iso14443a,   "使用密钥测试认证"},
    {"read",         CmdHF_ntag424_read,               IfPm3Iso14443a,   "读取文件"},
    {"write",        CmdHF_ntag424_write,              IfPm3Iso14443a,   "写入文件"},
    {"getfs",        CmdHF_ntag424_getfilesettings,    IfPm3Iso14443a,   "获取文件设置"},
    {"changefs",     CmdHF_ntag424_changefilesettings, IfPm3Iso14443a,   "更改文件设置"},
    {"changekey",    CmdHF_ntag424_changekey,          IfPm3Iso14443a,   "更改密钥"},
    {NULL, NULL, NULL, NULL}
};
