...
int demodPresco(bool verbose) {
    ...
    if (ASKDemod_ext(32, 0, 0, 0, false, false, false, 1, &st) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 Presco ASK解调失败");
        return PM3_ESOFT;
    }
    ...
    if (ans < 0) {
        if (ans == -1)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Presco: 找到的比特数太少");
        else if (ans == -2)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Presco: 未找到前导码");
        else if (ans == -3)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Presco: 大小不正确: %zu", size);
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Presco: 应答: %d", ans);
        return PM3_ESOFT;
    }
...
static int CmdPrescoDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频Presco解调",
                  "尝试查找presco前导码，如果找到则解码/解扰数据",
                  "低频Presco解调"
                 );
...
static int CmdPrescoClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Presco克隆",
                  "将 presco 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
    ...
    if (hex_len) {
        ...
        if (slen < 2)  {
            PrintAndLogEx(ERR, "必须至少包含 2 位数字");
            return PM3_EINVARG;
        }
    ...
    if ((sitecode & 0xFF) != sitecode) {
        sitecode &= 0xFF;
        PrintAndLogEx(INFO, "站点代码截断为 8 位 (Presco): %u", sitecode);
    }

    if ((usercode & 0xFFFF) != usercode) {
        usercode &= 0xFFFF;
        PrintAndLogEx(INFO, "用户代码截断为 16 位 (Presco): %u", usercode);
    }
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdPrescoDemod, AlwaysAvailable, "从 GraphBuffer 解调 Presco 标签"},
    {"reader",  CmdPrescoReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",   CmdPrescoClone,  IfPm3Lf,         "克隆 presco 标签到 T55x7、Q5/T5555 或 EM4305/4469"},
    {"sim",     CmdPrescoSim,    IfPm3Lf,         "模拟presco标签"},
    {NULL, NULL, NULL, NULL}
};
