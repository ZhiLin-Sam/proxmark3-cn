...
int mf_dark_side(uint8_t blockno, uint8_t key_type, uint64_t *key) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (WaitForResponseTimeout(CMD_HF_MIFARE_READER, &resp, 2000)) {
                ...
                if (resp.status != PM3_SUCCESS) {
                    ...
                    switch (package->isOK) {
                        case 2:
                            PrintAndLogEx(FAILED, "卡片不易受Darkside攻击（在认证请求时不发送NACK）。");
                            break;
                        case 3:
                            PrintAndLogEx(FAILED, "卡片不易受Darkside攻击（其随机数生成器不可预测）。");
                            break;
                        case 4:
                            PrintAndLogEx(FAILED, "卡片不易受Darkside攻击（其随机数生成器似乎基于众所周知的");
                            PrintAndLogEx(FAILED, "生成仅16有效位的多项式，但表现出意外行为。");
                            break;
                        case 5:
                        ...
                        case 6:
                            *key = 0101;
                            return PM3_SUCCESS;
                        default:
                            PrintAndLogEx(FAILED, "未知错误。暗侧攻击失败。");
                            break;
                    }
        ...
        if (par_list == 0 && first_run == true) {
            PrintAndLogEx(SUCCESS, "奇偶校验全为零。很可能这张卡在每次认证时发送NACK。");
        }
        first_run = false;
        ...
        if (keycount == 0) {
            PrintAndLogEx(FAILED, "未找到密钥（lfsr_common_prefix 列表为空）。Nt = %08x", nt);
            PrintAndLogEx(FAILED, "这在所有情况中有 25%% 的概率发生。");
            PrintAndLogEx(FAILED, "正在使用不同的读取器随机数重试...");
            continue;
        }
        ...
        *key = UINT64_C(-1);
        ...
        if (*key != UINT64_C(-1)) {
            ...
            PrintAndLogEx(FAILED, "所有密钥候选失败。重新启动darkside");
            free(last_keylist);
            last_keylist = keylist;
            first_run = true;
        }
...
int mf_check_keys_fast_ex(uint8_t sectorsCnt, uint8_t firstChunk, uint8_t lastChunk, uint8_t strategy,
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "块 %.1fs | 找到 %u/%u 个密钥 (%u)", (float)(t2 / 1000.0), curr_keys, (sectorsCnt << 1), size);
    }
...
int mf_check_keys_file(uint8_t *destfn, uint64_t *key) {
    ...
    while (WaitForResponseTimeout(CMD_HF_MIFARE_CHKKEYS, &resp, 2000) == false) {
        ...
        retry--;
        if (retry == 0) {
            PrintAndLogEx(WARNING, "检查密钥文件，命令执行超时");
            SendCommandNG(CMD_BREAK_LOOP, NULL, 0);
            return PM3_ETIMEOUT;
        }
    }
...
int mf_key_brute(uint8_t blockNo, uint8_t keyType, const uint8_t *key, uint64_t *resultkey) {
    ...
    for (i = 0, counter = 1; i < CANDIDATE_SIZE; i += KEYBLOCK_SIZE, ++counter) {
        ...
        // progress
        if (counter % 20 == 0) {
            PrintAndLogEx(SUCCESS, "尝试 %s.. \\t %u 个密钥", sprint_hex(candidates + i, 6),  counter * KEYS_IN_BLOCK);
        }
    }
    return found;
}
...
int mf_nested(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, uint8_t *resultKey, bool calibrate) {
    ...
    for (uint32_t i = 0; i < keycnt; i += max_keys) {
        ...
        PrintAndLogEx(INPLACE, "%6d/%u 密钥 | %5.1f 密钥/秒 | 最坏情况剩余 %6.1f 秒", i, keycnt, bruteforce_per_second, (keycnt - i) / bruteforce_per_second);
        looped = true;
    }
...
int mf_static_nested(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, uint8_t *resultKey, bool forceDetectDist) {
    ...
    for (uint32_t i = 0; i < keycnt; i += max_keys_chunk) {
        ...
        PrintAndLogEx(INPLACE, "%6u/%u 密钥 | %5.1f 密钥/秒 | 最坏情况 %6.1f 秒", i, keycnt, bruteforce_per_second, (keycnt - i) / bruteforce_per_second);
    }
...
int mf_write_block(uint8_t blockno, uint8_t keyType, const uint8_t *key, const uint8_t *block) {
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500) == false) {
        PrintAndLogEx(FAILED, "mfWriteBlock 执行超时");
        return PM3_ETIMEOUT;
    }
    int res = PM3_SUCCESS;
...
int mf_chinese_set_uid(uint8_t *uid, uint8_t uidlen, const uint8_t *atqa, const uint8_t *sak, uint8_t *old_uid, uint8_t *verifed_uid, uint8_t wipecard, uint8_t gdm) {
    ...
    if (res == 0) {
        PrintAndLogEx(SUCCESS, "旧块0... %s", sprint_hex_inrow(block0, sizeof(block0)));
        if (old_uid) {
            memcpy(old_uid, block0, uidlen);
        }
    } else {
        PrintAndLogEx(INFO, "无法获取旧数据。将覆盖块0的最后字节");
    }
    ...
    PrintAndLogEx(SUCCESS, "新块 0... %s", sprint_hex_inrow(block0, sizeof(block0)));
...
int mf_chinese_wipe(uint8_t *uid, const uint8_t *atqa, const uint8_t *sak, uint8_t gdm) {
    ...
    for (int blockNo = 0; blockNo < 4 * 16; blockNo++) {
        for (int retry = 0; retry < 3; retry++) {
            ...
            PrintAndLogEx(INPLACE, "擦除块 %d", blockNo);
            ...
            PrintAndLogEx(WARNING, "重试块 %d ...", blockNo);
        }

        if (res) {
            PrintAndLogEx(ERR, "设置块%d（%d）错误", blockNo, res);
            return res;
        }
    }
    DropField();
...
int mf_chinese_set_block(uint8_t blockNo, uint8_t *data, uint8_t *uid, uint8_t params) {
    ...
    if (isOK == 0) {
        ...
        if (reason == 4) {
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(WARNING, "GDM 魔术写入签名块失败");
        } else if (reason == 5) {
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(WARNING, "写入块失败");
        }
...
int try_decrypt_word(uint32_t nt, uint32_t ar_enc, uint32_t at_enc, uint8_t *data, int len) {
    ...
    PrintAndLogEx(SUCCESS, "加密数据... %s", sprint_hex(data, len));
...
int detect_classic_prng(void) {
    ...
    // check respA
    if (respA.oldarg[0] != 4) {
        PrintAndLogEx(ERR, "PRNG 数据错误: 长度错误: %"PRIu64, respA.oldarg[0]);
        return PM3_ESOFT;
    }
    ...
    while (true) {
        ...
        if (WaitForResponseTimeout(CMD_HF_MIFARE_NACK_DETECT, &resp, 500)) {
            ...
            switch (ok) {
                ...
                case 98 : {
                    if (verbose)
                        PrintAndLogEx(FAILED, "卡片随机数生成器不可预测。");
                    PrintAndLogEx(WARNING, "检测失败");
                    return PM3_SUCCESS;
                }
                case 97 : {
                    if (verbose) {
                        PrintAndLogEx(FAILED, "卡片随机数生成器似乎基于已知的生成多项式");
                        PrintAndLogEx(FAILED, "仅具有16个有效位，但显示意外行为，请重试。");
                    }
                    return PM3_SUCCESS;
                }
                case  2: {
...
#define PUBLIC_VIGIK_KEYLEN 128
    ...
    PrintAndLogEx(INFO, "RSA: 1024位");

    if (is_valid == false || i == ARRAYLEN(vigik_rsa_pk)) {
        PrintAndLogEx(INFO, "签名:");
        print_hex_noascii_break(d->rsa_signature, sizeof(d->rsa_signature),  MFBLOCK_SIZE * 2);
...
int vigik_annotate(mfc_vigik_t *d) {
    ...
    PrintAndLogEx(INFO, "制造商......... %s", sprint_hex(d->b0, sizeof(d->b0)));
    PrintAndLogEx(INFO, "MAD................. %s", sprint_hex(d->mad, sizeof(d->mad)));
    PrintAndLogEx(INFO, "计数器............ %u", d->counters);
    ...
    PrintAndLogEx(INFO, "信息标志........... %u -", d->info_flag); // ,  sprint_bin(d->info_flag, 1));
    PrintAndLogEx(INFO, "密钥版本......... %u", d->key_version);
    PrintAndLogEx(INFO, "PTR 计数器......... %u", d->ptr_counter);
    PrintAndLogEx(INFO, "计数器编号......... %u", d->counter_num);
    PrintAndLogEx(INFO, "槽位访问日期.... %s", sprint_hex(d->slot_access_date, sizeof(d->slot_access_date)));
    PrintAndLogEx(INFO, "槽位目标持续时间... %u", d->slot_dst_duration);
    PrintAndLogEx(INFO, "其他插槽......... %s", sprint_hex(d->other_slots, sizeof(d->other_slots)));
    PrintAndLogEx(INFO, "服务计数器.... %u", d->services_counter);
    PrintAndLogEx(INFO, "加载日期........ %s", sprint_hex(d->loading_date, sizeof(d->loading_date)));
    PrintAndLogEx(INFO, "保留空值....... %u", d->reserved_null);
    PrintAndLogEx(INFO, "----------------------------------------------------------------");
    PrintAndLogEx(INFO, "");
    vigik_verify(d);
    PrintAndLogEx(INFO, "----------------------------------------------------------------");
