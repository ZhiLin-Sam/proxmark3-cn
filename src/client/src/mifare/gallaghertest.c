...
static bool test_CAD(void) {
    ...
    if (result != 0x0F) {
        PrintAndLogEx(INFO, "Gallagher CAD 测试1失败：预期扇区0x0F，实际0x%02X", result);
        return false;
    }
    ...
    if (result != 0x0D) {
        PrintAndLogEx(INFO, "Gallagher CAD 测试 2 失败：预期扇区 0x0D，实际 0x%02X", result);
        return false;
    }
    ...
    if (result != -1) {
        PrintAndLogEx(INFO, "Gallagher CAD 测试 3 失败：预期 -1，实际 %d", result);
        return false;
    }
...
static bool test_creds(void) {
    ...
    if (!creds_match(&cred_result, &creds1)) {
        PrintAndLogEx(INFO, "Gallagher 编码/解码往返测试 1 失败");
        return false;
    }
    ...
    if (!creds_match(&cred_result, &creds2)) {
        PrintAndLogEx(INFO, "Gallagher 编码/解码往返测试 2 失败");
        return false;
    }
...
static bool test_known_vector_creds(void) {
    ...
    if (!creds_match(&expected, &result)) {
        PrintAndLogEx(INFO, "已知向量解码失败: RC=%d FC=%d CN=%d IL=%d",
    ...
    for (int i = 0; i < 8; i++) {
        if ((uint8_t)(known_block0[i] ^ 0xFF) != known_block0[i + 8]) {
            PrintAndLogEx(INFO, "按位取反检查在字节 %d 处失败", i);
            return false;
        }
    }
...
static bool test_mad_crc(void) {
    ...
    if (computed_crc != expected_crc) {
        PrintAndLogEx(INFO, "MAD CRC测试失败：预期0x%02X，得到0x%02X", expected_crc, computed_crc);
        return false;
    }
...
static bool test_MES(void) {
    ...
    if (memcmp(sector_result, known_sector, 16) != 0) {
        PrintAndLogEx(INFO, "Gallagher MES 编码测试失败");
        PrintAndLogEx(INFO, "预期：%s", sprint_hex_ascii(known_sector, 16));
    ...
    if (gallagher_decode_mes(known_sector, &result_creds) != PM3_SUCCESS) {
        PrintAndLogEx(INFO, "Gallagher MES 解码测试失败");
        return false;
    }
    if (!creds_match(&known_cred, &result_creds)) {
        PrintAndLogEx(INFO, "Gallagher MES 解码出的凭证与预期不同");
        return false;
    }
