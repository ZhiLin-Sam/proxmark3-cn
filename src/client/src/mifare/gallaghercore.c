...
int gallagher_encode_mes(uint8_t *sixteen_bytes, GallagherCredentials_t *creds) {
    ...
    if (creds->csn_len > 4) {
        PrintAndLogEx(ERR, "凭证无法编码到Mifare增强加密块中，仅支持4字节UUID");
        return PM3_ENOTIMPL;
    }
    ...
    PrintAndLogEx(DEBUG, "加密前的MES %s", sprint_hex_ascii(mes, 16));
    ...
    PrintAndLogEx(DEBUG, "加密后的MES %s", sprint_hex_ascii(sixteen_bytes, 16));
    return PM3_SUCCESS;
}
...
int gallagher_decode_mes(uint8_t *block, GallagherCredentials_t *creds) {
    ...
    gallagher_diversify_classic_key(creds->site_key, creds->csn, creds->csn_len, diversified_site_key);
    if (creds->csn_len > 4) {
        PrintAndLogEx(WARNING, "UUID 长度大于4，这可能不是有效的 Gallagher 凭证？");
    }
    ...
    PrintAndLogEx(DEBUG, "解密后的MES %s", sprint_hex_ascii(mes, 16));

    if (mes[0] != 0x01) {
        PrintAndLogEx(ERR, "MES块无效");
...
bool gallagher_is_valid_creds(uint64_t region_code, uint64_t facility_code, uint64_t card_number, uint64_t issue_level) {
    ...
    // validate input
    if (region_code > 0x0f) {
        PrintAndLogEx(ERR, "区域代码必须为 0 <= rc <= 15 (4位)，收到: %"PRIu64, region_code);
        is_valid = false;
    }
    if (facility_code > 0xffff) {
        PrintAndLogEx(ERR, "设施代码必须满足 0 <= fc <= 65535 (2字节)，收到: %"PRIu64, facility_code);
        is_valid = false;
    }
    if (card_number > 0xffffff) {
        PrintAndLogEx(ERR, "卡片号必须为0 <= cn <= 16777215（3字节），收到：%"PRIu64, card_number);
        is_valid = false;
    }
    if (issue_level > 0x0f) {
...
void print_gallagher_creds(GallagherCredentials_t *creds) {
    ...
    if (!gallagher_is_valid_creds_struct(creds)) {
        PrintAndLogEx(ERR, "无效的 Gallagher 凭证");
        return;
    }
