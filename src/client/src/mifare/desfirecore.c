...
void DesfirePrintContext(DesfireContext_t *ctx) {
    PrintAndLogEx(INFO, "密钥编号: %d 密钥算法: %s 密钥[%d]: %s",
                  ctx->keyNum,
    ...
    if (ctx->kdfAlgo != MFDES_KDF_ALGO_NONE) {
        PrintAndLogEx(INFO, "KDF 算法: %s KDF 输入[%d]: %s", CLIGetOptionListStr(DesfireKDFAlgoOpts, ctx->kdfAlgo), ctx->kdfInputLen, sprint_hex(ctx->kdfInput, ctx->kdfInputLen));
        PrintAndLogEx(INFO, "AID: %06x UID[%d]: %s", ctx->selectedAID, ctx->uidlen, sprint_hex(ctx->uid, ctx->uidlen));
    }

    PrintAndLogEx(INFO, "安全通道: %s 命令集: %s 通信模式: %s",
...
static int DESFIRESendApduEx(bool activate_field, sAPDU_t apdu, uint16_t le, uint8_t *result, uint32_t max_result_len, uint32_t *result_len, uint16_t *sw) {
    ...
    *result_len -= 2;
    ...
    if (isw != ISO7816_OK &&
        ...
        if (GetAPDULogging()) {
            if (isw >> 8 == 0x61) {
                PrintAndLogEx(ERR, "APDU链接长度: 0x%02x -->", isw & 0xff);
            } else {
                PrintAndLogEx(ERR, "APDU(%02x%02x)错误: [0x%4X] %s", apdu.CLA, apdu.INS, isw, GetAPDUCodeDescription(isw >> 8, isw & 0xff));
...
static int DESFIRESendRaw(bool activate_field, uint8_t *data, size_t datalen, uint8_t *result, uint32_t max_result_len, uint32_t *result_len, uint8_t *respcode) {
    *result_len = 0;
    ...
    if (GetAPDULogging()) {
        PrintAndLogEx(SUCCESS, "原始>> %s", sprint_hex(data, datalen));
    }
    ...
    if (GetAPDULogging()) {
        PrintAndLogEx(SUCCESS, "原始<< %s", sprint_hex(result, *result_len));
    }
    ...
    *result_len -= (1 + 2);
    ...
    if (rcode != MFDES_S_OPERATION_OK &&
        ...
        if (GetAPDULogging()) {
            PrintAndLogEx(ERR, "命令 (%02x) 错误: 0x%02x", data[0], rcode);
        }
...
static int DesfireExchangeNative(bool activate_field, DesfireContext_t *ctx, uint8_t cmd, uint8_t *data, size_t datalen, uint8_t *respcode, uint8_t *resp, size_t *resplen, bool enable_chaining, size_t splitbysize, size_t firsttxdatalen) {
    ...
    while (cdatalen > sentdatalen) {
        ...
        if (res != PM3_SUCCESS) {
            ...
            PrintAndLogEx(DEBUG, "错误 DESFIRESendRaw %s", DesfireGetErrorString(res, &ssw));
            free(buf);
            return res;
        }
        ...
        if ((rcode != MFDES_ADDITIONAL_FRAME) || (buflen > 0)) {
            if (sentdatalen != cdatalen) {
                PrintAndLogEx(WARNING, "Tx链错误。需要发送：%d，但已发送：%zu", cdatalen, sentdatalen);
            }
            break;
        }
    }
...
static int DesfireExchangeISONative(bool activate_field, DesfireContext_t *ctx, uint8_t cmd, uint8_t *data, size_t datalen, uint8_t *respcode, uint8_t *resp, size_t *resplen, bool enable_chaining, size_t splitbysize, size_t firsttxdatalen) {
    ...
    while (first_tx_frame || datalen > sentdatalen) {
        ...
        if (res != PM3_SUCCESS) {
            ...
            PrintAndLogEx(DEBUG, "错误 DESFIRESendApdu %s", DesfireGetErrorString(res, &sw));
            free(buf);
            return res;
        }
        ...
        if (sw != DESFIRE_GET_ISO_STATUS(MFDES_ADDITIONAL_FRAME) || buflen > 0) {
            if (sentdatalen != datalen) {
                PrintAndLogEx(WARNING, "Tx链错误。需要发送：%zu，但已发送：%zu", datalen, sentdatalen);
            }
            break;
        }
    }
    ...
    for (uint32_t x = 0; x < rndlen; x++) {
        if (RndA[x] != encRndA[x]) {
            if (g_debugMode > 1) {
                ...
                PrintAndLogEx(DEBUG, "生成的_RndA：%s", sprint_hex(encRndA, rndlen));
            }
            return 11;
        }
    }
...
static int DesfireAuthenticateEV2(DesfireContext_t *dctx, DesfireSecureChannel secureChannel, bool firstauth, bool verbose) {
    ...
    if (g_debugMode > 1) {
        ...
        PrintAndLogEx(DEBUG, "两者: %s", sprint_hex(tmp, CRYPTO_AES_BLOCK_SIZE * 2));
    }
    ...
    if (memcmp(rotRndA, recRndA, CRYPTO_AES_BLOCK_SIZE) != 0) {
        if (g_debugMode > 1) {
            ...
            PrintAndLogEx(DEBUG, "生成的_RndA'：%s", sprint_hex(recRndA, CRYPTO_AES_BLOCK_SIZE));
        }
        return 11;
    }
    ...
    if (verbose) {
        ...
        PrintAndLogEx(INFO, "会话密钥ENC: %s", sprint_hex(dctx->sessionKeyEnc, 16));
        PrintAndLogEx(INFO, "会话密钥MAC: %s", sprint_hex(dctx->sessionKeyMAC, 16));
    }
...
static int DesfireAuthenticateISO(DesfireContext_t *dctx, DesfireSecureChannel secureChannel, bool verbose) {
    ...
    if (verbose)
        PrintAndLogEx(INFO, "会话密钥: %s", sprint_hex(dctx->sessionKeyEnc, desfire_get_key_length(dctx->keyType)));
...
static int DesfireAuthenticateLRP(DesfireContext_t *dctx, DesfireSecureChannel secureChannel, bool firstauth, bool verbose) {
    ...
    if (memcmp(recCMAC, cmac, CRYPTO_AES_BLOCK_SIZE) != 0) {
        if (g_debugMode > 1) {
            ...
            PrintAndLogEx(DEBUG, "生成的 cmac：%s", sprint_hex(cmac, CRYPTO_AES_BLOCK_SIZE));
        }
        return 12;
    }
...
int DesfireAuthenticate(DesfireContext_t *dctx, DesfireSecureChannel secureChannel, bool verbose) {
    if (dctx->kdfAlgo == MFDES_KDF_ALGO_AN10922) {
        ...
        if (mfdes_kdf_input_gallagher(dctx->uid, dctx->uidlen, dctx->keyNum, dctx->selectedAID, dctx->kdfInput, &dctx->kdfInputLen) != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "无法生成 Gallagher KDF 输入");
        }
...
void DesfirePrintPICCInfo(DesfireContext_t *dctx, PICCInfo_t *PICCInfo) {
    ...
    if (PICCInfo->authCmdCheck.checked) {
        PrintAndLogEx(SUCCESS, "PICC 级别认证命令");
...
int DesfireFillFileList(DesfireContext_t *dctx, FileList_t FileList, size_t *filescount, bool *isopresent) {
    ...
    *filescount = buflen;
    ...
    if (buflen > 0) {
        ...
        if (isoindx * 2 != buflen)
            PrintAndLogEx(WARNING, "错误的ISO ID列表长度，应为%zu，实际为%zu", buflen, isoindx * 2);
    } else {
        PrintAndLogEx(WARNING, "ISO ID 列表未返回数据");
    }
...
int DesfireValueFileOperations(DesfireContext_t *dctx, uint8_t fid, uint8_t operation, uint32_t *value) {
    ...
    if ((res == 0x7E || res == -20) && dctx->commMode == DCMMACed) {
        PrintAndLogEx(INFO, "MAC 模式因长度错误失败，正在以明文模式重试");
...
static void PrintKeySettingsPICC(uint8_t keysettings, uint8_t numkeys, bool print2ndbyte) {
    PrintAndLogEx(SUCCESS, "PICC 级别权限");
    ...
    PrintAndLogEx(SUCCESS, "[..%c.] AID列表/获取密钥设置需要CMK：%s", (keysettings & (1 << 1)) ? '1' : '0', (keysettings & (1 << 1)) ? _GREEN_("NO") : _RED_("YES"));
...
static void PrintKeySettingsApp(uint8_t keysettings, uint8_t numkeys, bool print2ndbyte) {
    ...
    PrintAndLogEx(SUCCESS, "[..%c.] FID列表/获取密钥设置需要AMK：%s", (keysettings & (1 << 1)) ? '1' : '0', (keysettings & (1 << 1)) ? _GREEN_("NO") : _RED_("YES"));
    ...
    if (print2ndbyte) {
        ...
        PrintAndLogEx(SUCCESS, "Key cnt.... " _YELLOW_("%d"), numkeys & 0x0F);
        if (numkeys & 0x20) {
            PrintAndLogEx(SUCCESS, "ISO文件ID：已启用");
        }
    }
}
...
static const char *DesfireDisabledStr = "已禁用";
static const char *DesfireFreeStr = "free";
static const char *DesfireNAStr = "不适用";
static const DesfireCreateFileCommands_t DesfireFileCommands[] = {
    ...
    {0x02, "值",           MFDES_CREATE_VALUE_FILE,         16, 16, false},
...
void DesfirePrintFileSettingsExtended(FileSettings_t *fsettings) {
    ...
    switch (fsettings->fileType) {
        ...
        case 0x02: {
            ...
            PrintAndLogEx(SUCCESS, "Limited credit  : [%d - %s] %d (0x%08X)", fsettings->limitedCredit, (limited_credit_enabled) ? "enabled" : "已禁用", fsettings->value, fsettings->value);
            PrintAndLogEx(SUCCESS, "GetValue访问: %s", ((fsettings->limitedCredit & 0x02) != 0) ? "Free" : "Not Free");
            break;
        }
        case 0x03:
        case 0x04: {
            PrintAndLogEx(SUCCESS, "Record count    : %d [0x%x]", fsettings->curRecordCount, fsettings->curRecordCount);
            PrintAndLogEx(SUCCESS, "最大记录数: %d [0x%x]", fsettings->maxRecordCount, fsettings->maxRecordCount);
...
static void DesfirePrintFileSettDynPart(uint8_t filetype, uint8_t *data, size_t datalen, uint8_t *dynlen, bool create) {
    switch (filetype) {
        ...
        case 0x02: {
            ...
            PrintAndLogEx(INFO, "下限... %d / 0x%08X", lowerlimit, lowerlimit);
            PrintAndLogEx(INFO, "上限... %d / 0x%08X", upperlimit, upperlimit);
            if (create) {
                PrintAndLogEx(INFO, "值............ %d / 0x%08X", value, value);
                PrintAndLogEx(INFO, "有限信用... %d - %s"
                              , limited_credit_enabled
                              , ((limited_credit_enabled & 1) != 0) ? "enabled" : "已禁用"
                             );
            } else {
                PrintAndLogEx(INFO, "有限信用... %d - %s %d (0x%08X)"
                              , limited_credit_enabled
                              , ((limited_credit_enabled & 1) != 0) ? "enabled" : "已禁用"
                              , value
                              , value
            ...
            PrintAndLogEx(INFO, "GetValue访问... %s", ((limited_credit_enabled & 0x02) != 0) ? "Free" : "Not Free");
        ...
        case 0x04: {
            ...
            PrintAndLogEx(INFO, "记录大小....... %d / 0x%X 字节", recordsize, recordsize);
            PrintAndLogEx(INFO, "最大记录数... %d / 0x%X", maxrecords, maxrecords);
            PrintAndLogEx(INFO, "总大小........ %d / 0x%X 字节", recordsize * maxrecords, recordsize * maxrecords);
            if (create == false)
                PrintAndLogEx(INFO, "当前记录数... %d / 0x%X", currentrecord, currentrecord);

            *dynlen = (create) ? 6 : 9;
            break;
        }
        case 0x05: {
            PrintAndLogEx(INFO, "密钥类型 [0x%02x] ... %s", data[0], GetDesfireKeyType(data[0]));
            *dynlen = 1;

            if (create) {
                PrintAndLogEx(INFO, "密钥... %s", sprint_hex(&data[1], 16));
                *dynlen += 16;
            }

            PrintAndLogEx(INFO, "密钥版本... %d / 0x%X", data[*dynlen], data[*dynlen]);
            (*dynlen)++;
            break;
        }
        default: {
...
void DesfirePrintFileSettings(uint8_t *data, size_t len) {
    ...
    PrintAndLogEx(SUCCESS, "文件通信模式...... %s", GetDesfireCommunicationMode(data[1] & 0x03));
    bool addaccess = false;
    if (filetype != 0x05) {
        addaccess = ((data[1] & 0x80) != 0);
        PrintAndLogEx(SUCCESS, "额外访问... %s", (addaccess) ? "Yes" : "No");
    }

    PrintAndLogEx(SUCCESS, "访问权限....... %04x", MemLeToUint2byte(&data[2]));
    ...
    if (addaccess && filetype != 0x05 && reclen > 0 && len > reclen && len == reclen + data[reclen] * 2) {
        PrintAndLogEx(SUCCESS, "添加访问记录... %d", data[reclen]);
        for (int i = 0; i < data[reclen] * 2; i += 2) {
            PrintAndLogEx(SUCCESS, "添加访问权限：[%d] %04x", i / 2, MemLeToUint2byte(&data[reclen + 1 + i]));
...
void DesfirePrintSetFileSettings(uint8_t *data, size_t len) {
    ...
    if (addaccess && len > 3 && len == 4 + data[3] * 2) {
        PrintAndLogEx(SUCCESS, "添加访问记录：%d", data[3]);
        for (int i = 0; i < data[3] * 2; i += 2) {
            PrintAndLogEx(SUCCESS, "添加访问权限：[%d] %04x", i / 2, MemLeToUint2byte(&data[4 + i]));
...
void DesfirePrintCreateFileSettings(uint8_t filetype, uint8_t *data, size_t len) {
    ...
    if (ftyperec == NULL) {
        PrintAndLogEx(WARNING, "未知文件类型 0x%02x", filetype);
        return;
    }
...
int DesfireChangeKey(DesfireContext_t *dctx, bool change_master_key, uint8_t newkeynum, DesfireCryptoAlgorithm newkeytype, uint32_t newkeyver, uint8_t *newkey, DesfireCryptoAlgorithm oldkeytype, uint8_t *oldkey, bool verbose) {
    ...
    if (newkeytype != T_AES && newkeyver < 0x100) {
        ...
        if (verbose)
            PrintAndLogEx(INFO, "已更改新密钥: %s [%d] %s", CLIGetOptionListStr(DesfireAlgoOpts, newkeytype), desfire_get_key_length(newkeytype), sprint_hex(nkeybuf, desfire_get_key_length(newkeytype)));
    }
