...
static int open_mad_file(json_t **root, bool verbose) {
    ...
    *root = json_load_file(path, 0, &error);
    ...
    if (verbose) {
        PrintAndLogEx(SUCCESS, "已加载文件 `" _YELLOW_("%s") "` " _GREEN_("%zu") " records ( " _GREEN_("ok") " )"
                      , path
...
int MADCheck(uint8_t *sector0, uint8_t *sector16, bool verbose, bool *haveMAD2) {
    ...
    if (verbose) {
        if (GPB & 0x40)
            PrintAndLogEx(SUCCESS, "多应用卡");
        else
            PrintAndLogEx(SUCCESS, "单应用卡");
    }
    return res;
}
    ...
    if (mad_ver == 1) {
        ...
        if (info >= 0xF) {
            ...
            if (verbose) {
                ...
                PrintAndLogEx(WARNING, "MAD1信息字节指向MAD1扇区空间之外(0x%02x)，是否报告错误？", info);
            }
            return PM3_ESOFT;
        }
    } else {
