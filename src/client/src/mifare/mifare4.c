...
int CalculateMAC(mf4Session_t *mf4session, MACType_t mtype, uint8_t blockNum, uint8_t blockCount, uint8_t *data, int datalen, uint8_t *mac, bool verbose) {
    ...
    if (verbose)
        PrintAndLogEx(INFO, "MAC 数据[%d]：%s", macdatalen, sprint_hex(macdata, macdatalen));
...
int MifareAuth4(mf4Session_t *mf4session, const uint8_t *keyn, uint8_t *key, bool activateField, bool leaveSignalON, bool dropFieldIfError, bool verbose, bool silentMode) {
    ...
    if (res != PM3_SUCCESS) {
        ...
        if (silentMode == false) {
            PrintAndLogEx(ERR, "原始交换错误：%d", res);
        }
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "< 阶段1: %s", sprint_hex(data, datalen));
    }

    if (datalen < 1) {
        if (silentMode == false) {
            PrintAndLogEx(ERR, "卡片响应长度错误：%d", datalen);
        }
    ...
    if (data[0] != 0x90) {
        if (silentMode == false) {
            PrintAndLogEx(ERR, "卡片响应错误：%02x %s", data[0], mfpGetErrorDescription(data[0]));
        }
    ...
    if (datalen != 19) { // code 1b + 16b + crc 2b
        if (silentMode == false) {
            PrintAndLogEx(ERR, "卡片响应必须为19字节长，实际为：%d", datalen);
        }
    ...
    if (verbose) {
        PrintAndLogEx(INFO, ">阶段2: %s", sprint_hex(cmd2, 33));
    }
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "< 阶段2: %s", sprint_hex(data, datalen));
    }

    aes_decode(NULL, key, &data[1], raw, 32);

    if (verbose) {
        PrintAndLogEx(INFO, "结果: %s", sprint_hex(raw, 32));
    ...
    if (memcmp(&raw[4], &RndA[1], 16)) {
        ...
        if (verbose) {
            PrintAndLogEx(ERR, "RndA 读卡器：%s", sprint_hex(&RndA[1], 16));
    ...
    if (verbose) {
        ...
        PrintAndLogEx(INFO, "图片: %s", sprint_hex(&raw[20], 6));
...
int mfpReadSector(uint8_t sectorNo, uint8_t keyType, uint8_t *key, uint8_t *dataout, bool verbose) {
    ...
    if (res) {
        PrintAndLogEx(ERR, "扇区 %u 认证错误: %d", sectorNo, res);
        return res;
    }
    ...
    for (int n = firstBlockNo; n < firstBlockNo + mfNumBlocksPerSector(sectorNo); n++) {
        ...
        if (res) {
            PrintAndLogEx(ERR, "扇区 %u 读取错误: %d", sectorNo, res);
            DropField();
            return res;
        }

        if (datalen && data[0] != 0x90) {
            PrintAndLogEx(ERR, "扇区 %u 卡读取错误: %02x %s", sectorNo, data[0], mfpGetErrorDescription(data[0]));
            DropField();
            return 5;
        }
        if (datalen != 1 + 16 + 8 + 2) {
            PrintAndLogEx(ERR, "扇区 %u 错误返回数据长度: %d", sectorNo, datalen);
            DropField();
            return 6;
        }
        ...
        if (verbose)
            PrintAndLogEx(INFO, "数据[%03d]: %s", n, sprint_hex(&data[1], 16));

        if (memcmp(&data[1 + 16], mac, 8)) {
            PrintAndLogEx(WARNING, "警告：块 %d 上的mac不相等...", n);
