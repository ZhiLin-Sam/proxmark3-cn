...
int demodDestron(bool verbose) {
    ...
    if (FSKrawDemod(0, 0, 0, 0, false) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: FSK解调失败");
        return PM3_ESOFT;
    }
    ...
    if (ans < 0) {
        if (ans == -1)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: 找到的比特太少");
        else if (ans == -2)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: 未找到前导码");
        else if (ans == -3)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: 大小不正确：%zu", size);
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: ans: %d", ans);
    ...
    if (errCnt > 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: Manchester解码错误：%d", ans);
        return PM3_ESOFT;
    }
    ...
    if (parity_err > 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Destron: 奇偶校验错误：%d", parity_err);
        return PM3_ESOFT;
    }
...
static int CmdDestronDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频Destron解调",
                  "尝试查找Destron前导码，如果找到则解码/解扰数据",
                  "低频Destron解调"
                 );
...
static int CmdDestronReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Destron读取器",
                  "读取 Destron 标签",
...
static int CmdDestronClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Destron克隆",
                  "将 Destron 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
    ...
    if (datalen > 5) {
        PrintAndLogEx(FAILED, "UID 最多5字节（收到 %u）", datalen);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("低频Destron读取器") "` to verify");
    return res;
}

static int CmdDestronSim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Destron模拟",
                  "尝试查找Destron前导码，如果找到则解码/解扰数据",
                  "低频Destron模拟"
                 );
...
static command_t CommandTable[] = {
    ...
    {"demod",  CmdDestronDemod,  AlwaysAvailable, "从 GraphBuffer 解调 Destron 标签"},
    {"reader", CmdDestronReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",  CmdDestronClone,  IfPm3Lf,         "克隆Destron标签到T55x7、Q5/T5555或EM4305/4469"},
    {"sim",    CmdDestronSim,    IfPm3Lf,         "模拟Destron标签"},
    {NULL, NULL, NULL, NULL}
};
...
int detectDestron(uint8_t *dest, size_t *size) {
    ...
    PrintAndLogEx(DEBUG, "DEBUG: detectDestron FSK 找到前导码");
