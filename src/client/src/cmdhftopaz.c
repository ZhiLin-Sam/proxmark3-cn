...
static int topaz_send_cmd_raw(uint8_t *cmd, uint8_t len, uint8_t *response, uint16_t *response_len, bool verbose) {
    ...
    if (resp.oldarg[0] == *response_len) {
        *response_len = resp.oldarg[0];
        ...
        if (verbose) PrintAndLogEx(WARNING, "错误的响应长度(%d != %" PRIu64 ")", *response_len, resp.oldarg[0]);
        return PM3_ESOFT;
    }
    return PM3_SUCCESS;
}
...
// (data[3] & 0x0F) == 0 ? "Write access granted without any security" : (data[3] & 0x0F) == 0x0F ? "No write access granted at all" : "(RFU)");
    ...
    PrintAndLogEx(SUCCESS, "能力容器: %s", sprint_hex(data, 4));
...
static void topaz_print_control_TLVs(uint8_t *memory) {
    ...
    if (lock_TLV_present == false) {
        PrintAndLogEx(SUCCESS, "(无锁定控制TLV)");
    }

    if (reserved_memory_control_TLV_present == false) {
        PrintAndLogEx(SUCCESS, "(无保留内存控制TLV)");
    }
}
...
int CmdHFTopazInfo(const char *Cmd) {
    ...
    if (status == PM3_ESOFT) {
        PrintAndLogEx(SUCCESS, "无NDEF消息数据");
        topaz_switch_off_field();
        return PM3_SUCCESS;
    }
...
static int CmdHFTopazRaw(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频TOPAZ原始",
                  "向 Topaz 标签发送原始十六进制数据",
                  "高频TOPAZ原始");
    ...
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    CLIParserFree(ctx);
    PrintAndLogEx(INFO, "尚未实现，请暂时使用 `hf 14 raw --topaz`");
    return PM3_SUCCESS;
}
...
static int CmdHFTopazSniff(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf topaz list")"` to view captured tracelog");
...
static int CmdHFTopazView(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf topaz 查看",
                  "打印 Topaz 标签转储文件 (bin/eml/json)",
...
static int CmdHFTopazWrBl(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf topaz 写块",
                  "写入Topaz块，数据为8个十六进制字节",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int1(NULL, "blk", "<dec>", "块号"),
        arg_str1("d", "数据", "<hex>", "块数据（8 个十六进制字节）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (dlen != 8) {
        PrintAndLogEx(WARNING, "错误的数据长度。期望8，实际为%d", dlen);
        return PM3_EINVARG;
    }

    PrintAndLogEx(INFO, "块：%0d (0x%02X) [ %s]", blockno, blockno, sprint_hex(data, dlen));
    ...
    if (res == PM3_SUCCESS) {
        ...
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf topaz rdbl --blk %u") "` to verify", blockno);
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdHFTopazList,     AlwaysAvailable, "列出 Topaz 历史"},
    {"-----------", CmdHelp,            IfPm3Iso14443a,  "------------------- " _CYAN_("operations") " ---------------------"},
    {"dump",        CmdHFTopazDump,     IfPm3Iso14443a,  "将TOPAZ系列标签转储到文件"},
    {"info",        CmdHFTopazInfo,     IfPm3Iso14443a,  "标签信息"},
    {"raw",         CmdHFTopazRaw,      IfPm3Iso14443a,  "向标签发送原始十六进制数据"},
    {"rdbl",        CmdHFTopazRdBl,     IfPm3Iso14443a,  "读取块"},
    {"reader",      CmdHFTopazReader,   IfPm3Iso14443a,  "模拟 Topaz 读取器"},
    {"sim",         CmdHFTopazSim,      IfPm3Iso14443a,  "模拟Topaz标签"},
    {"sniff",       CmdHFTopazSniff,    IfPm3Iso14443a,  "嗅探 Topaz 读写器与标签通信"},
    {"view",        CmdHFTopazView,     AlwaysAvailable, "显示标签转储文件的内容"},
    {"wrbl",        CmdHFTopazWrBl,     IfPm3Iso14443a,  "写入块"},
...
int readTopazUid(bool loop, bool verbose) {
    ...
    do {
        ...
        if (status == PM3_ESOFT) {
            if (verbose) PrintAndLogEx(ERR, "错误：无法接收 ATQA");
        ...
        if (atqa[1] != 0x0c && atqa[0] != 0x00) {
            if (verbose) PrintAndLogEx(ERR, "标签不支持Topaz协议。");
        ...
        if (status == PM3_EWRONGANSWER) {
            if (verbose) PrintAndLogEx(ERR, "错误：标签未响应 RID");
        ...
        if (status == PM3_ESOFT) {
            PrintAndLogEx(ERR, "错误：标签未响应 RALL");
