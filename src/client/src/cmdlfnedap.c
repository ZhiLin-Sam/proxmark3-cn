...
int demodNedap(bool verbose) {
    ...
    if (ASKbiphaseDemod(0, 64, 1, 0, false) != PM3_SUCCESS) {
        if (g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: 错误 - NEDAP: ASK/双相解调失败");
        return PM3_ESOFT;
    }

    size = g_DemodBufferLen;
    if (!preambleSearch(g_DemodBuffer, (uint8_t *) preamble, sizeof(preamble), &size, &offset)) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - NEDAP: 未找到前导码");
        return PM3_ESOFT;
    }
    ...
    // sanity checks
    if ((size != 128) && (size != 64)) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - NEDAP: 大小不正确: %zu", size);
        return PM3_ESOFT;
    }

    if (bits_to_array(g_DemodBuffer, size, data) != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - NEDAP: bits_to_array 错误\\n");
        return PM3_ESOFT;
    }
    ...
    if (isEven_64_63(data) != (data[7] & 0x01)) {
        PrintAndLogEx(ERR, "错误奇偶校验 (%1u)", data[7] & 0x01);
        ret = PM3_ESOFT;
    }
    ...
    if (isValid == false) {
        PrintAndLogEx(ERR, "校验和 : %s (计算值 0x%04X != 0x%04X)", _RED_("fail"), checksum, checksum2);
        ret = PM3_ESOFT;
    }
    ...
    if ((idxC1 != 0xFF) && (idxC2 != 0xFF) && (idxC3 != 0xFF) && (idxC4 != 0xFF) && (idxC5 != 0xFF)) {
        ...
        PrintAndLogEx(DEBUG, "校验和 ( %s ) 0x%04X",  _GREEN_("ok"), checksum);

    } else {
        PrintAndLogEx(ERR, "无效索引 (1:%02x - 2:%02x - 3:%02x - 4:%02x - 5:%02x)", idxC1, idxC2, idxC3, idxC4, idxC5);
        ret = PM3_ESOFT;
    }

    if (size > 64) {
        // second part
        PrintAndLogEx(DEBUG, "NEDAP 标签，找到第二部分");

        if (isEven_64_63(data + 8) != (data[15] & 0x01)) {
            PrintAndLogEx(ERR, "错误奇偶校验 (%1u)", data[15] & 0x01);
            return ret;
        }
        ...
        if (!r0 && (r1 < 10) && (r2 < 10) && (r3 < 10) && (r4 < 10) && (r5 < 10)) {
            ...
            if ((fixed0 == FIXED_71) && (fixed1 == FIXED_40))
                PrintAndLogEx(DEBUG, "固定部分 {0 = 0x%02x, 1 = 0x%02x}", fixed0, fixed1);
            else
                PrintAndLogEx(DEBUG, "错误固定值：{0 = 0x%02x (%0x02x), 1 = 0x%02x (%0x02x)}", fixed0, FIXED_71, fixed1, FIXED_40);

            PrintAndLogEx(DEBUG, "Unknown part  {1 = 0x%02x, 2 = 0x%02x}", unk1, unk2);
        } else {
            PrintAndLogEx(ERR, "错误数字 (0:%1x - 1:%1x - 2:%1x - 3:%1x - 4:%1x - 5:%1x)", r0, r1, r2, r3, r4, r5);
            return ret;
        }
    }
...
static int CmdLFNedapDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 Nedap 解调",
                  "尝试查找Nedap前导码，如果找到则解码/解扰数据",
                  "低频 Nedap 解调"
                 );
...
static int CmdLFNedapClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Nedap 克隆",
                  "将 Nedap 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
    ...
    void *argtable[] = {
        ...
        arg_lit0("l", "long", "可选 - 长(128)，默认为短(64)"),
    ...
    if (sub_type > 0xF) {
        PrintAndLogEx(FAILED, "超出范围，有效子类型在0-15之间");
        return PM3_EINVARG;
    }

    if (customer_code > 0xFFF) {
        PrintAndLogEx(FAILED, "超出范围，有效客户代码在0-4095之间");
        return PM3_EINVARG;
    }

    if (id > 99999) {
        PrintAndLogEx(FAILED, "超出范围，ID最大值为99999");
        return PM3_EINVARG;
    }
    ...
    if (res == PM3_SUCCESS) {
        PrintAndLogEx(INFO, "块 0 已被更改（扩展），可能难以检测。");
...
static command_t CommandTable[] = {
    ...
    {"demod",  CmdLFNedapDemod,  AlwaysAvailable, "从 GraphBuffer 解调 Nedap 标签"},
    {"reader", CmdLFNedapReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",  CmdLFNedapClone,  IfPm3Lf,         "克隆Nedap标签到T55x7、Q5/T5555或EM4305/4469"},
    {"sim",    CmdLFNedapSim,    IfPm3Lf,         "模拟Nedap标签"},
    {NULL, NULL, NULL, NULL}
};
