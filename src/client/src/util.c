...
int kbd_enter_pressed(void) {
    ...
    if ((flags = fcntl(STDIN_FILENO, F_GETFL, 0)) < 0) {
        PrintAndLogEx(ERR, "kbd_enter_pressed中fcntl失败");
        return -1;
    }
    //non-blocking
    flags |= O_NONBLOCK;
...
char *sprint_bytebits_bin_break(const uint8_t *data, const size_t len, const uint8_t breaks) {
    ...
    for (int i = 0; i < rowlen; i++) {
        ...
        if (c == 7) {
            ...
            PrintAndLogEx(ERR, "传递给sprint_bytebits_bin_break()的数据无效");
            return buf;
        }
...
int param_getstr(const char *line, int paramnum, char *str, size_t buffersize) {
    ...
    if (en - bg + 1 >= buffersize) {
        PrintAndLogEx(ERR, "越界错误: 需要 %d 字节，有 %zu 字节\\n", en - bg + 1 + 1, buffersize);
        return 0;
    }
...
int hextobinarray_n(char *target, char *source, int sourcelen) {
    ...
    while (sourcelen--) {
        ...
        if (x >= '0' && x <= '9') {
            ...
            PrintAndLogEx(INFO, "(hextobinarray) 在索引 %d 处发现未知字符 %c %d，位于 %s", x, x, (int16_t)(source - start), start);
            return 0;
        }
...
int binstr_2_binarray(uint8_t *target, char *source, int length) {
    ...
    while (length--) {
        ...
        if (x >= '0' && x <= '1') {
            ...
            PrintAndLogEx(WARNING, "(binstring2binarray) 在索引 %d 处发现未知字符 %c %d，位于 %s", x, x, (int16_t)(source - start), start);
            return 0;
        }
        *(target++) = x;
        count++;
...
#else
    PrintAndLogEx(FAILED, "未使用 fct bitcount64 编译");
    return 0;
#endif
}
