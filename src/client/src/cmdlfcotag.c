...
int demodCOTAG(bool verbose) {
    ...
    if (err > 50) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - COTAG错误过多：%d", err);
        return PM3_ESOFT;
    }
...
static int CmdCOTAGDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频Cotag解调",
                  "尝试查找COTAG前导码，如果找到则解码/解扰数据",
                  "低频Cotag解调"
                 );
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdCOTAGDemod,   AlwaysAvailable, "解调 COTAG 标签"},
    {"reader",  CmdCOTAGReader,  IfPm3Lf,         "尝试读取并提取标签数据"},
    {NULL, NULL, NULL, NULL}
};
static int CmdHelp(const char *Cmd) {
