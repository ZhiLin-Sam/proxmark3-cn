...
static int testDES(uint8_t *key, testcase_t testcase) {
    ...
    if (memcmp(div_key, testcase.div_key, 8) != 0) {
        ...
        PrintAndLogEx(FAILED, "Div密钥 != 预期结果");
...
static void des_checkParity(uint8_t *key) {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        if (parity != (key[i] & 0x1)) {
            fails++;
            PrintAndLogEx(FAILED, "parity1失败，字节%d [%02x] 为%d，应为%d", i, key[i], (key[i] & 0x1), parity);
        }
    }
...
static int testKeyDiversificationWithMasterkeyTestcases(uint8_t *key) {
    ...
    if (error) {
        PrintAndLogEx(FAILED, "发生 %d 个错误，%d 个测试用例（%s）", error, i, _RED_("fail"));
    } else {
...
int doKeyTests(void) {
    ...
    PrintAndLogEx(SUCCESS, "正在检查密钥奇偶校验...");
    des_checkParity(key);

    // Test hashing functions
    testKeyDiversificationWithMasterkeyTestcases(key);
    PrintAndLogEx(INFO, "测试使用非敏感密钥的密钥分散...");
