...
void hash2(uint8_t *key64, uint8_t *outp_keytable) {
    ...
    **** The 64-bit HS Custom Key Value = 5B7C62C491C11B39 ******/
    ...
    if (g_debugMode > 0) {
        PrintAndLogEx(DEBUG, "高安全自定义密钥 (Kcus):");
...
int bruteforceItem(loclass_dumpdata_t item, uint16_t keytable[]) {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        if (numbytes_to_recover > 3) {
            PrintAndLogEx(FAILED, "CSN 需要超过 3 字节的暴力破解，不支持");
    ...
    if (loclass_found == 0xFF) {
        res = PM3_ESOFT;
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(WARNING, "使用以下 CSN 恢复 %d 字节失败", numbytes_to_recover);
...
int calculateMasterKey(uint8_t first16bytes[], uint8_t kcus[]) {
    ...
    if (memcmp(z_0, result, 4) != 0) {
        PrintAndLogEx(WARNING, "计算主密钥, k_cus ( %s )", _RED_("fail"));
        PrintAndLogEx(NORMAL, "");
        return PM3_ESOFT;
    }

    PrintAndLogEx(SUCCESS, "--- " _CYAN_("High security custom key (Kcus)") " ---");
    PrintAndLogEx(SUCCESS, "标准格式... %s", sprint_hex_inrow(key64_stdformat, sizeof(key64_stdformat)));
...
int bruteforceDump(uint8_t dump[], size_t dumpsize, uint16_t keytable[]) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "loclass 密钥恢复( %s )", _RED_("fail"));
        PrintAndLogEx(HINT, "尝试 `" _YELLOW_("hf iclass sim -t 2") "` again and collect new data");
        PrintAndLogEx(NORMAL, "");
        return PM3_ESOFT;
    }
...
static int _test_iclass_key_permutation(void) {
    ...
    if (memcmp(testcase_output, testcase_output_correct, 8) != 0) {
        PrintAndLogEx(ERR, "iclass密钥置换错误！");
    ...
    if (memcmp(testcase, testcase_output_rev, 8) != 0) {
        PrintAndLogEx(ERR, "反向iclass密钥置换错误");
...
static int _testHash1(void) {
    ...
    if (memcmp(k, expected, 8) != 0) {
        PrintAndLogEx(ERR, "哈希1错误！");
        printarr("calculated", k, 8);
        printarr("期望", expected, 8);
        return PM3_ESOFT;
    }
    return PM3_SUCCESS;
}
...
int testElite(bool slowtests) {
    PrintAndLogEx(INFO, "测试 iClass Elite 功能...");
    PrintAndLogEx(INFO, "测试 hash2...");
