    ...
    if (is_response) {
        ...
        if (nbits == 5) {
            ...
            // Hitag 1 commands
                ...
                exp_len += snprintf(exp + exp_len, size - exp_len, "未知 (%02X)", command);
            }
            ...
            concatbits(&command, 0, cmd, 0, 5, false);
            exp_len += snprintf(exp + exp_len, size - exp_len, "数据");
            ...
            if (command == HITAGS_SELECT) {
                ...
                exp_len = snprintf(exp, size, "选择 UID: %02X%02X%02X%02X", uid[0], uid[1], uid[2], uid[3]);
            }
        }
    } else {
...
static bool hts_get_uid(uint32_t *uid) {
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - 获取 Hitag S UID 失败");
        return false;
    }
...
static int process_hitags_common_args(CLIParserContext *ctx, lf_hitag_data_t *const packet) {
    ...
    if (key_len != 0 && key_len != HITAG_PASSWORD_SIZE && key_len != HITAG_CRYPTOKEY_SIZE) {
        PrintAndLogEx(WARNING, "错误的KEY长度，期望0、4或6，实际为%d", key_len);
        return PM3_EINVARG;
    }

    if (nrar_len && nrar_len != HITAG_NRAR_SIZE) {
        PrintAndLogEx(WARNING, "错误的NR/AR长度，期望%d，实际为%d", HITAG_NRAR_SIZE, nrar_len);
        return PM3_EINVARG;
    }
    ...
    if (mode > 3) {
        PrintAndLogEx(WARNING, "错误的响应协议模式，期望0、1、2或3，实际为%d", mode);
        return PM3_EINVARG;
    }
...
static void print_error(int8_t reason) {
    switch (reason) {
        case -2:
            PrintAndLogEx(FAILED, "UID 请求失败！");
            break;
        case -3:
            PrintAndLogEx(FAILED, "选择UID失败！");
            break;
        case -4:
        ...
        case -6:
            ...
            break;
        case -7:
            PrintAndLogEx(FAILED, "错误，未知功能");
            break;
        case -8:
            PrintAndLogEx(FAILED, "认证失败！");
            break;
        case -9:
            PrintAndLogEx(FAILED, "页面无写入权限");
            break;
        case -10:
            PrintAndLogEx(FAILED, "写入页面失败！");
            break;
        case -11:
            PrintAndLogEx(FAILED, "读取页失败！");
            break;
        default:
            // PM3_REASON_UNKNOWN
            PrintAndLogEx(FAILED, "错误 - Hitag S 失败");
    }
}
...
static int CmdLFHitagSRead(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf hitag hts 读块",
                  "读取 Hitag S 存储器\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("8", "82xx", "8268/8310模式"),
        arg_str0(NULL, "nrar", "<hex>", "nonce / answer writer, 8 hex bytes"),
        arg_lit0(NULL, "crypto", "crypto mode"),
        arg_str0("k", "key", "<hex>", "密码或密钥，4或6个十六进制字节"),
        arg_int0("m", "mode", "<0|1|2|3>", "响应协议模式（默认3）"),
        arg_int0("p", "page", "<dec>", "要读取的页地址"),
        arg_int0("c", "count", "<dec>", "要读取的页数。'0' 读取直到结束页的所有页（默认：1）"),
        arg_param_end
    };
    ...
    if (count > HITAGS_MAX_PAGES) {
        PrintAndLogEx(WARNING, "一次最多读取 64 页。");
        return PM3_EINVARG;
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (card->pages_reason[i] >= 0) {
            ...
            if (page_addr == HITAGS_UID_PADR) {
                ...
                PrintAndLogEx(NORMAL, "配置");
            } else if (page_addr == 2 && card->config_page.s.auth) {
                PrintAndLogEx(NORMAL, "密码/密钥");
            } else if (page_addr == 3 && card->config_page.s.auth) {
                PrintAndLogEx(NORMAL, "密钥");
            } else
                PrintAndLogEx(NORMAL, "数据");
        } else {
...
static int CmdLFHitagSDump(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Hitag HTS 转储",
                  "读取所有Hitag S内存并保存到文件\\n"
                  " Crypto mode: \n"
                  "    - key format ISK high + ISK low\n"
                  "    - default key 4F4E4D494B52 (ONMIKR)\n\n"
    ...
    void *argtable[] = {
        ...
        arg_lit0(NULL, "crypto", "crypto mode"),
        arg_str0("k", "key", "<hex>", "密码或密钥，4或6个十六进制字节"),
        arg_int0("m", "mode", "<dec>", "响应协议模式：0（标准00110），1（高级11000），2（高级11001），3（快速高级11010）（默认：3）"),
...
static int CmdLFHitagSRestore(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag hts 恢复",
                  "将转储文件恢复到Hitag S标签\\n"
                  " Crypto mode: \n"
                  "    - key format ISK high + ISK low\n"
                  "    - default key 4F4E4D494B52 (ONMIKR)\n\n"
    ...
    if (bytes_read != mem_size) {
        free(dump);
        PrintAndLogEx(FAILED, "错误的转储文件长度。期望%d字节，实际为%zu", mem_size, bytes_read);
        return PM3_EFILE;
    }
    ...
    for (int page = packet.page_count + 1; page < hts_mem_sizes[tag_config.MEMT]; page++) { // skip config page
        ...
        if (resp.status != PM3_SUCCESS) {
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(FAILED, "页面 %d 写入失败", page);
            print_error(resp.reason);
            free(dump);
            return PM3_ESOFT;
        }
        ...
        switch (page) {
            case 2: // auth first page
                if (packet.cmd == HTSF_82xx) {
                    ...
                    PrintAndLogEx(SUCCESS, "后续写入使用新密码");
                }
                break;
            case 3:  // crypto mode
                if (packet.cmd == HTSF_KEY) {
                    ...
                    PrintAndLogEx(SUCCESS, "后续写入使用新密钥");
                }
                break;
        }
    }
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(NORMAL, "");
        PrintAndLogEx(FAILED, "应用配置失败");
        print_error(resp.reason);
        free(dump);
        return PM3_ESOFT;
    }

    PrintAndLogEx(INFO, "写入过程完成");
...
static int CmdLFHitagSWrite(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag hts 写块",
                  "写入Hitag S存储器中的一个页。\\n"
                  "  Crypto mode: \n"
                  "    - key format ISK high + ISK low\n"
                  "    - default key 4F4E4D494B52 (ONMIKR)\n\n"
...
static int CmdLFHitagSReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag hts 读取器",
...
static int CmdLFHitagSSim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "lf hitag hts 模拟",
                  "模拟Hitag S应答器\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("8", "82xx", "模拟 8268/8310"),
        arg_int0("t", "threshold", "<dec>", "设置边沿检测阈值（默认：127）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static command_t CommandTable[] = {
    ...
    {"list",        CmdLFHitagSList,   AlwaysAvailable, "列出Hitag S跟踪历史"},
    {"-----------", CmdHelp,           IfPm3Hitag,      "----------------------- " _CYAN_("常规") " ------------------------"},
    {"reader",      CmdLFHitagSReader, IfPm3Hitag,      "模拟 Hitag S 读取器"},
    {"rdbl",        CmdLFHitagSRead,   IfPm3Hitag,      "读取Hitag S页面"},
    {"dump",        CmdLFHitagSDump,   IfPm3Hitag,      "将Hitag S页面转储到文件"},
    {"restore",     CmdLFHitagSRestore, IfPm3Hitag,      "从转储文件恢复Hitag S内存"},
    {"wrbl",        CmdLFHitagSWrite,  IfPm3Hitag,      "写入Hitag S页"},
    {"-----------", CmdHelp,           IfPm3Hitag,      "----------------------- " _CYAN_("Simulation") " -----------------------"},
    {"sim",         CmdLFHitagSSim,    IfPm3Hitag,      "模拟Hitag S应答器"},
    {NULL,          NULL,              0,               NULL}
};
