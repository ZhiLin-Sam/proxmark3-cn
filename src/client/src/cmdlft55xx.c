...
static void arg_add_t55xx_downloadlink(void *at[], uint8_t *idx, uint8_t show, uint8_t dl_mode_def) {
    ...
    snprintf(r0, r_len, "下行 - 固定位长度 %s", (dl_mode_def == 0) ? "(detected def)" : "");
    snprintf(r1, r_len, "下行 - 长前导参考 %s", (dl_mode_def == 1) ? "(detected def)" : "");
    snprintf(r2, r_len, "下行 - 前导零 %s", (dl_mode_def == 2) ? "(detected def)" : "");
    snprintf(r3, r_len, "下行 - 4种编码参考之一 %s", (dl_mode_def == 3) ? "(detected def)" : "");
    ...
    if (show == T55XX_DLMODE_ALL) {
        ...
        snprintf(r4, r_len, "尝试所有下行模式 %s", (dl_mode_def == 4) ? "(def)" : "");
...
static int CmdT55xxCloneHelp(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频T55xx克隆帮助",
                  "显示用于克隆T5xx标签特定技术的可用命令列表",
                  "低频T55xx克隆帮助"
                 );
    void *argtable[] = {
        arg_param_begin,
    ...
    CLIParserFree(ctx);
    PrintAndLogEx(NORMAL, "克隆T55xx标签特定技术，请参见相应低频子菜单中的命令，例如：");
    PrintAndLogEx(NORMAL, _GREEN_("低频AWID克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Destron克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频EM 410x克隆"));
// todo:  implement restore
//    PrintAndLogEx(NORMAL, _GREEN_("lf em 4x05 write"));
//    PrintAndLogEx(NORMAL, _GREEN_("lf em 4x50 restore"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 FDXB 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Gallagher 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 GProxII 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 HID 克隆"));
// todo:  implement restore
//    PrintAndLogEx(NORMAL, _GREEN_("lf hitag clone"));
    PrintAndLogEx(NORMAL, _GREEN_("lf idteck 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("lf indala 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 IO 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Jablotron 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Keri 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Motorola 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Nedap 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Nexwatch 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频 Noralsy 克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频PAC克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Paradox克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Presco克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Pyramid克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Securakey克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Viking克隆"));
    PrintAndLogEx(NORMAL, _GREEN_("低频Visa2000克隆"));
    return PM3_SUCCESS;
}
...
int clone_t55xx_tag(uint32_t *blockdata, uint8_t numblocks) {
    ...
    if (res == 0)
        PrintAndLogEx(SUCCESS, "数据已写入并验证");
...
static bool t55xxProtect(bool lock, bool usepwd, uint8_t override, uint32_t password, uint8_t downlink_mode, uint32_t new_password) {
    ...
    if (GetT55xxBlockData(&block0) == false) {
        PrintAndLogEx(DEBUG, "错误：解码的block0 == %08x", block0);
        return false;
    }
    PrintAndLogEx(DEBUG, "成功读取块0 == %08x", block0);


    bool isPwdBitAlreadySet = (block0 >> (32 - 28) & 1);
    if (isPwdBitAlreadySet) {
        PrintAndLogEx(INFO, "PWD 位已设置");
        usepwd = true;
    }
    ...
    if (t55xxWrite(T55x7_PWD_BLOCK, T55x7_PAGE0, usepwd, testmode, password, downlink_mode, new_password) != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入新密码失败");
        return false;
    } else {
        PrintAndLogEx(SUCCESS, "已写入新密码");
    }
    ...
    if (t55xxVerifyWrite(T55x7_PWD_BLOCK, T55x7_PAGE0, usepwd, override, curr_password, downlink_mode, new_password) == false) {
        PrintAndLogEx(WARNING, "验证密码写入失败。正在中止。");
        return false;
    } else {
        PrintAndLogEx(SUCCESS, "已验证新密码");
    }

    // write config
    if (t55xxWrite(T55x7_CONFIGURATION_BLOCK, T55x7_PAGE0, usepwd, testmode, curr_password, downlink_mode, block0) != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入修改后的配置块 %08X 失败", block0);
        return false;
    } else {
        PrintAndLogEx(SUCCESS, "已写入修改后的配置块");
    }
...
bool t55xxAcquireAndCompareBlock0(bool usepwd, uint32_t password, uint32_t known_block0, bool verbose) {
    ...
    if (verbose)
        PrintAndLogEx(INFO, "检测到块0写入，运行 `detect` 以查看是否可能验证");
...
int t55xxWrite(uint8_t block, bool page1, bool usepwd, bool testMode, uint32_t password, uint8_t downlink_mode, uint32_t data) {
    ...
    */
    ...
    if (WaitForResponseTimeout(CMD_LF_T55XX_WRITEBL, &resp, 2000) == false) {
        PrintAndLogEx(ERR, "发生错误，设备未确认写入操作。");
        return PM3_ETIMEOUT;
    }
    return resp.status;
}
...
static int CmdT55xxSetConfig(const char *Cmd) {
    ...
    void *argtable[1 + 12 + 6 + 5] = {
        ...
        arg_str0("c", "blk0", "<hex>", "从block0设置配置（4个十六进制字节）"),
    ...
    if (res == 0 || res == 2) {
        PrintAndLogEx(ERR, "块0数据必须是4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    if (res == 1) {
    ...
    if ((r0 + r1 + r2 + r3) > 1) {
        PrintAndLogEx(FAILED, "错误 多个下行链路编码");
        return PM3_EINVARG;
    }

    // validate user specified modulation FSK,FSK1,...BIA
    if (verify_mods > 1) {
        PrintAndLogEx(FAILED, "错误 多个解调结果，请选择一个");
        return PM3_EINVARG;
    }
    ...
    if (bitrate != -1) {
        ...
        if (i == 9) {
            PrintAndLogEx(FAILED, "错误：请选择有效的比特率");
            return PM3_EINVARG;
        }
    }
    ...
    if (usepwd) {
        ...
        if (override == 0) {
            ...
            if (t55xxTryDetectModulationEx(downlink_mode, false, 0, password) == false) {
                PrintAndLogEx(WARNING, "安全检查: 无法检测配置块中是否设置了PWD位。退出。");
                PrintAndLogEx(HINT, "提示: 考虑使用覆盖参数强制读取。");
                return PM3_EWRONGANSWER;
            } else {
                PrintAndLogEx(WARNING, "安全检查: 配置块中未设置PWD位。无需密码读取...");
                usepwd = false;
                page1 = false; // ??
            }
        } else if (override == 1) {
            PrintAndLogEx(INFO, "安全检查已覆盖 - 尽管有风险仍继续");
        }
    }
...
static int CmdT55xxReadBlock(const char *Cmd) {
    ...
    void *argtable[5 + 5] = {
        arg_param_begin,
        arg_int1("b", "blk", "<0-7>", "读取的块号"),
        arg_str0("p", "pwd", "<hex>", "密码(4个十六进制字节)"),
        arg_lit0("o", "override", "覆盖安全检查"),
    ...
    if (res == 0 || res == 2) {
        PrintAndLogEx(ERR, "密码应为4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    if (res == 1) {
    ...
    if (block > 7 && block != REGULAR_READ_MODE_BLOCK) {
        PrintAndLogEx(NORMAL, "块必须在 0 到 7 之间");
        return PM3_ESOFT;
    }
...
static int SanityOfflineCheck(bool useGraphBuffer) {
    if (!useGraphBuffer && !g_session.pm3_present) {
        PrintAndLogEx(WARNING, "您的proxmark3设备离线。请指定[1]以改用graphbuffer数据");
        return PM3_ENODATA;
    }
    return PM3_SUCCESS;
}
...
static int CmdT55xxWakeUp(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频T55xx唤醒",
                  "此命令发送应答请求命令，并随后保持读卡器场开启",
    ...
    if (verbose)
        PrintAndLogEx(SUCCESS, "已发送唤醒命令。请尝试读取");
...
bool t55xxTryDetectModulationEx(uint8_t downlink_mode, bool print_config, uint32_t wanted_conf, uint64_t pwd) {
    ...
    if (hits > 1) {
        ...
        for (int i = 0; i < hits; ++i) {
            ...
            if (retval || wanted) {
                PrintAndLogEx(NORMAL, "--[%d]--------------- << 选中此项", i + 1);
...
bool GetT55xxBlockData(uint32_t *blockdata) {
    ...
    if (idx + 32 > g_DemodBufferLen) {
        PrintAndLogEx(WARNING, "配置的偏移量 %d 过大。可能的偏移量：%zu)", idx, g_DemodBufferLen - 32);
        return false;
    }
...
static int CmdT55xxWriteBlock(const char *Cmd) {
    ...
    void *argtable[7 + 5] = {
        arg_param_begin,
        arg_int1("b", "blk", "<0-7>", "写入的块号"),
        arg_str0("d", "数据", "<hex>", "要写入的数据（4个十六进制字节）"),
        arg_str0("p", "pwd", "<hex>", "密码(4个十六进制字节)"),
    ...
    if (res == 0 || res == 2) {
        PrintAndLogEx(ERR, "数据必须是 4 个十六进制字节");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    ...
    if (validate) {
        ...
        if (isOK)
            PrintAndLogEx(SUCCESS, "写入成功，验证通过");
        else
            PrintAndLogEx(WARNING, "无法验证写入的数据");
    }
...
static int CmdT55xxDangerousRaw(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频T55xx危险原始操作",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<bitstr>", "原始比特串"),
        arg_int1("t", "时间", "<us>", "<0 - 200000> 场关闭前的微秒数"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (ng.time == 0 || ng.time > 200000) {
        PrintAndLogEx(ERR, "定时超出1..200000范围，得到%i", ng.time);
        return PM3_EINVARG;
    }
...
static int CmdT55xxReadTrace(const char *Cmd) {
    ...
    if (config.Q5) {
        ...
        if (hdr != 0x1FF) {
            PrintAndLogEx(FAILED, "无效的 Q5/T5555 追踪数据头（期望 0x1FF，发现 %X）", hdr);
            return PM3_ESOFT;
        }
...
static int CmdT55xxInfo(const char *Cmd) {
    ...
    void *argtable[5 + 5] = {
        ...
        arg_lit0("1", NULL, "extract using data from graphbuffer"),
        arg_str0("p", "pwd", "<hex>", "密码(4个十六进制字节)"),
        arg_str0("c", "blk0", "<hex>", "改用这些数据（4个十六进制字节）"),
    ...
    int res = arg_get_u32_hexstr_def_nlen(ctx, 2, 0, &password, 4, true);
    if (res == 0 || res == 2) {
        PrintAndLogEx(ERR, "密码必须是4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
    if (res == 1) {
    ...
    if (gotdata && use_gb) {
        PrintAndLogEx(FAILED, "必须选择用户提供的数据之一并使用 graphbuffer");
        return PM3_EINVARG;
    }

    if (dataasq5 && gotdata == false) {
        PrintAndLogEx(FAILED, "必须指定用户提供的Q5数据");
        return PM3_EINVARG;
    }
...
static int CmdT55xxDump(const char *Cmd) {
    ...
    void *argtable[5 + 5] = {
        arg_param_begin,
        arg_str0("f", "file", "<fn>", "文件名（默认在块0生成）"),
        arg_lit0("o", "override", "覆盖，强制读取密码，尽管可能损坏卡片"),
        arg_str0("p", "pwd", "<hex>", "密码(4个十六进制字节)"),
...
static int CmdT55xxRestore(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频T55xx恢复",
                  "从（bin/eml/json）转储文件恢复T55xx卡页面0/1的n个块",
    ...
    void *argtable[] = {
        ...
        arg_str0("p", "pwd", "<hex>", "如果目标卡设置了密码，则输入密码(4个十六进制字节)"),
        arg_param_end
    };
    ...
    for (idx = 1; idx <= 7; idx++) {
        ...
        if (CmdT55xxWriteBlock(wcmd) != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "警告：写入块%d时出错", idx);
        }
    }
    ...
    if (dump[0] != 0x00000000) {
        ...
        if (CmdT55xxWriteBlock(wcmd) != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "警告：写入块0时出错");
        }
    } else {
        PrintAndLogEx(WARNING, "警告：转储文件包含全零配置块。");
        PrintAndLogEx(HINT, "请确保您正确转储了卡片");
    }
    free(dump);
    ...
    switch (id) {
        ...
        case 3:
            if (q5)
                ...
                snprintf(retStr, sizeof(buf), "%u - " _RED_("(未知)"), id);
            break;
        default:
            snprintf(retStr, sizeof(buf), "%u - " _RED_("(未知)"), id);
            break;
...
char *GetModulationStr(uint32_t id, bool xmode) {
    ...
    switch (id) {
        ...
        case 24:
            ...
            break;
        default:
            snprintf(retStr, sizeof(buf), "0x%02X " _RED_("(未知)"), id);
            break;
    }
    return buf;
}
...
char *GetDownlinkModeStr(uint8_t downlink_mode) {
    ...
    switch (downlink_mode) {
        ...
        case T55XX_DLMODE_1OF4 :
            ...
            break;
        default:
            snprintf(retStr, sizeof(buf), _RED_("(未知)"));
            break;
    }
    return buf;
}
...
static int CmdT55xxWipe(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频T55xx擦除",
                  "此命令擦除标签，用零填充块1-7以及一个默认配置块",
    ...
    void *argtable[4 + 5] = {
        arg_param_begin,
        arg_str0("c", "cfg", "<hex>", "配置块0（4个十六进制字节）"),
        arg_str0("p", "pwd", "<hex>", "密码(4个十六进制字节)"),
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(WARNING, "配置块需要是4个十六进制字节");
        return PM3_EINVARG;
    }
    ...
    if (res == 2) {
        PrintAndLogEx(WARNING, "密码应为4字节，使用默认密码");
    }
    ...
    for (uint8_t blk = 1; blk < 8; blk++) {
        ...
        if (CmdT55xxWriteBlock(pwcmd) != PM3_SUCCESS)
            PrintAndLogEx(WARNING, "警告：写入块%d时出错", blk);
    ...
    if (config.downlink_mode != T55XX_DLMODE_FIXED) { // Detect found a different mode so card must support
        ...
        if (CmdT55xxWriteBlock(pwcmd) != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "警告：写入块3页1（配置）失败");
        }
        memset(wcmd, 0x00, sizeof(wcmd));
    }
    return PM3_SUCCESS;
...
static int CmdT55xxChkPwds(const char *Cmd) {
    ...
    */
    ...
    void *argtable[4 + 6] = {
        arg_param_begin,
        arg_lit0("m", "fm", "使用来自闪存的字典（RDV4）"),
        arg_str0("f", "file", "<fn>", "文件名"),
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(WARNING, "EM4100 ID 必须是 5 个十六进制字节");
        return PM3_EINVARG;
    }
    if (res == 0) {
        CLIParserFree(ctx);
    ...
    */
    ...
    if (use_calc_password) {
        ...
        PrintAndLogEx(INFO, "测试 %08"PRIX32" generated ", card_password);
    ...
    if ((found == false) && use_pwd_file) {
        ...
        if (res != PM3_SUCCESS || keycount == 0 || keyblock == NULL) {
            PrintAndLogEx(WARNING, "文件中未找到密钥");
            if (keyblock != NULL)
                free(keyblock);
        ...
        for (uint32_t c = 0; c < keycount && found == false; ++c) {
            ...
            PrintAndLogEx(INFO, "测试 %08"PRIX32, curr_password);
            for (dl_mode = downlink_mode; dl_mode <= 3; dl_mode++) {
...
static int CmdT55xxBruteForce(const char *Cmd) {
    ...
    void *argtable[3 + 6] = {
        arg_param_begin,
        arg_str1("s", "开始", "<hex>", "搜索起始密码（4个十六进制字节）"),
        arg_str1("e", "end", "<hex>", "搜索结束密码（4个十六进制字节）"),
    };
    uint8_t idx = 3;
    arg_add_t55xx_downloadlink(argtable, &idx, T55XX_DLMODE_ALL, T55XX_DLMODE_ALL);
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(FAILED, "起始密码应为4字节");
        return PM3_EINVARG;
    }
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(FAILED, "结束密码应为4字节");
        return PM3_EINVARG;
    }
...
//    else
    ...
    if (start_password > end_password) {
        PrintAndLogEx(FAILED, "错误，起始密码大于结束密码");
        return PM3_EINVARG;
    }

    PrintAndLogEx(INFO, "Press " _GREEN_("<Enter>") " to exit");
    PrintAndLogEx(INFO, "搜索密码范围 [%08X -> %08X]", start_password, end_password);
...
uint8_t t55xx_try_one_password(uint32_t password, uint8_t downlink_mode,  bool try_all_dl_modes) {
    ...
    PrintAndLogEx(INFO, "正在尝试密码 %08X", password);
...
static int CmdT55xxRecoverPW(const char *Cmd) {
    ...
    if (res == 2) {
        PrintAndLogEx(INFO, "密码应为4字节，改用默认密码");
    }
...
static int CmdT55xxDetectPage1(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频T55xx页面1检测",
                  "检测T55xx芯片的第1页",
                  "lf t55xx p1detect\n"
                  "lf t55xx p1detect -1\n"
    ...
    if (res == 2) {
        PrintAndLogEx(INFO, "密码应为4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    } else if (res == 1) {
        usepwd = true;
    ...
    if (found) {
        PrintAndLogEx(SUCCESS, "发现T55xx芯片!");
...
static int CmdT55xxSetDeviceConfig(const char *Cmd) {
    ...
    void *argtable[10 + 5] = {
        ...
        arg_lit0("p", "persist", "持久化到闪存(RDV4)"),
...
static int CmdT55xxProtect(const char *Cmd) {
    ...
    if (res == 2) {
        CLIParserFree(ctx);
        PrintAndLogEx(FAILED, "解析新密码字节错误");
        return PM3_EINVARG;
    } else if (res == 0) {
        PrintAndLogEx(FAILED, "必须指定新密码参数");
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
...
static int CmdT55xxSniff(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("1", NULL, "extract using data from graphbuffer"),
        arg_int0("t", "tol", "<dec>", "设置容差级别（默认5）"),
//        arg_int0(NULL, "signal", "<dec>", "set minimum signal level (default 20)"),
        arg_int0("o", "one", "<dec>", "设置一个脉冲的采样宽度（默认自动）"),
        arg_int0("z", "zero", "<dec>", "设置零脉冲的采样宽度（默认自动）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    bool use_graphbuf = arg_get_lit(ctx, 1);
    ...
    if (opt_width0 == 0) {
        PrintAndLogEx(ERR, "必须使用大于 0 的 --zero 调用");
        return PM3_EINVARG;
    }
    if (opt_width1 == 0) {
        PrintAndLogEx(ERR, "必须使用大于 0 的 --one 调用");
        return PM3_EINVARG;
    }
...
static int CmdT55xxView(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "----+----------+-------");
    PrintAndLogEx(SUCCESS, "块 | 十六进制数据 | ASCII");
    PrintAndLogEx(SUCCESS, "----+----------+-------");
...
static command_t CommandTable[] = {
    ...
    {"",             CmdHelp,                 AlwaysAvailable, "记得运行 `" _YELLOW_("低频T55xx检测") "` first whenever a new card"},
    {"",             CmdHelp,                 AlwaysAvailable, "已放置在Proxmark3上或配置块已更改。"},
    {"",             CmdHelp,                 AlwaysAvailable, ""},
    {"help",         CmdHelp,                 AlwaysAvailable, "此帮助"},
    {"-----------",  CmdHelp,                 AlwaysAvailable, "--------------------- " _CYAN_("operations") " ---------------------"},
    {"clonehelp",    CmdT55xxCloneHelp,       IfPm3Lf,         "显示可用的克隆命令"},
    {"config",       CmdT55xxSetConfig,       AlwaysAvailable, "设置/获取T55XX配置（调制、反转、偏移、速率）"},
    {"dangerraw",    CmdT55xxDangerousRaw,    IfPm3Lf,         "发送原始比特流。危险，请勿使用！！"},
    {"detect",       CmdT55xxDetect,          AlwaysAvailable, "尝试通过读取配置块检测标签调制方式"},
    {"deviceconfig", CmdT55xxSetDeviceConfig, IfPm3Lf,         "设置/获取T55XX设备配置"},
    {"dump",         CmdT55xxDump,            IfPm3Lf,         "转储T55xx卡第0页块0-7"},
    {"info",         CmdT55xxInfo,            AlwaysAvailable, "显示T55x7配置数据（第0页/块0）"},
    {"p1detect",     CmdT55xxDetectPage1,     IfPm3Lf,         "尝试通过读取第1页检测是否为 t55xx 标签"},
    {"read",         CmdT55xxReadBlock,       IfPm3Lf,         "读取T55xx块数据"},
    {"resetread",    CmdResetRead,            IfPm3Lf,         "发送复位命令，然后低频读取流以尝试识别其起始位置"},
    {"restore",      CmdT55xxRestore,         IfPm3Lf,         "恢复T55xx卡第0页/第1页块"},
    {"trace",        CmdT55xxReadTrace,       AlwaysAvailable, "显示T55x7可追溯性数据（第1页/块0-1）"},
    {"wakeup",       CmdT55xxWakeUp,          IfPm3Lf,         "发送AOR唤醒命令"},
    {"view",         CmdT55xxView,            AlwaysAvailable, "显示标签转储文件的内容"},
    {"write",        CmdT55xxWriteBlock,      IfPm3Lf,         "写入T55xx块数据"},
    {"-----------",  CmdHelp,                 AlwaysAvailable, "--------------------- " _CYAN_("恢复") " ---------------------"},
    {"bruteforce",   CmdT55xxBruteForce,      IfPm3Lf,         "简单暴力破解攻击以查找密码"},
    {"chk",          CmdT55xxChkPwds,         IfPm3Lf,         "检查密码"},
    {"protect",      CmdT55xxProtect,         IfPm3Lf,         "密码保护标签"},
    {"recoverpw",    CmdT55xxRecoverPW,       IfPm3Lf,         "尝试从克隆器的错误密码写入中恢复"},
    {"sniff",        CmdT55xxSniff,           AlwaysAvailable, "尝试从样本缓冲区恢复 T55xx 命令"},
    {"special",      CmdT55xxSpecial,         IfPm3Lf,         "显示64种不同偏移量的块变化"},
    {"wipe",         CmdT55xxWipe,            IfPm3Lf,         "擦除T55xx标签并恢复默认设置（将销毁标签上的所有数据）"},
    {NULL, NULL, NULL, NULL}
};
