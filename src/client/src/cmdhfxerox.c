...
static int read_xerox_block(iso14b_card_select_t *card, uint8_t blockno, uint8_t *out) {
    ...
    for (int retry = 0; retry < 2; retry++) {
        ...
        if (resp.status != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "再试一次");
            continue;
        }

        // sanity checks
        if (resp.length < 7) {
            PrintAndLogEx(FAILED, "再试一次，长度错误");
            continue;
        }
        ...
        if (d[0] != 0x02) {
            PrintAndLogEx(FAILED, "标签返回错误%x %x", d[0], d[1]);
            res = PM3_ERFTRANS;
            break;
        }
...
static int CmdHFXeroxReader(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf xerox 读取器",
...
static int CmdHFXeroxInfo(const char *Cmd) {
    ...
    if (status != PM3_SUCCESS) {
        if (verbose) {
            PrintAndLogEx(FAILED, "Fuji/Xerox 标签选择失败");
        }
        return status;
    }
    ...
    for (int i = 0; i < ARRAYLEN(info_blocks); i++) {
        ...
        if (status != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "Fuji/Xerox 标签读取失败");
            break;
        }
    }
...
static int CmdHFXeroxDump(const char *Cmd) {
    ...
    for (; blockno < 0x100; blockno++) {
        ...
        PrintAndLogEx(INPLACE, "块 %3d", blockno);
    }
    ...
    if (decrypt) {
        ...
        for (int n = 0; n < sizeof(var_list); n++) {
            ...
            if (dadr + 1 >= blockno) {
                PrintAndLogEx(INFO, "秘密块%02X已跳过。", dadr);
                continue;
            }
...
static int CmdHFXeroxView(const char *Cmd) {
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "文件大小 %zu 字节，文件块 %d (0x%x)", bytes_read, blockno, blockno);
    }
...
static int CmdHFXeroxRdBl(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf xerox 读块",
                  "读取Fuji/Xerox标签块\\n",
                  "hf xerox rdbl -b 1"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_int1("b", "blk", "<dec>", "页号(0-255)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static command_t CommandTable[] = {
    ...
    {"list",      CmdHFXeroxList,    AlwaysAvailable, "列出 ISO-14443B 历史"},
    {"--------",  CmdHelp,           AlwaysAvailable, "----------------------- " _CYAN_("操作") " -----------------------"},
    {"info",      CmdHFXeroxInfo,    IfPm3Iso14443b,  "标签信息"},
    {"dump",      CmdHFXeroxDump,    IfPm3Iso14443b,  "读取Fuji/Xerox标签的所有内存页面，保存到文件"},
    {"reader",    CmdHFXeroxReader,  IfPm3Iso14443b,  "模拟 Fuji/Xerox 读取器"},
    {"view",      CmdHFXeroxView,    AlwaysAvailable, "显示标签转储文件的内容"},
    {"rdbl",      CmdHFXeroxRdBl,    IfPm3Iso14443b,  "读取Fuji/Xerox块"},
