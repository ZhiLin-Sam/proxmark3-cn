...
int CmdHFSearch(const char *Cmd) {
    ...
    */
    ...
    if (success[THINFILM]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf thinfilm") "` commands\n");
    }

    if (success[LTO]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf lto") "` commands\n");
    }

    if (success[LEGIC]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf legic") "` commands\n");
    }

    if (success[TOPAZ]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf topaz") "` commands\n");
    }

    if (success[PROTO_TEXKOM]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf texkom") "` commands\n");
    }

    if (success[PROTO_XEROX]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf xerox") "` commands\n");
    }

    if (success[ISO_14443B]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 14b") "` commands\n");
    }

    if (success[ISO_15693]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 15") "` commands\n");
    }

    if (success[ICLASS]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf iclass") "` commands\n");
    }

    if (success[FELICA]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf felica") "` commands\n");
    }

    if (success[PROTO_CRYPTORF]) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf cryptorf") "` commands\n");
    }
...
int CmdHFTune(const char *Cmd) {
    ...
    if (WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_HF, &resp, 1000) == false) {
        PrintAndLogEx(WARNING, "等待Proxmark HF初始化超时，正在中止");
        return PM3_ETIMEOUT;
    }
    ...
    for (uint32_t i = 0; iter == 0 || i < iter; i++) {
        ...
        if (WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_HF, &resp, 1000) == false) {
            ...
            PrintAndLogEx(WARNING, "等待Proxmark HF测量超时，正在中止");
            break;
        }
    ...
    SendCommandNG(CMD_MEASURE_ANTENNA_TUNING_HF, mode, sizeof(mode));
    if (WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_HF, &resp, 1000) == false) {
        PrintAndLogEx(WARNING, "等待Proxmark HF关闭超时，正在中止");
        return PM3_ETIMEOUT;
    }
...
int CmdHFSniff(const char *Cmd) {
    ...
    if (params.skipMode != HF_SNOOP_SKIP_NONE) {
        PrintAndLogEx(INFO, "跳过模式。功能: %s, 每 %d 个样本",
    ...
    for (;;) {
        ...
        if (WaitForResponseTimeout(CMD_HF_SNIFF, &resp, 1000)) {
            ...
            if (resp.status == PM3_SUCCESS) {
                ...
                PrintAndLogEx(INFO, "HF嗅探 (%u 样本)", retval->len);

                PrintAndLogEx(HINT, "提示：使用 `" _YELLOW_("数据 高通滤波") "` to remove offset");
                PrintAndLogEx(HINT, "提示：使用 `" _YELLOW_("数据 绘图") "` to view");
                PrintAndLogEx(HINT, "提示：使用 `" _YELLOW_("数据保存") "` to save");
                ...
                if (res != PM3_SUCCESS) {
                    PrintAndLogEx(WARNING, "下载样本到客户端失败");
                    return res;
                }
                break;
            }
...
static command_t CommandTable[] = {
    ...
    {"aliro",       CmdHFAliro,       AlwaysAvailable, "{ ALIRO 数字访问凭证... }"},
//    {"cryptorf",    CmdHFCryptoRF,    AlwaysAvailable, "{ CryptoRF RFIDs...                   }"},
    ...
    {"emrtd",       CmdHFeMRTD,       AlwaysAvailable, "{ 机读旅行证件... }"},
    ...
    {"ksx6924",     CmdHFKSX6924,     AlwaysAvailable, "{ KS X 6924 (T-Money, Snapper+) 射频识别 }"},
    ...
    {"list",        CmdHFList,        AlwaysAvailable, "列出跟踪缓冲区中的协议数据"},
    {"plot",        CmdHFPlot,        IfPm3Hfplot,     "绘制信号"},
    {"tune",        CmdHFTune,        IfPm3Present,    "连续测量高频天线调谐"},
    {"search",      CmdHFSearch,      AlwaysAvailable, "搜索已知高频标签"},
    {"sniff",       CmdHFSniff,       IfPm3Hfsniff,    "通用高频嗅探"},
    {NULL, NULL, NULL, NULL}
};
