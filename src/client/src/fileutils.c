...
int saveFileEx(const char *preferredName, const char *suffix, const void *data, size_t datalen, savePaths_t e_save_path) {
    ...
    if (f == NULL) {
        PrintAndLogEx(WARNING, "文件未找到或已锁定 `" _YELLOW_("%s") "`", fileName);
        free(fileName);
        return PM3_EFILE;
    }
...
int saveFileJSONrootEx(const char *preferredName, const void *root, size_t flags, bool verbose, bool overwrite, savePaths_t e_save_path) {
    ...
    if (res == 0) {
        ...
        PrintAndLogEx(FAILED, "错误，无法保存文件`" _YELLOW_("%s") "`", filename);
    }
    free(filename);
    return PM3_EFILE;
}
...
int createMfcKeyDump(const char *preferredName, uint8_t sectorsCnt, const sector_t *e_sector) {
    ...
    FILE *f = fopen(fileName, "wb");
    if (f == NULL) {
        PrintAndLogEx(WARNING, "无法创建文件`" _YELLOW_("%s") "`", fileName);
        free(fileName);
        return PM3_EFILE;
    }
    PrintAndLogEx(SUCCESS, "正在生成二进制密钥文件");
    ...
    fflush(f);
    fclose(f);
    PrintAndLogEx(SUCCESS, "已找到的密钥已转储到 `" _YELLOW_("%s") "`", fileName);
    PrintAndLogEx(INFO, "--[ " _YELLOW_("FFFFFFFFFFFF") " ]-- 已将未知密钥填入 where " _YELLOW_("res") " is " _RED_("0"));
    free(fileName);
    return PM3_SUCCESS;
}
...
int loadFile_safeEx(const char *preferredName, const char *suffix, void **pdata, size_t *datalen, bool verbose) {
    ...
    if (f == NULL) {
        PrintAndLogEx(WARNING, "文件未找到或已锁定 `" _YELLOW_("%s") "`", path);
        free(path);
        return PM3_EFILE;
    }
    free(path);
    ...
    if (fsize <= 0) {
        PrintAndLogEx(FAILED, "错误，获取文件大小时出错");
        fclose(f);
        return PM3_EFILE;
    }
    ...
    *pdata = calloc(fsize, sizeof(uint8_t));
    ...
    if (bytes_read != fsize) {
        PrintAndLogEx(FAILED, "错误，读取字节数与文件大小不匹配");
        free(*pdata);
        return PM3_EFILE;
    }
...
static int load_file_sanity(char *s, uint32_t datalen, int i, size_t len) {
    if (len == 0) {
        PrintAndLogEx(DEBUG, "警告：json %s 块 %d 数据长度为零", s, i);
        PrintAndLogEx(DEBUG, "文件解析已停止");
        return false;
    } else if (len != datalen) {
        PrintAndLogEx(WARNING, "警告：json %s 块 %d 仅有 %zu 字节", s, i, len);
        PrintAndLogEx(INFO, "预期 %d - 用零填充", datalen);
    }
    return true;
}
...
int loadFileJSONex(const char *preferredName, void *data, size_t maxdatalen, size_t *datalen, bool verbose, void (*callback)(json_t *)) {
    ...
    *datalen = 0;
    ...
    if (verbose) {
        PrintAndLogEx(SUCCESS, "已加载 `" _YELLOW_("%s") "`", path);
    }
    ...
    if (!strcmp(ctype, "15693")) {
        PrintAndLogEx(WARNING, "loadFileJSONex: 正在加载已弃用的 15693 格式");
    ...
    if (!strcmp(ctype, "15693 v2")) {
        PrintAndLogEx(WARNING, "loadFileJSONex: 正在加载已弃用的 15693 v2 格式");
        // will set every metadata to 0 except 1st UID byte to E0 and memory layout
        iso15_tag_t *tag = (iso15_tag_t *)udata.bytes;
        tag->uid[7] = 0xE0;
        tag->bytesPerPage = 4;
    ...
    if (!strcmp(ctype, "15693 v3")) {
        PrintAndLogEx(WARNING, "loadFileJSONex: 正在加载已弃用的 15693 v3 格式");
        // will set every metadata to 0 except 1st UID byte to E0 and memory layout
        iso15_tag_t *tag = (iso15_tag_t *)udata.bytes;
        tag->uid[7] = 0xE0;
        tag->bytesPerPage = 8;
...
int loadFileDICTIONARY_safe_ex(const char *preferredName, const char *suffix, void **pdata, uint8_t keylen, uint32_t *keycnt, bool verbose) {
    ...
    *pdata = calloc(block_size, sizeof(uint8_t));
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strlen(line) > keylen) {
            PrintAndLogEx(INFO, "行太长 (%zu) ... %s", strlen(line), line);
            continue;
        }
...
int loadFileBinaryKey(const char *preferredName, const char *suffix, void **keya, void **keyb, size_t *alen, size_t *blen, bool verbose) {
    ...
    *blen = fread(*keyb, 1, fsize, f);
    ...
    if (verbose) {
        PrintAndLogEx(SUCCESS, "已加载二进制密钥文件 `" _YELLOW_("%s") "`", path);
    }
    free(path);
    return PM3_SUCCESS;
}
    ...
    if (verbose) {
        switch (retval) {
            ...
            case MFU_DF_PLAINBIN:
                ...
                break;
            case MFU_DF_UNKNOWN:
                PrintAndLogEx(WARNING, "检测mfu转储格式失败");
                break;
        }
    }
    return retval;
...
int detect_nfc_dump_format(const char *preferredName, nfc_df_e *dump_type, bool verbose) {
    ...
    *dump_type = NFC_DF_UNKNOWN;
    ...
    if (verbose) {
        ...
        switch (*dump_type) {
            case NFC_DF_MFU:
                PrintAndLogEx(INFO, "检测到基于 MIFARE Ultralight / NTAG 的转储格式");
                break;
            case NFC_DF_MFC:
                PrintAndLogEx(INFO, "检测到基于 MIFARE Classic 的转储格式");
                break;
            case NFC_DF_MFDES:
                PrintAndLogEx(INFO, "检测到基于 MIFARE DESFire 的转储格式");
                break;
            case NFC_DF_14_3A:
                PrintAndLogEx(INFO, "检测到基于 ISO14443-3A 的转储格式。无可用数据");
                break;
            case NFC_DF_14_3B:
                PrintAndLogEx(INFO, "检测到基于 ISO14443-3B 的转储格式。无可用数据");
                break;
            case NFC_DF_14_4A:
                PrintAndLogEx(INFO, "检测到基于 ISO14443-4A 的转储格式。无可用数据");
                break;
            case NFC_DF_PICOPASS:
                PrintAndLogEx(INFO, "检测到基于 PICOPASS 的转储格式");
                break;
            case NFC_DF_15:
                PrintAndLogEx(INFO, "检测到基于 ISO15693 的转储格式");
            case NFC_DF_UNKNOWN:
                PrintAndLogEx(WARNING, "检测转储格式失败");
                break;
        }
    }
    return PM3_SUCCESS;
...
static int searchFinalFile(char **foundpath, const char *pm3dir, const char *searchname, bool silent) {
    ...
    if ((g_debugMode == 2) && (!silent)) {
        ...
        PrintAndLogEx(INFO, "正在搜索... %s", filename);
    }

    // try implicit relative path
    PrintAndLogEx(DEBUG, "正在搜索隐式相对路径");
    if (fileExists(filename)) {
        *foundpath = filename;
        if ((g_debugMode == 2) && (!silent)) {
            PrintAndLogEx(INFO, "已找到 %s", *foundpath);
        }
        return PM3_SUCCESS;
    }
    ...
    PrintAndLogEx(DEBUG, "正在搜索偏好设置路径");
    for (int i = 0; i < spItemCount; i++) {
        ...
        if ((g_debugMode == 2) && (!silent)) {
            PrintAndLogEx(INFO, "正在搜索 %s", default_path);
        }
    ...
    // try pm3 dirs in user .proxmark3 (user mode)
    PrintAndLogEx(DEBUG, "正在搜索用户 .proxmark3 路径");
    const char *user_path = get_my_user_directory();
    if (user_path != NULL) {
        ...
        if ((g_debugMode == 2) && (!silent)) {
            PrintAndLogEx(INFO, "正在搜索 %s", path);
        }
    ...
    PrintAndLogEx(DEBUG, "正在搜索当前工作目录路径");
    const char *exec_path = get_my_executable_directory();
    if ((exec_path != NULL) &&
    ...
    PrintAndLogEx(DEBUG, "正在当前工作目录中搜索PM3目录");
    if ((exec_path != NULL) &&
    ...
    // try pm3 dirs in pm3 installation dir (install mode)
    PrintAndLogEx(DEBUG, "正在搜索PM3安装目录路径");
    if (exec_path != NULL) {
...
int searchFile(char **foundpath, const char *pm3dir, const char *searchname, const char *suffix, bool silent) {
    ...
    if (res != PM3_SUCCESS) {
        if ((res == PM3_EFILE) && (!silent)) {
            PrintAndLogEx(FAILED, "错误 - 找不到 `" _YELLOW_("%s") "`", filename);
        }
    }
    free(filename);
    return res;
        ...
        case JSON: {
            *pdump = calloc(maxdumplen, sizeof(uint8_t));
            ...
            if (res == PM3_ESOFT) {
                ...
                PrintAndLogEx(WARNING, "分配的内存大小错误。检查您的参数");
            }
            break;
        }
        case DICTIONARY: {
            PrintAndLogEx(ERR, "仅允许<BIN|EML|JSON|MCT|NFC格式");
            return PM3_EINVARG;
        }
        case MCT: {
        ...
        case FLIPPER: {
            ...
            if (dumptype == NFC_DF_MFC || dumptype == NFC_DF_MFU || dumptype == NFC_DF_PICOPASS || dumptype == NFC_DF_15) {
                ...
                *pdump = calloc(maxdumplen, sizeof(uint8_t));
                ...
                if (res == PM3_ESOFT) {
                    PrintAndLogEx(WARNING, "NFC 对象加载失败");
                } else if (res == PM3_EMALLOC) {
                    PrintAndLogEx(WARNING, "分配的内存大小错误。请检查您的参数");
                }
            } else {
...
int pm3_save_mf_dump(const char *fn, uint8_t *d, size_t n, JSONFileType jsft) {
    ...
    // Check for 7 bytes UID: double size uid bits in ATQA
        ...
        PrintAndLogEx(WARNING, "无效转储。未找到UID/SAK/ATQA");
    }
    jd.dump = d;
    jd.dumplen = n;
