    ...
    while (1) {
        ...
        if (WaitForResponseTimeout(CMD_ACK, &resp, 2000)) {
            if (resp.oldarg[0] > rxBufLen) {
                PrintAndLogEx(WARNING, "已接收 %"PRIu64 " bytes, rxBuf too small (%u)", resp.oldarg[0], rxBufLen);
        ...
        if ((retransmit) && (rxBuf[0] != 0 || rxBuf[1] != 0)) {
            ...
            if (fail_num > 10) {
                PROMPT_CLEARLINE;
                PrintAndLogEx(WARNING, "传输失败，请重试。");
                DropField();
                return PM3_ESOFT;
            }
        } else {
...
static int start_drawing_1in54B(uint8_t model_nr, uint8_t *black, uint8_t *red) {
    ...
    PrintAndLogEx(DEBUG, "1.54_步骤9: 电子纸配置2 (黑色)");
    if (model_nr == M1in54B) {      //1.54inch B Keychain
        for (i = 0; i < 50; i++) {
            ...
            progress = i * 100 / 100;
            PrintAndLogEx(INPLACE, "进度: %d %%", progress);
        }
    }
    PROMPT_CLEARLINE;
    PrintAndLogEx(DEBUG, "1.54_步骤6: 电子纸开机");
    ret = transceive_blocking(step_4, 2, rx, 20, actrxlen, true);          //cd 04
    if (ret != PM3_SUCCESS) {
        return ret;
    }

    PrintAndLogEx(DEBUG, "1.54_步骤7: 电子纸配置2 (红色)");
    if (model_nr == M1in54B) {       //1.54inch B Keychain
        for (i = 0; i < 50; i++) {
    ...
    PrintAndLogEx(DEBUG, "1.54_步骤8: EDP加载到主程序");
    ret = transceive_blocking(step_6, 2, rx, 20, actrxlen, true);          //cd 06
    if (ret != PM3_SUCCESS) {
        return ret;
    }

    PrintAndLogEx(DEBUG, "1.54_步骤9");
    return PM3_SUCCESS;
}
...
// uint8_t step14[2]={0xcd,0x0c};     // The end of the transmission
    ...
    msleep(100);
    PrintAndLogEx(DEBUG, "步骤2：电子纸正常模式类型");
    ...
    msleep(100);
    PrintAndLogEx(DEBUG, "步骤3：电子纸配置1");
    ...
    if (model_nr == M1in54B) {
        ...
        PrintAndLogEx(DEBUG, "开始绘制_1in54B");
        ...
        PrintAndLogEx(DEBUG, "步骤10：刷新电子纸");
    ...
    PrintAndLogEx(DEBUG, "步骤11：等待标签就绪");
    PrintAndLogEx(INPLACE, "电子纸正在刷新，请等待");
    ...
    while (1) {
        ...
        if (rx[0] == 0xff && rx[1] == 0) {
            ...
            PrintAndLogEx(SUCCESS, "电子纸刷新成功");
            msleep(200);
            break;
        } else {
            if (fail_num > 50) {
                PrintAndLogEx(WARNING, "更新失败，请重试。");
                DropField();
                return PM3_ESOFT;
            } else {
                fail_num++;
    ...
    PrintAndLogEx(DEBUG, "步骤12：电子纸断电命令");
    ret = transceive_blocking(step12, 2, rx, 20, actrxlen, true);          //cd 04
    if (ret != PM3_SUCCESS) {
        return ret;
    }
    msleep(200);
    PrintAndLogEx(SUCCESS, "电子纸更新成功");
    msleep(200);
    DropField();
    return PM3_SUCCESS;
}
...
static int CmdHF14AWSLoad(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf waveshare 加载",
                  "加载图像文件到 Waveshare NFC ePaper",
                  desc
                 );
    ...
    void *argtable[] = {
        ...
        arg_str1("f", "file", "<fn>", "指定要上传到标签的图像"),
        arg_str0("s", "save", "<fn>", "保存调色板版本到文件"),
        arg_param_end
    };
    ...
    //Validations
    if (infilelen < 1) {
        PrintAndLogEx(WARNING, "缺少输入文件");
        return PM3_EINVARG;
    }
    if (model_nr == -1) {
        PrintAndLogEx(WARNING, "缺少型号");
        return PM3_EINVARG;
    }
    if (model_nr >= MEND) {
        PrintAndLogEx(WARNING, "未知型号");
        return PM3_EINVARG;
    }
    if (!g_session.pm3_present && !outfilelen) {
        PrintAndLogEx(WARNING, "离线 - 只能执行图像转换");
        return PM3_ENOTTY;
    }
    ...
    if (scaled_img == NULL) {
        PrintAndLogEx(WARNING, "缩放输入图像失败");
    ...
    if (outfilelen) {
        if (gdImageFile(pal_img, outfile)) {
            ...
            PrintAndLogEx(WARNING, "无法保存转换后的图像", outfile);
            gdImageDestroy(pal_img);
            return PM3_EFILE;
        }
    }
    ...
    if (!black_plane) {
        PrintAndLogEx(WARNING, "无法将图像转换为位平面");
        gdImageDestroy(pal_img);
        return PM3_EMALLOC;
    }
...
static command_t CommandTable[] = {
    ...
    {"load",        CmdHF14AWSLoad,       AlwaysAvailable, "加载图像文件到 Waveshare NFC ePaper"},
    {NULL, NULL, NULL, NULL}
};
