...
int demodNoralsy(bool verbose) {
    ...
    if (ASKDemod_ext(32, 0, 0, 0, false, false, false, 1, &st) != PM3_SUCCESS) {
        if (g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: ASK/曼彻斯特解调失败");
        return PM3_ESOFT;
    }
    if (!st) {
        if (g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 未找到序列终止符");
        return PM3_ESOFT;
    }
    ...
    if (ans < 0) {
        if (g_debugMode) {
            if (ans == -1)
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 找到的位数太少");
            else if (ans == -2)
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 未找到前导码");
            else if (ans == -3)
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 大小不正确: %zu", size);
            else
                PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: ans: %d", ans);
        }
        return PM3_ESOFT;
    }
    ...
    // test checksums
    if (chk1 != calc1) {
        if (g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 校验和1失败 %x - %x\\n", chk1, calc1);
        return PM3_ESOFT;
    }
    if (chk2 != calc2) {
        if (g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: 错误 - Noralsy: 校验和2失败 %x - %x\\n", chk2, calc2);
        return PM3_ESOFT;
    }

    PrintAndLogEx(SUCCESS, "Noralsy - Card: " _GREEN_("%u")", Year: " _GREEN_("%u") ", Raw: %08X%08X%08X", cardid, year, raw1, raw2, raw3);
    if (raw1 != 0xBB0214FF) {
        PrintAndLogEx(WARNING, "第一个块中设置了未知位！期望0xBB0214FF，发现: 0x%08X", raw1);
        PrintAndLogEx(WARNING, "请将此输出发布到论坛以进一步研究此格式");
    }
    return PM3_SUCCESS;
}
...
static int CmdNoralsyDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 Noralsy 解调",
                  "尝试查找Noralsy前导码，如果找到则解码/解扰数据",
                  "低频 Noralsy 解调"
                 );
...
static int CmdNoralsyReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Noralsy 读取",
                  "读取 Noralsy 标签",
...
static int CmdNoralsyClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Noralsy 克隆",
                  "将 Noralsy 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
...
static int CmdNoralsySim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频 Noralsy 模拟",
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdNoralsyDemod,  AlwaysAvailable, "从GraphBuffer解调Noralsy标签"},
    {"reader",  CmdNoralsyReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",   CmdNoralsyClone,  IfPm3Lf,         "克隆 Noralsy 标签到 T55x7、Q5/T5555 或 EM4305/4469"},
    {"sim",     CmdNoralsySim,    IfPm3Lf,         "模拟Noralsy标签"},
    {NULL, NULL, NULL, NULL}
};
