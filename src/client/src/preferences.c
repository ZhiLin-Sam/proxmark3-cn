...
int preferences_save(void) {
    ...
    if (g_session.incognito) {
        PrintAndLogEx(INFO, "将不保存偏好文件");
        return PM3_SUCCESS;
    }
    PrintAndLogEx(DEBUG, "正在保存偏好设置...");
    ...
    if (fileExists(backupFilename)) {
        if (remove(backupFilename) != 0) {
            PrintAndLogEx(FAILED, "错误 - 无法删除旧的设置备份文件 \\\"%s\\\"", backupFilename);
            free(fn);
            free(backupFilename);
            return PM3_ESOFT;
        }
    ...
    if (fileExists(fn)) {
        if (rename(fn, backupFilename) != 0) {
            PrintAndLogEx(FAILED, "错误 - 无法将设置文件 \\\"%s\\\" 备份到 \\\"%s\\\"", fn, backupFilename);
            free(fn);
            free(backupFilename);
            return PM3_ESOFT;
        }
    ...
    if (saveFileJSONex(fn, jsfCustom, &dummyData, dummyDL, true, &preferences_save_callback, spItemCount) != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "保存偏好设置到\"%s\"时出错", fn);
    }
...
void preferences_load_callback(json_t *root) {
    ...
    if (json_unpack_ex(root, &up_error, 0, "{s:s}", "show.emoji", &s1) == 0) {
        ...
        if (strncmp(tempStr, "无", 4) == 0) g_session.emoji_mode = EMO_NONE;
    }
...
static void showEmojiState(prefShowOpt_t opt) {
    ...
    switch (g_session.emoji_mode) {
        ...
        case EMO_ALTTEXT:
            ...
            break;
        case EMO_NONE:
            PrintAndLogEx(INFO, "   %s emoji................... %s", pref_show_status_msg(opt), pref_show_value(opt, "无"));
            break;
        default:
...
static int setCmdEmoji(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0(NULL, "无", "don't show emoji or text"),
        arg_param_end
    };
...
int getDeviceDebugLevel(uint8_t *debug_level) {
    ...
    if (WaitForResponseTimeout(CMD_GET_DBGMODE, &resp, 2000) == false) {
        PrintAndLogEx(WARNING, "获取当前设备调试级别失败");
        return PM3_ETIMEOUT;
    }
...
int setDeviceDebugLevel(uint8_t debug_level, bool verbose) {
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "设置设备调试日志级别为%u", debug_level);
    }
    ...
    PacketResponseNG resp;
    if (WaitForResponseTimeout(CMD_SET_DBGMODE, &resp, 2000) == false) {
        PrintAndLogEx(WARNING, "设置设备调试日志级别失败");
        return PM3_EFAILED;
    }
    return PM3_SUCCESS;
}
...
static int setCmdExeDelay(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 客户端延迟",
                  "设置客户端中执行命令前延迟的持久偏好",
...
static int setCmdClientTimeout(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 客户端超时",
                  "设置客户端通信超时的持久偏好",
...
static int setCmdHfFieldTimeout(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 高频场超时秒数",
                  "设置PM3高频场不活动超时的持久偏好",
    ...
    if (g_session.pm3_present) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "无法对连接的PM3应用高频场超时");
        }
    }
...
static int setCmdPlotSliders(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 绘图滑块",
                  "设置客户端中显示绘图滑块控件的持久偏好",
...
static int setCmdSavePaths(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 保存路径",
                  "设置客户端中文件路径的持久偏好",
    ...
    if (deflen == 0 && dulen == 0 && tlen == 0) {
        PrintAndLogEx(FAILED, "必须至少提供一个路径");
        return PM3_EINVARG;
    }
...
static int setCmdBarMode(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 条模式",
                  "设置客户端中HF/LF调谐命令样式输出的持久偏好",
...
static int setCmdMqtt(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 设置 MQTT",
                  "设置客户端中MQTT服务器的持久偏好",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("s", "srv", "<path>", "默认MQTT服务器"),
        arg_str0("p", "port", "<path>", "默认MQTT端口"),
        arg_str0("t", "topic", "<path>", "默认MQTT主题"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    int deflen = 0;
    ...
    // sanity checks
    if (res) {
        PrintAndLogEx(FAILED, "解析输入字符串错误");
        return PM3_EINVARG;
    }
...
static int getCmdEmoji(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 表情符号",
                  "获取客户端中使用表情符号的偏好。",
                  "prefs 获取 表情符号"
                 );
    void *argtable[] = {
...
static int getCmdDebug(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 客户端调试",
                  "获取使用客户端调试级别的偏好。",
                  "prefs 获取 客户端调试"
                 );
    void *argtable[] = {
...
static int getCmdPlotSlider(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 绘图滑块",
                  "获取客户端中显示绘图滑块控件的偏好。",
                  "prefs 获取 绘图滑块"
                 );
    void *argtable[] = {
...
static int getCmdBarMode(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 条模式",
                  "获取客户端中HF/LF调谐命令样式输出的偏好。",
                  "prefs 获取 条模式"
                 );
    void *argtable[] = {
        arg_param_begin,
...
static int getCmdSavePaths(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 保存路径",
                  "获取客户端中文件路径的偏好。",
                  "prefs 获取 保存路径"
                 );
    void *argtable[] = {
...
static int getCmdExeDelay(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 客户端延迟",
                  "获取客户端中命令执行前延迟时间的偏好。",
                  "prefs 获取 客户端延迟"
                 );
    void *argtable[] = {
...
static int getCmdClientTimeout(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 客户端超时",
                  "获取客户端中命令执行前延迟时间的偏好。",
                  "prefs 获取 客户端超时"
                 );
    void *argtable[] = {
...
static int getCmdHfFieldTimeout(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 高频场超时秒数",
                  "获取PM3高频场空闲超时的偏好。",
                  "prefs 获取 高频场超时秒数"
                 );
    void *argtable[] = {
...
static int getCmdMqtt(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 获取 MQTT",
                  "获取客户端中MQTT设置的偏好。",
                  "prefs 获取 MQTT"
                 );
    void *argtable[] = {
...
static command_t CommandTableGet[] = {
    {"barmode",          getCmdBarMode,       AlwaysAvailable, "获取条形模式偏好"},
    {"client.debug",     getCmdDebug,         AlwaysAvailable, "获取客户端调试级别偏好"},
    {"client.delay",     getCmdExeDelay,      AlwaysAvailable, "获取客户端执行延迟偏好"},
    {"client.timeout",   getCmdClientTimeout, AlwaysAvailable, "获取客户端执行延迟偏好"},
    {"hf.field.timeout_sec", getCmdHfFieldTimeout, AlwaysAvailable, "获取PM3高频场空闲超时偏好"},
    {"color",            getCmdColor,         AlwaysAvailable, "获取颜色支持偏好"},
    ...
    {"emoji",            getCmdEmoji,         AlwaysAvailable, "获取表情符号显示偏好"},
    {"提示",            getCmdHint,          AlwaysAvailable, "获取提示显示偏好"},
    {"output",           getCmdOutput,        AlwaysAvailable, "获取转储输出样式偏好"},
    {"plotsliders",      getCmdPlotSlider,    AlwaysAvailable, "获取绘图滑块显示偏好"},
    {"mqtt",             getCmdMqtt,          AlwaysAvailable, "获取MQTT偏好"},
    {NULL, NULL, NULL, NULL}
};

static command_t CommandTableSet[] = {
    {"help",             setCmdHelp,          AlwaysAvailable, "此帮助"},
    {"barmode",          setCmdBarMode,       AlwaysAvailable, "设置条形模式"},
    {"client.debug",     setCmdDebug,         AlwaysAvailable, "设置客户端调试级别"},
    {"client.delay",     setCmdExeDelay,      AlwaysAvailable, "设置客户端执行延迟"},
    {"client.timeout",   setCmdClientTimeout, AlwaysAvailable, "设置客户端通信超时"},
    {"hf.field.timeout_sec", setCmdHfFieldTimeout, AlwaysAvailable, "设置PM3高频场非活动超时"},

    {"color",            setCmdColor,         AlwaysAvailable, "设置颜色支持"},
    {"emoji",            setCmdEmoji,         AlwaysAvailable, "设置表情符号显示"},
    {"提示",            setCmdHint,          AlwaysAvailable, "设置提示显示"},
    ...
    {"output",           setCmdOutput,        AlwaysAvailable, "设置转储输出样式"},
    {"plotsliders",      setCmdPlotSliders,   AlwaysAvailable, "设置绘图滑块显示"},
    {"mqtt",             setCmdMqtt,          AlwaysAvailable, "设置MQTT默认值"},
    {NULL, NULL, NULL, NULL}
};
...
static int CmdPrefShow(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "prefs 显示",
                  "显示所有持久偏好",
                  "prefs 显示"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("j", "json", "以JSON格式转储偏好设置"),
        arg_param_end
    };
    ...
    if (g_session.preferences_loaded) {
        ...
        PrintAndLogEx(WARNING, "偏好文件未加载");
    }

    PrintAndLogEx(INFO, "当前设置");
    showEmojiState(prefShowNone);
    showHintsState(prefShowNone);
    showColorState(prefShowNone);
    showPlotPosState();
...
static command_t CommandTable[] = {
    ...
    {"get",          CmdPrefGet,         AlwaysAvailable, "{ 获取偏好 }"},
    {"set",          CmdPrefSet,         AlwaysAvailable, "{ 设置偏好 }"},
    {"show",         CmdPrefShow,        AlwaysAvailable, "显示所有偏好设置"},
    {NULL, NULL, NULL, NULL}
};
