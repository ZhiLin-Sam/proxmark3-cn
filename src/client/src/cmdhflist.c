...
//        }
        ...
        switch (c) {
            ...
            case ICLASS_CMD_READ4:
                snprintf(exp, size, "读取4(%d)", cmd[1]);
                break;
            case ICLASS_CMD_READ_OR_IDENTIFY: {

                if (cmdsize > 1) {
                    snprintf(exp, size, "读取(%d)", cmd[1]);
                } else {
            ...
            case ICLASS_CMD_PAGESEL:
                snprintf(exp, size, "页选择(%d)", cmd[1]);
                curr_state = PICO_NONE;
                break;
            case ICLASS_CMD_UPDATE:
                snprintf(exp, size, "更新(%d)", cmd[1]);
                curr_state = PICO_NONE;
                break;
            case ICLASS_CMD_READCHECK:
                if (ICLASS_CREDIT(cmd[0])) {
                    snprintf(exp, size, "读取检查[Kc](%d)", cmd[1]);
                    curr_state = PICO_AUTH_EPURSE;
                } else {
                    snprintf(exp, size, "读取检查[Kd](%d)", cmd[1]);
                    curr_state = PICO_AUTH_EPURSE;
                }
                break;
            case ICLASS_CMD_ACT:
    ...
    if (cmdsize >= 2) {
        switch (cmd[1]) {
            ...
            case ISO15693_READ_MULTI_BLOCK:
                if (cmdsize == 6) {
                    snprintf(exp, size, "读取多块(%d-%d)", cmd[2], (cmd[2] + cmd[3]));
                } else {
                    snprintf(exp, size, "读取多块(%d-%d)", cmd[10], (cmd[10] + cmd[11]));
                }
                return;
...
void annotateMfDesfire(char *exp, size_t size, uint8_t *cmd, uint8_t cmdsize) {
    ...
    if (applyIso14443a(exp, size, cmd, cmdsize, false) != PM3_SUCCESS) {
        ...
        // R-block (ack) 101xx01x
            ...
            if ((cmd[0] & 0x10) == 0)
                snprintf(exp, size, "R-block 确认(%d)", (cmd[0] & 0x01));
            else
                snprintf(exp, size, "R-block 否认(%d)", (cmd[0] & 0x01));
        }
        // I-block 000xCN1x
            ...
            for (uint8_t i = 0; i < 2; i++, pos++) {
                ...
                switch (cmd[pos]) {
                    ...
                    case MFDES_SELECT_APPLICATION:
                        if (data_size >= 3) {
                            snprintf(exp, size, "选择应用 (应用ID %06x)", MemLeToUint3byte(data));
                        } else {
                    ...
                    case MFDES_READ_DATA:
                        if (data_size >= 7) {
                            snprintf(exp, size, "读取数据 (文件ID %02x, 偏移 %u, 长度 %u)", data[0], MemLeToUint3byte(data + 1), MemLeToUint3byte(data + 4));
                        } else {
                    ...
                    case MFDES_WRITE_DATA:
                        if (data_size >= 7) {
                            snprintf(exp, size, "写入数据 (文件ID %02x, 偏移 %u, 长度 %u)", data[0], MemLeToUint3byte(data + 1), MemLeToUint3byte(data + 4));
                        } else {
                    ...
                    case MFDES_LIMITED_CREDIT:
                        if (data_size >= 1) {
                            snprintf(exp, size, "有限信用 (文件ID %02x)", data[0]);
                        } else {
                    ...
                    case MFDES_WRITE_RECORD:
                        if (data_size >= 7) {
                            snprintf(exp, size, "写入记录 (文件ID %02x, 偏移 %u, 长度 %u)", data[0], MemLeToUint3byte(data + 1), MemLeToUint3byte(data + 4));
                        } else {
                    ...
                    case MFDES_READ_RECORDS:
                        if (data_size >= 7) {
                            snprintf(exp, size, "读取记录 (文件ID %02x, 偏移 %u, 长度 %u)", data[0], MemLeToUint3byte(data + 1), MemLeToUint3byte(data + 4));
                        } else {
                            snprintf(exp, size, "READ RECORDS");
                        }
                        break;
        ...
        for (uint8_t i = 0; i < 2; i++, pos++) {
            ...
            switch (opcode) {
                ...
                case MFP_WRITEPERSO:
                    if (data_size > 1) {
                        uint16_t uKeyNum = MemLeToUint2byte(data) ;
                        snprintf(exp, size, "写入个性化 (地址 0x%04X)", uKeyNum);
                    } else {
                ...
                case MFP_READPLAINMAC_UNMACED: {
                    ...
                    if (data_size > 2) {
                        ...
                        uint8_t uQty = data[2] ;
                        if (uQty > 1) {
                            snprintf(exp, size, "读取 %s(%u-%i) %s_%s", encrypted, uBlockNum, uBlockNum + uQty - 1, responseMaced, commandMaced);
                        } else {
                            snprintf(exp, size, "读取 %s(%u) %s_%s", encrypted, uBlockNum, responseMaced, commandMaced);
                        }
                    } else {
                        snprintf(exp, size, "读取 %s %s_%s ?", encrypted, responseMaced, commandMaced);
                    }
                    break;
                }
                    ...
                    if (data_size > 1) {
                        ...
                        switch (uBlockNum & 0xF000) {
                            ...
                            default:
                                ...
                                break;
                            case 0x4000:
                                snprintf(exp, size, "写入 (密钥号 0x%04X: %c 扇区 %d)", uBlockNum, uBlockNum & 0x0001 ? 'B' : 'A', (uBlockNum - 0x4000) / 2);
                                break;
                            case 0xA000: // There are virtual card encryption and MACing keys, but this is NOT their place!
                        ...
                        snprintf(exp, size, "写入 %s %s ？", encrypted, responseMaced);
                    }
                    break;
                }
    ...
    switch (bitsend) {
        ...
        case 11: {
            ...
            if (cmdBit == LEGIC_READ) {
                snprintf(exp, size, "读取字节(%d)", address);
            }

            if (cmdBit == LEGIC_WRITE) {
                snprintf(exp, size, "写入字节(%d)", address);
            }
            break;
        }
        case 21: {
            if (cmdBit == LEGIC_WRITE) {
                ...
                snprintf(exp, size, "写入字节(%d) %02X", address, val);
            }
            break;
        }
        case 23: {
