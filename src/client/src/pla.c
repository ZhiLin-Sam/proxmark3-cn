...
json_t *pla_load_ecplist(void) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "找不到 ecplist.json");
        return NULL;
    }
    ...
    if (!root) {
        PrintAndLogEx(ERR, "JSON错误在第%d行：%s", error.line, error.text);
        return NULL;
    }

    if (!json_is_array(root)) {
        PrintAndLogEx(ERR, "无效的ecplist.json格式。根必须为数组。");
        json_decref(root);
        return NULL;
    }
...
int pla_parse_ecp_subcommand(const char *cmd, uint8_t *frame, size_t frame_size) {
    ...
    if (!ecplist) {
        PrintAndLogEx(ERR, "加载 ecplist.json 失败");
        free(cmd_copy);
        return -1;
    }
    ...
    if (strncmp(p, "transit", 7) == 0) {
        ...
        if (*p != '\0') {
            ...
            if (result == -1) {
                ...
                if (search_term != endptr) {
                    ...
                    PrintAndLogEx(ERR, "未知传输密钥或无效 TCI：%s", search_term);
                }
            }
        } else {
            PrintAndLogEx(ERR, "传输类型需要密钥或TCI值");
        }
        ...
        if (second != NULL && *second != '\0') {
            ...
            if (result == -1 && third == NULL) {
                ...
                if (second == endptr) {
                    PrintAndLogEx(ERR, "未知的访问子类型/密钥或无效的TCI: %s", second);
                    free(second);
                    if (third) free(third);
                    json_decref(ecplist);
                    free(cmd_copy);
                ...
                PrintAndLogEx(ERR, "未找到子类型 '%s' 和密钥 '%s' 的匹配访问条目", second, third);
                free(second);
                free(third);
                json_decref(ecplist);
                free(cmd_copy);
        ...
        if (entry) {
            ...
            PrintAndLogEx(ERR, "未知的ECP类型: %s", search_term);
            PrintAndLogEx(HINT, "可用类型: access, transit, vasorpay, vasandpay, vasonly, payonly, gymkit, identity, aidrop");
        }
    }
