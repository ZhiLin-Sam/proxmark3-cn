...
int demodParadox(bool verbose, bool oldChksum) {
    ...
    if (size == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox 样本不足");
        free(bits);
        return PM3_ESOFT;
    }
    ...
    if (idx < 0) {
        if (idx == -1)
            ...
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox 仅检测到噪声");
        else if (idx == -3)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox FSK 解调过程中出现问题");
        else if (idx == -4)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox 未找到前导码");
        else
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox FSK 解调错误 %d", idx);
    ...
    for (uint32_t i = idx + PARADOX_PREAMBLE_LEN; i < (idx + 96); i += 2) {
        ...
        // not manchester data
        if (bits[i] == bits[i + 1]) {
            PrintAndLogEx(DEBUG, "错误 曼彻斯特编码在 %u", i);
            errors++;
        }
    ...
    if (errors) {
        PrintAndLogEx(DEBUG, "总曼彻斯特错误... %u", errors);
    }
    ...
    if (hi2 == 0 && hi == 0 && lo == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Paradox 未找到值");
        free(bits);
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(DEBUG, "DEBUG: Paradox 索引: %d, 长度: %zu, 打印解调缓冲区:", idx, size);
    if (g_debugMode) {
...
static int CmdParadoxDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频Paradox解调",
                  "尝试查找Paradox前导码，如果找到则解码/解扰数据",
...
static int CmdParadoxReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Paradox读取器",
                  "读取 Paradox 标签",
...
static int CmdParadoxClone(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频Paradox克隆",
                  "将 paradox 标签克隆到 T55x7、Q5/T5555 或 EM4305/4469 标签。",
    ...
    if ((fc || cn) && raw_len != 0) {
        PrintAndLogEx(FAILED, "不能同时指定FC/CN和RAW");
        return PM3_EINVARG;
    }

    if (fc > 999 || cn > 99999) {
        PrintAndLogEx(FAILED, "FC最大值为999，CN最大值为99999");
        return PM3_EINVARG;
    }
...
static int CmdParadoxSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("r", "raw", "<hex>", "原始十六进制数据，12字节"),
        arg_u64_0(NULL, "fc", "<dec>", "facility code"),
        arg_u64_0(NULL, "cn", "<dec>", "card number"),
        arg_param_end
    };
...
static command_t CommandTable[] = {
    ...
    {"demod",  CmdParadoxDemod,  AlwaysAvailable, "从 GraphBuffer 解调 Paradox FSK 标签"},
    {"reader", CmdParadoxReader, IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",  CmdParadoxClone,  IfPm3Lf,         "克隆 paradox 标签到 T55x7、Q5/T5555 或 EM4305/4469"},
    {"sim",    CmdParadoxSim,    IfPm3Lf,         "模拟paradox标签"},
    {NULL, NULL, NULL, NULL}
};
