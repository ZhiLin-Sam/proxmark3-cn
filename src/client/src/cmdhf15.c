...
#define ISO15_ERROR_HANDLING_RESPONSE { \
    ...
    if (resp.length < 2) { \
        PrintAndLogEx(ERR, "ISO15693命令失败"); \
...
#define ISO15_ERROR_HANDLING_CARD_RESPONSE(data, len) { \
 ...
 \
        PrintAndLogEx(ERR, "ISO15693卡片返回错误%i：%s", d[0], TagErrorStr(d[0])); \
        return PM3_EWRONGANSWER; \
    } \
}
#endif
...
static int CmdHF15Demod(const char *Cmd) {
    ...
    if (g_GraphTraceLen < 1000) {
        PrintAndLogEx(FAILED, "GraphBuffer中样本太少");
        PrintAndLogEx(HINT, "提示: 运行 `" _YELLOW_("hf 15 采样") "` to collect and download data");
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "SOF 在 %d，相关性 %zu", maxPos, max / (ARRAYLEN(FrameSOF) / skip));
    ...
    for (;;) {
        ...
        if (corrEOF > corr1 && corrEOF > corr0) {
            PrintAndLogEx(INFO, "在 %d 处遇到文件结束", i);
            break;
        } else if (corr1 > corr0) {
        ...
        if ((i + (int)ARRAYLEN(FrameEOF)) >= g_GraphTraceLen) {
            PrintAndLogEx(INFO, "超出末尾!");
            break;
        }

        if (k > 2048) {
            PrintAndLogEx(INFO, "缓冲区不足");
            break;
        }
    }

    if (mask != 0x01) {
        PrintAndLogEx(WARNING, "警告，丢弃多余的比特！");
    ...
    i = 0;
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(INFO, "获取 %d 字节，解码如下", k);
    PrintAndLogEx(NORMAL, "");
...
static int CmdHF15Samples(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 15 解调") "` to decode signal");
...
static int NxpTestEAS(const uint8_t *uid) {
    ...
    if (WaitForResponseTimeout(CMD_HF_ISO15693_COMMAND, &resp, 2000) == false) {
        PrintAndLogEx(DEBUG, "ISO15693超时");
        return PM3_ETIMEOUT;
    }
    if (resp.length < 2) {
        return PM3_EWRONGANSWER;
...
static int NxpCheckSig(uint8_t *uid) {
    ...
    if (resp.length < 2) {
        PrintAndLogEx(WARNING, "ISO15693卡片未响应读取签名命令");
        return PM3_EWRONGANSWER;
    }
...
static int NxpSysInfo(uint8_t *uid) {
    ...
    if (resp.length < 2) {
        PrintAndLogEx(WARNING, "ISO15693卡片未响应NXP系统信息命令");
        return PM3_EWRONGANSWER;
    }
    ...
    PrintAndLogEx(INFO, "    Page L read.... %s"
                  , (d[2] & 0x01) ?  _RED_("密码") : _GREEN_("no password")
                 );

    PrintAndLogEx(INFO, "    Page L write... %s"
                  , (d[2] & 0x02) ?  _RED_("密码") : _GREEN_("no password")
                 );

    PrintAndLogEx(INFO, "    Page H read.... %s"
                  , (d[2] & 0x10) ?  _RED_("密码") : _GREEN_("no password")
                 );

    PrintAndLogEx(INFO, "    Page H write... %s"
                  , (d[2] & 0x20) ?  _RED_("密码") : _GREEN_("no password")
                 );
...
static int CmdHF15Info(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 信息",
                  "使用可选命令`get_systeminfo` 0x2B尝试提取信息",
                  "hf 15 info\n"
                  "hf 15 info -*\n"
    ...
    if ((scan + unaddressed + (uidlen > 0)) > 1) {
        PrintAndLogEx(WARNING, "仅选择一个选项 /scan/unaddress/uid");
        return PM3_EINVARG;
    }
    ...
    if (resp.length < 2) {
        PrintAndLogEx(WARNING, "ISO15693卡片未响应系统信息命令(%d)", resp.length);
        return PM3_EWRONGANSWER;
    }
    ...
    printTagInfo_15(d + 2);
    PrintAndLogEx(SUCCESS, "系统信息... %s", sprint_hex(d, resp.length - 2));

    // DSFID
    if (d[1] & 0x01)
        PrintAndLogEx(SUCCESS, "DSFID..... 0x%02X", d[10]);
    else
        PrintAndLogEx(SUCCESS, "不支持 DSFID");

    // AFI
    if (d[1] & 0x02)
        PrintAndLogEx(SUCCESS, "AFI....... 0x%02X", d[11]);
    else
        PrintAndLogEx(SUCCESS, "不支持 AFI");

    // IC reference
    if (d[1] & 0x08)
        PrintAndLogEx(SUCCESS, "IC 参考.... 0x%02X", d[14]);
    else
        PrintAndLogEx(SUCCESS, "不支持的 IC 参考");

    // memory
    if (d[1] & 0x04) {
        PrintAndLogEx(SUCCESS, "标签内存布局（取决于厂商）");
    ...
    PrintAndLogEx(DEBUG, "NXP 版本: %02x", nxp_version);

    if (d[8] == 0x04) {
        // NXP
        if (d[7] == 0x01 && nxp_version == 0x08) {
            PrintAndLogEx(DEBUG, "检测到 SLIX2，正在获取 NXP 系统信息");
            return NxpSysInfo(uid);
        } else if (d[7] == 0x01 && nxp_version == 0x18) { // If it is an NTAG 5
            PrintAndLogEx(DEBUG, "检测到 NTAG 5，正在获取 NXP 系统信息");
            return NxpSysInfo(uid);
        } else if ((d[7] == 0x01 || d[7] == 0x02 || d[7] == 0x03)) { // If SLI, SLIX, SLIX-l, or SLIX-S check EAS status
            PrintAndLogEx(DEBUG, "检测到 SLI、SLIX、SLIX-L 或 SLIX-S，正在检查 EAS 状态");
        ...
        if (d[7] == 0x08) {
            PrintAndLogEx(DEBUG, "检测到 ST25TVxC，获取 ST 签名");
...
static int CmdHF15Sniff(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 15 list") "` to view captured tracelog");
...
static int CmdHF15ELoad(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 加载EEPROM",
                  "从文件加载内存转储以用于'hf 15 sim'",
                  "hf 15 eload -f hf-15-01020304.bin\n"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_str1("f", "file", "<fn>", "转储文件名"),
        arg_param_end
    };
    ...
    if (bytes_read == 0) {
        PrintAndLogEx(FAILED, "内存镜像为空。");
        free(tag);
        return PM3_EINVARG;
    }

    if (bytes_read != sizeof(iso15_tag_t)) {
        PrintAndLogEx(FAILED, "内存镜像与标签结构不匹配。");
        free(tag);
        return PM3_EINVARG;
    }
    ...
    if ((tag->pagesCount > ISO15693_TAG_MAX_PAGES) ||
        ...
        PrintAndLogEx(FAILED, "标签大小错误：pagesCount=%d，bytesPerPage=%d",
                      tag->pagesCount,
                      tag->bytesPerPage
                     );
        free(tag);
    ...
    while (bytes_read > 0) {
        ...
        if (hf15EmlSetMem((uint8_t *)tag + offset, bytestosend, offset) != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "无法在偏移 %zu / 0x%zx 设置仿真器内存", offset, offset);
            free(tag);
            return PM3_ESOFT;
        }
    ...
    PrintAndLogEx(HINT, "提示：您可以模拟了。参见 `" _YELLOW_("hf 15 sim -h") "`");
    PrintAndLogEx(INFO, "完成！");
    return PM3_SUCCESS;
}
...
static int CmdHF15ESave(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 保存EEPROM",
    ...
    PrintAndLogEx(INFO, "从模拟器内存下载%u字节", bytes);
...
static void print_tag_15693(iso15_tag_t *tag, bool dense_output, bool verbose) {
    if (verbose) {
        ...
        PrintAndLogEx(SUCCESS, "IC 参考.... 0x%02X", tag->ic);
        PrintAndLogEx(SUCCESS, "标签内存布局（取决于厂商）");
...
static int CmdHF15EView(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 查看EEPROM",
                  "显示模拟器内存",
                  "hf 15 eview\n"
                  "hf 15 eview -z\n"
                 );
...
static int CmdHF15Sim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 模拟",
                  "模拟ISO-15693标签\\n",
                  "hf 15 sim\n"
                  "hf 15 sim -u E011223344556677");
    void *argtable[] = {
        arg_param_begin,
        arg_str0("u", "uid", "<hex>", "UID，8 个十六进制字节"),
        arg_int0("b", "blocksize", "<dec>", "块大小（默认4）"),
        arg_int0("t", "timeout", "<dec>", "超时时间（毫秒，默认-1，即直到按键）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (uidlen == 0) {
        ...
        if (resp.status != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "从模拟器内存获取 UID 失败");
            return resp.status;
        }
...
static int CmdHF15WriteAfi(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str0("u", "uid", "<hex>", "完整 UID，8 个十六进制字节"),
        arg_int1(NULL, "afi", "<dec>", "AFI number (0-255)"),
        arg_str0("p", "pwd", "<hex>", "可选AFI/EAS密码"),
        arg_param_end
    };
    ...
    if (pwdlen > 0 && pwdlen != 4) {
        PrintAndLogEx(WARNING, "如果提供，密码必须是4个十六进制字节");
        return PM3_ESOFT;
    }
...
static int CmdHF15WriteDsfid(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 写入DSFID",
                  "写入卡片DSFID",
    ...
    argtable[arglen++] = arg_lit0("v", "详细", "详细输出");
    argtable[arglen++] = arg_param_end;
...
static int CmdHF15Dump(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 转储",
                  "此命令转储 ISO-15693 标签的内容并保存到文件（bin/json）",
                  "hf 15 dump\n"
                  "hf 15 dump -*\n"
    ...
    void *argtable[6 + 6] = {0};
    uint8_t arglen = arg_add_default(argtable);
    argtable[arglen++] = arg_str0("f", "file", "<fn>", "指定转储文件的文件名");
    argtable[arglen++] = arg_int0(NULL, "bs", "<dec>", "块大小（默认4）");
    argtable[arglen++] = arg_lit0(NULL, "ns", "no save to file");
    argtable[arglen++] = arg_lit0("v", "详细", "详细输出");
    argtable[arglen++] = arg_lit0("z", "dense", "密集转储输出样式");
    argtable[arglen++] = arg_param_end;
    ...
    if (blocksize < 4) {
        PrintAndLogEx(WARNING, "块大小太小，使用默认4字节");
        blocksize = 4;
    }
    ...
    for (int retry = 0; (retry < 2 && blocknum < tag->pagesCount); retry++) {
        ...
        if (WaitForResponseTimeout(CMD_HF_ISO15693_COMMAND, &resp, 2000)) {
            ...
            if (resp.length < 2) {
                PrintAndLogEx(NORMAL, "");
                PrintAndLogEx(FAILED, "ISO15693命令失败");
                continue;
            }
            ...
            if ((d[0] & ISO15_RES_ERROR) == ISO15_RES_ERROR) {
                ...
                PrintAndLogEx(NORMAL, "");
                PrintAndLogEx(FAILED, "标签返回错误%i：%s", d[1], TagErrorStr(d[1]));
                break;
            }
            ...
            PrintAndLogEx(INPLACE, "块 %3d", blocknum);
        }
    }
    ...
    if (tag->bytesPerPage != blocksize) {
        ...
        PrintAndLogEx(INFO, "正在使用 %u ...", tag->bytesPerPage);
    }
...
static int CmdHF15Raw(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 原始命令",
                  "通过ISO-15693向卡片发送原始字节",
    ...
    void *argtable[] = {
        ...
        arg_lit0("c", "crc", "计算并附加CRC"),
        arg_lit0("k", NULL, "接收后保持信号场开启"),
        arg_lit0("2", NULL, "use slower '1 out of 256' mode"),
        arg_lit0("r", NULL, "do not read response"),
        arg_str1("d", "数据", "<hex>", "要发送的原始字节"),
        arg_lit0("w", "wait", "等待响应更长时间。用于写入等操作。"),
        arg_param_end
    };
    ...
    if (crc) {
        ...
        if ((datalen - 2) < PM3_CMD_DATA_SIZE) {
            ...
            PrintAndLogEx(FAILED, "原始数据太长，无法添加 CRC。");
            return PM3_ECRC;
        }
    }
...
static int CmdHF15Readmulti(const char *Cmd) {
    ...
    if (add_option == false) {
        if (verbose) {
            PrintAndLogEx(INFO, "覆盖 OPTION 参数以获取锁定信息响应 (ENFORCE)");
        }
        add_option = true;
    }
    ...
    PacketResponseNG resp;
    if (WaitForResponseTimeout(CMD_HF_ISO15693_COMMAND, &resp, 2000) == false) {
        PrintAndLogEx(FAILED, "ISO15693卡片超时");
        return PM3_ETIMEOUT;
    }
    ...
    if (resp.length > (1 + ((blockcnt + 1) * (blocksize + 1)) + 2)) {
        PrintAndLogEx(WARNING, "收到更长的响应。检查块大小！");
    }
...
static int CmdHF15Readblock(const char *Cmd) {
    ...
    void *argtable[6 + 4] = {0};
    uint8_t arglen = arg_add_default(argtable);
    argtable[arglen++] = arg_int1("b", "blk", "<dec>", "页号(0-255)");
    argtable[arglen++] = arg_int0(NULL, "bs", "<dec>", "块大小（默认4）");
    argtable[arglen++] = arg_lit0("v", "详细", "详细输出");
    argtable[arglen++] = arg_param_end;
...
static int hf_15_write_blk(const uint8_t *pm3flags, uint16_t flags, const uint8_t *uid, bool fast, uint8_t blockno, const uint8_t *data, uint8_t dlen) {
    ...
    PacketResponseNG resp;
    if (WaitForResponseTimeout(CMD_HF_ISO15693_COMMAND, &resp, 2000) == false) {
        PrintAndLogEx(FAILED, "ISO15693卡片超时，数据可能已写入");
        return PM3_ETIMEOUT;
    }
...
static int CmdHF15Write(const char *Cmd) {
    ...
    uint8_t arglen = arg_add_default(argtable);
    argtable[arglen++] = arg_int1("b", "blk", "<dec>", "页号(0-255)");
    argtable[arglen++] = arg_str1("d", "数据", "<hex>", "数据，4字节");
    argtable[arglen++] = arg_lit0("v", "详细", "详细输出");
    argtable[arglen++] = arg_param_end;
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (uid[7] == 0xE0 && uid[6] == 0x07) {
        if (verbose) {
            PrintAndLogEx(INFO, "覆盖 OPTION 参数，写入 TI 标签");
        }
        add_option = true;
    }
...
static int CmdHF15Restore(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 恢复",
                  "此命令将转储文件（bin/eml/json）的内容恢复到ISO-15693标签上",
                  "hf 15 restore\n"
                  "hf 15 restore -*\n"
    ...
    uint8_t arglen = arg_add_default(argtable);
    argtable[arglen++] = arg_str0("f", "file", "<fn>", "指定转储文件的文件名");
    argtable[arglen++] = arg_int0("r", "retry", "<dec>", "重试次数（默认3）");
    argtable[arglen++] = arg_lit0("v", "详细", "详细输出");
    argtable[arglen++] = arg_param_end;
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    while (bytes < (tag->pagesCount * tag->bytesPerPage)) {
        ...
        if (tried >= retries) {
            ...
            free(tag);
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(FAILED, "重试次数过多（" _RED_("fail") " )");
            DropField();
            return retval;
        }
    ...
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 15 dump --ns") "` to verify");
    PrintAndLogEx(INFO, "完成！");
    return PM3_SUCCESS;
}
...
static int CmdHF15CSetUID(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("u", "uid", "<hex>", "UID，8 个十六进制字节"),
        arg_lit0("2", "v2", "使用 gen2 魔术命令"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF15SlixEASEnable(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 启用SLIX EAS",
                  "在SLIX ISO-15693标签上启用EAS模式",
                  "hf 15 slixeasenable -p 0F0F0F0F");

    void *argtable[] = {
        arg_param_begin,
        arg_str0("p", "pwd", "<hex>", "可选密码，4个十六进制字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    struct {
    ...
    if (pwdlen > 0) {
        ...
        PrintAndLogEx(INFO, "正在尝试不使用密码启用EAS模式");
...
static int CmdHF15SlixEASDisable(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 禁用SLIX EAS",
                  "禁用SLIX ISO-15693标签的EAS模式",
...
static int CmdHF15SlixDisable(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 禁用SLIX隐私",
                  "禁用SLIX ISO-15693标签的隐私模式",
                  "hf 15 slixprivacydisable -p 0F0F0F0F");

    void *argtable[] = {
        arg_param_begin,
        arg_str1("p", "pwd", "<hex>", "密码，4个十六进制字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    struct {
...
static int CmdHF15SlixEnable(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 启用SLIX隐私",
                  "在SLIX ISO-15693标签上启用隐私模式",
...
static int CmdHF15SlixWritePassword(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 SLIX写入密码",
                  "在SLIX系列ISO-15693标签上写入密码。"
                  "Some tags do not support all different password types.",
                  "hf 15 slixwritepwd -t READ -o 00000000 -n 12131415");

    void *argtable[] = {
        arg_param_begin,
        arg_str1("t", "type", "<read|write|privacy|destroy|easafi>", "要写入哪个密码字段"),
        arg_str0("o", "old", "<hex>", "旧密码（如果存在），4个十六进制字节"),
        arg_str1("n", "new", "<hex>", "新密码，4个十六进制字节"),
        arg_param_end
    };
    ...
    if (pwdlen > 0 && pwdlen != 4) {
        PrintAndLogEx(WARNING, "如果提供，旧密码必须是4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    if (pwdlen != 4) {
        PrintAndLogEx(WARNING, "新密码必须是 4 个十六进制字节");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    if (vlen > 0) {
        if (strcmp(value, "read") == 0) {
            PrintAndLogEx(SUCCESS, "已选择读取密码");
            payload.pwd_id = 0x01;
        } else if (strcmp(value, "write") == 0) {
            PrintAndLogEx(SUCCESS, "已选择写入密码");
            payload.pwd_id = 0x02;
        } else if (strcmp(value, "privacy") == 0) {
            PrintAndLogEx(SUCCESS, "已选择隐私密码");
            payload.pwd_id = 0x04;
        } else if (strcmp(value, "destroy") == 0) {
            PrintAndLogEx(SUCCESS, "已选择销毁密码");
            payload.pwd_id = 0x08;
        } else if (strcmp(value, "easafi") == 0) {
            PrintAndLogEx(SUCCESS, "已选择 easafi 密码");
            payload.pwd_id = 0x10;
        } else {
            PrintAndLogEx(ERR, "t 参数必须是 'read', 'write', 'privacy', 'destroy' 或 'easafi'");
            return PM3_EINVARG;
        }
    }
...
static int CmdHF15SlixProtectPage(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 SLIX保护页",
                  "定义用户内存的保护指针地址和页面的访问条件",
                  "hf 15 slixprotectpage -w deadbeef -p 3 -h 3");

    void *argtable[] = {
        arg_param_begin,
        arg_str0("r", "readpw", "<hex>", "读取密码，4个十六进制字节"),
        arg_str0("w", "writepw", "<hex>", "写入密码，4个十六进制字节"),
        arg_int0("p", "ptr", "<dec>", "保护指针页（0-78），若为0则保护整个用户内存"),
        arg_int1("l", "lo", "<dec>", "低页的页保护标志(0-无, 1-读保护, 2-写保护)"),
        arg_int1("i", "hi", "<dec>", "高页的页保护标志(0-无, 1-读保护, 2-写保护)"),
        arg_param_end
    };
    ...
    if (pwdlen > 0 && pwdlen != 4) {
        PrintAndLogEx(WARNING, "如果提供，读取密码必须是 4 个十六进制字节");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    if (pwdlen > 0 && pwdlen != 4) {
        PrintAndLogEx(WARNING, "如果提供，写入密码必须为4个十六进制字节");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    if (payload.divide_ptr > 78) {
        PrintAndLogEx(WARNING, "保护指针页无效（当前为 %d，应 <=78）。", payload.divide_ptr);
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    if (pwdlen > 3) {
        PrintAndLogEx(WARNING, "页保护标志必须在0到3之间");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
...
static int CmdHF15AFIPassProtect(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 密码保护AFI",
                  "此命令启用 AFI 的密码保护。\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("p", "pwd", "<hex>", "EAS/AFI密码，4个十六进制字节"),
    ...
    if (pwdlen != 4) {
        PrintAndLogEx(WARNING, "密码必须是4个十六进制字节");
        return PM3_ESOFT;
    }

    if (force == false) {
        PrintAndLogEx(WARNING, "使用 `--force` 标志覆盖。注意！不可逆命令");
        return PM3_ESOFT;
    }

    PrintAndLogEx(INFO, "正在尝试启用AFI密码保护...");
...
static int CmdHF15EASPassProtect(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 密码保护EAS",
                  "此命令启用 EAS 的密码保护。\\n"
                  "*** OBS!  This action can not be undone! ***",
    ...
    PrintAndLogEx(INFO, "正在尝试启用EAS密码保护...");
...
static int CmdHF15View(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 查看",
                  "打印 ISO-15693 标签转储文件 (bin/eml/json)",
...
static int CmdHF15Wipe(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 擦除",
                  "通过用零填充内存来擦除ISO-15693标签",
                  "hf 15 wipe\n"
                 );
    void *argtable[6 + 3] = {0};
    uint8_t arglen = arg_add_default(argtable);
...
static command_t CommandTable[] = {
    {"-----------",         CmdHF15Help,              AlwaysAvailable, "----------------------- " _CYAN_("常规") " -----------------------"},
    {"help",                CmdHF15Help,              AlwaysAvailable, "此帮助"},
    {"list",                CmdHF15List,              AlwaysAvailable, "列出 ISO-15693 历史"},
    {"-----------",         CmdHF15Help,              AlwaysAvailable, "----------------------- " _CYAN_("操作") " -----------------------"},
    {"demod",               CmdHF15Demod,             AlwaysAvailable, "从标签解调ISO-15693"},
    {"dump",                CmdHF15Dump,              IfPm3Iso15693,   "读取ISO-15693标签的所有内存页面，保存到文件"},
    {"info",                CmdHF15Info,              IfPm3Iso15693,   "标签信息"},
    {"sniff",               CmdHF15Sniff,             IfPm3Iso15693,   "嗅探 ISO-15693 通信"},
    {"raw",                 CmdHF15Raw,               IfPm3Iso15693,   "向标签发送原始十六进制数据"},
    {"rdbl",                CmdHF15Readblock,         IfPm3Iso15693,   "读取一个块"},
    {"rdmulti",             CmdHF15Readmulti,         IfPm3Iso15693,   "读取多个块"},
    {"reader",              CmdHF15Reader,            IfPm3Iso15693,   "模拟 ISO-15693 读取器"},
    {"restore",             CmdHF15Restore,           IfPm3Iso15693,   "从文件恢复到ISO-15693标签的所有内存页"},
    {"samples",             CmdHF15Samples,           IfPm3Iso15693,   "作为读取器获取样本（启用载波，发送查询）"},
    {"view",                CmdHF15View,              AlwaysAvailable, "显示标签转储文件的内容"},
    {"wipe",                CmdHF15Wipe,              IfPm3Iso15693,   "擦除卡片为全零"},
    {"wrbl",                CmdHF15Write,             IfPm3Iso15693,   "写入一个块"},
    {"-----------",         CmdHF15Help,              IfPm3Iso15693,   "--------------------- " _CYAN_("Simulation") " ----------------------"},
    {"sim",                 CmdHF15Sim,               IfPm3Iso15693,   "伪造ISO-15693标签"},
    {"eload",               CmdHF15ELoad,             IfPm3Iso15693,   "上传文件到模拟器内存"},
    {"esave",               CmdHF15ESave,             IfPm3Iso15693,   "将模拟器内存保存到文件"},
    {"eview",               CmdHF15EView,             IfPm3Iso15693,   "查看模拟器内存"},
    {"-----------",         CmdHF15Help,              IfPm3Iso15693,   "------------------------ " _CYAN_("SLIX") " -------------------------"},
    {"slixwritepwd",        CmdHF15SlixWritePassword, IfPm3Iso15693,   "在SLIX ISO-15693标签上写入密码"},
    {"slixeasdisable",      CmdHF15SlixEASDisable,    IfPm3Iso15693,   "禁用SLIX ISO-15693标签的EAS模式"},
    {"slixeasenable",       CmdHF15SlixEASEnable,     IfPm3Iso15693,   "在SLIX ISO-15693标签上启用EAS模式"},
    {"slixprivacydisable",  CmdHF15SlixDisable,       IfPm3Iso15693,   "禁用SLIX ISO-15693标签的隐私模式"},
    {"slixprivacyenable",   CmdHF15SlixEnable,        IfPm3Iso15693,   "在SLIX ISO-15693标签上启用隐私模式"},
    {"slixprotectpage",     CmdHF15SlixProtectPage,   IfPm3Iso15693,   "保护SLIX ISO-15693标签上的页面"},
    {"passprotectafi",      CmdHF15AFIPassProtect,    IfPm3Iso15693,   "密码保护AFI - 不可撤销"},
    {"passprotecteas",      CmdHF15EASPassProtect,    IfPm3Iso15693,   "密码保护EAS - 不可撤销"},
    {"-----------",         CmdHF15Help,              IfPm3Iso15693,  "-------------------------- " _CYAN_("afi") " ------------------------"},
    {"findafi",             CmdHF15FindAfi,           IfPm3Iso15693,   "暴力破解ISO-15693标签的AFI"},
    {"writeafi",            CmdHF15WriteAfi,          IfPm3Iso15693,   "在ISO-15693标签上写入AFI"},
    {"writedsfid",          CmdHF15WriteDsfid,        IfPm3Iso15693,   "在ISO-15693标签上写入DSFID"},
    {"-----------",         CmdHF15Help,              IfPm3Iso15693,  "------------------------- " _CYAN_("Magic") " -----------------------"},
    {"csetuid",             CmdHF15CSetUID,           IfPm3Iso15693,   "为魔术卡设置UID"},
    {NULL, NULL, NULL, NULL}
};
