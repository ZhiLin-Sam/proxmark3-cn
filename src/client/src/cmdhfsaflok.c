...
static inline uint8_t get_days_in_month(uint16_t year, uint8_t month) {
    if (month < 1 || month > 12) { // programmer error ... this function uses 1-based indexing!
        PrintAndLogEx(ERR, "无效月份查询：%d，预期范围[1..12]...返回0天！", month);
        return 0;
    }
...
static inline bool is_saflok_mfc_datetime_valid(const saflok_mfc_datetime_t *date) {
    ...
    if ((date->year < 1980) || (date->year > (1980u+0xFFu))) {
        PrintAndLogEx(WARNING, "年份超出范围 (%d)", date->year);
        result = false;
    }
    if ((date->month < 1) || (date->month > 12)) {
        PrintAndLogEx(WARNING, "月份超出范围 (%d)", date->month);
        result = false;
    }
    uint8_t max_days = get_days_in_month(date->year, date->month);
    if ((date->day < 1u) || (date->day > max_days)) {
        PrintAndLogEx(WARNING, "月份 %d 年份 %d 的日期 (%d) 超出范围", date->day, date->month, date->year);
        result = false;
    }
    if (date->hour > 23) {
        PrintAndLogEx(WARNING, "小时超出范围 (%d)", date->hour);
        result = false;
    }
    if (date->minute > 59) {
        PrintAndLogEx(WARNING, "分钟超出范围 (%d)", date->minute);
        result = false;
    }
    return result;
}
static inline saflok_mfc_datetime_t get_saflok_mfc_card_creation_datetime(
    ...
    if (!is_saflok_mfc_datetime_valid(&date)) {
        PrintAndLogEx(WARNING, "警告：创建日期似乎无效：%04d-%02d-%02dT%02d:%02d",
...
static inline uint8_t _get_saflok_mfc_interval_months_impl(const saflok_mfc_data_t *data) {
    ...
    if (months >= 12u) {
        PrintAndLogEx(WARNING, "警告：间隔月数似乎无效（%u）", months);
    }
    return months;
}
...
static inline uint8_t _get_saflok_mfc_interval_days_impl(const saflok_mfc_data_t *data) {
    ...
    if (days > 31u) {
        PrintAndLogEx(WARNING, "警告：间隔天数似乎无效（%u）", days);
    }
    return days;
}
...
static inline uint8_t _get_saflok_mfc_interval_hours_impl(const saflok_mfc_data_t *data) {
    ...
    if (hours > 23u) {
        PrintAndLogEx(WARNING, "警告：间隔小时数似乎无效（%u）", hours);
    }
    return hours;
}
...
static inline uint8_t _get_saflok_mfc_interval_minutes_impl(const saflok_mfc_data_t *data) {
    ...
    if (minutes > 59u) {
        PrintAndLogEx(WARNING, "警告：间隔分钟数似乎无效（%u）", minutes);
    }
    return minutes;
}
...
static saflok_mfc_datetime_t add_offset(
    ...
    PrintAndLogEx(MY_DEBUG_LEVEL, "基准日期：%04u-%02u-%02uT%02u:%02u",
                  base->year, base->month, base->day,
                  base->hour, base->minute
                 );
    PrintAndLogEx(MY_DEBUG_LEVEL, "作为struct tm: %04d-%02d-%02dT%02d:%02d",
                  tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                  tm.tm_hour, tm.tm_min
                 );
    PrintAndLogEx(MY_DEBUG_LEVEL, "添加偏移: %u年 %u月 %u天 %u小时 %u分钟",
    ...
    PrintAndLogEx(MY_DEBUG_LEVEL, "带偏移（预归一化）：%04d-%02d-%02dT%02d:%02d",
                  tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                  tm.tm_hour, tm.tm_min
                 );
    ...
    if (tm.tm_min >= 60) {
        ...
        PrintAndLogEx(MY_DEBUG_LEVEL, "标准化分钟: %04d-%02d-%02dT%02d:%02d",
                      tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                      tm.tm_hour, tm.tm_min
                     );
    ...
    if (tm.tm_hour >= 24) {
        ...
        PrintAndLogEx(MY_DEBUG_LEVEL, "标准化小时: %04d-%02d-%02dT%02d:%02d",
                      tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                      tm.tm_hour, tm.tm_min
                     );
    }
    ...
    for (uint8_t current_days_in_month = get_days_in_month(tm.tm_year + 1900, tm.tm_mon + 1);
        ...
        PrintAndLogEx(MY_DEBUG_LEVEL, "标准化天数: %04d-%02d-%02dT%02d:%02d",
                      tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                      tm.tm_hour, tm.tm_min
                     );
    }
    if (tm.tm_mon >= 12) {
        ...
        PrintAndLogEx(MY_DEBUG_LEVEL, "标准化月份: %04d-%02d-%02dT%02d:%02d",
                      tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                      tm.tm_hour, tm.tm_min
                     );
    }
    PrintAndLogEx(MY_DEBUG_LEVEL, "最终标准化日期：%04d-%02d-%02dT%02d:%02d",
                  tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                  tm.tm_hour, tm.tm_min
                 );
    ...
    PrintAndLogEx(MY_DEBUG_LEVEL, "已转换回 saflok_mfc_datetime_t: %04d-%02d-%02dT%02d:%02d",
...
static saflok_mfc_datetime_offset_t get_datetime_offset(
    ...
    PrintAndLogEx(MYDBG, "计算从 %04u-%02u-%02uT%02u:%02u 到 %04u-%02u-%02uT%02u:%02u 的偏移量",
    ...
    PrintAndLogEx(MYDBG, "初始差值: %d 年, %d 月, %d 天, %d 小时, %d 分钟",
    ...
    if (diff.tm_mday < 0) {
        ...
        PrintAndLogEx(MYDBG, "从年份 %04u 的月份 %02u 借用天数，该月有 %02u 天",
    ...
    PrintAndLogEx(MYDBG, "最终偏移：%d 年，%d 月，%d 天，%d 小时，%d 分钟",
...
static saflok_mfc_datetime_t get_saflok_mfc_card_expiration_datetime(
    ...
    PrintAndLogEx(MY_DEBUG_LEVEL, "卡片创建日期: %04u-%02u-%02uT%02u:%02u",
                  creation_date.year, creation_date.month, creation_date.day,
                  creation_date.hour, creation_date.minute
                 );
    PrintAndLogEx(MY_DEBUG_LEVEL, "卡片间隔偏移: %u 年, %u 月, %u 天, %u 小时, %u 分钟",
                  offset.years, offset.months, offset.days,
                  offset.hours, offset.minutes
                 );
    saflok_mfc_datetime_t expiration = add_offset(&creation_date, offset);
    PrintAndLogEx(MY_DEBUG_LEVEL, "计算到期日期: %04u-%02u-%02uT%02u:%02u",
...
static uint32_t get_bitfield(const saflok_mfc_data_t *data, size_t start_bit, size_t num_bits) {
    ...
    if (start_bit >= total_available_bits) {
        ...
        PrintAndLogEx(ERR, "get_bitfield: 越界访问 (start_bit=%zu, total_available_bits=%zu)", start_bit, total_available_bits);
        return 0;
    }
    if (num_bits > 32) {
        // Exceeds maximum supported bit extraction
        PrintAndLogEx(ERR, "get_bitfield: 越界访问 (num_bits=%zu, max=32)", num_bits);
        return 0;
    }
    if (total_available_bits - start_bit < num_bits) {
        // Out of bounds access
        PrintAndLogEx(ERR, "get_bitfield: 越界访问 (start_bit=%zu + num_bits=%zu > total_available_bits=%zu)", start_bit, num_bits, total_available_bits);
        return 0;
    }
...
static bool set_bitfield(saflok_mfc_data_t *data, size_t start_bit, size_t num_bits, uint32_t value) {
    ...
    if (start_bit >= total_available_bits) {
        // Out of bounds access
        PrintAndLogEx(ERR, "set_bitfield: 越界访问（start_bit=%zu, total_available_bits=%zu）", start_bit, total_available_bits);
        return false;
    }
    if (num_bits > 32) {
        // Exceeds maximum supported bit extraction
        PrintAndLogEx(ERR, "set_bitfield: num_bits超过32");
        return false;
    }
    if (total_available_bits - start_bit < num_bits) {
        // Out of bounds access
        PrintAndLogEx(ERR, "set_bitfield: 越界访问（start_bit=%zu + num_bits=%zu > total_available_bits=%zu）", start_bit, num_bits, total_available_bits);
        return false;
    }
    if ((num_bits < 32) && ((value >> num_bits) != 0)) {
        // Value exceeds the size of the specified bit field
        PrintAndLogEx(ERR, "set_bitfield: 值超出位域大小（value=%u (0x%08x), num_bits=%zu）", value, value, num_bits);
        return false;
    }
...
static int CmdHFSaflokRead(const char *Cmd) {
    ...
    if (res == PM3_SUCCESS) {
        PrintAndLogEx(SUCCESS, "找到有效的Saflok卡！");
    } else {
        PrintAndLogEx(FAILED, "无效的 Saflok 卡");
        return PM3_EFAILED;
    }
    ...
    PrintAndLogEx(INFO,    "明文数据：%s", sprint_hex_inrow(decrypted.raw, sizeof(saflok_mfc_data_t)));
...
static int CmdHFSaflokDecode(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", NULL, "数据", "Encrypted 17 byte card data"),
        arg_param_end
    };
    ...
    if (dlen != 17) {
        PrintAndLogEx(WARNING, "saflok数据必须包含17个十六进制字节，实际得到%i", dlen);
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(INFO,    "明文数据：%s", sprint_hex_inrow(decrypted.raw, sizeof(saflok_mfc_data_t)));
    PrintAndLogEx(INFO,    "加密数据: %s", sprint_hex_inrow(encrypted.raw, sizeof(saflok_mfc_data_t)));
    saflok_decode(&decrypted);
...
static int CmdHFSaflokModify(const char *Cmd) {
    ...
    if (dlen != 17) {
        PrintAndLogEx(WARNING, "块数据必须包含17个十六进制字节。得到 %i", dlen);
        return PM3_EINVARG;
    }
...
static int CmdHFSaflokEncrypt(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", NULL, "数据", "17-byte unencrypted hex block"),
        arg_param_end
    };
    ...
    if (len != 17) {
        PrintAndLogEx(WARNING, "预期17字节。实际 %d。", len);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
...
static int CmdHFSaflokDecrypt(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", NULL, "数据", "17-byte encrypted hex block"),
        arg_param_end
    };
...
static int CmdHFSaflokSelfTest(const char *Cmd) {
    ...
    PrintAndLogEx(WARNING, "Saflok自检非穷举，仅4字节MFC。");
    ...
    if (true) {
        ...
        if (result_bitfield == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_bitfield通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint8_t i = 0; i < 16; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_card_level失败，值%u", i);
                result_card_level = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_card_level(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_card_level 返回 %u，预期 %u", x, i);
                    result_card_level = PM3_EFAILED;
                }
            }
        }
        if (result_card_level == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_card_level通过");
        } else {
            result = PM3_EFAILED;
        }
    ...
    if (true) {
        ...
        for (uint8_t i = 0; i < 16; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_card_type失败，值%u", i);
                result_card_type = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_card_type(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_card_type 返回 %u，预期 %u", x, i);
                    result_card_type = PM3_EFAILED;
                }
            }
        }
        if (result_card_type == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_card_type通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint16_t i = 0; i < 256; ++i) {
            ...
            if (!success) {
                ...
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_card_id 返回 %u，预期 %u", x, i);
                    result_card_id = PM3_EFAILED;
                }
            }
        }
        if (result_card_id == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_card_id通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint8_t i = 0; i < 4; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_opening_key失败，值%u", i);
                result_opening_key = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_opening_key(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_opening_key 返回 %u，预期 %u", x, i);
                    result_opening_key = PM3_EFAILED;
                }
            }
        }
        if (result_opening_key == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_opening_key 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint16_t i = 0; i < 16384; i += 11) { // step by prime to test various bits
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_lock_id失败，值%u", i);
                result_lock_id = PM3_EFAILED;
            } else {
                uint16_t x = get_saflok_mfc_lock_id(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_lock_id 返回 %u，预期 %u", x, i);
                    result_lock_id = PM3_EFAILED;
                }
            }
        }
        if (result_lock_id == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_lock_id 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint16_t i = 0; i < 4096; i += 7) { // step by prime to test various bits
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_pass_number失败，值%u", i);
                result_pass_number = PM3_EFAILED;
            } else {
                uint16_t x = get_saflok_mfc_pass_number(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_pass_number 返回 %u，预期 %u", x, i);
                    result_pass_number = PM3_EFAILED;
                }
            }
        }
        if (result_pass_number == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_pass_number 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint16_t i = 0; i < 4096; i += 5) { // step by prime to test various bits
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_sequence_and_combination失败，值%u", i);
                result_seq_combo = PM3_EFAILED;
            } else {
                uint16_t x = get_saflok_mfc_sequence_and_combination(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_sequence_and_combination 返回 %u，预期 %u", x, i);
                    result_seq_combo = PM3_EFAILED;
                }
            }
        }
        if (result_seq_combo == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_sequence_and_combination 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint8_t i = 0; i < 2; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_deadbolt_override失败，值%u", i);
                result_deadbolt = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_deadbolt_override(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_deadbolt_override 返回 %u，预期 %u", x, i);
                    result_deadbolt = PM3_EFAILED;
                }
            }
        }
        if (result_deadbolt == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_deadbolt_override通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint8_t i = 0; i < 128; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_restricted_days失败，值%u", i);
                result_restricted_days = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_restricted_days(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_restricted_days 返回 %u，预期 %u", x, i);
                    result_restricted_days = PM3_EFAILED;
                }
            }
        }
        if (result_restricted_days == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_restricted_days 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (uint16_t i = 0; i < 4096; i += 13) { // step by prime to test various bits
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_property_id失败，值%u", i);
                result_property_id = PM3_EFAILED;
            } else {
                uint16_t x = get_saflok_mfc_property_id(&testdata);
                if (x != i) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_property_id 返回 %u，预期 %u", x, i);
                    result_property_id = PM3_EFAILED;
                }
            }
        }
        if (result_property_id == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_property_id 通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    if (true) {
        ...
        for (size_t i = 0; i < 0x100; ++i) {
            ...
            if (!success) {
                PrintAndLogEx(FAILED, "set_saflok_mfc_checksum失败，值%02x", value);
                result_checksum = PM3_EFAILED;
            } else {
                uint8_t x = get_saflok_mfc_checksum(&testdata);
                if (x != value) {
                    PrintAndLogEx(FAILED, "get_saflok_mfc_checksum 返回 %02x，预期 %02x", x, value);
                    result_checksum = PM3_EFAILED;
                }
            }
        }
        if (result_checksum == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "get/set_saflok_mfc_checksum通过");
        } else {
            result = PM3_EFAILED;
        }
    }
    ...
    if (true) {
        ...
        for (size_t i = 0; i < ARRAYLEN(saflok_test_cases); ++i) {
            ...
            if (memcmp(tmp.raw, tc->encoded.raw, sizeof(saflok_mfc_data_t)) != 0) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 加密与预期数据不匹配。", i);
            ...
            if (memcmp(tmp.raw, tc->decoded.raw, sizeof(saflok_mfc_data_t)) != 0) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 解密与预期数据不匹配。", i);
                PrintAndLogEx(DEBUG,  "               raw result: %s", sprint_hex_inrow(tmp.raw, sizeof(saflok_mfc_data_t)));
            ...
            if (memcmp(&tmp_dt, &tc->card_creation_datetime, sizeof(saflok_mfc_datetime_t)) != 0) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 卡片创建日期时间与预期数据不匹配。", i);
            ...
            if (memcmp(&tmp_dt, &tc->card_expiration_datetime, sizeof(saflok_mfc_datetime_t)) != 0) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 卡片过期日期时间与预期数据不匹配。", i);
                PrintAndLogEx(DEBUG,  "               api result: %4u-%02u-%02u %02u:%02u",
                              tmp_dt.year, tmp_dt.month, tmp_dt.day,
                              tmp_dt.hour, tmp_dt.minute);
                PrintAndLogEx(DEBUG,  "          expected result: %4u-%02u-%02u %02u:%02u",
            ...
            if (memcmp(&tmp_offset, &tc->card_interval, sizeof(saflok_mfc_datetime_offset_t)) != 0) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 卡片间隔与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_lock_id(&tc->decoded) != tc->lock_id) {
                PrintAndLogEx(FAILED, "测试用例 %zu: lock_id 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_pass_number(&tc->decoded) != tc->pass_number) {
                PrintAndLogEx(FAILED, "测试用例 %zu: pass_number 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_sequence_and_combination(&tc->decoded) != tc->sequence_and_combination) {
                PrintAndLogEx(FAILED, "测试用例 %zu: sequence_and_combination 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_property_id(&tc->decoded) != tc->property_id) {
                PrintAndLogEx(FAILED, "测试用例 %zu: property_id 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_card_level(&tc->decoded) != tc->card_level) {
                PrintAndLogEx(FAILED, "测试用例 %zu: card_level 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_card_type(&tc->decoded) != tc->card_type) {
                PrintAndLogEx(FAILED, "测试用例 %zu: card_type 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_card_id(&tc->decoded) != tc->card_id) {
                PrintAndLogEx(FAILED, "测试用例 %zu: card_id 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_opening_key(&tc->decoded) != tc->opening_key) {
                PrintAndLogEx(FAILED, "测试用例 %zu: opening_key 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_restricted_days(&tc->decoded) != tc->restricted_days) {
                PrintAndLogEx(FAILED, "测试用例 %zu: restricted_days 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_deadbolt_override(&tc->decoded) != tc->deadbolt_override) {
                PrintAndLogEx(FAILED, "测试用例 %zu: deadbolt_override 与预期数据不匹配。", i);
            ...
            if (get_saflok_mfc_checksum(&tc->decoded) != tc->checksum) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 存储的校验和与预期数据不匹配。", i);
            ...
            if (checksum != tc->checksum) {
                PrintAndLogEx(FAILED, "测试用例 %zu: 计算出的校验和与预期数据不匹配。", i);
        ...
        if (result_test_cases == PM3_SUCCESS) {
            PrintAndLogEx(SUCCESS, "所有预定义测试用例通过。");
        } else {
            result = PM3_EFAILED;
        }
    }
...
static int CmdHFSaflokChecksum(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", NULL, "数据", "16-byte decrypted Saflok block"),
        arg_param_end
    };
    ...
    if (len != 16) {
        PrintAndLogEx(WARNING, "预期16字节。实际 %d。", len);
        CLIParserFree(ctx);
        return PM3_EINVARG;
    }
...
static int CmdHFSaflokProvision(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", NULL, "数据", "17-byte block"),
        arg_param_end,
    };
    ...
    if (mf_read_uid(uid, &uid_len, NULL) != PM3_SUCCESS || uid_len < 4) {
        PrintAndLogEx(WARNING, "从卡片读取 UID 失败。");
        CLIParserFree(ctx);
        return PM3_ESOFT;
    }
    ...
    uint8_t set_keys = 0;
    if (!write_success) {
        PrintAndLogEx(WARNING, "初始写入失败。正在尝试设置扇区0密钥...");
        ...
        if (mf_write_block(3, 1, all_F, trailer0) != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "在扇区 0 设置密钥失败。请先擦除卡片。");
            CLIParserFree(ctx);
            return PM3_ESOFT;
        }
        ...
        write_success = mf_write_block(1, 0, keyA.key, block1) == PM3_SUCCESS &&
                        mf_write_block(2, 0, keyA.key, block2) == PM3_SUCCESS;
        if (!write_success) {
            PrintAndLogEx(WARNING, "设置密钥后仍然写入失败。");
            CLIParserFree(ctx);
            return PM3_ESOFT;
        }
        set_keys = 1;
    ...
    PrintAndLogEx(SUCCESS, "Saflok卡配置成功。");
    CLIParserFree(ctx);
    return PM3_SUCCESS;
}
...
static int CmdHFSaflokInterrogate(const char *Cmd) {
    ...
    if (mf_read_uid(uid, &uid_len, NULL) != PM3_SUCCESS || uid_len < 4) {
        PrintAndLogEx(WARNING, "读取 UID 失败。");
        return PM3_ESOFT;
    }
    ...
    if (mf_read_block(2, 0, key.key, block2) != PM3_SUCCESS) {
        PrintAndLogEx(WARNING, "使用派生密钥读取块 2 失败。");
        return PM3_ESOFT;
    }
    ...
    if (subblock_stop == 0) {
        ...
        PrintAndLogEx(SUCCESS, "卡片没有可变密钥");
        return PM3_EINVARG;
    }
    ...
    while (1) {
        ...
        if (mf_read_block(current_block, 0, key.key, buffer + total_bytes) != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "读取块 %d 失败", current_block);
            break;
        }
    ...
    if (total_bytes > 0) {
        PrintAndLogEx(SUCCESS, "卡片对以下锁具有可变密钥:");
    } else {
        PrintAndLogEx(SUCCESS, "卡片没有可变密钥");
    }
    int cursor = 0;
...
static command_t CommandTable[] = {
    ...
    {"read",  CmdHFSaflokRead,  IfPm3NfcBarcode, "读取Saflok卡"},
    {"provision",  CmdHFSaflokProvision,  IfPm3NfcBarcode, "配置Saflok卡"},
    {"encode",  CmdHFSaflokEncode,  AlwaysAvailable, "编码Saflok卡数据"},
    {"decode",  CmdHFSaflokDecode,  AlwaysAvailable, "解码Saflok卡片数据"},
    {"modify",  CmdHFSaflokModify,  AlwaysAvailable, "修改Saflok卡片数据"},
    {"encrypt", CmdHFSaflokEncrypt, AlwaysAvailable, "加密17字节解密块"},
    {"decrypt", CmdHFSaflokDecrypt, AlwaysAvailable, "解密17字节加密块"},
    {"interrogate", CmdHFSaflokInterrogate, IfPm3NfcBarcode, "查询saflok卡"},
    {"cksum",   CmdHFSaflokChecksum, IfPm3NfcBarcode, "生成数据块的校验和"},
    {"selftest", CmdHFSaflokSelfTest, AlwaysAvailable, "运行自检"},
    {NULL, NULL, NULL, NULL}
};
