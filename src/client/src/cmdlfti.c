...
int demodTI(bool verbose) {
    ...
    */
    ...
    PrintAndLogEx(DEBUG, "实际数据位从样本 %d 开始", maxPos);
    PrintAndLogEx(DEBUG, "长度 %d/%d", highLen, lowLen);
    ...
    PrintAndLogEx(DEBUG, "TI标签：原始标签位 | %s", bits);

    TagType = (shift3 >> 8) & 0xFF;
    if (TagType != ((shift0 >> 16) & 0xFF)) {
        PrintAndLogEx(DEBUG, "TI标签: 错误: 起始位和停止位不匹配!");
        goto out;
    } else if (TagType == 0x7E) {
        PrintAndLogEx(INFO, "检测到只读 TI 标签。");
        retval = PM3_SUCCESS;
        goto out;
    } else if (TagType == 0xFE) {
        PrintAndLogEx(INFO, "检测到可重写 TI 标签。");
        ...
        if ((shift3 ^ shift0) & 0x7FFF) {
            PrintAndLogEx(WARNING, "警告：标识不匹配！");
        }
        ...
        if (calccrc != (shift2 & 0xFFFF))
            PrintAndLogEx(WARNING, "警告：CRC不匹配，计算值为%04X，实际为%04X", calccrc, shift2 & 0xFFFF);

        retval = PM3_SUCCESS;
        goto out;
    } else {
        PrintAndLogEx(WARNING, "未知标签类型。");
    }
...
static int CmdTIDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频TI解调",
                  "尝试查找TI前导码，如果找到则解码/解扰数据",
                  "低频TI解调"
                 );
...
static int CmdTIWrite(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("r", "raw", "<hex>", "原始十六进制数据，最多8字节"),
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdTIDemod,   AlwaysAvailable, "从GraphBuffer解调TI低频标签的原始比特"},
    {"reader",  CmdTIReader,  IfPm3Lf,         "读取并解码TI 134 kHz标签"},
    {"write",   CmdTIWrite,   IfPm3Lf,         "向可读写TI 134 kHz标签写入新数据"},
    {NULL, NULL, NULL, NULL}
};
