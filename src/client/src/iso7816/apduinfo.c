...
void APDUPrintEx(APDU_t apdu, size_t maxdatalen) {
    ...
    if (maxdatalen > 0)
        PrintAndLogEx(INFO, "数据：%s%s", sprint_hex(apdu.data, MIN(apdu.lc, maxdatalen)), apdu.lc > maxdatalen ? "..." : "");
}
...
void SAPDUPrint(sAPDU_t apdu, size_t maxdatalen) {
    ...
    PrintAndLogEx(INFO, "数据 { %s%s }", sprint_hex(apdu.data, len), apdu.Lc > len ? "..." : "");
}
