...
int hfgal_diversify_desfire_key(uint8_t *site_key, uint8_t *uid, uint8_t uid_len,
    ...
    if (site_key == NULL) {
        PrintAndLogEx(INFO, "hfgal_diversify_desfire_key 正在使用默认站点密钥");
...
static int select_aid(DesfireContext_t *ctx, uint32_t aid, bool verbose) {
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "已选择 AID %06X", aid);
    }
...
static uint32_t find_available_gallagher_aid(DesfireContext_t *ctx, bool verbose) {
    ...
    if (verbose) {
        // Print what we got
        PrintAndLogEx(INFO, "获取的 AID 列表：" NOLF);
...
static int hfgal_read_desfire_cad(DesfireContext_t *ctx, uint8_t *dest_buf,
    ...
    if (dest_buf_len < 3 * 36) {
        PrintAndLogEx(ERR, "hfgal_read_desfire_cad 目标缓冲区大小错误。收到长度 %d，必须至少为 %d",
                      dest_buf_len,
                      3 * 36
                     );
        return PM3_EINVARG;
    ...
    *num_entries_out = num_entries;
    ...
    if (num_entries == 0) {
        PrintAndLogEx(WARNING, "卡片应用目录为空");
    } else if (verbose) {
        // Print what we found
        // iceman maybe on seperate lines for easier reading.
        PrintAndLogEx(SUCCESS, "卡片应用目录包含:" NOLF);
...
static int hfgal_add_aid_to_cad(DesfireContext_t *ctx, uint8_t *key, bool should_diversify,
    ...
    if (aid_exists(ctx, DESFIRE_CAD_AID, false)) {
        if (verbose) {
            PrintAndLogEx(INFO, "卡片应用目录存在，正在读取条目...");
        }
        ...
        if (verbose) {
            PrintAndLogEx(INFO, "卡片应用目录不存在，正在创建...");
        }
    ...
    if (entry_num == 0) {
        if (verbose) {
            PrintAndLogEx(INFO, "正在CAD中创建新文件");
        }
...
static int hfgal_read_desfire_card(uint32_t aid, uint8_t *site_key, bool verbose, bool quiet) {
    ...
    for (uint16_t i = 0; i < num_entries * 6; i += 6) {
        ...
        if (res == HFGAL_AUTH_FAIL) {
            PrintAndLogEx(WARNING, "AID %06X的站点密钥无效", current_aid);
            continue;
        }
...
static int hfgal_write_site_specific_sector(uint8_t sector_num, GallagherCredentials_t *creds,
    ...
    if (mfNumBlocksPerSector(sector_num) > 4) {
        PrintAndLogEx(ERR, "仅支持4块扇区（扇区0-31）");
        return PM3_ENOTIMPL;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(ERR, "写入块 %d（扇区 %d，块 %d）失败", first_block + i, sector_num, i);
            return res;
        }
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入扇区尾部（扇区 %d）失败", sector_num);
        return res;
    }
...
static int hfgal_update_mad(uint8_t cred_sector, uint8_t cad_sector,
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "读取 MAD 扇区0失败");
        return res;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入 MAD 块 1 失败");
        return res;
    }

    res = mf_write_block(2, mad_key_type, mad_key, &sector0[2 * MFBLOCK_SIZE]);
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入 MAD 块 2 失败");
        return res;
    }

    if (verbose) {
        PrintAndLogEx(SUCCESS, "已更新 MAD 扇区 0");
    }
    return PM3_SUCCESS;
}
...
static int hfgal_write_classic_card(GallagherCredentials_t *creds, uint8_t cred_sector,
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "卡片UID: %s", sprint_hex(card.uid, card.uidlen));
    }
    ...
    if (res != PM3_SUCCESS) {
        if (verbose) {
            PrintAndLogEx(INFO, "Gallagher Key B 失败，尝试默认密钥...");
        }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "写入凭证扇区 %d 失败", cred_sector);
        return res;
    }
    ...
    if (update_mad) {
        ...
        if (res != PM3_SUCCESS) {
            if (verbose) {
                PrintAndLogEx(INFO, "MAD密钥B失败，尝试默认密钥...");
            }
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "更新 MAD 失败 - 凭据已写入但 MAD 未更新");
        }
    }
...
static int hfgal_read_site_specific_sector(uint8_t sector_num, GallagherCredentials_t cred, uint8_t *csn, uint8_t *diversified_site_key, bool verbose) {
    ...
    if (memcmp(block1, cardax_string, 16) != 0) {
        if (verbose) {
            PrintAndLogEx(FAILED, "扇区 %d 不包含 Gallagher 站点密钥块", sector_num);
        }
        return PM3_ENODATA;
    }
    ...
    if (block3[9] == 0x1D) {
        if (verbose) {
            PrintAndLogEx(INFO, "扇区 %d 包含 MIFARE 增强安全块", sector_num);
        }
...
static int CmdGallagherReader(const char *cmd) {
    ...
    CLIParserInit(&ctx, "hf gallagher 读取器",
    ...
    void *argtable[] = {
        ...
        arg_lit0("@",  "continuous",          "连续读取器模式"),
        arg_lit0(NULL, "apdu",                "显示 APDU 请求和响应"),
        arg_lit0("v",  "详细",             "详细输出"),
        arg_lit0("c", "classic",              "读取 Gallagher MIFARE Classic 卡"),
        arg_param_end
    };
...
static int CmdGallagherClone(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 克隆",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("c",   "classic",             "写入MIFARE Classic卡而非DESFire"),
        arg_int0("n",   "keynum",  "<dec>",    "DESFire：PICC密钥编号[默认=0]"),
        arg_str0("t",   "algo",    "<DES|2TDEA|3TDEA|AES>", "DESFire：PICC加密算法"),
        arg_str0("k",   "key",     "<hex>",    "DESFire：用于PICC认证的密钥"),
        arg_u64_1(NULL, "rc",      "<dec>",    "区域代码，最多 4 位"),
        arg_u64_1(NULL, "fc",      "<dec>",    "设施代码，最多2字节"),
        arg_u64_1(NULL, "cn",      "<dec>",    "卡号，最多3字节"),
        arg_u64_1(NULL, "il",      "<dec>",    "发行级别，最多4位"),
    ...
    if (classic_mode) {
        ...
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf gallagher reader -c") "` to verify");
        return PM3_SUCCESS;
    }
    ...
    if (aid_len == 0) {
        ...
        if (verbose) {
            PrintAndLogEx(INFO, "正在使用可用 AID: %06X", aid);
        }
    ...
    PrintAndLogEx(SUCCESS, "完成！");
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf gallagher 读取器") "` to verify");
    return PM3_SUCCESS;
}

static int CmdGallagherDelete(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 删除",
...
static int CmdGallagherDiversify(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 多样化",
                  "派生Gallagher密钥\\n"
                  "Specify site key is required if using non-default key",
    ...
    if (IfPm3Iso14443()) {
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf mfdes auth --aid %06X --keyno %d --algo AES --key %s") "` to verify",
                      aid,
                      key_num,
                      key_str
                     );
...
static int CmdGallagherDecode(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 解码",
                  "解码Gallagher凭证块。\\n"
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1(NULL, "数据", "<hex>", "Credential block (8 or 16 bytes)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, cmd, argtable, false);
...
static int CmdGallagherEncode(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 编码",
                  "编码Gallagher凭证块\\n"
                  "Credential block can be specified with or without the bitwise inverse.",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_u64_1("r", "rc",      "<dec>", "区域代码，最多 4 位"),
        arg_u64_1("f", "fc",      "<dec>", "设施代码，最多2字节"),
        arg_u64_1("c", "cn",      "<dec>", "卡号，最多3字节"),
        arg_u64_1("i", "il",      "<dec>", "发行级别，最多4位"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, cmd, argtable, false);
...
static int CmdGallagherTest(const char *cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf gallagher 测试",
...
static command_t CommandTable[] = {
    ...
    {"reader",       CmdGallagherReader,    IfPm3Iso14443,   "读取并解码DESFire或Classic卡上的所有Gallagher凭证"},
    {"clone",        CmdGallagherClone,     IfPm3Iso14443,   "将Gallagher凭证克隆到DESFire或Classic卡"},
    {"delete",       CmdGallagherDelete,    IfPm3Iso14443,   "从DESFire卡删除Gallagher凭证"},
    {"diversifykey", CmdGallagherDiversify, AlwaysAvailable, "多样化Gallagher密钥"},
    {"decode",       CmdGallagherDecode,    AlwaysAvailable, "解码Gallagher凭证块"},
    {"encode",       CmdGallagherEncode,    AlwaysAvailable, "编码Gallagher凭证块"},
    {"test",         CmdGallagherTest,      AlwaysAvailable, "测试 Gallagher Mifare Core 的功能"},
    {NULL, NULL, NULL, NULL}
};
