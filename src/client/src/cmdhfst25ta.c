...
static void print_st25ta_system_info(uint8_t *d, uint8_t n) {
    if (n < 0x12) {
        PrintAndLogEx(WARNING, "从系统文件读取的字节数不足");
        return;
    }
    ...
    if (d[2] != 0x80) {
        ...
        PrintAndLogEx(SUCCESS, "GPO 配置... 0x%02X", d[2]);
    ...
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(SUCCESS, "事件计数器配置.... 0x%02X", d[3]);
    ...
    */
    ...
    if (resplen != 32) {
        if ((resplen == 2) && (resp[0] == 0x69) && (resp[1] == 0x82)) {
            PrintAndLogEx(WARNING, "GetSignature: 安全状态不满足");
        }
        DropField();
        return PM3_ESOFT;
    }
...
static int infoHFST25TA(void) {
    ...
    sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "选择系统文件失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
    ...
    sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "读取系统文件失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static int CmdHFST25TASim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频ST25TA模拟",
                  "模拟 7 字节 UID 的 ST25TA512B 标签",
                  "hf st25ta sim -u 02E2007D0FCA4C\n");

    void *argtable[] = {
        arg_param_begin,
        arg_str1("u", "uid", "<hex>", "7字节UID"),
        arg_param_end
    };
    ...
    if (uidlen != 7) {
        PrintAndLogEx(ERR, "UID必须是7个十六进制字节");
        return PM3_EINVARG;
    }
...
int CmdHFST25TANdefRead(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频ST25TA NDEF读取",
                  "读取 ST25TA 上的 NFC 数据交换格式 (NDEF) 文件",
    ...
    if (pwdlen == 0) {
        ...
        if (pwdlen != 16) {
            PrintAndLogEx(ERR, "密码必须是16个十六进制字节");
            return PM3_EINVARG;
        }
        with_pwd = true;
    }
    ...
    if (with_pwd) {
        ...
        if (sw == 0x6300) {
            ...
            sw = get_sw(response, resplen);
            if (sw != ISO7816_OK) {
                PrintAndLogEx(ERR, "验证密码失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
                DropField();
                return PM3_ESOFT;
            }
        }
...
static int CmdHFST25TAProtect(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频ST25TA保护",
                  "更改ST25TA上NFC数据交换格式（NDEF）文件的读取或写入保护",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("e",  "enable",            "启用保护"),
        arg_lit0("d",  "disable",           "禁用保护（默认）"),
        arg_lit0("r",  "read",              "更改读保护"),
        arg_lit0("w",  "write",             "更改写保护（默认）"),
        arg_str1("p",  "密码", "<hex>", "16字节写入密码"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (enable_protection && disable_protection) {
        PrintAndLogEx(ERR, "必须指定启用或禁用保护，不能同时指定两者");
        return PM3_EINVARG;
    }
    ...
    if (read_protection && write_protection) {
        PrintAndLogEx(ERR, "必须指定读保护或写保护，不能同时指定两者");
        return PM3_EINVARG;
    }
    if (read_protection) {
        state[2] = 0x01;
    ...
    sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "更改保护失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }

    PrintAndLogEx(SUCCESS, " %s protection ( %s )", ((state[2] & 0x01) == 0x01) ? _YELLOW_("read") : _YELLOW_("write"),
                  ((state[0] & 0x28) == 0x28) ? _RED_("enabled") : _GREEN_("已禁用"));
...
static int CmdHFST25TAPwd(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "高频ST25TA密码",
                  "更改ST25TA上NFC数据交换格式（NDEF）文件的读取或写入密码",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("r", "read",              "更改读密码（默认）"),
        arg_lit0("w", "write",             "更改写密码"),
        arg_str1("p", "密码", "<hex>", "当前16字节写密码"),
        arg_str1("n", "new",      "<hex>", "新16字节密码"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (change_read_password && change_write_password) {
        PrintAndLogEx(ERR, "必须指定读或写，不能同时指定两者");
        return PM3_EINVARG;
    }
    ...
    if (pwdlen != 16) {
        PrintAndLogEx(ERR, "原始写入密码必须为16个十六进制字节");
        return PM3_EINVARG;
    }
    if (newpwdlen != 16) {
        PrintAndLogEx(ERR, "新密码必须是16个十六进制字节");
        return PM3_EINVARG;
    }
    ...
    sw = get_sw(response, resplen);
    if (sw != ISO7816_OK) {
        PrintAndLogEx(ERR, "密码更改失败 (%04x - %s)。", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        DropField();
        return PM3_ESOFT;
    }
...
static command_t CommandTable[] = {
    ...
    {"info",     CmdHFST25TAInfo,       IfPm3Iso14443a,  "标签信息"},
    {"list",     CmdHFST25TAList,       AlwaysAvailable, "列出ISO 14443A/7816历史"},
    {"ndefread", CmdHFST25TANdefRead,   AlwaysAvailable, "读取标签上的NDEF文件"},
    {"protect",  CmdHFST25TAProtect,    IfPm3Iso14443a,  "更改标签保护"},
    {"pwd",      CmdHFST25TAPwd,        IfPm3Iso14443a,  "更改标签密码"},
    {"sim",      CmdHFST25TASim,        IfPm3Iso14443a,  "伪造ISO 14443A/ST标签"},
    {NULL, NULL, NULL, NULL}
};
