...
serial_port uart_open(const char *pcPortName, uint32_t speed, bool slient) {
    ...
    if (isTCP || isUDP) {
        ...
        if (bindAddrPortStr != NULL) {
            *bindAddrPortStr = '\0'; // as the end of target address (and port)
            ...
            if (result != PM3_SUCCESS) {
                if (result == PM3_ESOFT) {
                    PrintAndLogEx(ERR, "错误：地址错误：bind 选项中的 [] 不匹配");
                } else {
                    PrintAndLogEx(ERR, "错误：解析绑定选项中的地址和端口失败");
                }
                free(addrPortStr);
                free(sp);
                return INVALID_SERIAL_PORT;
        ...
        if (result != PM3_SUCCESS) {
            if (result == PM3_ESOFT) {
                PrintAndLogEx(ERR, "错误：地址错误：[] 不匹配");
            } else {
                PrintAndLogEx(ERR, "错误：解析地址和端口失败");
            }
            free(addrPortStr);
            free(sp);
            return INVALID_SERIAL_PORT;
        ...
        if (s != 0) {
            PrintAndLogEx(ERR, "错误：getaddrinfo: %d: %s", s, gai_strerror(s));
            freeaddrinfo(addr);
            free(addrPortStr);
            free(sp);
            return INVALID_SERIAL_PORT;
        ...
        for (rp = addr; rp != NULL; rp = rp->ai_next) {
            ...
            if (!uart_bind(&sfd, bindAddrStr, bindPortStr, isBindingIPv6)) {
                PrintAndLogEx(ERR, "错误：无法绑定。errno: %d", errno);
                close(sfd);
                freeaddrinfo(addr);
                free(addrPortStr);
                free(sp);
    ...
    if (isBluetooth) {
        ...
        if (strlen(pcPortName) != 20) {
            PrintAndLogEx(ERR, "错误：蓝牙 MAC 地址长度错误");
            free(sp);
            return INVALID_SERIAL_PORT;
        }
        ...
        return sp;
#else // HAVE_BLUEZ
        PrintAndLogEx(ERR, "抱歉，此客户端不支持原生蓝牙地址");
        free(sp);
        return INVALID_SERIAL_PORT;
#endif // HAVE_BLUEZ
    }
    ...
    if (tcgetattr(sp->fd, &sp->tiOld) == -1) {
        PrintAndLogEx(ERR, "错误：UART 获取终端信息属性");
        uart_close(sp);
        return INVALID_SERIAL_PORT;
    }
    ...
    if (tcsetattr(sp->fd, TCSANOW, &sp->tiNew) == -1) {
        PrintAndLogEx(ERR, "错误：UART 设置终端信息属性");
    ...
    if (!uart_set_speed(sp, speed)) {
        ...
        if (!uart_set_speed(sp, speed)) {
            uart_close(sp);
            PrintAndLogEx(ERR, "设置波特率时 UART 错误");
            return INVALID_SERIAL_PORT;
        }
    }
...
int uart_send(const serial_port sp, const uint8_t *pbtTx, const uint32_t len) {
    ...
    while (pos < len) {
        ...
        // Write error
        if (res < 0) {
            PrintAndLogEx(ERR, "UART:: 写入错误 (%d)", res);
            return PM3_ENOTTY;
        }

        // Write time-out
        if (res == 0) {
            PrintAndLogEx(ERR, "UART:: 写入超时");
            return PM3_ETIMEOUT;
        }
