...
serial_port uart_open(const char *pcPortName, uint32_t speed, bool slient) {
    ...
    if (isTCP || isUDP) {
        ...
        if (iResult != 0) {
            PrintAndLogEx(ERR, "错误：WSAStartup 失败，错误：%d", iResult);
            free(addrPortStr);
            free(sp);
            return INVALID_SERIAL_PORT;
        }
        ...
        for (rp = addr; rp != NULL; rp = rp->ai_next) {
            ...
            if (!uart_bind(&hSocket, bindAddrStr, bindPortStr, isBindingIPv6)) {
                PrintAndLogEx(ERR, "错误：无法绑定。错误：%u", WSAGetLastError());
                closesocket(hSocket);
    ...
    if (!BuildCommDCBA("baud=115200 parity=N data=8 stop=1", &sp->dcb)) {
        uart_close(sp);
        PrintAndLogEx(WARNING, "UART 错误 CDC 设置\\n");
        return INVALID_SERIAL_PORT;
    }
    ...
    if (!SetCommState(sp->hPort, &sp->dcb)) {
        uart_close(sp);
        PrintAndLogEx(WARNING, "设置 COM 状态时 UART 错误\\n");
        return INVALID_SERIAL_PORT;
    }
    ...
    if (!uart_set_speed(sp, speed)) {
        ...
        if (!uart_set_speed(sp, speed)) {
            uart_close(sp);
            PrintAndLogEx(WARNING, "设置波特率时 UART 错误\\n");
            return INVALID_SERIAL_PORT;
        }
    }
