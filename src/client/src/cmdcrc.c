    ...
    if (width[0] == 0) { //reveng -D
        ...
        *count = mcount();
        ...
        if (~model.flags & P_MULXN) {
            PrintAndLogEx(WARNING, "无法搜索非 Williams 兼容模型");
            return 0;
        }
        ...
        if (uflags & C_NOBFS && ~rflags & R_HAVEP) {
            PrintAndLogEx(WARNING, "未找到模型");
            return 0;
        }

        if (!(model.flags & P_REFIN) != !(model.flags & P_REFOUT)) {
            PrintAndLogEx(WARNING, "无法搜索交叉字节序模型");
            return 0;
        }
            ...
            if (memcmp(result, inCRC, crcChars) == 0) {
                ...
                if (crcChars > 2) {
                    ...
                    if (memcmp(swapEndian, inCRC, crcChars) == 0) {
                        PrintAndLogEx(SUCCESS, "model... " _YELLOW_("%s"), Models[i]);
                        PrintAndLogEx(SUCCESS, "值字节序交换... %s\\n", swapEndian);
                        // optional - stop searching if found...
                        found = true;
                    }
                    free(swapEndian);
