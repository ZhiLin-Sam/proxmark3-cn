...
static int lf_hid_validate_packed_transport(const wiegand_input_t *input, const char *command_name) {
    ...
    if (input->packed_valid == false) {
        PrintAndLogEx(ERR, "凭证编码成功，但%" PRIuMAX "-bit Wiegand data cannot be represented as a packed HID credential", (uintmax_t)input->bin_len);
        PrintAndLogEx(ERR, "压缩 HID 编码支持最多 84 位 Wiegand 位");
        return PM3_EINVARG;
    }
    ...
    if (input->packed.Length > 37) {
        PrintAndLogEx(ERR, "%s 仅支持最多 37 位的打包凭证", command_name);
        return PM3_EINVARG;
    }
...
static int lf_hid_resolve_input(const lf_hid_cli_input_t *cli, wiegand_input_t *input, int *format_idx) {
    ...
    if (input_modes != 1) {
        PrintAndLogEx(ERR, "请精确使用 `--raw`、`--bin`、`--new` 或 `--wiegand/--fc/--cn` 之一");
        return PM3_EINVARG;
    }
...
static int CmdHIDDemod(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "低频 HID 解调",
                  "尝试查找HID Prox前导码，如果找到则解码/解扰数据",
                  "低频 HID 解调"
                 );
...
static int CmdHIDWatch(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "正在监听 HID Prox 卡片 - 将标签放在天线上");
...
static int CmdHIDSim(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_u64_0("i",    NULL,     "<dec>", "问题级别"),
        arg_u64_0("o",   "oem",     "<dec>", "OEM代码"),
    ...
    if (cli.bin_len > 96) {
        PrintAndLogEx(ERR, "二进制Wiegand字符串必须小于97位");
        return PM3_EINVARG;
    }
...
static int CmdHIDClone(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_u64_0(NULL, "fc",      "<dec>", "facility code"),
        arg_u64_0(NULL, "cn",      "<dec>", "card number"),
        arg_int0("i",    NULL,     "<dec>", "问题级别"),
        arg_int0("o",   "oem",     "<dec>", "OEM代码"),
        arg_str0("r",  "raw",     "<hex>", "原始字节"),
    ...
    if (cli.raw_len == 0) {
        PrintAndLogEx(INFO, "准备克隆 HID 标签");
        if (format_idx >= 0) {
...
static int CmdHIDBrute(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_u64_0(NULL, "fc",     "<dec>",   "facility code"),
        arg_u64_0(NULL, "cn",     "<dec>",   "card number"),
        arg_u64_0("i",  "issue",  "<dec>",   "问题级别"),
        arg_u64_0("o", "oem",     "<dec>",   "OEM代码"),
        arg_u64_0("d", "delay",   "<dec>",   "尝试间隔延迟（毫秒），默认1000"),
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "Wiegand 格式... %i", format_idx);
        PrintAndLogEx(INFO, "OEM.............. %u", card_hi.OEM);
        PrintAndLogEx(INFO, "ISSUE............ %u", card_hi.IssueLevel);
        PrintAndLogEx(INFO, "设施代码.... %u", card_hi.FacilityCode);
        PrintAndLogEx(INFO, "卡片号...... %" PRIu64, card_hi.CardNumber);
    ...
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(INFO, "开始暴力破解 HID Prox 读卡器");
...
static command_t CommandTable[] = {
    ...
    {"demod",   CmdHIDDemod,    AlwaysAvailable, "从 GraphBuffer 解调 HID Prox 标签"},
    {"reader",  CmdHIDReader,   IfPm3Lf,         "尝试读取并提取标签数据"},
    {"clone",   CmdHIDClone,    IfPm3Lf,         "克隆HID标签到T55x7、Q5/T5555或EM4305/4469"},
    {"sim",     CmdHIDSim,      IfPm3Lf,         "模拟HID标签"},
    {"brute",   CmdHIDBrute,    IfPm3Lf,         "暴力破解设施码或卡片号以匹配读卡器"},
