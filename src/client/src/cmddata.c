...
static int CmdSetDebugMode(const char *Cmd) {
    ...
    switch (g_debugMode) {
        case 0:
            PrintAndLogEx(INFO, "客户端调试级别... %u (无调试消息)", g_debugMode);
            break;
        case 1:
            PrintAndLogEx(INFO, "客户端调试级别... %u (调试消息)", g_debugMode);
            break;
        case 2:
            PrintAndLogEx(INFO, "客户端调试级别... %u (详细调试消息)", g_debugMode);
            break;
        default:
            break;
    }
...
int printDemodBuff(uint8_t offset, bool strip_leading, bool invert, bool print_hex) {
    ...
    if (len == 0) {
        PrintAndLogEx(WARNING, "DemodBuffer为空");
        return PM3_EINVARG;
    }
...
int CmdPrintDemodBuff(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("i", "inv", "打印前反转 DemodBuffer"),
//        arg_int0("l","len", "<dec>", "length to print in # of bits or hex characters respectively"),
        arg_int0("o", "offset", "<dec>", "偏移量（以位为单位）"),
        arg_lit0("s", "strip", "去除前导零，即将偏移量设置为第一个等于 1 的位"),
        arg_lit0("x", "十六进制", "以十六进制输出（省略则为二进制输出）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdConvertBitStream(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 转换比特流",
                  "将GraphBuffer的0|1值转换为127|-127",
                  "数据 转换比特流"
                 );
    void *argtable[] = {
        arg_param_begin,
...
int ASKDemod_ext(int clk, int invert, int maxErr, size_t maxlen, bool amplify, bool verbose, bool emSearch, uint8_t askType, bool *stCheck) {
    ...
    PrintAndLogEx(DEBUG, "DEBUG: (ASKDemod_ext) 来自图形缓冲区的样本数: %zu", bitlen);
...
//    if (*stCheck)
    ...
    if (st) {
        *stCheck = st;
        ...
        g_MarkerD.pos = stend;
        if (verbose)
            PrintAndLogEx(DEBUG, "已找到序列终止符 - 第一个由橙色/蓝色图形标记显示");
    }
    ...
    if (errCnt < 0 || bitlen < 16) { //if fatal error (or -1)
        PrintAndLogEx(DEBUG, "DEBUG: (ASKDemod_ext) 未找到数据，错误数:%d, %s 位长:%zu, 时钟:%i"
                      , errCnt
                      , (invert) ? "inverted," : ""
                      , bitlen
                      , clk
    ...
    if (errCnt > maxErr) {
        PrintAndLogEx(DEBUG, "DEBUG: (ASKDemod_ext) 发现过多错误，错误数:%d, 位数:%zu, 时钟:%i"
                      , errCnt
                      , bitlen
                      , clk
                     );
    ...
    if (verbose) {
        PrintAndLogEx(DEBUG, "DEBUG: (ASKDemod_ext) 使用时钟:%i, %s找到的位数:%zu, 起始索引 %d"
                      , clk
                      , (invert) ? "inverted, " : ""
                      , bitlen
                      , start_idx
...
static int Cmdaskmandemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 原始解调 --am",
                  "ASK/MANCHESTER 解调 GraphBuffer 中的数据并输出二进制",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("a", "amp", "尝试使用ASK放大解调（默认无放大）"),
        arg_int0("c", "clk", "<dec>", "手动设置时钟（默认自动检测）"),
        arg_lit0("i", "inv", "反转输出"),
        arg_lit0("s", "st", "检查序列终止符"),
...
static int Cmdmandecoderaw(const char *Cmd) {
    ...
    if (high > 7 || low < 0) {
        PrintAndLogEx(ERR, "错误：请先进行原始解调，然后进行曼彻斯特原始解码");
        free(bits);
        return PM3_ESOFT;
    }
    ...
    PrintAndLogEx(INFO, "曼彻斯特解码 %s", (invert) ? "( inverted )" : "");
...
static int CmdBiphaseDecodeRaw(const char *Cmd) {
    ...
    PrintAndLogEx(INFO, "使用偏移%d%s解码的双相数据", offset, (invert) ? "( inverted )" : "");
    PrintAndLogEx(INFO, "%s", sprint_bytebits_bin_break(bits, size, 32));
...
int ASKbiphaseDemod(int offset, int clk, int invert, int maxErr, bool verbose) {
    ...
    if (size == 0) {
        PrintAndLogEx(DEBUG, "DEBUG: graphbuf 中无数据");
        free(bs);
        return PM3_ESOFT;
    }
    int startIdx = 0;
    ...
    if (errCnt < 0 || errCnt > maxErr) {
        PrintAndLogEx(DEBUG, "DEBUG: 未找到数据或错误 %d, 时钟: %d", errCnt, clk);
        free(bs);
        return PM3_ESOFT;
    }
    ...
    if (errCnt < 0) {
        if (g_debugMode || verbose) PrintAndLogEx(DEBUG, "DEBUG: 错误 双相原始解码: %d", errCnt);
        free(bs);
        return PM3_ESOFT;
    }
    if (errCnt > maxErr) {
        if (g_debugMode || verbose) PrintAndLogEx(DEBUG, "DEBUG: 错误 双相原始解码错误过多: %d", errCnt);
        free(bs);
        return PM3_ESOFT;
    }
    ...
    if (g_debugMode || verbose) {
        PrintAndLogEx(DEBUG, "使用偏移%d | 时钟%d | 错误数%d | 起始索引%d解码的双相数据\\n数据\\n", offset, clk, errCnt, (startIdx + clk * offset / 2));
...
static int Cmdaskbiphdemod(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_int0("c", "clk", "<dec>", "手动设置时钟（默认自动检测）"),
        arg_lit0("i", "inv", "反转输出"),
        arg_int0("o", "offset", "<dec>", "开始双相位的偏移量（默认0）"),
...
static int Cmdaskrawdemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 原始解调 --ar",
                  "ASK/RAW 解调 GraphBuffer 中的数据并输出二进制",
...
int AutoCorrelate(const int *in, int *out, size_t len, size_t window, bool SaveGrph, bool verbose) {
    ...
    if (distance > -1) {
        ...
        PrintAndLogEx(HINT, "提示: 未找到重复模式，尝试增加窗口大小");
...
static int CmdAutoCorr(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_u64_0("w", "win", "<dec>", "相关窗口长度。默认4000"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (g_GraphTraceLen == 0) {
        PrintAndLogEx(WARNING, "图形缓冲区为空");
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("低频读取") "` to collect samples");
        return PM3_ESOFT;
    }
...
static int CmdUndecimate(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据反抽取",
                  "执行反抽取，将 graphbuf 中的每个样本重复 N 次",
                  "data undecimate\n"
                  "data undecimate -n 4\n"
                 );
...
static int CmdAskEdgeDetect(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("t", "thres", "<dec>", "阈值，使用20-45（默认25）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdDetectClockRate(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 检测时钟",
                  "检测GraphBuffer中波的ASK、FSK、NRZ、PSK时钟速率",
    ...
    int tmp = (a + f + n + p);
    if (tmp > 1) {
        PrintAndLogEx(WARNING, "仅指定一种调制方式");
        return PM3_EINVARG;
    } else if (tmp == 0) {

        int clock = GetFskClock("", false);
        if (clock > 0) {
            PrintAndLogEx(SUCCESS, "FSK时钟... %d", clock);
        }
        clock = GetAskClock("", false);
        if (clock > 0) {
            PrintAndLogEx(SUCCESS, "ASK时钟... %d", clock);
        }
        clock = GetNrzClock("", false);
        if (clock > 0) {
            PrintAndLogEx(SUCCESS, "NRZ 时钟... %d", clock);
        }
        clock = GetPskClock("", false);
        if (clock > 0) {
            PrintAndLogEx(SUCCESS, "PSK 时钟... %d", clock);
        }
        return PM3_SUCCESS;
    }
...
int FSKrawDemod(uint8_t rfLen, uint8_t invert, uint8_t fchigh, uint8_t fclow, bool verbose) {
    ...
    if (size > 0) {
        ...
        if (verbose || g_debugMode) {
            PrintAndLogEx(DEBUG, "DEBUG: (FSKrawDemod) 使用时钟:%u, %s高频:%u, 低频:%u"
                          , rfLen
                          , (invert) ? "inverted, " : ""
                          , fchigh
                          , fclow
        ...
        PrintAndLogEx(DEBUG, "未找到 FSK 数据");
    }
...
static int CmdFSKrawdemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 原始解调 --fs",
                  "对 GraphBuffer 中的数据进行 FSK 解调并输出二进制",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("c", "clk", "<dec>", "手动设置时钟（默认：自动检测）"),
        arg_lit0("i", "inv", "反转输出"),
...
int PSKDemod(int clk, int invert, int maxErr, bool verbose) {
    ...
    if (errCnt > maxErr) {
        if (g_debugMode || verbose) PrintAndLogEx(DEBUG, "DEBUG: (PSKdemod) 发现过多错误，clk: %d, invert: %d, 位数: %zu, 错误计数: %d", clk, invert, bitlen, errCnt);
        free(bits);
        return PM3_ESOFT;
    }
    if (errCnt < 0 || bitlen < 16) { //throw away static - allow 1 and -1 (in case of threshold command first)
        if (g_debugMode || verbose) PrintAndLogEx(DEBUG, "DEBUG: (PSKdemod) 未找到数据，clk: %d, invert: %d, 位数: %zu, 错误计数: %d", clk, invert, bitlen, errCnt);
        free(bits);
        return PM3_ESOFT;
    }
    if (verbose || g_debugMode) {
        PrintAndLogEx(DEBUG, "DEBUG: (PSKdemod) 使用时钟:%d, invert:%d, 找到的位数:%zu", clk, invert, bitlen);
        if (errCnt > 0) {
            PrintAndLogEx(DEBUG, "DEBUG: (PSKdemod) 解调过程中的错误（位流中显示为7）: %d", errCnt);
        }
    }
...
int NRZrawDemod(int clk, int invert, int maxErr, bool verbose) {
    ...
    if (errCnt > maxErr) {
        PrintAndLogEx(DEBUG, "DEBUG: (NRZrawDemod) 发现过多错误，clk: %d, invert: %d, 位数: %zu, 错误计数: %d", clk, invert, bitlen, errCnt);
        free(bits);
        return PM3_ESOFT;
    }
    if (errCnt < 0 || bitlen < 16) { //throw away static - allow 1 and -1 (in case of threshold command first)
        PrintAndLogEx(DEBUG, "DEBUG: (NRZrawDemod) 未找到数据，clk: %d, invert: %d, 位数: %zu, 错误计数: %d", clk, invert, bitlen, errCnt);
        free(bits);
        return PM3_ESOFT;
    }

    if (verbose || g_debugMode) PrintAndLogEx(DEBUG, "DEBUG: (NRZrawDemod) 尝试使用时钟: %d - invert: %d - 找到的位数: %zu 进行NRZ解调", clk, invert, bitlen);
    //prime g_DemodBuffer for output
    setDemodBuff(bits, bitlen, 0);
    setClockGrid(clk, clkStartIdx);


    if (errCnt > 0 && (verbose || g_debugMode)) PrintAndLogEx(DEBUG, "DEBUG: (NRZrawDemod) 解调过程中的错误（位流中显示为7）: %d", errCnt);
    if (verbose || g_debugMode) {
        PrintAndLogEx(SUCCESS, "NRZ 解调比特流");
        PrintAndLogEx(INFO, "---------------------");
        // Now output the bitstream to the scrollback by line of 16 bits
...
static int CmdNRZrawDemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据原始解调 --nr",
                  "对 GraphBuffer 中的数据进行 NRZ/DIRECT 解调并输出二进制",
...
int CmdPSK1rawDemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据原始解调 --p1",
                  "对 GraphBuffer 中的数据进行 PSK1 解调并输出二进制",
    ...
    //output
    if (ans != PM3_SUCCESS) {
        if (g_debugMode) PrintAndLogEx(ERR, "错误 解调：%d", ans);
        return PM3_ESOFT;
    }
...
static int CmdPSK2rawDemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据原始解调 --p2",
                  "对 GraphBuffer 中的数据进行 PSK2 解调并输出二进制",
...
static int CmdRawDemod(const char *Cmd) {
    ...
    if (foo > 1) {
        PrintAndLogEx(WARNING, "请仅选择一种调制方式");
        return PM3_EINVARG;
    }
    if (foo == 0) {
...
void setClockGrid(uint32_t clk, int offset) {
    ...
    g_DemodClock = clk;
    if (clk == 0 && offset == 0)
        PrintAndLogEx(DEBUG, "DEBUG: (setClockGrid) 清除设置");
    else
        PrintAndLogEx(DEBUG, "DEBUG: (setClockGrid) 解调偏移 %d, 时钟 %d", offset, clk);
...
static int CmdHexsamples(const char *Cmd) {
    ...
    if (requested > g_pm3_capabilities.bigbuf_size) {
        ...
        PrintAndLogEx(INFO, "n 大于大缓冲区大小，将使用 %u", requested);
    }

    uint8_t got[g_pm3_capabilities.bigbuf_size];
    if (offset + requested > sizeof(got)) {
        PrintAndLogEx(NORMAL, "尝试读取超出缓冲区末尾，<字节 %u> + <偏移 %u> > %d"
                      , requested
                      , offset
...
int getSamplesEx(uint32_t start, uint32_t end, bool verbose, bool ignore_lf_config) {
    ...
    if (end < start) {
        PrintAndLogEx(WARNING, "错误，结束（%u）小于开始（%u）", end, start);
        return PM3_EINVARG;
    }
...
int getSamplesFromBufEx(uint8_t *data, size_t sample_num, uint8_t bits_per_sample, bool verbose) {
    ...
    if (bits_per_sample < 8) {
        ...
        if (verbose) PrintAndLogEx(INFO, "正在解包...");
        ...
        if (verbose) PrintAndLogEx(INFO, "解包 %zu 个样本", j);
...
static int CmdLoad(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("f", "file", "<fn>", "要加载的文件"),
        arg_lit0("b", "bin", "二进制文件"),
        arg_lit0("n",  "no-fix",  "从文件加载数据，不进行任何转换"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (f == NULL) {
        PrintAndLogEx(WARNING, "无法打开`" _YELLOW_("%s") "`", path);
        free(path);
        return PM3_EFILE;
    }
    free(path);
...
int CmdLtrim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 左修剪",
                  "从迹线左侧裁剪样本",
                  "data ltrim -i 300   --> remove from start 0 to index 300"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_u64_1("i", "idx", "<dec>", "图形缓冲区中的索引"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdMtrim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_u64_1("s", "开始", "<dec>", "起始点"),
        arg_u64_1("e", "end", "<dec>", "终点"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (start > g_GraphTraceLen || stop > g_GraphTraceLen || start >= stop) {
        PrintAndLogEx(WARNING, "起始点和结束点不对齐");
        return PM3_EINVARG;
    }
...
int CmdSave(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("w", "wave", "保存为波形格式（.wav）"),
        arg_str1("f", "file", "<fn w/o ext>", "保存文件名"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (g_GraphTraceLen == 0) {
        PrintAndLogEx(WARNING, "图形缓冲区为空，无内容可保存");
        return PM3_SUCCESS;
    }
...
static int CmdTimeScale(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0("u", "unit", "<str>", "显示的时间单位（最多10个字符）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    g_CursorScaleFactor = arg_get_dbl_def(ctx, 1, 1);
    if (g_CursorScaleFactor <= 0) {
        PrintAndLogEx(FAILED, "错误，时间比例因子不能为负或零");
...
static int CmdDirectionalThreshold(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据 脏阈值",
                  "最大上升高于上限阈值/最小下降低于下限阈值，其余保持先前值。",
                  "data dirthreshold -u 10 -d -10"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_int1("d", "down", "<dec>", "阈值下限"),
        arg_int1("u", "up", "<dec>", "阈值上限"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int FSKToNRZ(int *data, size_t *dataLen, uint8_t clk, uint8_t LowToneFC, uint8_t HighToneFC) {
    ...
    if (clk == 0 || LowToneFC == 0 || HighToneFC == 0) {
        ...
        if (g_debugMode > 1) {
            PrintAndLogEx(NORMAL, "调试FSKtoNRZ：检测到时钟：fc_low %i, fc_high %i, clk %i, firstClockEdge %i, ans %u", LowToneFC, HighToneFC, clk, firstClockEdge, ans);
        }
    }
    ...
    if (ans == 0 || clk == 0 || LowToneFC == 0 || HighToneFC == 0 || LowToneFC > 10 || HighToneFC < 4) {
        if (g_debugMode > 1) {
            PrintAndLogEx(NORMAL, "调试FSKtoNRZ：未找到FSK时钟");
        }
        return PM3_ESOFT;
    }
...
#define LF_NUM_OF_TESTS     6
    ...
    if (hits) {
        PrintAndLogEx(SUCCESS, "找到 [%d] 个可能的调制匹配。", hits);
...
static int CmdDiff(const char *Cmd) {
    ...
    if (IfPm3Rdv4Fw() == false && (splenA > 0 || splenB > 0)) {
        PrintAndLogEx(WARNING, "无可用RDV4闪存");
        return PM3_EINVARG;
    }

    if (splenA > 32) {
        PrintAndLogEx(WARNING, "SPIFFS 文件名 A 长度超过 32 字节，实际为 %d", splenA);
        return PM3_EINVARG;
    }
    if (splenB > 32) {
        PrintAndLogEx(WARNING, "SPIFFS 文件名 B 长度超过 32 字节，实际为 %d", splenB);
        return PM3_EINVARG;
    }

    //
    if (width > 16 || width < 1) {
        PrintAndLogEx(INFO, "宽度超出范围，使用默认 16 字节宽度");
        width = 16;
    }
    ...
    if (inA == NULL) {
        PrintAndLogEx(INFO, "inA 为空");
    }

    if (inB == NULL) {
        PrintAndLogEx(INFO, "inB 为空");
    }
...
static int CmdNumCon(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0(NULL,  "十六进制", "<hex>", "hexadecimal value"),
...
static int CmdCryptography(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<hex>", "待处理数据"),
        arg_str1("k", "key", "<hex>", "要使用的密钥"),
        arg_lit0("r", "rev", "解密，非加密"),
    ...
    if ((type & 0x04) == 0x04) { // Use AES(0) or DES(1)?
        ...
        if (datilen % 8 != 0) {
            PrintAndLogEx(ERR, "<data> 长度必须是 8 的倍数。实际为 %d", datilen);
            return PM3_EINVARG;
        }

        if (keylen != 8 && keylen != 16 && keylen != 24 && keylen != 32) {
            PrintAndLogEx(ERR, "<key> 必须是 8、16、24、32 字节。实际为 %d", keylen);
            return PM3_EINVARG;
        }
        ...
        if (datilen % 16 != 0 && ((type & 0x02) == 0)) {
            PrintAndLogEx(ERR, "<data> 长度必须是 16 的倍数。实际为 %d", datilen);
            return PM3_EINVARG;
        }

        if (keylen != 16 && keylen != 32) {
            PrintAndLogEx(ERR, "<key> 必须是 16 或 32 字节。实际为 %d", keylen);
            return PM3_EINVARG;
        }
    }
    ...
    if ((type & 0x08) == 0x08) {
        ...
        if ((type & 0x04) == 0x04)  { // AES or DES?
            if ((type & 0x02) == 0x02) { // If we will calculate a MAC
                ...
                PrintAndLogEx(INFO, "尚未实现 - 欢迎贡献代码！");
                return PM3_SUCCESS;
            } else {
...
static int CmdXor(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("d", "数据", "<hex>", "输入十六进制字符串"),
        arg_str0("x", "xor", "<str>", "输入异或字符串"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (hlen != xlen) {
        PrintAndLogEx(FAILED, "长度不匹配，得到 %i != %i", hlen, xlen);
        return PM3_EINVARG;
    }

    PrintAndLogEx(SUCCESS, "输入... %s", sprint_hex_inrow(hex, hlen));
...
static int CmdTestSaveState8(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据测试_ss8",
                  "测试缓冲区保存状态（8 位缓冲区）的实现",
                  "数据测试_ss8");
    void *argtable[] = {
        arg_param_begin,
        arg_param_end
    ...
    PrintAndLogEx(DEBUG, "测试长度 = %llu", length);
    ...
    PrintAndLogEx(DEBUG, "保存状态已创建，长度=%llu，填充=%i，类型=%i", test8.bufferSize, test8.padding, test8.type);

    test8.clock = rand();
    test8.offset = rand();
    PrintAndLogEx(DEBUG, "保存状态时钟 = %u, 偏移 = %u", test8.clock, test8.offset);
    ...
    if (returnedLength != length) {
        ...
        PrintAndLogEx(WARNING, "返回长度 = %llu 缓冲区长度 = %llu 预期 = %llu", returnedLength, test8.bufferSize, length);
    } else {
    ...
    for (size_t i = 0; i < returnedLength; i++) {
        if (srcBuffer[i] != destBuffer[i]) {
            PrintAndLogEx(FAILED, "缓冲区在索引 %lu 处不匹配！期望 %i，实际 %i", i, srcBuffer[i], destBuffer[i]);
            free(srcBuffer);
            free(destBuffer);
            return PM3_EFAILED;
        }
        PrintAndLogEx(DEBUG, "索引 %lu 匹配: %u", i, destBuffer[i]);
    }
...
static int CmdTestSaveState32(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据测试_ss32",
                  "测试缓冲区保存状态（32 位缓冲区）的实现",
                  "数据测试_ss32");
    void *argtable[] = {
        arg_param_begin,
        arg_param_end
    ...
    PrintAndLogEx(DEBUG, "保存状态已创建，长度=%llu，类型=%i", test32.bufferSize, test32.type);

    test32.clock = rand();
    test32.offset = rand();
    PrintAndLogEx(DEBUG, "保存状态时钟=%u, 偏移=%u", test32.clock, test32.offset);

    uint32_t *destBuffer = (uint32_t *)calloc(length, sizeof(uint32_t));
    size_t returnedLength = restore_buffer32(test32, destBuffer);

    if (returnedLength != length) {
        PrintAndLogEx(FAILED, "返回长度 != 缓冲区长度！预期 '%llu'，收到 '%llu'", g_DemodBufferLen, returnedLength);
        free(srcBuffer);
        free(destBuffer);
        return PM3_EFAILED;
    }
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        PrintAndLogEx(DEBUG, "索引 %lu 匹配: %i", i, destBuffer[i]);
    }
...
static int CmdTestSaveState32S(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "数据测试_ss32s",
                  "测试缓冲区保存状态（32 位有符号缓冲区）的实现",
                  "数据测试_ss32s");
    void *argtable[] = {
        arg_param_begin,
        arg_param_end
    ...
    for (int i = 0; i < length; i++) {
        if (srcBuffer[i] != destBuffer[i]) {
            PrintAndLogEx(FAILED, "缓冲区在索引 %i 处不匹配！期望 %i，实际 %i", i, srcBuffer[i], destBuffer[i]);
            free(srcBuffer);
            free(destBuffer);
            return PM3_EFAILED;
        }
        PrintAndLogEx(DEBUG, "索引 %i 匹配: %i", i, destBuffer[i]);
    }
...
static int CmdQRcode(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("a", "ascii", "使用 ASCII 安全字符渲染"),
        arg_str0("f", "file", "<fn>", "指定文件名"),
        arg_str0("d", "数据", "<hex>", "消息作为单个十六进制字节字符串"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (data_count > 1) {
        CLIParserFree(ctx);
        PrintAndLogEx(ERR, "QR 数据只接受一个 -d 值。使用单个连续的十六进制字符串，并将空格编码为 20。");
        return PM3_EINVARG;
    }
    ...
    if (data_count == 1) {
        ...
        if (strpbrk(data_arg, " \t") != NULL) {
            CLIParserFree(ctx);
            PrintAndLogEx(ERR, "QR 数据必须是一个连续的十六进制字符串。不支持空格；将空格字节编码为 20。");
            return PM3_EINVARG;
        }
        ...
        for (int i = 0; i < data_arg_len; i++) {
            if (isxdigit((unsigned char)data_arg[i]) == 0) {
                CLIParserFree(ctx);
                PrintAndLogEx(ERR, "QR 数据只能包含十六进制字符 0-9、a-f 或 A-F。");
                return PM3_EINVARG;
            }
        }
        ...
        if (data_arg_len & 1) {
            CLIParserFree(ctx);
            PrintAndLogEx(ERR, "QR 数据必须包含偶数个十六进制数字。");
            return PM3_EINVARG;
        }
        ...
        if (dlen < 0) {
            CLIParserFree(ctx);
            PrintAndLogEx(ERR, "QR 数据必须是有效的十六进制字节。");
            return PM3_EINVARG;
        }
    }
    ...
    if (fnlen && dlen) {
        PrintAndLogEx(WARNING, "请指定文件名或数据字符串");
        return PM3_EINVARG;
    }
    ...
    if (dlen) {
        ...
        if (smallest_version < 1) {
            PrintAndLogEx(ERR, "QR 数据太大，无法放入支持的 QR 码中。");
            return PM3_EINVARG;
        }
...
static command_t CommandTable[] = {
    ...
    {"清除",            CmdBuffClear,            AlwaysAvailable,  "清除图形窗口使用的各种缓冲区"},
    {"hide",             CmdHide,                 AlwaysAvailable,  "隐藏图形窗口"},
    {"load",             CmdLoad,                 AlwaysAvailable,  "加载文件内容到图形窗口"},
    {"num",              CmdNumCon,               AlwaysAvailable,  "转换十进制/十六进制/二进制"},
    {"plot",             CmdPlot,                 AlwaysAvailable,  "显示图形窗口"},
    {"print",            CmdPrintDemodBuff,       AlwaysAvailable,  "打印解调缓冲区中的数据"},
    {"save",             CmdSave,                 AlwaysAvailable,  "保存信号迹线数据"},
    {"setdebugmode",     CmdSetDebugMode,         AlwaysAvailable,  "设置客户端调试级别"},
    {"xor",              CmdXor,                  AlwaysAvailable,  "对输入字符串进行异或运算"},

    {"-----------",      CmdHelp,                 AlwaysAvailable, "------------------------- " _CYAN_("Modulation") "-------------------------"},
    {"biphaserawdecode", CmdBiphaseDecodeRaw,     AlwaysAvailable,  "对DemodBuffer中的双相解码二进制流"},
    {"detectclock",      CmdDetectClockRate,      AlwaysAvailable,  "检测GraphBuffer中波的ASK、FSK、NRZ、PSK时钟速率"},
    {"fsktonrz",         CmdFSKToNRZ,             AlwaysAvailable,  "将fsk2转换为nrz波形以进行交替FSK解调（用于弱FSK）"},
    {"manrawdecode",     Cmdmandecoderaw,         AlwaysAvailable,  "Manchester 解码 DemodBuffer 中的二进制流"},
    {"modulation",       CmdDataModulationSearch, AlwaysAvailable,  "识别低频信号的时钟和调制"},
    {"rawdemod",         CmdRawDemod,             AlwaysAvailable,  "解调GraphBuffer中的数据并输出二进制"},

    {"-----------",      CmdHelp,                 AlwaysAvailable, "------------------------- " _CYAN_("Graph") "-------------------------"},
    {"askedgedetect",    CmdAskEdgeDetect,        AlwaysAvailable,  "调整图形以进行手动 ASK 解调"},
    {"autocorr",         CmdAutoCorr,             AlwaysAvailable,  "窗口自相关"},
    {"convertbitstream", CmdConvertBitStream,     AlwaysAvailable,  "将GraphBuffer的0/1值转换为127/-127"},
    {"cthreshold",       CmdCenterThreshold,      AlwaysAvailable,  "平均所有介于之间的值"},
    {"dirthreshold",     CmdDirectionalThreshold, AlwaysAvailable,  "最大上升高阈值/最小下降低阈值"},
    {"decimate",         CmdDecimate,             AlwaysAvailable,  "抽取样本"},
    {"envelope",         CmdEnvelope,             AlwaysAvailable,  "生成样本的方波包络"},
    {"grid",             CmdGrid,                 AlwaysAvailable,  "在图形窗口上叠加网格"},
    {"getbitstream",     CmdGetBitStream,         AlwaysAvailable,  "将GraphBuffer的>=1值转换为1，<1转换为0"},
    {"hpf",              CmdHpf,                  AlwaysAvailable,  "从迹线中移除直流偏移"},
    {"iir",              CmdDataIIR,              AlwaysAvailable,  "对绘图数据应用 IIR 巴特沃斯滤波器"},
    {"ltrim",            CmdLtrim,                AlwaysAvailable,  "从迹线左侧裁剪样本"},
    {"mtrim",            CmdMtrim,                AlwaysAvailable,  "从指定起始位置到指定结束位置裁剪样本"},
    {"norm",             CmdNorm,                 AlwaysAvailable,  "将最大值/最小值归一化到+/-128"},
    {"rtrim",            CmdRtrim,                AlwaysAvailable,  "从迹线右侧裁剪样本"},
    {"setgraphmarkers",  CmdSetGraphMarkers,      AlwaysAvailable,  "设置图形窗口中的标记"},
    {"shiftgraphzero",   CmdGraphShiftZero,       AlwaysAvailable,  "移位0用于图形波形+或-移位值"},
    {"timescale",        CmdTimeScale,            AlwaysAvailable,  "设置光标显示时间刻度"},
    {"undecimate",       CmdUndecimate,           AlwaysAvailable,  "恢复采样率"},
    {"zerocrossings",    CmdZerocrossings,        AlwaysAvailable,  "计算过零点之间的时间"},

    {"-----------",      CmdHelp,                 AlwaysAvailable, "------------------------- " _CYAN_("操作") "-------------------------"},
    {"asn1",             CmdAsn1Decoder,          AlwaysAvailable,  "ASN1 解码器"},
    {"atr",              CmdAtrLookup,            AlwaysAvailable,  "ATR 查找"},
    {"bitsamples",       CmdBitsamples,           IfPm3Present,     "获取原始样本作为位字符串"},
    {"bmap",             CmdBinaryMap,            AlwaysAvailable,  "根据二进制模板转换十六进制值"},
    {"crypto",           CmdCryptography,         AlwaysAvailable,  "加密和解密数据"},
    {"diff",             CmdDiff,                 AlwaysAvailable,  "输入文件的差异"},
    {"hexsamples",       CmdHexsamples,           IfPm3Present,     "以大缓冲区形式转储十六进制字节"},
    {"samples",          CmdSamples,              IfPm3Present,     "获取图形窗口的原始样本（GraphBuffer）"},
    {"qrcode",           CmdQRcode,               AlwaysAvailable,  "创建二维码"},

    {"-----------",      CmdHelp,                 IfClientDebugEnabled, "------------------------- " _CYAN_("Debug") "-------------------------"},
    {"test_ss8",         CmdTestSaveState8,       IfClientDebugEnabled, "测试缓冲区保存状态的实现（8位缓冲区）"},
    {"test_ss32",        CmdTestSaveState32,      IfClientDebugEnabled, "测试缓冲区保存状态的实现（32位缓冲区）"},
    {"test_ss32s",       CmdTestSaveState32S,     IfClientDebugEnabled, "测试缓冲区保存状态的实现（32位有符号缓冲区）"},
