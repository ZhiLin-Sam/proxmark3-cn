...
static int CmdHFCipurseInfo(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf cipurse 信息",
                  "从CIPURSE标签获取信息。",
                  "hf cipurse 信息");
    ...
    PrintAndLogEx(INFO, "应用`" _YELLOW_("AF F1") "` selected " _GREEN_("successfully"));
    ...
    if (sw != ISO7816_OK) {
        if (sw == 0x0000) {
            ...
            if (!mfExist)
                PrintAndLogEx(INFO, "不是 CIPURSE 卡。APDU 响应: %04x - %s", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
            else
    ...
    if (useAID && aidLen > 0) {
        ...
        if (res != 0 || *sw != ISO7816_OK) {
            if (verbose) {
                ...
                PrintAndLogEx(ERR, "卡片返回0x%04x", *sw);
            }
            return PM3_ESOFT;
        }
        if (verbose) {
...
static int CmdHFCipurseSelect(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 选择",
                  "选择应用程序或文件",
...
static int CmdHFCipurseAuth(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 认证",
                  "使用密钥 ID 和密钥进行认证。如果未提供密钥，将使用默认密钥 737373...7373",
...
static int CmdHFCipurseReadFile(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 读取",
                  "通过文件ID、密钥ID和密钥读取应用中的文件。如果未提供密钥，将使用默认密钥737373...7373",
    ...
    void *argtable[] = {
        ...
        arg_int0("o",  "offset",  "<dec>", "从文件读取数据的偏移量"),
    ...
    if (verbose)
        PrintAndLogEx(INFO, "选择文件 0x%x ( %s )", fileId, _GREEN_("ok"));
...
static int CmdHFCipurseWriteFile(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 写入",
                  "通过文件ID、密钥ID和密钥在应用中写入文件。如果未提供密钥，将使用默认密钥737373...7373",
    ...
    if (verbose) {
        ...
        PrintAndLogEx(INFO, "数据 [%d]: %s", hdatalen, sprint_hex(hdata, hdatalen));
    }
...
static int CmdHFCipurseReadFileAttr(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 认证读取",
                  "通过文件ID、密钥ID和密钥读取文件属性。如果未提供密钥，将使用默认密钥737373...7373",
    ...
    if (len == 0) {
        PrintAndLogEx(WARNING, "文件属性为空");
        DropField();
        return PM3_SUCCESS;
    }

    if (verbose)
        PrintAndLogEx(INFO, "属性原始数据 [%zu]: %s", len, sprint_hex(buf, len));
...
static int CmdHFCipurseWriteFileAttr(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 认证写入",
                  "通过文件ID、密钥ID和密钥写入文件属性。如果未提供密钥，将使用默认密钥737373...7373",
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "属性数据[%d]: %s", hdatalen, sprint_hex(hdata, hdatalen));
...
static int CmdHFCipurseFormatAll(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 全部格式化",
                  "格式化卡片。擦除卡片级别的所有数据！",
...
static int CmdHFCipurseCreateDGI(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 创建",
                  "通过提供适当的DGI创建应用/文件/密钥。如果未提供密钥，将使用默认密钥737373...7373。",
    ...
    void *argtable[] = {
        ...
        arg_str0("d",  "数据",    "<hex>", "用于创建的数据（含DGI）"),
        arg_str0(NULL, "sreq",    "<plain|mac|encode>", "Communication reader-PICC security level (def: mac)"),
        arg_str0(NULL, "sresp",   "<plain|mac|encode>", "Communication PICC-reader security level (def: mac)"),
        arg_lit0(NULL, "no-auth", "Execute without authentication"),
...
static int CmdHFCipurseDeleteFile(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 删除",
                  "通过文件ID、密钥ID和密钥删除文件。如果未提供密钥，将使用默认密钥737373...7373。",
...
static int CmdHFCipurseUpdateKey(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 更新密钥",
                  "更新密钥",
    ...
    if (verbose) {
        PrintAndLogEx(INFO, "新密钥编号: %d", newKeyId);
        PrintAndLogEx(INFO, "新密钥附加信息: 0x%02x", newKeyAInfo);
        PrintAndLogEx(INFO, "新密钥: %s", sprint_hex_inrow(newKey, 16));
        PrintAndLogEx(INFO, "新密钥kvv: %s", sprint_hex_inrow(kvv, 3));
        PrintAndLogEx(INFO, "新密钥数据: %s", sprint_hex_inrow(keydata, sizeof(keydata)));
        if (encKeyId) {
            PrintAndLogEx(INFO, "编码密钥编号: %d", encKeyId);
            PrintAndLogEx(INFO, "编码密钥: %s", sprint_hex_inrow(encKey, 16));

            aes_encode(NULL, encKey, newKey, &keydata[3], CIPURSE_AES_KEY_LENGTH);

            PrintAndLogEx(INFO, "编码的新密钥数据: %s", sprint_hex_inrow(keydata, sizeof(keydata)));
        }
        PrintAndLogEx(NORMAL, "");
    }
...
static int CmdHFCipurseTest(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 测试",
                  "回归测试",
                  "hf cipurse 测试");
...
static int CmdHFCipurseDefault(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf cipurse 默认",
                  "设置访问cipurse卡片的默认参数",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0(NULL, "清除",   "Resets to defaults"),
        arg_int0("n",  NULL,      "<dec>", "Key ID"),
        arg_str0("k",  "key",     "<hex>", "认证密钥"),
        arg_str0(NULL, "aid",     "<hex>", "Application ID (AID) ( 1..16 bytes )"),
    ...
    PrintAndLogEx(INFO, "密钥 ID : %d", defaultKeyId);
    ...
    PrintAndLogEx(INFO, "文件 ID: 0x%04x", defaultFileId);
...
static command_t CommandTable[] = {
    ...
    {"info",        CmdHFCipurseInfo,          IfPm3Iso14443a,  "标签信息"},
    {"select",    CmdHFCipurseSelect,        IfPm3Iso14443a,  "选择CIPURSE应用或文件"},
    {"auth",      CmdHFCipurseAuth,          IfPm3Iso14443a,  "认证CIPURSE标签"},
    {"read",      CmdHFCipurseReadFile,      IfPm3Iso14443a,  "读取二进制文件"},
    {"write",     CmdHFCipurseWriteFile,     IfPm3Iso14443a,  "写入二进制文件"},
    {"aread",     CmdHFCipurseReadFileAttr,  IfPm3Iso14443a,  "读取文件属性"},
    {"awrite",    CmdHFCipurseWriteFileAttr, IfPm3Iso14443a,  "写入文件属性"},
    {"formatall", CmdHFCipurseFormatAll,     IfPm3Iso14443a,  "擦除芯片中的所有数据"},
    {"create",    CmdHFCipurseCreateDGI,     IfPm3Iso14443a,  "通过DGI记录创建文件、应用、密钥"},
    {"delete",    CmdHFCipurseDeleteFile,    IfPm3Iso14443a,  "删除文件"},
    {"updkey",    CmdHFCipurseUpdateKey,     IfPm3Iso14443a,  "更新密钥"},
    {"updakey",   CmdHFCipurseUpdateKeyAttr, IfPm3Iso14443a,  "更新密钥属性"},
    {"default",   CmdHFCipurseDefault,       IfPm3Iso14443a,  "为所有其他命令设置默认密钥和文件ID"},
    {"test",      CmdHFCipurseTest,          AlwaysAvailable, "回归测试"},
    {NULL, NULL, 0, NULL}
};
