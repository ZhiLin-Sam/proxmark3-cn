...
static void piv_tag_dump_tlv(const struct tlv *tlv, const struct piv_tag *tag, int level) {
    ...
    while (left) {
        ...
        if (!tlv_parse_tl(&buf, &left, &sub_tlv)) {
            PrintAndLogEx(INFO, "%*s无效 Tag-Len", (level * 4), " ");
            continue;
        }
        sub_tlv.value = buf;
...
static void PrintTLVFromBuffer(const uint8_t *buf, size_t len) {
    ...
    if (tlvdb_parse_root_multi(root) == true) {
        ...
        PrintAndLogEx(WARNING, "TLV错误：无法将缓冲区解析为TLV树。");
    }
...
static int PivGetData(Iso7816CommandChannel channel, const uint8_t tag[], size_t tag_len, bool verbose, struct tlvdb_root **result, uint16_t *sw) {
    ...
    *sw = 0;
    ...
    do {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "发送APDU失败，代码%d", res);
            free(root);
            return res;
        }
        root->len += received;
        if (((*sw) & 0xff00) == 0x6100) {
            ...
            if ((capacity - root->len) < PM3_CMD_DATA_SIZE) {
                PrintAndLogEx(DEBUG, "正在为缓冲区增加更多容量...");
    ...
    *result = root;
    ...
    if (verbose == true) {
        PrintAndLogEx(WARNING, "无法解析 TLV 应答。");
    }
    return PM3_SUCCESS;
}
...
static int PivGetDataByCidAndPrint(Iso7816CommandChannel channel, const struct piv_container *cid, bool decodeTLV, bool verbose) {
    ...
    PrintAndLogEx(INFO, "正在获取 %s [" _GREEN_("%s") "]", cid->name, sprint_hex_inrow(cid->tlv_tag, cid->len));
    ...
    if (PivGetData(channel, cid->tlv_tag, cid->len, verbose, &root, &sw) == PM3_SUCCESS) {
        switch (sw) {
            ...
            case ISO7816_FILE_NOT_FOUND:
                PrintAndLogEx(FAILED, "未找到容器。");
                break;
            case ISO7816_SECURITY_STATUS_NOT_SATISFIED:
                PrintAndLogEx(WARNING, "安全条件未满足。");
                break;
            default:
                if (verbose == true) {
                    PrintAndLogEx(INFO, "APDU响应状态: %04" PRIx16 " - %s", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
                }
                break;
        }
        tlvdb_root_free(root);
...
static int PivAuthenticateSign(Iso7816CommandChannel channel, uint8_t alg_id, uint8_t key_id, uint8_t nonce[], size_t nonce_len, void **result, bool decodeTLV, bool verbose) {
    ...
    if (nonce_len > MAX_NONCE_LEN) {
        if (verbose == true) {
            PrintAndLogEx(WARNING, "Nonce 不能超过 %" PRIu64 " bytes. Got %" PRIu64 " bytes.", MAX_NONCE_LEN, nonce_len);
        }
        return PM3_EINVARG;
    }
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "发送APDU失败，代码%" PRId32, res);
        return res;
    }
    if (sw != ISO7816_OK) {
        if (verbose == true) {
            PrintAndLogEx(INFO, "意外的APDU响应状态: %04" PRIx16 " - %s", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        }
        return PM3_EFAILED;
    }
    if (verbose == true) {
...
static int PivSelect(Iso7816CommandChannel channel, bool activateField, bool leaveFieldOn, bool decodeTLV, bool silent, uint8_t applet[], size_t appletLen) {
    ...
    if (res != PM3_SUCCESS || sw != ISO7816_OK) {
        PrintAndLogEx(FAILED, "小程序选择失败。卡片不是PIV卡。");
        return res;
    }
...
static int CmdPIVSelect(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("sS",  "select",  "激活场并选择小程序"),
        arg_lit0("kK",  "keep",    "保持场以执行下一条命令"),
        arg_lit0("aA",  "apdu",    "显示 APDU 请求和响应"),
        arg_lit0("tT",  "tlv",     "TLV解码结果"),
        arg_lit0("wW",  "wired",   "通过接触式（ISO7816）接口发送数据（默认：非接触式接口）"),
...
static int CmdPIVGetData(const char *Cmd) {
    ...
    if ((tag_len < 1) || (tag_len > 3)) {
        PrintAndLogEx(WARNING, "标签应为1到3字节。实际为%" PRIi32, tag_len);
        return PM3_EINVARG;
    }
...
static int CmdPIVAuthenticateSign(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "piv 签名",
                  "发送一个 nonce 并要求 PIV 卡签名",
    ...
    if (key_slot > 0xff) {
        PrintAndLogEx(FAILED, "密钥槽必须占用 1 字节。");
        return PM3_EINVARG;
    }
    if (alg_id > 0xff) {
        PrintAndLogEx(FAILED, "算法 ID 必须适合 1 字节");
        return PM3_EINVARG;
    }
...
static command_t CommandTable[] =  {
    ...
    {"select",      CmdPIVSelect,                   IfPm3Iso14443,   "选择PIV小程序"},
    {"getdata",     CmdPIVGetData,                  IfPm3Iso14443,   "获取PIV卡上的容器"},
    {"authsign",    CmdPIVAuthenticateSign,         IfPm3Iso14443,   "与卡片认证"},
    {"scan",        CmdPIVScan,                     IfPm3Iso14443,   "扫描PIV卡以查找已知容器"},
    {"list",        CmdPIVList,                     AlwaysAvailable, "列出 ISO7816 历史"},
    {NULL, NULL, NULL, NULL}
};
