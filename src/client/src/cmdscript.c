...
static void set_python_path(const char *path) {
    ...
    if (syspath == 0) {
        PrintAndLogEx(WARNING, "Python 获取对象失败");
    }

    PyObject *pName = PyUnicode_FromString(path);
    if (PyList_Insert(syspath, 0, pName)) {
        PrintAndLogEx(WARNING, "将额外路径插入 sys.path 列表时出错");
    }

    if (PySys_SetObject("path", syspath)) {
        PrintAndLogEx(WARNING, "设置sys.path对象错误");
    }
}
...
static int CmdScriptList(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "脚本列表",
                  "列出可用的Lua、Cmd和Python脚本",
                  "脚本列表"
                 );
    void *argtable[] = {
        arg_param_begin,
...
pyexception:
        ...
        if (PyStatus_IsExit(status)) {
            PrintAndLogEx(WARNING, "\\nPython初始化失败，退出码=%i", status.exitcode);
        }
        if (PyStatus_IsError(status)) {
            PrintAndLogEx(WARNING, "\\nPython初始化失败，异常：%s", status.err_msg);
        }
        return PM3_ESOFT;
#endif
    }
    ...
    if (ext == PM3_LUA)
        ...
        PrintAndLogEx(FAILED, "错误 - 找不到 %s.[lua|cmd|py]", filename);
#else
    else if (ext == PM3_UNSPECIFIED)
        PrintAndLogEx(FAILED, "错误 - 找不到 %s.[lua|cmd]", filename);
#endif
    free(script_path);
    return ret;
}
...
static command_t CommandTable[] = {
    ...
    {"list",  CmdScriptList,    AlwaysAvailable, "列出可用脚本"},
    {"run",   CmdScriptRun,     AlwaysAvailable, "<name> - 执行脚本"},
    {NULL, NULL, NULL, NULL}
};
