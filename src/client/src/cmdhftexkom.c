...
static bool TexcomTK13Decode(uint32_t *implengths, uint32_t implengthslen, char *bitstring, char *cbitstring, bool verbose) {
    ...
    if (verbose)
        PrintAndLogEx(INFO, "原始位字符串 [%3zu]... %s", strlen(bitstring), bitstring);
    ...
    for (uint32_t i = 0; i < strlen(bitstring); i = i + 2) {
        if (bitstring[i] == bitstring[i + 1]) {
            cbitstring[0] = 0;
            if (verbose)
                PrintAndLogEx(WARNING, "原始位字符串在偏移 %d 处有错误。", i);
            break;
        }
        if (bitstring[i] == '1')
            strcat(cbitstring, "1");
    ...
    if (verbose)
        PrintAndLogEx(INFO, "位字符串 [%3zu].... %s", strlen(cbitstring), cbitstring);
...
static bool TexcomTK17Decode(uint32_t *implengths, uint32_t implengthslen, char *bitstring, char *cbitstring, bool verbose) {
    ...
    if (verbose)
        PrintAndLogEx(INFO, "TK17原始位字符串 [%zu]: %s", strlen(bitstring), bitstring);
    ...
    if (verbose)
        PrintAndLogEx(INFO, "TK17位字符串 [%zu]: %s", strlen(cbitstring), cbitstring);
...
static bool TexcomGeneralDecode(uint32_t *implengths, uint32_t implengthslen, char *bitstring, bool verbose) {
    ...
    for (uint32_t i = 0; i < implengthslen; i++) {
        if (TexcomCalculateBit(implengths[i], hilength, threshold))
            ...
            if (verbose) {
                PrintAndLogEx(INFO, "错误：字符串[%zu]：%s，位：%d，长度：%d", strlen(bitstring), bitstring, i, implengths[i]);

                PrintAndLogEx(INFO, "长度数组：");
    ...
    if (verbose)
        PrintAndLogEx(INFO, "通用原始比特串 [%zu]... %s", strlen(bitstring), bitstring);
    ...
    while (sindx < samplesCount - 5) {
        ...
        if (sindx == 0 || sindx > samplesCount - 5) {
            if (TexkomAVGField() > 30 && verbose) {
                PrintAndLogEx(WARNING, "环境噪声太大。尝试将标签稍微移开天线。");
            }
            break;
        }
    ...
    do {
        ...
        if (loop) {
            ...
            switch (res) {
                ...
                case PM3_ESOFT:
                    if (verbose) {
                        PrintAndLogEx(WARNING, "texkom 卡片选择失败");
                    }
                    return PM3_ESOFT;
                default:
                    break;
...
static int CmdHFTexkomReader(const char *Cmd) {
    ...
    if (codefound != TexkomModError) {
        ...
        if (verbose) {
            PrintAndLogEx(INFO, "十六进制代码............ %s", sprint_hex(tcode, 8));
            PrintAndLogEx(INFO, "十六进制代码修订版........ %s", sprint_hex(rtcode, 8));
        }
        ...
        if (tcode[0] == 0xff && tcode[1] == 0xff) {
            ...
            if (verbose == false) {
                ...
                PrintAndLogEx(SUCCESS, "Texkom 复制器... %s", sprint_hex(rtcode, 8));
            }
            ...
            if (tcode[2] == 0x63) {
                ...
                PrintAndLogEx(INFO, "UID................ %s（可能）", sprint_hex(&tcode[3], 4));
            }
        } else {
            PrintAndLogEx(ERR, "代码没有前导码 FFFF... %s", sprint_hex(tcode, 8));
        }
    } else {
        if (strlen(genbitstring) > 0)
            PrintAndLogEx(INFO, "通用解码比特串... %s", genbitstring);
        if (strlen(bitstring) > 0)
            PrintAndLogEx(INFO, "最后原始位串 [%zu].... %s", strlen(bitstring), bitstring);
        if (strlen(cbitstring) > 0)
            PrintAndLogEx(INFO, "最后位串 [%zu]........ %s", strlen(cbitstring), cbitstring);

        PrintAndLogEx(ERR, "未找到 Texkom 卡");
    }
...
static int CmdHFTexkomSim(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("v",  "详细",  "详细输出"),
        arg_lit0("t",  "tk17",     "使用 TK-17 调制（默认 TK-13）"),
    ...
    if (rawdatalen == 0 && iddatalen == 0) {
        PrintAndLogEx(ERR, "必须指定 <raw data> 或 <id> 以进行模拟");
        return PM3_EINVARG;
    }

    if (iddatalen > 0 && iddatalen != 4) {
        PrintAndLogEx(ERR, "<id> 必须是 4 字节长，实际为：%d", iddatalen);
        return PM3_EINVARG;
    }
    ...
    if (rawdatalen > 0 && rawdatalen != 8) {
        PrintAndLogEx(ERR, "<raw data> 必须是 8 字节长，实际为：%d", rawdatalen);
        return PM3_EINVARG;
    }
    ...
    if (payload.timeout > 0 && payload.timeout < 2800) {
        PrintAndLogEx(INFO, "模拟命令已启动");
        PacketResponseNG resp;
        if (WaitForResponseTimeout(CMD_HF_TEXKOM_SIMULATE, &resp, 3000) == false) {
            if (verbose) {
                PrintAndLogEx(WARNING, "(hf texkom simulate) 命令执行超时");
            }
            return PM3_ETIMEOUT;
        }
        PrintAndLogEx(INFO, "模拟命令执行完成");
    } else {
        PrintAndLogEx(INFO, "模拟命令已启动...");
    }
...
static command_t CommandTable[] = {
    ...
    {"reader",  CmdHFTexkomReader,  IfPm3Iso14443a,   "模拟 Texkom 读取器"},
    {"sim",     CmdHFTexkomSim,     IfPm3Iso14443a,   "模拟Texkom标签"},
