...
void printEM410x(uint32_t hi, uint64_t id, bool verbose, int type) {
    ...
    if (type & 0x4) { // Short Extended ID
        PrintAndLogEx(SUCCESS, "在 128 位帧上发现 EM 410x 短 ID");
        uint64_t data = (id << 20) >> 20;
...
static int ask_em410x_binary_decode(bool verbose, uint32_t *hi, uint64_t *lo, uint8_t *bits, size_t *size, size_t *idx) {
    ...
    if (ans < 0) {
        if (ans == -2)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x解调后样本不足");
        else if (ans == -4)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x未找到前导码");
        else if (ans == -5)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x大小不正确：%zu", *size);
        else if (ans == -6)
            PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x奇偶校验失败");

        return PM3_ESOFT;
    }

    if (!lo && !hi) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x解码结果全零");
        return PM3_ESOFT;
    }

    PrintAndLogEx(DEBUG, "DEBUG: Em410x 索引: %zu, 长度: %zu, 打印解调缓冲区:", *idx, *size);
    if (g_debugMode) {
...
int AskEm410xDecode(bool verbose, uint32_t *hi, uint64_t *lo) {
    ...
    if (getDemodBuff(bits, &size) == false) {
        PrintAndLogEx(DEBUG, "DEBUG: 错误 - Em410x从ASK解调复制时出现问题");
        return PM3_ESOFT;
    }
...
static int CmdEM410xWatch(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "正在监听 EM410x 卡片 - 将标签放在 Proxmark3 天线上");
    clearCommandBuffer();
...
static int CmdEM410xDemod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 410x解调",
                  "尝试查找EM 410x前导码，如果找到则解码/解扰数据",
...
static int CmdEM410xReader(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 410x读取器",
                  "读取 EM 410x 标签",
                  "lf em 410x reader\n"
...
static int CmdEM410xBrute(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 410x暴力破解",
                  "通过模拟EM 410x标签进行暴力破解",
    ...
    void *argtable[] = {
        ...
        arg_str1("f", "file", "<hex>", "包含EM标签ID的文件，每行一个ID"),
    ...
    if (uidcount == 0) {
        PrintAndLogEx(FAILED, "文件中未找到EM标签ID");
        free(uidblock);
        return PM3_EINVARG;
    }
...
static int CmdEM410xClone(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "低频EM 410x克隆",
                  "将 EM410x ID 克隆到 T55x7、Q5/T5555、EM4305/4469、Hitag S/8211/8268/8310 或 Hitag µ/8265 标签。",
    ...
    if (q5 + em + hts + htu > 1) {
        PrintAndLogEx(FAILED, "仅指定一种标签类型");
        return PM3_EINVARG;
    }

    if ((hts || htu) && IfPm3Hitag() == false) {
        PrintAndLogEx(FAILED, "设备未编译支持 Hitag");
        return PM3_EINVARG;
    }
    ...
    PrintAndLogEx(INFO, "编码为 %s", sprint_hex(data, sizeof(data)));
    ...
    if (hts) {
        ...
        for (size_t step = 0; step < 3; step++) {
            ...
            if (resp.status != PM3_SUCCESS) {
                PrintAndLogEx(WARNING, "步骤%zu出错", step);
                return resp.status;
            }
        }
    } else if (htu) {
    ...
    switch (resp.status) {
        case PM3_SUCCESS: {
            ...
            PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("低频EM 410x读取器") "` to verify");
            break;
        }
        default: {
            PrintAndLogEx(WARNING, "出错了");
            break;
        }
    }
    return resp.status;
...
static command_t CommandTable[] = {
    ...
    {"demod",  CmdEM410xDemod,    AlwaysAvailable, "从 GraphBuffer 解调 EM410x 标签"},
    {"reader", CmdEM410xReader,   IfPm3Lf,         "尝试读取并提取标签数据"},
    {"sim",    CmdEM410xSim,      IfPm3Lf,         "模拟EM410x标签"},
    {"brute",  CmdEM410xBrute,    IfPm3Lf,         "通过模拟EM410x标签进行读取器暴力破解攻击"},
    {"watch",  CmdEM410xWatch,    IfPm3Lf,         "监听EM410x 125/134 kHz标签"},
    {"spoof",  CmdEM410xSpoof,    IfPm3Lf,         "监听 EM410x 125/134 kHz 标签并重放" },
    {"clone",  CmdEM410xClone,    IfPm3Lf,         "克隆EM410x标签ID到T55x7、Q5/T5555或EM4305/4469"},
    {NULL, NULL, NULL, NULL}
};
