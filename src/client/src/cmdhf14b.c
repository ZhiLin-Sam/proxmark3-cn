            ...
            if (dfnamelen) {
                if (dfnamelen == vaidlen) {
                    if (memcmp(dfname, vaid, vaidlen) == 0) {
                        if (verbose) {
                            PrintAndLogEx(INFO, "(DF) 名称找到且等于 AID");
                        }
                    } else {
                        PrintAndLogEx(INFO, "(DF) 名称不等于 AID: %s :", sprint_hex(dfname, dfnamelen));
...
static bool wait_14b_response(bool only_first, uint8_t *datalen, uint8_t *data) {
    ...
    */
    ...
    if (WaitForResponseTimeout(CMD_HF_ISO14443B_COMMAND, &resp, TIMEOUT) == false) {
        PrintAndLogEx(WARNING, "等待回复超时（第一次）");
        return false;
    }
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "首次响应失败... %d", resp.status);
        return false;
    }
    ...
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(DEBUG, "第二次响应失败... %d", resp.status);
        return false;
    }
...
static bool wait_cmd_14b(bool verbose, bool is_select, uint32_t timeout) {
    ...
    if (is_select) {
        if (resp.status == PM3_ECARDEXCHANGE) {
            PrintAndLogEx(INFO, "标签无响应");
            return false;
        }
        if (resp.status != PM3_SUCCESS) {
            if (verbose) {
                PrintAndLogEx(INFO, "状态值失败... %d",  resp.status);
            }
            return false;
        }
    }
...
uint8_t *get_uid_from_filename(const char *filename) {
    ...
    if (strlen(filename) < 23) {
        PrintAndLogEx(ERR, "无法从文件名 ` 获取 UID" _YELLOW_("%s") "` expected format is hf-14b-<uid>...", filename);
        return uid;
    }
    ...
    if (len == 8) {
        ...
        PrintAndLogEx(ERR, "get_uid_from_filename 失败：hex_to_bytes 返回 %d", len);
        memset(uid, 0, 8);
    }
    return uid;
}
...
static int print_atqb_resp(uint8_t *data, uint8_t cid) {
    ...
    PrintAndLogEx(SUCCESS, "最大帧大小: %u%s 字节", maxFrame, (maxFrame == 257) ? "+ RFU" : "");
    ...
    if (fwt < 15) {
        ...
        PrintAndLogEx(SUCCESS, "帧等待整数：%u - %u ETU | %u 微秒", fwt, etus, fwt_time);
    } else {
        PrintAndLogEx(SUCCESS, "帧等待整数：%u - RFU", fwt);
    }
    ...
    if (memcmp(data, "\x54\x43\x4F\x53", 4) == 0) {
        ...
        if (exchange_14b_apdu(tcos_version, sizeof(tcos_version), true, false, out, PM3_CMD_DATA_SIZE, &outlen, -1) == PM3_SUCCESS) {
            if (outlen > 2) {
                ...
                PrintAndLogEx(SUCCESS, "Tiananxin TCOS CPU card... " _RED_("不适用"));
            }
            PrintAndLogEx(SUCCESS, "魔术能力........ 很可能");
        }
...
static int CmdHF14BSim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14b 模拟",
                  "模拟具有4字节UID / PUPI的ISO/IEC 14443 type B标签",
                  "hf 14b sim -u 11AA33BB"
                 );

    void *argtable[] = {
        arg_param_begin,
        arg_str1("u", "uid", "十六进制", "4字节UID/PUPI"),
        arg_param_end
    };
...
static int CmdHF14BSniff(const char *Cmd) {
    ...
    PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 14b list") "` to view captured tracelog");
...
static int CmdHF14BRaw(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("k", "keep", "接收响应后保持信号场开启"),

        arg_str0("d", "数据", "<hex>", "数据，要发送的字节"),
        arg_lit0("r", NULL, "do not read response from card"),
        arg_int0("t", "timeout", "<dec>", "超时时间（毫秒）"),

        arg_lit0("s", "std", "使用ISO14B选择"),
    ...
    if (user_timeout > 0) {
        ...
        if (user_timeout > MAX_14B_TIMEOUT_MS) {
            ...
            PrintAndLogEx(INFO, "设置超时为4.9秒。这是等待响应的最大时间");
        }
    ...
    if (select_std) {
        ...
        if (verbose && success) {
            PrintAndLogEx(SUCCESS, "已获取标准选择的响应");
        }
    }
    ...
    if (select_sr) {
        success = wait_cmd_14b(verbose, true, user_timeout);
        if (verbose && success) {
            PrintAndLogEx(SUCCESS, "已获取ST/SRx选择的响应");
        }
    }
    ...
    if (select_cts) {
        success = wait_cmd_14b(verbose, true, user_timeout);
        if (verbose && success) {
            PrintAndLogEx(SUCCESS, "已获取ASK/C-ticket选择的响应");
        }
    }
    ...
    if (select_xrx) {
        success = wait_cmd_14b(verbose, true, user_timeout);
        if (verbose && success) {
            PrintAndLogEx(SUCCESS, "已获取Fuji/Xerox选择的响应");
        }
    }
    ...
    if (select_pico) {
        success = wait_cmd_14b(verbose, true, user_timeout);
        if (verbose && success) {
            PrintAndLogEx(SUCCESS, "已获取Picopass选择的响应");
        }
    }
...
static bool HF14B_Std_Info(bool verbose, bool do_aid_search) {
    ...
    switch (resp.status) {
        ...
        case PM3_ELENGTH:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 STD ATTRIB 失败");
            break;
        case PM3_ECRC:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 STD CRC 失败");
            break;
        default:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-b STD 选择失败");
            break;
    }
...
static int CmdHF14Binfo(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14b 信息",
                  "基于 ISO/IEC 14443 B 型标签的信息",
                  "hf 14b info\n"
                 );
...
static bool HF14B_st_reader(bool verbose) {
    ...
    switch (resp.status) {
        ...
        case PM3_ELENGTH:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 ST ATTRIB 失败");
            break;
        case PM3_ECRC:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 ST CRC 失败");
            break;
        case PM3_EWRONGANSWER:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 ST 随机芯片 ID 失败");
            break;
        default:
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-b ST 选择 SRx 失败");
            break;
    }
    return false;
}
...
static bool HF14B_ask_ct_reader(bool verbose) {
    ...
    SendCommandNG(CMD_HF_ISO14443B_COMMAND, (uint8_t *)&packet, sizeof(iso14b_raw_cmd_t));
    if (WaitForResponseTimeout(CMD_HF_ISO14443B_COMMAND, &resp, TIMEOUT) == false) {
        if (verbose) PrintAndLogEx(WARNING, "等待回复超时");
        return false;
    }
    ...
    switch (resp.status) {
        ...
        case PM3_ELENGTH: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 CTS 长度错误");
            break;
        }
        case PM3_ECRC: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 CTS CRC 失败");
            break;
        }
        default: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-b CTS 选择失败");
            break;
        }
    }
    return false;
...
static bool HF14B_picopass_reader(bool verbose) {
    ...
    switch (resp.status) {
        ...
        case PM3_ELENGTH: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 长度错误");
            break;
        }
        case PM3_ECRC: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-3 CRC 失败");
        ...
        default: {
            if (verbose) PrintAndLogEx(FAILED, "ISO 14443-b Picopass 选择失败");
            break;
        }
    }
    return false;
...
static int CmdHF14BDump(const char *Cmd) {
    ...
    if (select_cardtype == ISO14B_CT) {
        ...
        PrintAndLogEx(FAILED, "转储CT标签尚未实现。");
    ...
    if (select_cardtype == ISO14B_STANDARD) {
        // Have to figure out how large one of these are..
        PrintAndLogEx(FAILED, "转储标准ISO14443-B标签尚未实现。");
...
static int CmdHF14BRestore(const char *Cmd) {
    ...
    if (bytes_read != (block_cnt * ST25TB_SR_BLOCK_SIZE) && bytes_read != ((block_cnt + 1) * ST25TB_SR_BLOCK_SIZE)) {
        PrintAndLogEx(ERR, "文件内容错误。读取了 %zu", bytes_read);
        free(data);
        return PM3_EFILE;
    }
    PrintAndLogEx(INFO, "正在复制到 %s", s);
...
//    uint32_t decodedBlock19 = srix4kDecode(block19);
    ...
    PrintAndLogEx(SUCCESS, "魔术字节 | %08X", result);
    return result;
}

static int CmdSRIX4kValid(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14b 验证",
                  "SRIX 校验和测试",
                  "hf 14b valid\n"
                 );
...
int select_card_14443b_4(bool disconnect, iso14b_card_select_t *card) {
    ...
    // check result
    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(WARNING, "场中无ISO14443-B卡");
        switch_off_field_14b();
        return PM3_ESOFT;
    }
...
static int handle_14b_apdu(bool chainingin, uint8_t *datain, int datainlen,
    ...
    *chainingout = false;
    ...
    if (WaitForResponseTimeout(CMD_HF_ISO14443B_COMMAND, &resp, MAX(APDU_TIMEOUT, user_timeout)) == false) {
        PrintAndLogEx(ERR, "APDU: 回复超时");
        return PM3_ETIMEOUT;
    }

    if (resp.status != PM3_SUCCESS) {
        PrintAndLogEx(ERR, "APDU: 无APDU响应");
        return resp.status;
    }
...
static int CmdHF14BAPDU(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_str0("m",  "make",     "<hex>", "使用此字段的头部和数据字段的数据构建APDU。\\n"
                 "                                   must be 4 bytes: <CLA INS P1 P2>"),
        arg_lit0("e",  "extended", "如果包含 `m` 参数，则构建扩展长度APDU"),
        arg_int0("l",  "le",       "<int>", "如果包含`m`参数，则为Le APDU参数"),
        arg_str1("d", "数据",     "<hex>", "<APDU | data> 如果包含 `m` 参数"),
        arg_int0(NULL, "timeout",   "<dec>", "超时时间（毫秒）"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
...
int CmdHF14BNdefRead(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14b NDEF读取",
                  "打印 NFC 数据交换格式 (NDEF)",
                  "hf 14b ndefread\n"
    ...
    if (ndef_size + offset > 0xFFFF) {
        PrintAndLogEx(ERR, "CmdHF14BNdefRead() 中 NDEF 大小异常过大。正在中止...\\n");
        free(ndef_file);
        switch_off_field_14b();
        return PM3_EOVFLOW;
    }
...
static int CmdHF14BCalypsoRead(const char *Cmd) {
    ...
    */
    ...
    for (int i = 0; i < ARRAYLEN(cmds); i++) {
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(FAILED, "发送命令失败，正在中止！");
            switch_off_field_14b();
            return res;
        }
        ...
        if (sw == ISO7816_OK) {
            ...
            PrintAndLogEx(INFO, "%-22s - 发送命令失败 (%04x - %s)。", cmds[i].desc, sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
        }
...
static int CmdHF14BSriTearoff(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int1("b", "block",   "<dec>", "块号（对于ST25TB计数器通常为5或6）"),
        arg_str1("d", "数据",    "<hex>", "目标计数器值（4 个十六进制字节，例如 FFFFFFFE）"),
        ...
        arg_int0(NULL, "开始",  "<dec>", "initial tear-off delay in us (default: 150)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (dlen != 4) {
        PrintAndLogEx(FAILED, "目标值必须是4个十六进制字节，得到 %d", dlen);
        return PM3_EINVARG;
    }

    if (blockno < 0 || blockno > 255) {
        PrintAndLogEx(FAILED, "块号必须为0-255，得到 %d", blockno);
        return PM3_EINVARG;
    }
    ...
    if (WaitForResponseTimeout(CMD_HF_ISO14443B_ST25TB_TEAROFF, &resp, -1) == false) {
        PrintAndLogEx(WARNING, "命令失败或连接丢失");
        return PM3_ETIMEOUT;
    }
    ...
    if (resp.status == PM3_SUCCESS) {
        ...
        PrintAndLogEx(INFO, "最后已知值: 0x%08X", final_value);
    } else {
...
static int CmdHF14BSetUID(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14b 设置UID",
                  "设置魔术卡片的UID（仅适用于此类卡片）\\n",
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_str1("u", "uid", "<hex>", "UID，4 个十六进制字节"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    if (memcmp(card->uid, uid, uidlen) == 0) {
        ...
        PrintAndLogEx(HINT, "提示：尝试 `" _YELLOW_("hf 14b 读卡器") "` to verify");
        PrintAndLogEx(NORMAL, "");
        return PM3_SUCCESS;;
    }
...
static command_t CommandTable[] = {
    ...
    {"config",    CmdHf14BConfig,      IfPm3Iso14443b,  "配置14b设置（谨慎使用）"},
    {"list",      CmdHF14BList,        AlwaysAvailable, "列出ISO-14443-B历史"},
    {"---------", CmdHelp,             AlwaysAvailable, "----------------------- " _CYAN_("操作") " -----------------------"},
    {"apdu",      CmdHF14BAPDU,        IfPm3Iso14443b,  "向标签发送ISO 14443-4 APDU"},
    {"dump",      CmdHF14BDump,        IfPm3Iso14443b,  "读取ISO-14443-B标签的所有内存页面，保存到文件"},
    {"info",      CmdHF14Binfo,        IfPm3Iso14443b,  "标签信息"},
    {"ndefread",  CmdHF14BNdefRead,    IfPm3Iso14443b,  "读取标签上的NDEF文件"},
    {"raw",       CmdHF14BRaw,         IfPm3Iso14443b,  "向标签发送原始十六进制数据"},
    {"rdbl",      CmdHF14BSriRdBl,     IfPm3Iso14443b,  "读取SRI512/SRIX4块"},
    {"reader",    CmdHF14BReader,      IfPm3Iso14443b,  "模拟 ISO-14443-B 读取器识别标签"},
    {"restore",   CmdHF14BRestore,     IfPm3Iso14443b,  "从文件恢复到ISO-14443-B标签的所有内存页"},
    {"sim",       CmdHF14BSim,         IfPm3Iso14443b,  "伪造ISO ISO-14443-B标签"},
    {"sniff",     CmdHF14BSniff,       IfPm3Iso14443b,  "窃听ISO-14443-B"},
    {"wrbl",      CmdHF14BSriWrbl,     IfPm3Iso14443b,  "将数据写入SRI512/SRIX4标签"},
    {"tearoff",   CmdHF14BSriTearoff,  IfPm3Iso14443b,  "对 ST25TB/SRx 计数器块的撕拉攻击"},
    {"view",      CmdHF14BView,        AlwaysAvailable, "显示标签转储文件的内容"},
    {"valid",     CmdSRIX4kValid,      AlwaysAvailable, "SRIX4校验和测试"},
    {"---------", CmdHelp,             AlwaysAvailable, "------------------ " _CYAN_("Calypso / Mobib") " ------------------"},
    {"calypso",   CmdHF14BCalypsoRead, IfPm3Iso14443b,  "读取Calypso卡片的内容"},
    {"mobib",     CmdHF14BMobibRead,   IfPm3Iso14443b,  "读取Mobib卡片的内容"},
    {"---------", CmdHelp,             IfPm3Iso14443b,  "------------------------- " _CYAN_("Magic") " -----------------------"},
    {"setuid",    CmdHF14BSetUID,      IfPm3Iso14443b,   "为魔术卡设置UID"},
    {NULL, NULL, NULL, NULL}
};
...
int infoHF14B(bool verbose, bool do_aid_search) {
    ...
    if (verbose) {
        PrintAndLogEx(FAILED, "未找到 14443-B 标签");
    }
...
plot:
    ...
    if (verbose && found == false) {
        PrintAndLogEx(FAILED, "未找到 ISO 14443-B 标签");
    }
